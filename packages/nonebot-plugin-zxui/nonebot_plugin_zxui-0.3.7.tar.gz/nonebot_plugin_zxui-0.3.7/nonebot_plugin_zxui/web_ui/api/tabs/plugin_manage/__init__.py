...
async def _(
    ...
    try:
        ...
        logger.error(f"WebUi {router.prefix}/get_plugin_list 调用错误 {type(e)}:{e}")
...
async def _() -> Result[PluginCount]:
    try:
        ...
        logger.error(f"WebUi {router.prefix}/get_plugin_count 调用错误 {type(e)}:{e}")
        return Result.fail(f"发生了一点错误捏 {type(e)}: {e}")
...
async def _(param: UpdatePlugin) -> Result:
    try:
        ...
        logger.error(f"WebUi {router.prefix}/update_plugin 调用错误 {type(e)}:{e}")
...
async def _(param: PluginSwitch) -> Result:
    try:
        ...
        logger.error(f"WebUi {router.prefix}/change_switch 调用错误 {type(e)}:{e}")
        return Result.fail(f"{type(e)}: {e}")
...
async def _() -> Result[list[str]]:
    try:
        ...
        logger.error(
            f"WebUi {router.prefix}/get_plugin_menu_type 调用错误 {type(e)}:{e}"
        )
        return Result.fail(f"{type(e)}: {e}")
...
async def _(module: str) -> Result[PluginDetail]:
    try:
        ...
        logger.error(f"WebUi {router.prefix}/get_plugin 调用错误 {type(e)}:{e}")
        return Result.fail(f"{type(e)}: {e}")
