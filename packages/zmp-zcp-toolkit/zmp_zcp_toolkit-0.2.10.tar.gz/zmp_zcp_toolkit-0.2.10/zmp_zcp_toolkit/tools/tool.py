    ...
    def _run(
        ...
        **kwargs: Any,
        ...
        if self.request_body is not None:
            ...
            elif isinstance(request_body, BaseModel):
                logger.debug("Request body is BaseModel")
                for field_name, field in request_body.model_fields.items():
                    logger.debug(f"Field: {field_name}")
                    logger.debug(f"Field type: {type(field)}")
                    logger.debug(f"Field value: {field.default}")
            else:
