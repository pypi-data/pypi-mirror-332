...
class ZmpToolkit(BaseToolkit):
    ...
    def from_zmp_api_wrapper(cls, *, zmp_api_wrapper: ZmpAPIWrapper) -> "ZmpToolkit":
        ...
        for i, tool in enumerate(tools):
            ...
            log_str += f"  Description: {tool.description}\n"
            ...
            if tool.request_body:
                log_str += "    Request body fields:\n"
                for field_name, field_info in tool.request_body.model_fields.items():
                    log_str += f"      - {field_name}: {field_info.annotation}\n"
