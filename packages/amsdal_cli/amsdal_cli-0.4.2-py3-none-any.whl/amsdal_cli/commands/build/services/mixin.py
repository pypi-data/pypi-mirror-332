...
class BuildMixin:
    ...
    def build_fixtures(cli_app_path: Path) -> None:
        ...
        from amsdal_cli.commands.build.schemas.loaders.cli_fixtures_loader import CliMultiFixturesLoader
        ...
        fixtures_loader = CliMultiFixturesLoader(schemas_dirs)
        ...
        for fixtures_path in fixtures_loader.iter_fixtures():
            shutil.copy(
                fixtures_path,
                target_fixtures_path / fixtures_path.name,
            )

        for files_path in fixtures_loader.iter_fixture_files():
            shutil.copytree(
                files_path,
                target_fixtures_path / 'files',
                dirs_exist_ok=True,
            )
