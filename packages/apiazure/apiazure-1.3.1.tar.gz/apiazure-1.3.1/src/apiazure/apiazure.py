        ...
        if response.status_code == 200:
            ...
            for pipeline_run in json.loads(response.text)['value']:
                json_response['pipelines_runs'].append({
                    ...
                    "variables": pipeline_run['variables']
                })
        else:
            json_response={
                "function_code":f"{response.status_code}",
...
def get_build_status_by_source_branch(API_URL=str, PROJECT=str, TOKEN=str, SOURCE_BRANCH=int, PIPELINE_NAME=str|None, headers=object|None, params=object|None) -> object:
    """
    Retrieves the status of a pipeline run.
    ...
    try:
        ...
        for pipeline_run in list_pipelines_runs_response['pipeline_runs']:
            if pipeline_run['variables']['system.pullRequest.sourceBranch'].split('/')[2] == SOURCE_BRANCH:
                json_response={
                    "function_code":200,
                    "build": {
                        "id": pipeline_run['id'],
