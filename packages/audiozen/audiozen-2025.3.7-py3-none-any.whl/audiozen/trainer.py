...
class Trainer(ABC):
    ...
    def _parse_ckpt_path_or_alias(self, path_or_alias: str) -> Optional[Path]:
        ...
        If the path_or_alias is "no", it means that the training starts from scratch. Returns None.
    ...
    def evaluate(self):
        ...
        # If the current run is not in training (doesnot use train mode), we need to prepare the model for DDP.
        ...
        for dl_idx, (dl_id, dataloader) in enumerate(self.eval_dataloaders.items()):
            ...
            for batch_idx, batch in enumerate(
                ...
                It is advised against computing metrics within the `evaluation_epoch_end` method for several reasons:
                    1. Most evaluation metrics are inherently sequential and not parallelizable. Hence, computing them
                       in `evaluation_epoch_end` does not offer a speed advantage during evaluation.
                    2. By not aggregating all outputs for metric calculation at the epoch's end, we reduce the risk of
                       memory overflow, which can occur when gathering results across all processes.
                    3. Calculating the metric score during `evaluation_step` allows for earlier detection of any errors
                       in the code.
                ...
                Recommendations for metric calculation:
                    ...
                    3. If necessary, compute the average or aggregate metric score in the `evaluation_epoch_end`
                       method.
                """
                with torch.no_grad():
