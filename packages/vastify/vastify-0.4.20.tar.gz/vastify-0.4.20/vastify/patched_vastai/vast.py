...
def parse_query(query_str: str, res: Dict = None, fields = {}, field_alias = {}, field_multiplier = {}) -> Dict:
    ...
    for field, op, _, value, _ in opts:
        ...
        if not op_name:
            raise ValueError(f"Unknown operator. '{field}' '{op}' '{value}' Did you forget to quote your query? " + repr(op).strip("u"))
    ...
    if args.region:
        if args.region != 'World':
            if not _is_valid_region(args.region):
                print(f"Invalid region or country codes provided. {args.region}")
                return
