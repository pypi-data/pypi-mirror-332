...
def ensure_available_max_directory(directory: Optional[Path]) -> tuple[Path, bool]:
    ...
    if not is_empty and not is_max:
        print(
            f"[red]{_("Le dossier n'est pas vide et n'est pas un projet MaX valide, veuillez en choisir un autre.")}[/red]"
        )
        raise typer.Exit(code=1)
...
def new(
    ...
    root_directory, is_max = ensure_available_max_directory(directory)
    if is_max:
        print(_("Le dossier contient une instance de MaX. Utiliser la commande init"))
        raise typer.Exit()
    else:
