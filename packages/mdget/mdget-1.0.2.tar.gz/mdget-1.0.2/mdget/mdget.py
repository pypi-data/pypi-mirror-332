...
def main():
    ...
    parser.add_argument('-u', '--upload', metavar=('lc_path', 'rm_path'), type=lambda x: x.split(), nargs=2, help='')
    parser.add_argument('-d', '--download', metavar=('rm_path', 'lc_path'), type=lambda x: x.split(), nargs=2, help='')
    args = parser.parse_args()

    before = time.time()
    file_path = '/Users/xigua/数据中心2/spd.txt'
    ...
    cloud.skip = ['.DS_Store', '__pycache__', '__MACOSX', 'desktop.ini', 'logfile', 'spd.txt']
    if args.upload:
        local_path, remoto_path = args.upload
        cloud.upload(local_path=local_path, remote_path=remoto_path)
    if args.download:
        remoto_path, local_path = args.upload
