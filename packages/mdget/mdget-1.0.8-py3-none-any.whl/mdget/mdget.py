...
def main():
    ...
    parser.add_argument('-v', '--version', action='version', version=f'%(prog)s {__version__.VERSION}', help='')
    parser.add_argument('-u', '--upload', nargs=2, help='')
    parser.add_argument('-d', '--download', nargs=2, help='')
    args = parser.parse_args()

    before = time.time()
    content = config.read_config(file_path='spd.txt'   )
    cloud = SCPCloud(
