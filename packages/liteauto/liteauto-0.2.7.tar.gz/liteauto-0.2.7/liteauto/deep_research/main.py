...
class DeepResearchSystem:
    ...
    def generate_search_queries(self, num_queries: int = 3) -> List[SearchQuery]:
        ...
        queries = self._llm_call(
            system_prompt=system_prompt,
            prompt="Generate search queries for the current research phase.",
        ...
        if len(filtered_queries) < num_queries // 2 and len(filtered_queries) > 0:
            ...
            system_prompt += "\n\nIMPORTANT: Your previous suggestions were too similar to past queries. Generate completely different queries."
    ...
    def should_advance_phase(self) -> bool:
        ...
        if current_phase == "discovery":
            ...
            if self.phase_summaries.get("discovery"):
                ...
                if confidence >= 0.75 and len(knowledge_gaps) <= 2:
                    logger.info(
    ...
    def save_report_md(self, report: Dict, query: str, start_time, save_md=None):
...
\n## Comprehensive Analysis\n{report['knowledge_summary']}\n
        ...
        if save_md:
            ...
            # Create filename
            filename = f"reports/research_report_{safe_query}_{timestamp}.md"
    ...
    def conduct_research(self, query: str, max_urls, deep_focus_k, save_md_report=None) -> Dict:
        ...
        report_markdown, filename = self.save_report_md(final_report, query, start_time,save_md=save_md_report)
        logger.info(f"Report saved to: {filename}")
...
if __name__ == "__main__":
    ...
    # Conduct research
    research_system.conduct_research(query, max_urls, deep_focus_k)
