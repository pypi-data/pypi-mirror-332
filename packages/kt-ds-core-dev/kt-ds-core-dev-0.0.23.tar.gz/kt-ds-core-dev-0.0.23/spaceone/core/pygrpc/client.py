...
def client(endpoint=None, ssl_enabled=False, max_message_length=None, **client_opts):
    ...
    if endpoint not in _GRPC_CHANNEL:
        options = [
            ('grpc.keepalive_time_ms', 60000),  # 60초마다 KeepAlive Ping 전송
            ('grpc.keepalive_timeout_ms', 20000),  # 20초 내로 응답이 없으면 연결 종료
            ('grpc.http2.max_pings_without_data', 0),  # 제한 없이 KeepAlive 전송 허용
            ('grpc.keepalive_permit_without_calls', 1)  # 활성 호출이 없어도 KeepAlive 유지
        ]
