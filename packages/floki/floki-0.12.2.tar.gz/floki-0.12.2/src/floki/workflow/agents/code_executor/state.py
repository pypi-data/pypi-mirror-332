...
class CodeExecWorkflowMessage(BaseModel):
...
class CodeExecWorkflowEntry(BaseModel):
    ...
    messages: List[CodeExecWorkflowMessage] = Field(default_factory=list, description="Messages exchanged during the workflow")
    last_message: Optional[CodeExecWorkflowMessage] = Field(default=None, description="Last processed message in the workflow")
...
class CodeExecWorkflowState(BaseModel):
    """Represents the state of multiple Code Executor workflows."""
    instances: Dict[str, CodeExecWorkflowEntry] = Field(default_factory=dict, description="Workflow entries indexed by their instance_id.")