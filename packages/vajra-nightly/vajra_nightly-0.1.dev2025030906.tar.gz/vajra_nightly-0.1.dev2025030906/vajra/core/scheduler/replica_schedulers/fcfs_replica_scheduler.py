...
from vajra.core.scheduler.replica_schedulers.batch_formation_tracker import (
    ...
    def _get_seq_next_num_q_tokens(
        ...
        if self.parallel_config.kv_parallel_size > 1:
            last_group_tokens = (
                ...
                % self.kvp_state_tracker.max_num_tokens_per_kvp_group
            )
            next_num_tokens = min(
                next_num_tokens,
                self.kvp_state_tracker.max_num_tokens_per_kvp_group - last_group_tokens,
            )
