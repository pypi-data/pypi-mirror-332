...
class DataExtractor(Tool):
    ...
    def update_data_info(self, selected_data, data_extract_store):
        ...
        if not any([i['value'] for i in ctx.triggered]):
            return [html.Div('Selected data descriptions will appear here')], [True], [json.dumps({'selected_data':[]})]
    ...
    def pick_download_type(self, download_info):
        ...
        if download_info['type']=='Properties':
            self.download_property_data(download_info)
        elif download_info['type'] in ['Images','Masks','Images & Masks']:
            self.download_image_data(download_info)
        elif download_info['type'] == 'Annotations':
            self.download_annotations(download_info)
    ...
    def update_download_data(self, new_interval, download_info_store):
        ...
        if not download_info_store['current_task'] in current_threads:
            ...
            if len(download_info_store['download_tasks'])==1:
                ...
                task_name = f'{download_info_store["download_tasks"][0]["structure"]} {download_info_store["download_tasks"][0]["type"]}'
        ...
        return [interval_disabled], [modal_open], [modal_children], [new_n_intervals], [download_data]
