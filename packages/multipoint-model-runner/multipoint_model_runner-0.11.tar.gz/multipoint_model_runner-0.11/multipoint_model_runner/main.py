...
def get_raw_samples(url, api_key, uuids):
    query = """query GetMultipointSamples($uuids: [ID]!){
        getMultipointSamples(uuids:$uuids){
            uuid
            y {
                nodes{
    ...
    return res['data']['getMultipointSamples']


def get_dataset(url, api_key, dataset_uuid):
    query = """query GetMultipointSamplesByDataset($uuid: ID!){
        getMultipointSamplesByDataset(uuid: $uuid){ 
            uuid 
            y {
                nodes{
                    evalLevel tagUuids actionUuids
    ...
    res = run_gql(url, api_key, query, variables)
    return res['data']['getMultipointSamplesByDataset']
