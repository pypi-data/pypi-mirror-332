...
class Individual:
	...
	fitnesses = None
	fitness = None
	numberOfDominatedSolutions: int = -1
	...
	def __gt__(self, other):
		return self.numberOfDominatedSolutions >  other.numberOfDominatedSolutions
	...
	def getNumberOfDominatedSolutions(self, population):
		if self.numberOfDominatedSolutions == -1:
			count = 0 
			for other in population:
				dominates = True
				i = 0
				while i < len(self.fitnesses) and dominates:
					if self.fitness[i] <= other.fitness[i]:
						dominates = False
					i+=1
				if dominates:
					count += 1
			self.numberOfDominatedSolutions = count
		return self.numberOfDominatedSolutions
	...
	def getFitnesses(self, tr_x = None, tr_y = None):
		...
		if self.fitness is None:
			...
			for fitnessType in self.fitnesses:
				if fitnessType == "Accuracy":
				...
				if fitnessType == "WAF":
					...
					self.fitness.append(waf)

				if fitnessType == "2FOLD":
					...
					self.fitness.append( (f1+f2)/2 )

				if fitnessType == "Size":
					self.fitness.append( 0 - self.getSize())

				if fitnessType == "Size(Classifier)":
					self.getTrainingPredictions()
					size = sum([x.tree_.node_count for x in self.model.estimators_ ])
					self.fitness.append( 0 - size )
				...
				if fitnessType == "MDL":
					...
					size_fe = self.getSize()
					size_classifier = sum([x.tree_.node_count for x in self.model.estimators_ ])

					self.fitness.append( 0- (size_fe + size_classifier + (1-acc)*len(self.training_Y)) )
