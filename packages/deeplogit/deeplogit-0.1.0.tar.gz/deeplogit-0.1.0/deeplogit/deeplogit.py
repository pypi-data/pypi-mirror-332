...
class DeepLogit:
    ...
    def fit(
        ...
        Args:
            ...
            attributes_varnames : list
                The list of variable names for the attributes that must match the columns in the data.

            unstructured_data : dict
                The unstructured data for each product. The keys are the product IDs and the values are the unstructured data. (See example.py to see how to store the unstructured data.)

            unstructured_data_type : str
                The type of unstructured data. Must be either 'images' or 'text'.

            select_optimal_PC_RCs : bool, optional
                Whether to select the optimal number of principal components and random coefficients by trying all possible combinations. If False, the model will try all combinations with the maximum number of random coefficients. Default is True.
