...
def parse_dependencies(dep_file):
    ...
    if isinstance(raw_content, dict):
        ...
        try:
            import json
            processed_dependencies = json.loads(raw_content)
    ...
    for attr in ["features", "optional"]:
        if attr in processed_dependencies:
            global_attributes[attr] = processed_dependencies[attr]
    ...
    for pkg_name in list(formatted_dependencies.keys()):
        for attr_name, attr_value in global_attributes.items():
            attr_pkg_key = f"{attr_name}_packages"
                ...
                if pkg_name in pkg_list:
                    ...
                    if attr_name == "features":
                        if isinstance(attr_value, str):
                            formatted_dependencies[pkg_name]["features"] = [attr_value]
                        elif isinstance(attr_value, list):
                            formatted_dependencies[pkg_name]["features"] = attr_value
                    elif attr_name == "optional":
                        if isinstance(attr_value, str):
                            attr_value = attr_value.lower() == "true"
                        formatted_dependencies[pkg_name]["optional"] = bool(attr_value)
                    else:
                        formatted_dependencies[pkg_name][attr_name] = attr_value
    ...
    for pkg_name, info in list(processed_dependencies.items()):
        if isinstance(info, dict) and pkg_name not in global_attributes:
            formatted_dependencies[pkg_name] = {
