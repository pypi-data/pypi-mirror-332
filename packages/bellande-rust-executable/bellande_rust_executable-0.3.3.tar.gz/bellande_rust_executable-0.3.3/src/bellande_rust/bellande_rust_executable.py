...
def parse_dependencies(dep_file):
        ...
        while i < len(parts):
            if ":" in parts[i]:
                ...
                if value.strip():
                    ...
                    processed_dependencies[key] = value
                    i += 1
                else:
                    if i + 1 < len(parts):
    ...
    global_attributes = {}
    for attr in ["features", "optional"]:
        if attr in processed_dependencies:
            global_attributes[attr] = processed_dependencies.pop(attr)
    ...
    for name, info in processed_dependencies.items():
        ...
        if isinstance(info, str):
            formatted_dependencies[name] = info
        elif isinstance(info, dict):
            formatted_dependencies[name] = info
        else:
            formatted_dependencies[name] = {"version": str(info)}
    
    for pkg_name in list(formatted_dependencies.keys()):
        # Handle features attribute
        if "features" in global_attributes:
            feature_value = global_attributes["features"]
            
            attr_pkg_key = "features_packages"
            pkg_list = []
            
            if attr_pkg_key in processed_dependencies:
                pkg_list = processed_dependencies[attr_pkg_key]
                if isinstance(pkg_list, str):
                    pkg_list = [pkg.strip() for pkg in pkg_list.split(',')]
            
            if not pkg_list or pkg_name in pkg_list:
                if isinstance(formatted_dependencies[pkg_name], str):
                    formatted_dependencies[pkg_name] = {"version": formatted_dependencies[pkg_name]}
                
                if isinstance(feature_value, str):
                    formatted_dependencies[pkg_name]["features"] = [feature_value]
                elif isinstance(feature_value, list):
                    formatted_dependencies[pkg_name]["features"] = feature_value
        
        if "optional" in global_attributes:
            optional_value = global_attributes["optional"]
            
            attr_pkg_key = "optional_packages"
            pkg_list = []
            ...
            if not pkg_list or pkg_name in pkg_list:
                ...
                if isinstance(optional_value, str):
                    optional_value = optional_value.lower() == "true"
                formatted_dependencies[pkg_name]["optional"] = bool(optional_value)
