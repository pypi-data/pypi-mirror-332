...
def parse_dependencies(dep_file):
    ...
    # Debug: Print raw content to see what we're working with
    ...
    while i < len(lines):
        ...
        if "=" in line:  # Look for standard Cargo dependency format with =
            parts = line.split("=", 1)
            dep_name = parts[0].strip()
            dep_value = parts[1].strip().strip('"')
            
            # Debug: Print the extracted dependency name and value
            print(f"Found dependency: {dep_name} = {dep_value}")
            
            # Check if it's a complex dependency with features
            if "{" in line and "}" in line:
                # This is a complex dependency with inline attributes like { version = "1.0", features = [...] }
                dep_dict = {}
                
                # Extract version
                if "version" in line:
                    version_parts = line.split("version")[1].split("=")[1].strip()
                    version = version_parts.split(",")[0].strip().strip('"')
                    dep_dict["version"] = version
                    print(f"  Extracted version: {version}")
                
                # Extract features if present
                if "features" in line:
                    features_part = line.split("features")[1].split("=")[1].strip()
                    if "[" in features_part and "]" in features_part:
                        features_text = features_part.split("[")[1].split("]")[0]
                        features_list = [f.strip().strip('"').strip("'") for f in features_text.split(",")]
                        dep_dict["features"] = features_list
                        print(f"  Extracted features: {features_list}")
                
                # Extract optional flag if present
                if "optional" in line:
                    optional_part = line.split("optional")[1].split("=")[1].strip()
                    optional_value = optional_part.split(",")[0].strip().lower() == "true"
                    dep_dict["optional"] = optional_value
                    print(f"  Extracted optional: {optional_value}")
                
                processed_dependencies[dep_name] = dep_dict
            else:
                # Simple dependency with just a version
                processed_dependencies[dep_name] = dep_value.strip('"')
            
            i += 1
        else:
            i += 1
    
    # Debug: Print the final processed dependencies
...
def build_project(project_dir, output_path, binary_name):
    ...
    # First check if cargo is installed and available
    try:
...
def main():
    ...
    try:
        ...
        # Only remove build directory if not in debug mode
        if not args.debug:
