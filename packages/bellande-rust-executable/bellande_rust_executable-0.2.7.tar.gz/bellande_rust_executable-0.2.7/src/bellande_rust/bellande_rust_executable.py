...
def parse_dependencies(dep_file):
    ...
    while i < len(lines):
        ...
        if "=" in line:
            ...
            if has_special_attributes and "{" in line:
                version_part = dep_value.split(",")[0].split("=")[1].strip().strip('"')
                dep_dict = {"version": version_part}
                
                if "features" in line:
                    features_part = line.split("features")[1].split("=")[1].strip()
                    if "[" in features_part and "]" in features_part:
                        features_list = features_part.split("[")[1].split("]")[0].split(",")
                        dep_dict["features"] = [f.strip().strip('"').strip("'") for f in features_list]
                    ...
                    if "=" in attr_line:
                        ...
                        if attr_name == "features":
                            if "[" in attr_value and "]" in attr_value:
                                features_list = attr_value.split("[")[1].split("]")[0].split(",")
                                dep_dict["features"] = [f.strip().strip('"').strip("'") for f in features_list]
                            else:
                                dep_dict["features"] = [attr_value.strip('"').strip("'")]
                        elif attr_name == "optional":
                            dep_dict["optional"] = attr_value.lower().strip('"') == "true"
                        else:
                            dep_dict[attr_name] = attr_value.strip('"')
        ...
        if isinstance(value, dict):
            ...
            if "optional" in value:
                if isinstance(value["optional"], bool):
                    dep_dict["optional"] = value["optional"]
                elif isinstance(value["optional"], str):
                    dep_dict["optional"] = value["optional"].lower() == "true"
            
            # Add any other attributes
            for attr, attr_value in value.items():
                if attr not in ["version", "features", "optional"]:
                    dep_dict[attr] = attr_value
            
            processed_deps[name] = dep_dict
        else:
            processed_deps[name] = value
...
def main():
    ...
    try:
        ...
        print(dependencies)
        print(update_cargo_toml_dependencies)
