...
def check_valid_ref(item_count: int) -> Callable[[Union[str, int]], int]:
  def _check_valid_ref(ref: Union[str, int]) -> int:
    ref = int(ref)
    if ref < 0:
      raise argparse.ArgumentTypeError("%s is an invalid positive int value" % ref)
    elif ref > item_count:
      raise argparse.ArgumentTypeError("%s is out of range" % ref)
    return ref
