...
class SkillSynchronizer:
    ...
    def __get_skill(self) -> None:
        ...
        req = skill_api.GetSkillRequest(
            workspaceID=self.config.skill_name.workspace_id,
            localName=self.config.skill_name.local_name,
            version=self.config.skill_version)
        skill = self.__skill_cli.get_skill(req=req)
        if hasattr(skill, 'success') and skill.success == False:
            raise Exception(f"GetSkillFailed: {skill.code}, {skill.message}")
    ...
    def __get_device_configuration(self) -> dict:
        ...
        req = device_client.GetConfigurationRequest(
            ...
            local_name="default")
        resp = {}
        resp = self.__device_cli.get_configuration(req=req)
    ...
    def run(self) -> None:
        ...
        try:
            ...
            if len(self.config.sync_model_result) == 0:
                logger.warning("SyncSkill sync_model_result len is 0, success")
                return
        ...
        for device_local_name, device in device_map.items():
...
def main() -> int:
    ...
    try:
        ...
        if not artifact_name or not (
            ...
            raise ValueError(
                f"Invalid artifact name: {args.skill_name}")
        config.skill_version = artifact_name.version
        ...
        if not skill_name or not (
            ...
            raise ValueError(f"Invalid skill name: {args.skill_name}")
