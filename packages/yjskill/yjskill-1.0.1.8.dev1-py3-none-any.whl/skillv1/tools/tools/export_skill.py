...
from skillv1.client.skill_api_skill import GetSkillRequest, parse_skill_name
from skillv1.client.skill_client import SkillClient
...
def run():
    ...
    try:
        ...
        job_name = os.getenv("JOB_NAME","workspaces/wsgdessn/jobs/job-z503psa7")
        windmill_endpoint = os.getenv("WINDMILL_ENDPOINT","0.0.0.0:8080")
        vistudio_endpoint = os.getenv("VISTUDIO_ENDPOINT","0.0.0.0:80")
        ...
        tracker.log_event(
            ...
            reason=f"技能名称(版本)：{str(skill.display_name)}(v{str(artifact_name.version)})",
        )
        tracker.log_event(
            kind=EventKind.Succeed,
        ...
        if export_kind == EXPORT_SKILL_TEMPLATE_KIND:
            upload_skill_template(job_name, skill, output_artifact_path, job_client, windmill_client)
            # 模板导出记录成功指标
            tracker.log_metric(
...
def upload_skill_template(job_name, skill, output_artifact_path, job_client, windmill_client):
    ...
    job.tags['skillDisplayName'] = skill.display_name

    filesystem = windmill_client.suggest_first_filesystem(skill.workspace_id,
                                                          guest_name=f"workspaces/{skill.workspace_id}")
    # 上传文件
