...
class SqlAlchemyDapiValidator(DapiValidator):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._current_sha = kwargs.get("commit_sha") or head_sha
        # sqlalchemy docker isn't compatible to generate files at base
        # as the docker image is from the head commit
        # HACK: needs rework
        self._skip = self._current_sha != head_sha
    ...
    def validate_projects(self, projects: List[SqlAlchemyProjectInfo]):
        ...
        if self._skip:
            # Possible that the project and artifacts may not exist at base commit
            return
