# ding_walk.walk : a python library for metabolic networks sampling and analysis
# ding_walk.walk is part of GeomScale project
...
import warnings
import math
from ding_walk.walk.MetabolicNetwork import MetabolicNetwork
from ding_walk.walk.fva import slow_fva
from ding_walk.walk.utils import (
...
try:
    import gurobipy
    from ding_walk.walk.gurobi_based_implementations import (
        fast_fba,
        fast_fva,
        fast_inner_ball,
...
class PolytopeSampler:
    ...
    def facet_redundancy_removal(self, value):
        ...
        if (not self._parameters["fast_computations"]) and value:
            warnings.warn(
                "Since you are in slow mode the redundancy removal step is skipped (ding_walk.walk does not currently support this functionality in slow mode)"
            )
