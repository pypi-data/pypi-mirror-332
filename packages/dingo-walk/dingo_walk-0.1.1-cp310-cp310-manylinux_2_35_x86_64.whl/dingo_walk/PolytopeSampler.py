...
from dingo_walk.fva import slow_fva
...
try:
    import gurobipy
    from dingo_walk.gurobi_based_implementations import (
        fast_fba,
        fast_fva,
        fast_inner_ball,
...
class PolytopeSampler:
    ...
    def facet_redundancy_removal(self, value):
        ...
        if (not self._parameters["fast_computations"]) and value:
            warnings.warn(
                "Since you are in slow mode the redundancy removal step is skipped (dingo_walk does not currently support this functionality in slow mode)"
            )
