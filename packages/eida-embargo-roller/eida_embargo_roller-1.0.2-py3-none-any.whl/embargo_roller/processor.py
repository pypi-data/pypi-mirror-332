...
class StationXMLProcessor:
    ...
    def _save_file(self, root, file, inplace):
        ElementTree.register_namespace('', 'http://www.fdsn.org/xml/station/1')  # Set default namespace
        ...
        if inplace:
            ...
            with open(path, 'wb') as f:
                root.write(f, encoding="UTF-8", xml_declaration=True)  # Ensure UTF-8 and XML declaration
            logger.info(f"File saved in place: {path}")
        else:
            # Correct the tostring usage by extracting the root element
            xml_string = ElementTree.tostring(root.getroot(), encoding="UTF-8", xml_declaration=True).decode("UTF-8")
            print(xml_string)
