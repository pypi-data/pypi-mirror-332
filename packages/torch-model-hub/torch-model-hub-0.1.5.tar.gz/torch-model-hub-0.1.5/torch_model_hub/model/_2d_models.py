...
class Base2DModel(nn.Module, ABC):
    ...
    def __init__(self, in_features: int, embed_dim: int, hidden_size: int):
        ...
        if embed_dim <= 1:
            ...
            assert hidden_size % embed_dim == 0
            self.reshape = nn.Sequential(
                nn.Linear(in_features, hidden_size), nn.Unflatten(1, (-1, embed_dim))
            )
        self.model = None
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        output = self.reshape(
            inputs
        )  # [(batch_size, hidden_size / embed_dim, embed_dim)]]
...
class MultiheadSelfAttention2D(nn.Module):
    def __init__(
        ...
        self.att = nn.MultiheadAttention(
            embed_dim, num_heads, dropout, bias=bias, batch_first=True
        )
