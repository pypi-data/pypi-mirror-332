...
def str_diff(str1: str, str2: str) -> str:
    """
    Generates a character-level diff between two strings.
    ...
    Returns:
        A string representing the diff, or None if the strings are identical.
    """
    differ = difflib.Differ()
    diff = differ.compare(str1.splitlines(keepends=True), str2.splitlines(keepends=True))
    diff_str = ''.join(diff)
    if not any(line.startswith(('+', '-', '?')) for line in diff_str.splitlines()): #check for changes
        return None
    return diff_str

def str_diff(str1: str, str2: str) -> str:
    """
    Generates a human-readable diff between two strings.
    ...
    Returns:
        A string representing the diff, or None if the strings are identical.
    """
    diff = difflib.unified_diff(str1.splitlines(keepends=True), str2.splitlines(keepends=True), fromfile='string1', tofile='string2')
    diff_str = ''.join(diff)
    if not diff_str:
        return ""
    return diff_str
...
def _update_function_section(function_file_path: str, schema: Dict[str, Any]):
    ...
    # Output vars needs special handling
    output_vars_section = sections_to_update['output_vars']
    start_marker = output_vars_section['start_marker']
    end_marker = output_vars_section['end_marker']
    ...
    if start_index != -1 and end_index != -1:
        ...
        existing_output_section = file_content[start_replace_index:end_index].strip()
        existing_output_content = ""
        if existing_output_section.startswith('return '):
            existing_output_content = existing_output_section[len('return '):].strip()
        else:
            existing_output_content = existing_output_section

        updated_output_content_items = []
        existing_keys = set()

        if existing_output_content and existing_output_content != '{}':
            content_without_braces = existing_output_content.strip()[1:-1].strip()
            if content_without_braces: # check if not empty after removing braces
                output_pairs = content_without_braces.split(',')
                for pair in output_pairs:
                    pair = pair.strip()
                    if ':' in pair:
                        parts = pair.split(':', 1) # split only once at the first ':'
                        key = parts[0].strip()
                        value = parts[1].strip()
                        existing_keys.add(key.strip())
                        # Keep existing key-value pairs initially
                        updated_output_content_items.append(f"{key}: {value}")

        final_output_content_items = []
        final_existing_keys = set() # To track keys we are keeping

        for item_str in updated_output_content_items:
            key = item_str.split(':', 1)[0].strip()
            if key in required_outputs_step:
                final_output_content_items.append(item_str)
                final_existing_keys.add(key)

        for output_name in required_outputs_step:
            if output_name not in final_existing_keys:
                final_output_content_items.append(f"{output_name}: ''")

        updated_output_str = "{" + ", ".join(final_output_content_items) + "}" if final_output_content_items else "{}"
        updated_output_content = f"    return {updated_output_str}"


        sections_to_update['output_vars']['new_content'] = updated_output_content
