...
def BreadcrumbContainer(*li, # List items for the breadcrumb (usually `Li` with `A` or `Span` inside)
                        cls='', # Additional classes on the breadcrumb
                        aria_label='Breadcrumb', # Accessibility label for the breadcrumb navigation
                        **kwargs # Additional args for breadcrumb navigation
                       )->FT: # Nav containing a breadcrumb Ul
    "Creates a breadcrumb navigation container following FrankenUI's pattern"
    return fh.Nav(
        Ul(*li, cls=('uk-breadcrumb', stringify(cls))),
...
def BreadcrumbItem(content, # Content for the breadcrumb item (text or component)
                  href=None, # Link URL (if None, creates a non-clickable item with Span)
                  cls='', # Additional classes on the item
                  current=False, # Whether this item represents the current page
                  disabled=False, # Whether this item is disabled
                  **kwargs # Additional args for the Li tag
                 )->FT: # Li containing either an A link or a Span
    "Creates a breadcrumb item, either as a link (with href) or text (without href)"
    cls = stringify(cls)
    if disabled:
        cls = stringify((cls, 'uk-disabled'))
        href = None
    
    if href is not None:
        # Create a link item
        item = fh.A(content, href=href, aria_current="page" if current else None)
    else:
        # Create a text item
        item = Span(content)
    
    return Li(item, cls=cls, **kwargs)

# %% ../nbs/02_franken.ipynb 118
def Breadcrumb(*items, # Items to include in the breadcrumb
              cls='', # Additional classes on the breadcrumb
              container_cls='', # Additional classes on the container
              aria_label='Breadcrumb', # Accessibility label for breadcrumb
              make_last_current=True, # Whether to automatically mark the last item as current
              **kwargs # Additional args for the container
             )->FT: # Complete breadcrumb navigation component
    "Creates a complete breadcrumb navigation with provided items"
    if not items:
        return BreadcrumbContainer(cls=cls, aria_label=aria_label, **kwargs)
    ...
    for i, item in enumerate(items):
        ...
        if isinstance(item, tuple) and len(item) >= 2:
            # Format: (content, href, kwargs)
            content = item[0]
            href = item[1]
            item_kwargs = item[2] if len(item) > 2 and isinstance(item[2], dict) else {}
            breadcrumb_items.append(BreadcrumbItem(content, href=href, current=is_current, **item_kwargs))
            ...
            # Format: simple string or component
