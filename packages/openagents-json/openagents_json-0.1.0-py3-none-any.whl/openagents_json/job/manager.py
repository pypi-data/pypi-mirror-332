...
class JobManager:
        ...
        if job_store is None:
            ...
            if store_type == "memory":
                ...
                job_store = FileJobStore(
                    storage_dir=settings.job.store_path
                )
                ...
                job_store = SQLAlchemyJobStore(
                    ...
                    password=settings.storage.database.password.get_secret_value() if settings.storage.database.password else None,
                    database=settings.storage.database.name,
                    path=settings.storage.database.path
                )
            else:
                logger.warning(f"Unknown job store type: {store_type}, defaulting to memory")
                job_store = MemoryJobStore()
        
        self.job_store = job_store
        self.retention_days = retention_days if retention_days is not None else settings.job.job_retention_days
        self.max_concurrent_jobs = max_concurrent_jobs if max_concurrent_jobs is not None else settings.job.max_concurrent_jobs
    ...
    def start_job(self, job_id: str) -> bool:
        ...
        if job.has_dependencies():
            ...
            for dep_job in dependencies:
                if dep_job.status != JobStatus.COMPLETED:
                    logger.info(f"Cannot start job {job_id}: Dependency {dep_job.job_id} is not completed")
                    return False
        ...
        if not self.distributed_mode:
            if len(self.running_jobs) >= self.max_concurrent_jobs:
                logger.info(f"Cannot start job {job_id}: Maximum concurrent jobs reached")
    ...
    def resume_job(self, job_id: str) -> bool:
        ...
        if job.status != JobStatus.PAUSED:
            logger.warning(f"Cannot resume job {job_id}: Job is not paused (status: {job.status})")
            return False
            ...
            if job.timeout and job.timeout > 0:
                ...
                try:
                    ...
                    if job.timeout and elapsed_time > job.timeout:
                        timed_out = True
                        raise TimeoutError(f"Job {job_id} timed out after {elapsed_time} seconds")
                    ...
                    timed_out = True
                    raise TimeoutError(f"Job {job_id} timed out after {job.timeout} seconds")
            else:
        ...
        if job.status == JobStatus.RUNNING:
            if error:
                ...
                if job.max_retries > 0 and job.retry_count < job.max_retries:
                    logger.info(f"Job {job_id} failed, retrying ({job.retry_count + 1}/{job.max_retries})")
                    job.retry_count += 1
                    self.job_store.update_job(job)
    ...
    async def _retry_job_after_delay(self, job_id: str, delay_seconds: float) -> None:
        ...
        """
        logger.debug(f"Scheduling retry for job {job_id} after {delay_seconds:.2f} seconds")
    ...
    def get_dependencies(self, job_id: str) -> List[Job]:
        ...
        # Fallback implementation
        return [self.job_store.get(dep_id) for dep_id in job.dependencies if self.job_store.get(dep_id)]
