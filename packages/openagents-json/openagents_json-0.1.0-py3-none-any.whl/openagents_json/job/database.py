...
from sqlalchemy import create_engine, Column, String, Float, Integer, JSON, Text, Enum, ForeignKey, Table
from sqlalchemy.ext.declarative import declarative_base
from sqlalchemy.orm import sessionmaker, relationship, scoped_session
from sqlalchemy.pool import QueuePool, NullPool

from openagents_json.job.model import JobStatus, JobPriority, Job
...
class JobModel(Base):
    ...
    def to_job(self) -> Job:
        ...
        job = Job(
            ...
            created_at=datetime.fromtimestamp(self.created_at) if self.created_at else None,
            updated_at=datetime.fromtimestamp(self.updated_at) if self.updated_at else None,
            started_at=datetime.fromtimestamp(self.started_at) if self.started_at else None,
    ...
    def from_job(cls, job: Job) -> 'JobModel':
        ...
        model = cls(
            ...
            created_at=job.created_at.timestamp() if job.created_at else datetime.now().timestamp(),
            updated_at=job.updated_at.timestamp() if job.updated_at else datetime.now().timestamp(),
...
def get_engine(
    ...
    if pooling:
        if 'sqlite' in connection_string:
            ...
            logging.info(f"Creating database engine with connection pooling (pool_size={pool_size}, max_overflow={max_overflow})")
            return create_engine(
                connection_string,
                poolclass=QueuePool,
                pool_size=pool_size,
...
def init_db(engine, create_tables=True):
    ...
    try:
        with engine.connect() as connection:
            ...
            connection.execute("CREATE INDEX IF NOT EXISTS idx_jobs_priority ON jobs (priority)")
            connection.execute("CREATE INDEX IF NOT EXISTS idx_jobs_batch_id ON jobs (batch_id)")
