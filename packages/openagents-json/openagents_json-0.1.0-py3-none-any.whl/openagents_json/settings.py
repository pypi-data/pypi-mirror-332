...
class JobSettings(BaseSettings):
    ...
    # Storage settings - will be used for backward compatibility
    ...
    def validate_db_configuration(self) -> "DatabaseSettings":
        ...
        if self.dialect in ("postgresql", "mysql") and not self.connection_string:
            if not (self.host and self.name):
                raise ValueError(f"Either connection_string or host and name must be provided for {self.dialect}")
...
class StorageSettings(BaseSettings):
    ...
    storage_type: Literal["memory", "file", "redis", "postgres", "sqlite", "mysql"] = Field(
        default="memory",
    ...
    def validate_storage_configuration(self) -> "StorageSettings":
        ...
        if self.storage_type == "file" and not self.file_storage_path:
            raise ValueError("file_storage_path must be set when storage_type is 'file'")
...
class Settings(BaseSettings):
    ...
    APP_NAME: str = Field(
        default=None,
        description="DEPRECATED: Use app.app_name instead"
    )
    DEBUG: bool = Field(
        default=None,
        description="DEPRECATED: Use app.debug instead"
    )
    API_PREFIX: str = Field(
        default=None,
        description="DEPRECATED: Use app.api_prefix instead"
    )
    REGISTRY_AUTO_DISCOVER: bool = Field(
        default=None,
        description="DEPRECATED: Use registry.auto_discover instead"
    )
    REGISTRY_MODULES: List[str] = Field(
        default=None,
        description="DEPRECATED: Use registry.modules instead"
    )
    WORKFLOW_VALIDATION_MODE: str = Field(
        default=None,
        description="DEPRECATED: Use workflow.validation_mode instead"
    )
    JOB_STORE_TYPE: str = Field(
        default=None,
        description="DEPRECATED: Use job.store_type instead"
    )
    JOB_STORE_FILE_DIR: str = Field(
        default=None,
        description="DEPRECATED: Use job.store_path instead"
    )
    JOB_RETENTION_DAYS: int = Field(
        default=None,
        description="DEPRECATED: Use job.job_retention_days instead"
    )
    DB_DIALECT: str = Field(
        default=None,
        description="DEPRECATED: Use storage.database.dialect instead"
    )
    DB_PATH: str = Field(
        default=None,
        description="DEPRECATED: Use storage.database.path instead"
    )
    DB_HOST: str = Field(
        default=None,
        description="DEPRECATED: Use storage.database.host instead"
    )
    DB_PORT: int = Field(
        default=None,
        description="DEPRECATED: Use storage.database.port instead"
    )
    DB_USERNAME: str = Field(
        default=None,
        description="DEPRECATED: Use storage.database.username instead"
    )
    DB_PASSWORD: str = Field(
        default=None,
        description="DEPRECATED: Use storage.database.password instead"
    )
    DB_NAME: str = Field(
        default=None,
        description="DEPRECATED: Use storage.database.name instead"
    )
    ...
    version: str = Field(
        default="0.1.0",
        description="OpenAgents JSON version."
    )
    ...
    def sync_legacy_fields(self) -> "Settings":
        ...
        elif self.app.app_name != "OpenAgents JSON" and self.APP_NAME != self.app.app_name:
        ...
        if self.REGISTRY_AUTO_DISCOVER is not None and self.registry.auto_discover != self.REGISTRY_AUTO_DISCOVER:
            self.registry.auto_discover = self.REGISTRY_AUTO_DISCOVER
        elif self.registry.auto_discover is not True and self.REGISTRY_AUTO_DISCOVER != self.registry.auto_discover:
            self.REGISTRY_AUTO_DISCOVER = self.registry.auto_discover
            
        if self.REGISTRY_MODULES is not None and self.registry.modules != self.REGISTRY_MODULES:
        ...
        if self.WORKFLOW_VALIDATION_MODE is not None and self.workflow.validation_mode != self.WORKFLOW_VALIDATION_MODE:
            self.workflow.validation_mode = self.WORKFLOW_VALIDATION_MODE
        elif self.workflow.validation_mode != "strict" and self.WORKFLOW_VALIDATION_MODE != self.workflow.validation_mode:
            self.WORKFLOW_VALIDATION_MODE = self.workflow.validation_mode
            
        # Two-way sync for job settings
        if self.JOB_STORE_TYPE is not None and self.job.store_type != self.JOB_STORE_TYPE:
            self.job.store_type = self.JOB_STORE_TYPE
        elif self.job.store_type != "memory" and self.JOB_STORE_TYPE != self.job.store_type:
            self.JOB_STORE_TYPE = self.job.store_type
            
        if self.JOB_STORE_FILE_DIR is not None and self.job.store_path != self.JOB_STORE_FILE_DIR:
        ...
        if self.JOB_RETENTION_DAYS is not None and self.job.job_retention_days != self.JOB_RETENTION_DAYS:
            self.job.job_retention_days = self.JOB_RETENTION_DAYS
        elif self.job.job_retention_days != 30 and self.JOB_RETENTION_DAYS != self.job.job_retention_days:
            self.JOB_RETENTION_DAYS = self.job.job_retention_days
            
        # Two-way sync for database settings
        if self.DB_DIALECT is not None and self.storage.database.dialect != self.DB_DIALECT:
            self.storage.database.dialect = self.DB_DIALECT
        elif self.storage.database.dialect != "sqlite" and self.DB_DIALECT != self.storage.database.dialect:
        ...
        elif self.storage.database.path != "./openagents.db" and self.DB_PATH != self.storage.database.path:
        ...
        elif self.storage.database.host != "localhost" and self.DB_HOST != self.storage.database.host:
        ...
        elif self.storage.database.port is not None and self.DB_PORT != self.storage.database.port:
            self.DB_PORT = self.storage.database.port
            
        if self.DB_USERNAME is not None and self.storage.database.username != self.DB_USERNAME:
            self.storage.database.username = self.DB_USERNAME
        elif self.storage.database.username and self.DB_USERNAME != self.storage.database.username:
        ...
        elif self.storage.database.name != "openagents" and self.DB_NAME != self.storage.database.name:
            self.DB_NAME = self.storage.database.name
            
        if self.DB_CONNECTION_STRING is not None and self.storage.database.connection_string != self.DB_CONNECTION_STRING:
            self.storage.database.connection_string = self.DB_CONNECTION_STRING
        elif self.storage.database.connection_string and self.DB_CONNECTION_STRING != self.storage.database.connection_string:
