...
from fastapi import APIRouter, Depends, Query, Body, Path, HTTPException, Request
from pydantic import BaseModel, Field

from openagents_json.core.app import OpenAgentsApp
from openagents_json.workflow import Workflow, Step, Connection, Parameter
...
class JobRequest(BaseModel):
    ...
    inputs: Dict[str, Any] = Field(default_factory=dict, description="Input values for the workflow")
...
class JobResponse(BaseModel):
    ...
    outputs: Optional[Dict[str, Any]] = Field(None, description="Output values (if completed)")
...
class WorkflowResponse(BaseModel):
    ...
    connections: List[Dict[str, Any]] = Field(..., description="Connections between steps")
    inputs: List[Dict[str, Any]] = Field(..., description="Input parameters")
    outputs: List[Dict[str, Any]] = Field(..., description="Output parameters")
    metadata: Optional[Dict[str, Any]] = Field(None, description="Metadata about the workflow")
...
def create_api_router(openagents_app: OpenAgentsApp) -> APIRouter:
    ...
    async def get_workflows(
        ...
        category: Optional[str] = Query(None, description="Filter workflows by category")
    ) -> List[Dict[str, Any]]:
    ...
    async def create_workflow(
        ...
        try:
            if app.register_workflow(workflow):
                ...
                raise HTTPException(status_code=400, detail="Failed to register workflow")
