...
from openagents_json.job.database import (
    JobModel, TagModel, get_engine, get_session_factory, 
    build_connection_string, init_db, Base
)
...
class JobStore(abc.ABC):
    ...
    def update_worker_heartbeat(self, worker_id: str, last_heartbeat: str, running_jobs: List[str]) -> bool:
        """
    ...
    def claim_jobs_for_worker(self, worker_id: str, batch_size: int, tags: Optional[List[str]] = None) -> List[Job]:
        """
    ...
    def list(
        ...
        with self.lock:
            ...
            if workflow_id is not None:
                filtered_jobs = [job for job in filtered_jobs if job.workflow_id == workflow_id]
            ...
            filtered_jobs.sort(
                key=lambda job: getattr(job, sort_by, job.created_at),
                reverse=reverse
            )
...
class FileJobStore(JobStore):
    ...
    Stores jobs as JSON files in a directory structure. Each job is saved
    as a separate file named by its ID. Provides persistence across
...
class SQLAlchemyJobStore(JobStore):
    """
    SQLAlchemy-based implementation of JobStore.
    
    Stores jobs in a SQL database using SQLAlchemy ORM, with support for
    SQLite, PostgreSQL, and MySQL backends. Suitable for production use with
    ...
    def __init__(
        ...
        """
        Initialize SQLAlchemy job store.
        ...
        try:
            ...
            logging.info(f"Initialized SQLAlchemyJobStore with {dialect} backend")
        except Exception as e:
            logging.error(f"Failed to initialize SQLAlchemyJobStore: {str(e)}")
            raise
    ...
    def close(self):
        ...
        if hasattr(self, 'engine'):
            self.engine.dispose()
            logging.info("SQLAlchemyJobStore closed and resources released")
        ...
        try:
            ...
            if existing_job:
                ...
                for attr, value in vars(job_model).items():
                    if attr != '_sa_instance_state' and attr != 'tags' and attr != 'dependencies':
    ...
    def cleanup_old_jobs(self, days: int) -> int:
        ...
        try:
            ...
            # Find jobs to delete
            jobs_to_delete = session.query(JobModel).filter(JobModel.created_at < cutoff_date).all()
    ...
    def add_job_dependency(self, job_id: str, depends_on_id: str) -> bool:
        ...
        try:
            ...
            session.rollback()
            logging.error(f"Error adding dependency {job_id} -> {depends_on_id}: {str(e)}")
            return False
        finally:
            session.close()
    ...
    def remove_job_dependency(self, job_id: str, depends_on_id: str) -> bool:
        ...
        try:
            ...
            session.rollback()
            logging.error(f"Error removing dependency {job_id} -> {depends_on_id}: {str(e)}")
            return False
        finally:
            session.close()
    ...
    def register_worker(self, worker_info: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            existing_worker = session.query(WorkerModel).filter_by(worker_id=worker_id).first()
            ...
            session.rollback()
            logging.error(f"Error registering worker {worker_info.get('worker_id')}: {str(e)}")
            return False
        finally:
            session.close()
    ...
    def claim_jobs_for_worker(self, worker_id: str, batch_size: int, tags: Optional[List[str]] = None) -> List[Job]:
        ...
        try:
            ...
            if tags and len(tags) > 0:
                from openagents_json.job.database import job_tags, TagModel
                tag_subquery = session.query(TagModel.id).filter(TagModel.name.in_(tags)).subquery()
            ...
            for job_model in candidate_jobs:
                ...
                if job_model.dependencies:
                    ...
                    dependencies_satisfied = all(dep.status == JobStatus.COMPLETED for dep in dependencies)
