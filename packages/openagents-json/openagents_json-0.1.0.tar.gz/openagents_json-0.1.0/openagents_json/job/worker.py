...
class Worker:
    ...
    async def register(self) -> bool:
        ...
        try:
            if not hasattr(self.job_store, 'register_worker'):
                logger.error(f"Job store {self.job_store.__class__.__name__} does not support worker registration")
                return False
    ...
    async def heartbeat(self) -> bool:
        ...
        try:
            if not hasattr(self.job_store, 'update_worker_heartbeat'):
                logger.error(f"Job store {self.job_store.__class__.__name__} does not support worker heartbeats")
                return False
                
            success = await self.job_store.update_worker_heartbeat(self.worker_id, heartbeat_data)
            ...
            logger.error(f"Error sending heartbeat for worker {self.worker_id}: {str(e)}")
            return False
    ...
    async def claim_jobs(self) -> List[Job]:
        ...
        try:
            if not hasattr(self.job_store, 'claim_jobs'):
                logger.error(f"Job store {self.job_store.__class__.__name__} does not support claiming jobs")
                return []
    ...
    async def execute_job(self, job: Job) -> None:
        ...
        """
        if not self.running:
            logger.warning(f"Worker {self.worker_id} not running, cannot execute job {job.job_id}")
            return
        ...
        try:
            ...
            if self.executor:
                ...
                for i in range(10):
                    ...
                    if not self.running:
                        logger.warning(f"Worker {self.worker_id} stopped during job {job.job_id} execution")
            ...
            try:
                ...
                logger.error(f"Failed to update job {job.job_id} final status: {str(e)}")
    ...
    async def stop(self) -> None:
        ...
        try:
            ...
            logger.error(f"Error updating worker {self.worker_id} status to offline: {str(e)}")
...
class WorkerManager:
    ...
    def __init__(
        ...
        logger.info(f"WorkerManager initialized with heartbeat timeout {heartbeat_timeout}s")
    ...
    async def _check_workers(self) -> None:
        ...
        if not hasattr(self.job_store, 'get_workers'):
            logger.error(f"Job store {self.job_store.__class__.__name__} does not support worker management")
            return
        ...
        for worker in workers:
            ...
            try:
                ...
                if time_since_heartbeat > self.heartbeat_timeout:
                    logger.warning(f"Worker {worker_id} appears to be dead (no heartbeat for {time_since_heartbeat:.1f}s)")
    ...
    async def _handle_dead_worker(self, worker: Dict[str, Any]) -> None:
        ...
        try:
            if hasattr(self.job_store, 'release_worker_jobs'):
                ...
                logger.info(f"Released {released_count} jobs from dead worker {worker_id}")
        except Exception as e:
