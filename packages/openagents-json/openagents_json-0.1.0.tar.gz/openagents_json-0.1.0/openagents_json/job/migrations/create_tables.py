...
from openagents_json.job.database import create_engine, Base
...
def create_tables(connection_string=None, dialect=None, host=None, port=None, 
                 username=None, password=None, database=None, path=None):
    """
    ...
    try:
        ...
        # Create all tables
        logger.info(f"Creating database tables for dialect: {dialect or 'from connection string'}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Create database tables for the job store")
    
    # Connection options
    parser.add_argument("--connection-string", help="SQLAlchemy connection string")
    parser.add_argument("--dialect", choices=["sqlite", "postgresql", "mysql"], 
