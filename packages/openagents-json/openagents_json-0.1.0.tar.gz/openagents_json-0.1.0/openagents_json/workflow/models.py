...
class RetryConfig(BaseModel):
    ...
    max_attempts: int = Field(default=3, description="Maximum number of retry attempts", ge=0)
    delay_seconds: int = Field(default=5, description="Delay between retries in seconds", ge=0)
    backoff_multiplier: float = Field(default=1.0, description="Multiplier for exponential backoff", ge=1.0)
...
class Parameter(BaseModel):
    ...
    enum: Optional[List[Any]] = Field(default=None, description="Enumeration of allowed values")
    pattern: Optional[str] = Field(default=None, description="Regex pattern for string validation")
    minimum: Optional[float] = Field(default=None, description="Minimum value for number validation")
    maximum: Optional[float] = Field(default=None, description="Maximum value for number validation")
...
class WorkflowMetadataModel(BaseModel):
    ...
    custom: Optional[Dict[str, Any]] = Field(default=None, description="Custom metadata fields")
...
class Step(BaseModel):
    ...
    name: Optional[str] = Field(default=None, description="Human-readable name for the step")
    description: Optional[str] = Field(default=None, description="Detailed description of the step")
    inputs: Dict[str, str] = Field(default_factory=dict, description="Input mappings for the step")
    outputs: Dict[str, str] = Field(default_factory=dict, description="Output mappings from the step")
    condition: Optional[str] = Field(default=None, description="Condition for step execution")
    retry: Optional[RetryConfig] = Field(default=None, description="Retry configuration for the step")
    timeout: Optional[int] = Field(default=None, description="Timeout for step execution in seconds", ge=0)
    ...
    def validate_id(cls, v):
        ...
        if not v.replace("_", "").replace("-", "").isalnum():
            raise ValueError("Step ID can only contain alphanumeric characters, underscores, and hyphens")
        return v
...
class Connection(BaseModel):
    ...
    condition: Optional[str] = Field(default=None, description="Condition for connection activation")
    transform: Optional[str] = Field(default=None, description="Transformation to apply to the data")
...
class Workflow(BaseModel):
    ...
    version: str = Field(default="1.0.0", description="Semantic version of the workflow")
    name: Optional[str] = Field(default=None, description="Human-readable name for the workflow")
    description: Optional[str] = Field(default=None, description="Detailed description of the workflow")
    steps: List[Step] = Field(description="Sequential steps in the workflow")
    connections: List[Connection] = Field(default_factory=list, description="Connections between steps")
    inputs: Dict[str, Parameter] = Field(default_factory=dict, description="Input definition for the workflow")
    outputs: Dict[str, Parameter] = Field(default_factory=dict, description="Output definition for the workflow")
    metadata: Optional[WorkflowMetadataModel] = Field(default=None, description="Additional metadata for the workflow")
    ...
    def validate_id(cls, v):
        ...
        if not v.replace("_", "").replace("-", "").isalnum():
            raise ValueError("Workflow ID can only contain alphanumeric characters, underscores, and hyphens")
        return v
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Workflow":
        ...
        if "steps" in data and isinstance(data["steps"], list):
            for i, step in enumerate(data["steps"]):
                if isinstance(step, dict) and "retry" in step and isinstance(step["retry"], dict):
