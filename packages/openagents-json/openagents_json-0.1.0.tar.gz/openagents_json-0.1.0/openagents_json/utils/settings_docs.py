    ...
    for section_name, section_docs in docs.items():
        ...
        for field_name, field_info in section_docs["fields"].items():
            ...
            if "fields" in field_info:
                ...
                md_lines.append(f"| **{field_name}** | {field_info['type']} | - | {field_info['description']} |")
                
                # Add nested fields with dot notation
                for sub_name, sub_info in field_info["fields"].items():
                    default_val = sub_info["default"] if sub_info["default"] not in (None, "None") else "-"
                    md_lines.append(f"| {field_name}.{sub_name} | {sub_info['type']} | {default_val} | {sub_info['description']} |")
            else:
                default_val = field_info["default"] if field_info["default"] not in (None, "None") else "-"
                md_lines.append(f"| {field_name} | {field_info['type']} | {default_val} | {field_info['description']} |")
