...
from openagents_json.core.state.decorators import StatefulBase, stateful
...
class BaseAgent(StatefulBase, abc.ABC):
    """
    ...
    def __init__(self, name: str, state_manager: Optional[StateManager] = None, **kwargs):
        """
        ...
        """
        # Initialize StatefulBase first
        StatefulBase.__init__(self, state_manager)
    ...
    @stateful(persistent=True)
    def state(self) -> AgentState:
        """Get the agent's state object."""
        return AgentState(
    ...
    async def execute(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Always save state after execution
            await self.save_execution_state()
    
    async def prepare_state(self) -> None:
        """Prepare state before capability execution."""
        # Default implementation loads state
        self.load_state()
    
    async def save_execution_state(self) -> None:
        """Save state after capability execution."""
        # Save all stateful properties
        self.save_state()
    ...
    def register_capability(
        ...
        Args:
            ...
            func: Function that implements the capability
            description: Description of the capability
            inputs: Dictionary describing the inputs
            outputs: Dictionary describing the outputs
        """
    ...
    def update_state(self, **kwargs) -> None:
        ...
        """
        state = self.state  # This uses the stateful property getter
        ...
        self.state = state  # This uses the stateful property setter
    ...
    def get_state_snapshot(self) -> Dict[str, Any]:
        ...
        """
        # Get base snapshot from StatefulBase
    ...
    def restore_from_snapshot(self, snapshot: Dict[str, Any]) -> None:
        ...
        """
        # Restore agent-specific information
        if "agent_id" in snapshot:
            self.agent_id = snapshot["agent_id"]
            
        # Restore base state from StatefulBase
        super().restore_from_snapshot(snapshot)
    
    async def cleanup(self) -> None:
        """Clean up agent resources."""
        # Ensure state is saved before cleanup
        await self.save_execution_state()
...
class LLMAgent(BaseAgent):
    ...
    def __init__(self, name: str, model: str, state_manager: Optional[StateManager] = None, **kwargs):
        """
        ...
        Args:
            ...
            **kwargs: Additional configuration parameters
        """
        super().__init__(name, state_manager, **kwargs)
        self.model = model
        self.prompts = {}
