...
class Tool:
    ...
    def __init__(
        ...
        for param_name, param_info in self.metadata.input_schema.items():
            # Use deserialize_type instead of eval() for safe type deserialization
            try:
                field_type = deserialize_type(param_info["type"]) if isinstance(param_info["type"], str) else param_info["type"]
                ...
                logger.warning(f"Failed to resolve type for parameter {param_name}: {e}. Defaulting to str.")
                input_fields[param_name] = (str, param_info.get("default", ... if param_info.get("required", True) else None))
