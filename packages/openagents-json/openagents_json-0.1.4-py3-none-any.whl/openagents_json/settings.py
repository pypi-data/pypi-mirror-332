...
class RegistrySettings(BaseSettings):
    ...
    modules: List[str] = Field(
        default_factory=lambda: ["openagents_json.agent"],
...
class WorkflowSettings(BaseSettings):
    ...
    validation_mode: Literal["strict", "lenient", "none"] = Field(
        default="strict",
        description="Validation mode for workflow schemas",
    )
...
class AgentSettings(BaseSettings):
    ...
    max_retries: int = Field(
        default=3,
        description="Default maximum number of retries for failed agent operations.",
        ge=0,
    )

    retry_delay_seconds: int = Field(
        default=1,
        description="Default delay in seconds between agent operation retries.",
        ge=0,
    )
...
class JobSettings(BaseSettings):
    ...
    store_path: Optional[str] = Field(
        default="~/.openagents/jobs",
        description="Path for file-based storage (only used with store_type='file')",
    )

    # Multiprocessing settings
    use_multiprocessing: bool = Field(
        default=True,
        description="Whether to use multiprocessing for job execution. Set to False to disable multiprocessing.",
    )
    ...
    def validate_store_type(cls, v):
        ...
        if v not in valid_types:
            raise ValueError(f"Job store_type must be one of: {', '.join(valid_types)}")
        return v
...
class DatabaseSettings(BaseSettings):
    ...
    def validate_db_configuration(self) -> "DatabaseSettings":
        ...
        # Nothing to check if connection_string is provided
        ...
        if not self.host:
            raise ValueError("Database host is required for PostgreSQL/MySQL")
        if not self.port:
...
class StorageSettings(BaseSettings):
    ...
    def validate_storage_configuration(self) -> "StorageSettings":
        ...
        if self.storage_type == "file" and not self.file_storage_path:
            # Default to ~/.openagents/storage if not specified
            self.file_storage_path = Path.home() / ".openagents" / "storage"
            return self
...
class StateSettings(BaseSettings):
    ...
    manager: Literal["memory", "file", "database", "redis"] = Field(
        default="memory",
        description="""
        State manager type. Options:
        - 'memory': Volatile in-memory storage (fastest, no persistence)
        - 'file': JSON file-based storage (good for development/simple deployments)
        - 'database': SQL database storage via SQLAlchemy (recommended for production)
        - 'redis': Redis-based storage (good for distributed setups)
        """,
    )
    
    file_path: str = Field(
        default="~/.openagents/state",
        description="Path for file-based state storage (only used with manager='file')",
    )
    
    database_url: Optional[str] = Field(
        default=None,
        description="Database URL for database-based state storage (only used with manager='database')",
    )
    
    redis_url: Optional[str] = Field(
        default=None,
        description="Redis URL for Redis-based state storage (only used with manager='redis')",
    )
    
    serializer: Literal["json", "pickle", "msgpack"] = Field(
        default="json",
        description="""
        Serializer to use for state storage. Options:
        - 'json': Human-readable JSON (good for debugging, limited types)
        - 'pickle': Python pickle (supports all types, Python-only)
        - 'msgpack': MessagePack (compact binary format, good performance)
        """,
    )
    
    compression: bool = Field(
        default=False,
        description="Whether to compress state data (useful for large state objects)",
    )
    
    cache_enabled: bool = Field(
        default=True,
        description="Whether to cache state values in memory for performance",
    )
    
    transaction_mode: Literal["none", "optimistic", "pessimistic"] = Field(
        default="optimistic",
        description="""
        Transaction mode for concurrent access. Options:
        - 'none': No transaction support (fastest, but unsafe for concurrent access)
        - 'optimistic': Optimistic locking (good performance, detects conflicts)
        - 'pessimistic': Pessimistic locking (safest, but slower performance)
        """,
    )
    
    encryption_key: Optional[SecretStr] = Field(
        default=None,
        description="Encryption key for sensitive state data. If not provided, data is stored unencrypted.",
    )
    
    @model_validator(mode="after")
    def validate_state_configuration(self) -> "StateSettings":
        """Validate state configuration."""
        if self.manager == "database" and not self.database_url:
            # Try to build database URL from database settings
            from openagents_json.settings import settings
            
            try:
                if settings.storage.database.connection_string:
                    self.database_url = settings.storage.database.connection_string
                    return self
                    
                dialect = settings.storage.database.dialect
                if dialect == "sqlite":
                    path = settings.storage.database.path or "./openagents_state.db"
                    ...
                    password = settings.storage.database.password.get_secret_value() if settings.storage.database.password else ""
                    db_name = settings.storage.database.name
                    
                    self.database_url = f"{dialect}://{username}:{password}@{host}:{port}/{db_name}"
            except (AttributeError, ImportError):
                # Default to SQLite if database settings not available
                self.database_url = "sqlite:///./openagents_state.db"
                
        if self.manager == "redis" and not self.redis_url:
            # Default to localhost if not specified
