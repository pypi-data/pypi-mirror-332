from django.test import TestCase, Client, RequestFactory, modify_settings
...
from apollo.models import Form, FormFieldTemplate, ExternalWebhook, FormSubmission
from apollo.settings import apollo_settings, reload_apollo_settings
from apollo.api.v1.serializers import FormSerializer, FormFieldSerializer, FormSubmissionSerializer
...
class TestCreateFormSubmission(APITestCase):
    ...
    # TODO (steinbachr): This is failing because for some reason, we can't get the apollo settings to reload in this test with the new Django settings (for an override of the serializer)
    # @factory.django.mute_signals(signals.form_submission_cleaned)
    # def test_form_submission_serializer_override(self):
    #     from crm.models import Contact
    #
    #     # need some specialized fields for this
    #     FormFieldFactory(form=self.form, template=FormFieldTemplateFactory(name='first_name'))
    #     FormFieldFactory(form=self.form, template=FormFieldTemplateFactory(name='last_name'))
    #     contact_raw_data = {
    #         'form': self.form.id,
    #         'raw_data': [
    #             {
    ...
    #         ]
    #     }
    #     contact = Contact.objects.create(first_name=contact_raw_data['raw_data'][0]['value'])
    #
    #     with self.settings(APOLLO={'DEFAULT_FORM_SUBMISSION_SERIALIZER': 'crm.serializers.CRMFormSubmissionSerializer'}):
    #         resp = self.post('/submissions', contact_raw_data)
    #
    #     resp_json = resp.json()
    #     submission = FormSubmission.objects.get(id=resp_json['id'])
    #
    #     self.assertEqual(resp.status_code, 201)
    #     self.assertEqual(submission.cleaned_data['first_name'], 'Bob')
    #     self.assertEqual(resp_json['contact_id'], contact.id)
