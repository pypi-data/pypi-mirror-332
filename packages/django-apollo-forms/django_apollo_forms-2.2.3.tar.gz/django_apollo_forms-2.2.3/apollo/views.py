...
class DownloadFormSubmissions(TemplateView):
    def get(self, request, *args, **kwargs):
        ...
        submissions = form.submissions.filter(cleaned_data__isnull=False)
        ...
        with open(f.name, 'w') as csvfile:
            # we need to create the set of all field names across all submissions to avoid situations where the form
            # has changed which fields are included and it results in the header not including all fields sent
            field_names = set([field for submission in submissions for field in submission.cleaned_data.keys()])
