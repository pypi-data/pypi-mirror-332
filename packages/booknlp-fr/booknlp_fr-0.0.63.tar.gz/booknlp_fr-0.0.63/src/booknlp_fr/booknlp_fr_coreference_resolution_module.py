...
def assign_coreference_to_proper_mentions_if_A_is_part_of_B(entities_df):
    ...
    Returns:
        pd.DataFrame: DataFrame containing pairs of indices (`A`, `B`) that are predicted to be coreferent,
                      along with the `coreference_prediction` column set to 1.
    """
    entities_df['text'] = entities_df['text'].str.lower()  # Lowercase text once

    # Filter for proper nouns or specific nominal forms
    nominal_starts_with_list = {'monsieur', 'm.', 'madame', 'mme', 'mademoiselle', 'mon ami', 'mon amie', 'mon cher',
                                'ma chère', 'cher', 'chère', 'maître', 'le malheureux', 'malheureux'}

    def is_valid_nominal(text):
        return any(text.startswith(prefix + ' ') for prefix in nominal_starts_with_list)

    filtered_df = entities_df[
        (entities_df['prop'] == 'PROP') |
        ((entities_df['prop'] == 'NOM') & entities_df['text'].apply(is_valid_nominal))
    ]

    # Filter based on gender and number
    filtered_df = filtered_df[
        filtered_df['gender'].isin(['Male', 'Female', 'Not_Assigned']) &
        filtered_df['number'].isin(['Singular', 'Plural'])
    ]

    # Convert text to sets of words to optimize containment checks
    filtered_df['word_set'] = filtered_df['text'].str.split().apply(set)

    # Use combinations instead of merging entire DataFrame
    mention_pairs = []
    for (idx_A, row_A), (idx_B, row_B) in combinations(filtered_df.iterrows(), 2):
        if row_A['gender'] == row_B['gender'] and row_A['number'] == row_B['number']:
            if row_A['word_set'].issubset(row_B['word_set']) or row_B['word_set'].issubset(row_A['word_set']):
                mention_pairs.append((idx_A, idx_B, 1))  # Coreference prediction

    # Create result DataFrame
    result_df = pd.DataFrame(mention_pairs, columns=['A', 'B', 'coreference_prediction'])
...
def rule_based_postprocessing(CAT_entities_df):
    ...
    rule_based_postprocess_pairs_df = pd.DataFrame()
        # Apply additional rules based on the provided configuration
    print("assign_coreference_to_proper_mentions_if_A_is_part_of_B")
    post_processed_mention_pairs.append(assign_coreference_to_proper_mentions_if_A_is_part_of_B(CAT_entities_df))
    print("assign_coreference_to_direct_nested_mentions")
    post_processed_mention_pairs.append(assign_coreference_to_direct_nested_mentions(CAT_entities_df))
    print("assign_coreference_to_explicit_proper_and_noun_phrase_mentions")
