...
def process_data(input_file, output_file):
    ...
    # Treinar GAN
    encoder, decoder, _ = train_gan(encoder, decoder, discriminator, X_train, args)

    # Gerar amostras sintéticas
    n_to_sample_dict, _ = calculate_n_to_sample(y_train)
    synthetic_data, synthetic_labels = [], []

    for cl, n_samples in n_to_sample_dict.items():
        if n_samples > 0:
            X_synthetic, y_synthetic = G_SM1(X_train, y_train, n_samples, cl, encoder, decoder, args)
