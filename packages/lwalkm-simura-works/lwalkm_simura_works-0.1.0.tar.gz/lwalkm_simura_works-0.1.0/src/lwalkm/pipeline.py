...
2. CorrectionNet/NEB Optimization:
   ...
   "localwalk_*_refined.pdb") are loaded as a baseline pathway ("beads on a string").
   A CorrectionNet neural network is pre-trained and integrated with a NEB framework
   (using cyclic simulated annealing and a SAC agent) to drive the pathway toward a
   lower–energy transition.
3. Final Minimization:
   A separate GROMACS minimization pipeline can be run on each final image, with energy
...
def global_superimpose(structA, structB):
    ...
    if fixed_atoms and moving_atoms:
        from Bio.PDB.Superimposer import Superimposer  # Ensure correct import
        sup = Superimposer()
...
def main_run() -> None:
    ...
    if REF_INTP_FILES is not None and len(REF_INTP_FILES) > 0:
        localwalk_files = REF_INTP_FILES
    else:
