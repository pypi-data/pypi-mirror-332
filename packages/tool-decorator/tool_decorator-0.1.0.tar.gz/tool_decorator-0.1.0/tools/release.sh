...
check_pypi_version() {
    local package=$1
    pip index versions "$package" 2>/dev/null | grep -o "([0-9.]*)" | head -n1 | tr -d '()' || echo "not found"
}
...
PYPI_VERSION=$(check_pypi_version "tool_decorator")
echo "Current version on PyPI: $PYPI_VERSION"

# Phase 1: Version Validation
echo -e "\nValidating versions..."
# Get current version from pyproject.toml
CURRENT_VERSION=$(grep "^version = " pyproject.toml | grep -o '"[0-9.]*"' | tr -d '"')
...
PYPI_VERSION=$(pip index versions tool_decorator 2>/dev/null | grep -o "([0-9.]*)" | head -n1 | tr -d '()' || echo "not found")
echo "Current version on PyPI: $PYPI_VERSION"
