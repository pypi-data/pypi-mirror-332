...
class FileHandler:
    ...
    def read_file(self, file_path: str, mode: str='rb'):
        """
        Lit un fichier local ou SMB en batchs
        """
        if self.smb_config and file_path.startswith("\\"):
            return self._read_smb_file(file_path, mode)
        return self._read_local_file(file_path, mode)

    def _read_smb_file(self, file_path, mode):
        """
    ...
    def _read_local_file(self, file_path, mode):
        """
