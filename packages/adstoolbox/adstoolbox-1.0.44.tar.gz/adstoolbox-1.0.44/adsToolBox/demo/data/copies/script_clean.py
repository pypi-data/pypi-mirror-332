...
def alim_NUMOF_X3():
    """Met a jour la table REF_NOF"""
    db.connect()
    logger.info(f"Suppression des anciennes donnees de {table}.")
    db.sqlExec(f"DELETE FROM {database}.{schema}.{table}")
    logger.info(f"Insertion des nouvelles donnees dans {table}.")
    db.sqlExec(f"""
    ...
    logger.info(f"Mise a jour de {table} terminee.")

def process_SSCC_Auzances_X():
    """Traite les fichiers SSCC pour Auzances"""
    logger.info("Demarrage du traitement SSCC Auzances.")
    try:
        ...
        if not sscc_exists:
            logger.warning("Fichier SSCC non trouve. Fin du traitement.")
            return
        logger.info("Fichier SSCC trouve, debut du traitement.")
        trg_lines = ["Ligne transformee\n"]
