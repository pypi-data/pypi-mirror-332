        ...
        def request_hook(span, request_obj):
            if request_obj.headers:
                for k, v in request_obj.headers.items():
                    span.set_attribute(f"http.request.header.{k.lower()}", v)
            if request_obj.body:
        ...
        def response_hook(span, request_obj, response):
            if response.headers:
                for k, v in response.headers.items():
                    span.set_attribute(f"http.response.header.{k.lower()}", v)
            if response.text:
...
def configure_custom_env_vars(options: MXLOptions, resource: Resource):
