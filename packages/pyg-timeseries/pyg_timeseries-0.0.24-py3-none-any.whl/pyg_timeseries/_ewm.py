...
def ewmcovar_(a, n, min_sample = 0.25, bias = False, instate = None, join = 'outer', method = None):
    ...
    state = {} if instate is None else instate
    arr = df_concat(a, join = join, method = method)
    state['prev'] = state.pop('prev', np.zeros((arr.shape[1], arr.shape[1])))
    return ewmcovariance_(cumsum(arr), n = n, min_sample=min_sample, bias = bias, instate = state, join = join, method = method)   
...
def ewmcorr_(a, n, min_sample = 0.25, bias = False, instate = None, join = 'outer', method = None):
    ...
    arr = df_concat(a, join = join, method = method)
    state['prev'] = state.pop('prev', np.zeros((arr.shape[1], arr.shape[1])))
    return ewmcorrelation_(cumsum(arr), n = n, min_sample=min_sample, bias = bias, instate = state, join = join, method = method)   
