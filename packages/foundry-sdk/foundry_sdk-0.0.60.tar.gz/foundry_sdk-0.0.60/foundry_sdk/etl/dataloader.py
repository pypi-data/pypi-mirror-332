...
class DataLoader:
    ...
    def write_flags(self, not_for_sales_flag: pd.DataFrame, not_available_flag: pd.DataFrame, company_object: Company) -> t.Tuple[Flags]:
        ...
        flag_ids = []
        rows_to_remove = [] 
        for idx, row in tqdm(flags.iterrows()):
            datapoint_id = self.db.execute_query(query, (row["product"], row["date"], row["store"], company_id, company_id), fetchone=True)
            if not datapoint_id:
                logger.warning(f"Could not find a datapoint for product {row['product']}, date {row['date']} and store {row['store']}, removing from flags ...")
                rows_to_remove.append(idx)
                continue
            flag_ids.append(datapoint_id[0])
        
        flags = flags.drop(rows_to_remove)

        flags["datapointID"] = flag_ids
        ...
        flags_object.write_to_db_multi_row(flags, save_ids=False, show_progress_bar=False)
    ...
    def write_product_data(self, product_feature_description_map: pd.DataFrame, product_feature_map: pd.DataFrame, product_object: Stores, company_object: Company) -> t.Tuple[ProductFeatureDescription, ProductFeatures]:
        ...
        product_features.write_to_db_multi_row(product_feature_map, save_ids=False, show_progress_bar=False)
    ...
    def write_time_product_data(self, time_product_feature_description_map: pd.DataFrame, time_product_feature_map: pd.DataFrame, product_object: Products, company_object: Company) -> t.Tuple[TimeProductFeatureDescription, TimeProductFeatures]:
        ...
        if len(time_product_feature_description_map) == 0:
            ...
            time_product_features.write_to_db_multi_row(time_product_feature_map, save_ids=False, show_progress_bar=False)
