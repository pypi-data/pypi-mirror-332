...
class DataLoader:
    ...
    def write_categories(self, company: Company, categories_dict: t.Dict, categories_level_description: pd.DataFrame) -> t.Any:
        ...
        for idx, row in categories_level_description.iterrows():
            ...
            sub_category_ids_df = get_category_ids(self.db, company.ids, sub_categories_unique)
            ...
            parent_ids_df = get_category_ids(self.db, company.ids, parent_categories_unique)
    ...
    def write_products(self, products: pd.DataFrame, company: Company) -> t.Any:
        ...
        products_object.write_to_db_multi_row(unique_products_df, save_ids=True, show_progress_bar=False)
    ...
    def write_product_categories(self, products: pd.DataFrame, unique_products_df: pd.DataFrame, company_object: Company, category_object: Categories, product_object: Products) -> t.Any:
        ...
        # get category ids 
        condition = f"""
            "companyID"={company_object.ids}
            AND
            "name" IN ({", ".join([f"'{cat}'" for cat in products_with_ids["category"].unique()])})
        """

        category_ids = retrieve_data(self.db, "categories", ["ID", "name"], condition = condition)
        category_ids_df = pd.DataFrame(category_ids, columns=["categoryID", "name"])
        category_ids_df.rename(columns={"name": "category"}, inplace=True)
        ...
        product_categories.write_to_db_multi_row(products_with_ids_and_category_ids, save_ids=False, show_progress_bar=False)
    ...
    def write_skus(self, time_sku_data: pd.DataFrame, store_object: Stores, product_object: Products, company_object: Company) -> t.Any:
        ...
        condition = f"""
            ...
            AND
            "name" IN ({", ".join([f"'{store}'" for store in unique_stores])})
        """
        store_ids = retrieve_data(self.db, "stores", ["ID", "name"], condition=condition)
        store_ids_df = pd.DataFrame(store_ids, columns=["storeID", "name"])
        store_ids_df.rename(columns={"name": "store"}, inplace=True)

        condition = f"""
            "companyID"={company_id}
            AND
            "name" IN ({", ".join([f"'{product}'" for product in unique_products])})
        """
        product_ids = retrieve_data(self.db, "products", ["ID", "name"], condition=condition)
        product_ids_df = pd.DataFrame(product_ids, columns=["productID", "name"])
        product_ids_df.rename(columns={"name": "product"}, inplace=True)
        ...
        skus.write_to_db_multi_row(unique_store_product_pairs, save_ids=True, show_progress_bar=False)
    ...
    def write_datapoints(self, time_sku_data_with_sku_id: pd.DataFrame) -> t.Any:
        ...
        condition = f"""
            "date" IN ({", ".join([f"'{date}'" for date in unique_dates])})
        """
        date_ids = retrieve_data(self.db, "dates", ["ID", "date"], condition=condition)
        date_ids_df = pd.DataFrame(date_ids, columns=["dateID", "date"])
        ...
        time_sku_data_with_sku_id["datapointID"] = datapoints.ids
        del time_sku_data_with_sku_id["skuID"]
        del time_sku_data_with_sku_id["dateID"]

        return datapoints, time_sku_data_with_sku_id
    ...
    def write_time_sku_data(self, time_sku_data_with_datapoint_id: pd.DataFrame, create_new_time_sku_table: bool = True) -> t.Any:
        ...
        for column in columns_without_id:
            ...
            data = time_sku_data_with_datapoint_id[["datapointID", column]]
    ...
    def write_store_data(self, store_feature_description_map: pd.DataFrame, store_feature_map: pd.DataFrame, store_object: Stores, company_object: Company) -> t.Tuple[StoreFeatureDescription, StoreFeatures]:
        ...
        condition = f"""
            "companyID"={company_id}
            AND
            "name" IN ({", ".join([f"'{store}'" for store in store_feature_map["store"].unique()])})
        """
        store_ids = retrieve_data(self.db, "stores", ["ID", "name"], condition=condition)
        store_ids_df = pd.DataFrame(store_ids, columns=["storeID", "store"])
    ...
    def write_time_product_data(self, time_product_feature_description_map: pd.DataFrame, time_product_feature_map: pd.DataFrame, product_object: Products, company_object: Company) -> t.Tuple[TimeProductFeatureDescription, TimeProductFeatures]:
        ...
        if len(time_product_feature_description_map) == 0:
            ...
            raise AssertionError("Time product features not implemented yet")
    ...
    def write_time_region_data(self, time_region_feature_description_map: pd.DataFrame, time_region_feature_map: pd.DataFrame, company_object: Company) -> t.Tuple[TimeRegionFeatureDescription, TimeRegionFeatures]:
        ...
        condition = f"""
            "companyID" = '{company_id}'
            AND
            "name" IN ({", ".join([f"'{feature}'" for feature in unique_feature_names])})
        """

        feature_ids = retrieve_data(self.db, "time_region_features_description", ["ID", "name"], condition=condition)
        feature_ids_df = pd.DataFrame(feature_ids, columns=["trfID", "name"])
        feature_ids_df.rename(columns={"name": "feature"}, inplace=True)
    ...
    def write_time_store_data(self, time_store_feature_description_map: pd.DataFrame, time_store_feature_map: pd.DataFrame, store_object: Stores, company_object: Company) -> t.Tuple[TimeStoreFeatureDescription, TimeStoreFeatures]:
        ...
        condition = f"""
            "companyID" = {company_id}
            AND
            "name" IN ({", ".join([f"'{store}'" for store in time_store_feature_map["store"].unique()])})
        """
        store_ids = retrieve_data(self.db, "stores", ["ID", "name"], condition=condition)
        store_ids_df = pd.DataFrame(store_ids, columns=["storeID", "store"])
