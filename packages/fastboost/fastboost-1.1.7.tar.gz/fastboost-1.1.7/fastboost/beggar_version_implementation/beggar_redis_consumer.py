        ...
        while True:
            try:
                ...
                if redis_task:
                    ...
                    logging.debug(f'从redis的 [{self.queue_name}] 队列中 取出的消息是： {task_str}  ')
                    task_dict = json.loads(task_str)
                    self.pool.submit(self.consume_function, **task_dict)
                else:
                    logging.debug(f'redis的 {self.queue_name} 队列中没有任务')
    ...
    while True:
        try:
            ...
            if redis_task:
                ...
                logging.info(f'redis的 {queue_name} 队列中没有任务')
        except redis.RedisError as e:
            logging.info(e)
