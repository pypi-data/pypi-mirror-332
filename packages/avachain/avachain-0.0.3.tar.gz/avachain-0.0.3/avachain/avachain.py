...
class LLM:
    ...
    def ava_llm_completions(
        ...
        # return self.LLMClient.chat.completions.create(**self.kwargs)
...
class OpenaiLLM(LLM, openai.OpenAI):
    ...
    def ava_llm_completions(
        ...
        print("tools in requests: ", len(tools))
        if tools != []:
            if not is_function_based:
                # print("sending tools in requests: ", len(tools))
                # print("is function based in avallm", is_function_based)
                # print("tools in ava_llm", tools)
                self.kwargs["tools"] = tools
                # if tool_choice:
                #     self.kwargs["tool_choice"] = tool_choice
    ...
    for tool in tools_list:
        if tool.name == tool_name:
            ...
            if tool_params:
                ...
                print("unpacking success tool param as its empty!")

                args_model = tool.args_schema(**tool_params)
                return tool._run(**args_model.dict()), tool.return_direct
            else:
                print("not unpacking tool param as its empty!")
