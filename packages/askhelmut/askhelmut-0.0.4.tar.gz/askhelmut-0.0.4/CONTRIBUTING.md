# Contributing to askhelmut

Thank you for considering contributing to askhelmut!

## Setup

Clone this GitHub repository via ```git clone git@github.com:helmut-hoffer-von-ankershoffen/askhelmut.git``` and change into the directory of your local askhelmut repository: ```cd askhelmut```
...
```shell
docker build -t askhelmut .
```

```shell
docker run --env THE_VAR=THE_VALUE askhelmut --help
```
