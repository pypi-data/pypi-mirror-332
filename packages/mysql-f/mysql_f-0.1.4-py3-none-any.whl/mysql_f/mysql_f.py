...
class MySQL_F:
    ...
    def create(self, columns: Dict[str, str]) -> bool:
        try:
            with self.pool.connection() as conn:
                ...
                self._logWithIcon('📝 ✅ ', f'表创建成功: {self.table}')
                return True
        except Error as e:
    ...
    # 创建表，主持中文约束描述
    @_timerDecorator
    def createC(self, fieldsDef: Dict[str, List[str]]) -> bool:
        columns = []
        for field in fieldsDef:
            typeInfo = fieldsDef[field][0]
            constraints = fieldsDef[field][1:]
            ...
            if '(' in typeInfo and ')' in typeInfo:
                typeName, args = typeInfo.split('(', 1)
                args = args.rstrip(')').strip()
                typeName = typeName.strip()

                if typeName == '字符串': convertedType = f'VARCHAR({args})'
                elif typeName in ['枚举', '集合']:
                    args = args.replace("'", "''")
                    convertedType = f'{typeName.upper()}({args})'
                else: convertedType = typeMapping.get(typeName, typeName.upper())
            else: convertedType = typeMapping.get(typeInfo, typeInfo.upper())
            ...
            for constraint in constraints:
                if constraint == '非空': isNullable = False
                elif constraint == '唯一': unique = True
                elif constraint == '自增': autoIncrement = True
                elif constraint == '主键': primaryKey = True

            # 构建列定义
            columnDef = convertedType
            if not isNullable: columnDef += ' NOT NULL'
            if unique: columnDef += ' UNIQUE'
            if autoIncrement: columnDef += ' AUTO_INCREMENT'
            if primaryKey: columnDef += ' PRIMARY KEY'
            columns.append(f'{field} {columnDef}')

        # 生成完整SQL语句
        createTableSql = f'CREATE TABLE IF NOT EXISTS {self.table} ({", ".join(columns)})'
