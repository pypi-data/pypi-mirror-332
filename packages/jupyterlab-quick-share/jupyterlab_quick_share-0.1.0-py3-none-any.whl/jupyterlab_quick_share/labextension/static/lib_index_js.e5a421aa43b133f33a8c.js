...
const plugin = {
    ...
    autoStart: true,
    requires: [_jupyterlab_filebrowser__WEBPACK_IMPORTED_MODULE_1__.IDefaultFileBrowser, _jupyterlab_settingregistry__WEBPACK_IMPORTED_MODULE_2__.ISettingRegistry],
    activate: async (app, fileBrowser, settingRegistry) => {
        ...
        app.commands.addCommand('jupyterlab-quick-share:share', {
            ...
            icon: _jupyterlab_ui_components__WEBPACK_IMPORTED_MODULE_3__.shareIcon,
            execute: async () => {
                const itemsIter = fileBrowser.selectedItems();
                const selectedFile = itemsIter.next().value;
                if (itemsIter.next().value) {
                    (0,_jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_0__.showErrorMessage)('Quick Share Failed', 'Only one file can be shared at a time');
                    return;
                }
                try {
                    const data = await (0,_handler__WEBPACK_IMPORTED_MODULE_4__.requestAPI)(`share?path=${encodeURIComponent(selectedFile.path)}`);
                    await navigator.clipboard.writeText(data.url);
                    console.log('Copied to clipboard:', data.url);
                }
                catch (error) {
                    console.error('Failed to copy quick share link:', error);
                    (0,_jupyterlab_apputils__WEBPACK_IMPORTED_MODULE_0__.showErrorMessage)('Quick Share Failed', error);
                }
            }
        });
...
function fixJupytextIssue1344(app) {
    app.shell.layoutModified.connect(() => {
        ...
        const jupytextSection = findLauncherSectionWithTitle('Console', launcherSectionTitles);
        if (!jupytextSection) {
            return;
        }
        container.prepend(jupytextSection);
        (_a = findLauncherSectionWithTitle('Notebook', launcherSectionTitles)) === null || _a === void 0 ? void 0 : _a.remove();
    });
}
function findLauncherSectionWithTitle(title, els) {
    var _a;
    return (_a = Array.from(els)
        .find(el => { var _a; return ((_a = el.textContent) === null || _a === void 0 ? void 0 : _a.trim()) === title; })) === null || _a === void 0 ? void 0 : _a.closest('.jp-Launcher-section');
}
...
//# sourceMappingURL=lib_index_js.e5a421aa43b133f33a8c.js.map