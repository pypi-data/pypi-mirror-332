...
function fixJupytextIssue1344(app) {
    app.shell.layoutModified.connect(() => {
        // Check if launcher is currently in the main area
        const widgets = app.shell.widgets('main');
        let widget = widgets.next().value;
        while (widget) {
            if (widget.id.includes('launcher')) {
                console.log('it worked'); // This will log each time a launcher is shown
                break;
            }
            widget = widgets.next();
        }
...
//# sourceMappingURL=lib_index_js.181a6710b1b9473d1039.js.map