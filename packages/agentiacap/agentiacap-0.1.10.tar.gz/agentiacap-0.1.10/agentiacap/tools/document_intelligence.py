    ...
    try:
        ...
        if extractions:
            for idx, element in enumerate(extractions):
                ...
                for field in fields_to_extract:
                    fields_data[field] = element.get(field, "none")
                    if fields_data[field] == "none":
                        missing_fields.append(field)
...
def process_binary_files_layout(binary_files: list, fields_to_extract: list) -> list:
    client = initialize_client()
    final_results = {}
    user_text_prompt = f"""Extrae los datos de la tabla. 
        - Si un valor no esta presente, devolve un string vacío.
        - Busca en la columna "Referencia" el numero de factura: {'0002A00000010'}. Si no lo encontras devolvé un string vacío.
        - Si lo encontras obtené el numero de 10 digitos que esta en la misma fila sobre la columna "Doc. comp.".
        - Vas a buscar alguna fila que tenga el número obtenido en la columna "Nº doc." y el valor 'OP' en la columna "Clas". Si no encontras devolve un string vacío.
        - Si encontras dicha fila entonces devolvé el numero de 10 digitos obtenido como 'op' y el la fecha 'due_date' de la columna "Vence El"."""
    ...
    for file_data in binary_files:
        ...
        try:
            ...
            text_result = analyze_document_layout(client, content, fields_to_extract, user_text_prompt)
