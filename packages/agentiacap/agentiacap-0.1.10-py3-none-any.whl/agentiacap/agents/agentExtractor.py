...
from agentiacap.tools.document_intelligence import ImageFieldExtractor, process_binary_files, process_binary_files_layout
...
class PrebuiltLayoutNode():
    async def __call__(self, state: State) -> State:
        try:
            result = process_binary_files_layout(binary_files=state["pdfs"], fields_to_extract=fields_to_extract_rpa)
            return {"aggregate": result}
        except Exception as e:
