...
from .models import (
    ...
    CreateScanInput,
    CreateTaskManagementIntegrationInput,
    ...
    EventsFilter,
    FileInfo,
    FindTaskManagementIntegrationsInput,
    ...
    RemoveTaskManagementIntegrationInput,
...
async def _serialize_create_task_management_integration(
    input: CreateTaskManagementIntegrationInput, config: Config
) -> HTTPRequest:
    path = "/taskmanagement/integrations/add"
    query: str = f""
    ...
    if input.api_token is not None:
        result["apiToken"] = input.api_token

    if input.integration_params is not None:
        result["integrationParams"] = _serialize_integration_parameters(
            input.integration_params, config
        )
...
async def _serialize_find_task_management_integrations(
    input: FindTaskManagementIntegrationsInput, config: Config
) -> HTTPRequest:
    path = "/taskmanagement/integrations/describe"
    query: str = f""
...
async def _serialize_get_chatbot_response(
    ...
    if input.session_id is not None:
        result["sessionId"] = input.session_id
...
async def _serialize_remove_task_management_integration(
    input: RemoveTaskManagementIntegrationInput, config: Config
) -> HTTPRequest:
    if not input.integration_id:
        raise ServiceError("integration_id must not be empty.")

    path = "/taskmanagement/integrations/{integration_id}".format(
        integration_id=urlquote(input.integration_id, safe=""),
    )
    query: str = f""
...
async def _serialize_validate_jira_integration(
    input: ValidateJiraIntegrationInput, config: Config
) -> HTTPRequest:
    if not input.integration_id:
        raise ServiceError("integration_id must not be empty.")

    path = "/taskmanagement/integrations/validation/{integration_id}".format(
        integration_id=urlquote(input.integration_id, safe=""),
    )
    query: str = f""
...
def _serialize_integration_parameters(
    input: IntegrationParameters, config: Config
) -> dict[str, DocumentValue]:
    result: dict[str, DocumentValue] = {}

    if input.jira is not None:
        result["jira"] = _serialize_jira_params(input.jira, config)
...
def _serialize_jira_params(
    input: JiraParams, config: Config
) -> dict[str, DocumentValue]:
    result: dict[str, DocumentValue] = {}
