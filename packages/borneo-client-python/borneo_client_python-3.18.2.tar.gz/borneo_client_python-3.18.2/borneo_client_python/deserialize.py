...
from .models import (
    ...
    CreateScanOutput,
    CreateTaskManagementIntegrationOutput,
    ...
    FalsePositiveReport,
    FileInfo,
    FindTaskManagementIntegrationsOutput,
    ...
    RemoveTaskManagementIntegrationOutput,
...
async def _deserialize_create_task_management_integration(
    http_response: HTTPResponse, config: Config
) -> CreateTaskManagementIntegrationOutput:
    if http_response.status != 200 and http_response.status >= 300:
        raise await _deserialize_error_create_task_management_integration(
            http_response, config
        )
    ...
    if (_integration_id := output.get("integrationId")) is not None:
        kwargs["integration_id"] = expect_type(str, _integration_id)

    return CreateTaskManagementIntegrationOutput(**kwargs)


async def _deserialize_error_create_task_management_integration(
    http_response: HTTPResponse, config: Config
) -> ApiError:
    code, message, parsed_body = await parse_rest_json_error_info(http_response)
...
async def _deserialize_find_task_management_integrations(
    http_response: HTTPResponse, config: Config
) -> FindTaskManagementIntegrationsOutput:
    if http_response.status != 200 and http_response.status >= 300:
        raise await _deserialize_error_find_task_management_integrations(
            http_response, config
        )
    ...
    if (_last_validation_time := output.get("lastValidationTime")) is not None:
        kwargs["last_validation_time"] = epoch_seconds_to_datetime(
            expect_type(int | float, _last_validation_time)
        )
    ...
    if (_integration_params := output.get("integrationParams")) is not None:
        kwargs["integration_params"] = _deserialize_integration_parameters(
            _integration_params, config
        )

    return FindTaskManagementIntegrationsOutput(**kwargs)


async def _deserialize_error_find_task_management_integrations(
    http_response: HTTPResponse, config: Config
) -> ApiError:
    code, message, parsed_body = await parse_rest_json_error_info(http_response)
...
async def _deserialize_get_chatbot_response(
    ...
    if (_relevant_links := output.get("relevantLinks")) is not None:
        kwargs["relevant_links"] = expect_type(str, _relevant_links)

    if (_id := output.get("id")) is not None:
        kwargs["id"] = expect_type(str, _id)

    if (_session_id := output.get("sessionId")) is not None:
        kwargs["session_id"] = expect_type(str, _session_id)
...
async def _deserialize_remove_task_management_integration(
    http_response: HTTPResponse, config: Config
) -> RemoveTaskManagementIntegrationOutput:
    if http_response.status != 200 and http_response.status >= 300:
        raise await _deserialize_error_remove_task_management_integration(
            http_response, config
        )

    kwargs: dict[str, Any] = {}

    return RemoveTaskManagementIntegrationOutput(**kwargs)


async def _deserialize_error_remove_task_management_integration(
    http_response: HTTPResponse, config: Config
) -> ApiError:
    code, message, parsed_body = await parse_rest_json_error_info(http_response)
    ...
    match code.lower():
        ...
        case "dependentassetsexist":
            return await _deserialize_error_dependent_assets_exist(
                http_response, config, parsed_body, message
            )
...
async def _deserialize_validate_jira_integration(
    http_response: HTTPResponse, config: Config
) -> ValidateJiraIntegrationOutput:
    if http_response.status != 200 and http_response.status >= 300:
        raise await _deserialize_error_validate_jira_integration(http_response, config)
    ...
    return ValidateJiraIntegrationOutput(**kwargs)


async def _deserialize_error_validate_jira_integration(
    http_response: HTTPResponse, config: Config
) -> ApiError:
    code, message, parsed_body = await parse_rest_json_error_info(http_response)
...
async def _deserialize_error_dependent_assets_exist(
    http_response: HTTPResponse,
    config: Config,
    parsed_body: dict[str, DocumentValue] | None,
    default_message: str,
    ...
    kwargs["assets"] = _deserialize_assets_list(output["assets"], config)

    return DependentAssetsExist(**kwargs)
...
def _deserialize_asset_info(output: DocumentValue, config: Config) -> AssetInfo:
    if not isinstance(output, dict):
        raise ServiceError(f"Expected dict, found {type(output)}")
...
def _deserialize_assets_list(output: DocumentValue, config: Config) -> list[AssetInfo]:
    if not isinstance(output, list):
        raise ServiceError(f"Expected list, found {type(output)}")
    return [_deserialize_asset_info(e, config) for e in output]
...
def _deserialize_integration_parameters(
    output: DocumentValue, config: Config
) -> IntegrationParameters:
    if not isinstance(output, dict):
        raise ServiceError(f"Expected dict, found {type(output)}")
    ...
    if (_jira := output.get("jira")) is not None:
        kwargs["jira"] = _deserialize_jira_params(_jira, config)

    return IntegrationParameters(**kwargs)
...
def _deserialize_jira_params(output: DocumentValue, config: Config) -> JiraParams:
    if not isinstance(output, dict):
        raise ServiceError(f"Expected dict, found {type(output)}")

    kwargs: dict[str, Any] = {}

    if (_host := output.get("host")) is not None:
        kwargs["host"] = expect_type(str, _host)

    if (_username := output.get("username")) is not None:
        kwargs["username"] = expect_type(str, _username)

    return JiraParams(**kwargs)
