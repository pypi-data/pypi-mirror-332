...
def print_json(routers, unique_routers, sorted_connections, networks, tags):
    ...
    output = {"nodes": [], "links": []}
    for router in unique_routers:
        node = {"id": ip2name(routers, router), "name": ip2name(routers, router), "ip": router, "tags": routers_tags_dict.get(router, []), "networks": routers_networks_dict.get(router, []), "position": routers_position_dict.get(router, [])}
