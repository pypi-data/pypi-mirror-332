...
def get_flat_channels(raw, threshold_min=3e-6, threshold_max=300e-6, threshold_length=0.5):
    ...
    return empty_channels, max_channels, empty_scores, max_scores

def search_bridge_cluster(raw, threshold=0.99):
    data = raw.get_data()
    ...
    G = nx.from_numpy_array(binary_matrix) # get graph from matrix
    components = list(nx.connected_components(G)) # calculate connected components
    multi_node_components = [comp for comp in components if len(comp) > 1]
    clusters_indices = [list(comp) for comp in multi_node_components]
...
class ENDetector():
    ...
    def predict_proba(self, X):
        ...
        snr_db_new = np.array(X, dtype=float)
        prob_noises = []
        ...
        return np.array(prob_noises)



def DNC_SN_ratio(raw, noise_threshold=0.8, optimized=False):  
    ch_names = raw.ch_names
    ...
    for ch_name, snr_db in results:
        proba = detector.predict_proba(snr_db)
        snr_probabilities[ch_name] = proba
        if proba > noise_threshold:
            bad_channels.append(ch_name)
            scores.append(proba)

    variable = list(snr_probabilities.values())
...
def DNC_electrical_distance(raw, vis=False, thr_std=3):
    ...
    label = 'Probability Coefficient'
    vlim = (0, 1.0)
    fig = plot_topomap(variable, title, label, vlim)
    plt.close(fig)
