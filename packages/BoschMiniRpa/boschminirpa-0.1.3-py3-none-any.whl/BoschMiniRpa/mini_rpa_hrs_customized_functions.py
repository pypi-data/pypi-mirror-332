...
def hrs_generate_email_content(service_year, birthday_year, card_type, user_name, seq_id, template_folder_path,
    ...
    try:
        ...
        # font_path = "/opt/msyh.ttc"
        # font = ImageFont.truetype(font_path, 80)
        # bold_font = ImageFont.truetype(font_path, 80)
        ...
        is_card_path_exist, card_obj = smb_check_file_exist(smb_user_name, user_password, server_name, share_name, card_path, port)

        if not is_card_path_exist:
            return {'is_successful': False, 'email_content': 'No card template,please check~'}
        else:

            byte_io = BytesIO()
            bk_img = read_image_from_bytesio(smb_user_name, user_password, server_name, share_name, card_path, port)
            img_pil = Image.fromarray(bk_img)
            draw = ImageDraw.Draw(img_pil)
            # draw.text((5150, 800), user_name, font=font, fill=(0, 0, 0))
            # draw.text((5150, 800), user_name, font=bold_font, fill=(0, 0, 0))
            draw.text((5150, 800), user_name, fill=(0, 0, 0))
            draw.text((5150, 800), user_name, fill=(0, 0, 0))
            bk_img = np.array(img_pil)

            success, encoded_image = cv2.imencode('.jpg', bk_img)
            if success:
...
def hrs_send_anniversary_email(card_type,email_to_manager_column, manager_name_column, anniversary_year_column, email_to_column, user_name_column, email_cc, email_subject, email_header,
                               email_account, email_password, email_address, anniversary_file_path, template_folder_path, birthday_year, smb_user_name, user_password, server_name,
                               share_name, port):
        ...
        for row_index in anniversary_data.index:
            ...
            if email_to:
                ...
                thr = Thread(target=hrs_send_html_content_email,
                             args=[mail_host, mail_user, mail_pass, email_to, email_cc, email_header, email_subject, service_year, birthday_year, card_type,user_name, sender, seq_id,
                                   template_folder_path, smb_user_name, user_password, server_name, share_name, port])
                thr.start()
