...
def main():
    ...
    try:
        if args.handle:
            ...
            # Using the correct method name that exists in the BlueskyPostsFetcher class
            list_result = fetcher.get_posts_from_bluesky_list_url(args.list, args.limit)
            
            # Flatten the dict of user posts into a single list
            posts = []
            for user_posts in list_result.values():
                posts.extend(user_posts)
            
            # Extract list name from URL for the output file
            try:
                # Simple extraction of list ID from URL
                list_id = args.list.split('/')[-1]
                list_name = list_id.lower()
            except Exception:
                list_name = "unknown_list"
