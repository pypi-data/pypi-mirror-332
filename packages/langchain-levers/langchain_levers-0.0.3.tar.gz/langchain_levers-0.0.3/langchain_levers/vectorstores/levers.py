...
class Levers(VectorStore):
    ...
    def __init__(
        ...
        *,
        ...
        self.user = connection_args.get("USER", None)
        self.password = connection_args.get("PASSWORD", None)
        self.db_name = connection_args.get("DB_NAME", DEFAULT_DB_NAME)
        self.collection_name = f"{self.db_name}_{collection_name}"
    ...
    def delete(
        self, ids: Optional[List[str]] = None, **kwargs: str
    ):
        ...
        status, results = self.cli.delete(self.collection_name, del_ids)
        # if status.code == 0:
        #     deleted_ids.extend(results.succ_index)
        print(f"Deleted {len(del_ids)} entities with status {status}")
        print(f"Deleted entities: {results}")
        logger.info(f"Deleted {len(del_ids)} entities with status {status}")
