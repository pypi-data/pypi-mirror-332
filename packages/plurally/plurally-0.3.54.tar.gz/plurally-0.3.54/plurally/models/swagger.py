...
def get_response_kls(method_schema):
    schema = dpath.get(method_schema, "responses|200|content|application/json|schema", separator='|', default=None)
    if not schema:
...
class Swagger(Node, NameableInputFields):
        ...
        for path, methods in self.swagger.get("paths", {}).items():
            for method, method_schema in methods.items():
                ...
                for param_def in method_schema.get("parameters", []):
                    ...
                    method_fields[param_name] = (ptype, Field(default, description=param_schema.get("description"), title=param_name))
                ...
                if payload_kls:
                    ...
                    method_fields["payload"] = (payload_kls, Field(default, description=method_schema.get("summary"), title=class_name))

                method_schema = create_model(class_name, **method_fields)
                fields[class_name] = (List[method_schema], Field(None, description=method_schema.__doc__, title=class_name))
    ...
    def _forward(self, session: requests.Session, node_inputs):
        ...
        for class_name, (path, method) in self.ops.items():
            for payload in (getattr(output, class_name, None) or []):
                ...
                response = session.request(method, url, json=json_args["payload"], params=query_params)
    ...
    def serialize(self):
        return super().serialize() | {"swagger": yaml.dump(self.swagger), 
                                        "prompt": self.prompt, "examples": self.examples, "general_info": self.general_info, 
                                        "add_explanation": self.add_explanation, "model": self.model, 
