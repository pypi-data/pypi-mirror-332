...
class DistributedIMM:
    ...
    def predict(self, x_data: DataFrame):
        ...
        tree_broadcast = self.spark.sparkContext.broadcast(self.tree)

        # Define Pandas UDF for batch processing
        @pandas_udf(IntegerType())
        def predict_batch(features_batch: pd.Series) -> pd.Series:
            return features_batch.apply(lambda features: self._predict_subtree(tree_broadcast.value, np.array(features)))
