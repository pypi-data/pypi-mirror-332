import requests
from py.src.test_cosmic.protocol import (
    AISearchPayload,
...
class DaturaApiSDK:
    ...
    def basic_twitter_search(
        ...
        return self.handle_request(
            self.client.post, f"{BASE_URL}/twitter", json=payload.__dict__
        )

    def basic_web_search(self, payload: WebSearchPayload) -> BasicWebSearchResponse:
        return self.handle_request(
            self.client.get, f"{BASE_URL}/web", params=payload.__dict__
        )
