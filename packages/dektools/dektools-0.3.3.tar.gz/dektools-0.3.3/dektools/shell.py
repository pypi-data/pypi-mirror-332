...
def shell_output(command, error=True, env=None):
    return shell_result(command, error, env)[1]


def shell_exitcode(command, error=True, env=None):
    return shell_result(command, error, env)[0]


def shell_result(command, error=True, env=None):
    return getstatusoutput(command, error, env)


def getstatusoutput(command, error=True, env=None):
    try:
        data = subprocess.check_output(
            shell_quote(command), shell=True, text=True, env=env,
