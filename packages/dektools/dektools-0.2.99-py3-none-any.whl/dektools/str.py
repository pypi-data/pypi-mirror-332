...
def deep_format(s, left, right, handler, escape='\\'):
    cursor = 0
    count = 0
    escape_str = ''
    escape_marker = set()
    while True:
        ...
        if index == -1:
            if cursor != 0:
                escape_str += s[cursor:]
            break
        escape_str += s[cursor:index] + s[index + len(escape)]
        escape_marker.add(index - count * len(escape))
        count += 1
...
if __name__ == '__main__':
    ...
    print(deep_format(
        "-\\\\{abc{de}f{123}}gh{ijk}lmn{>>abc}x{//abc}", r"\{(>>|>|=|$|\*|//|/)?", "\\}",
