...
def deep_format(s, left, right, handler, escape='\\', escape_set=None):
    def is_match(x, y):
        if isinstance(x, str):
            return x == y
        else:
    ...
    while True:
        ...
        if index == -1:
            ...
            break
        retain = all(
            not is_match(x, s[index + len(escape):index + len(escape) + len(x if isinstance(x, str) else x.pattern)])
            for x in escape_set)
        escape_str = f'{escape_str}{s[cursor:index]}{escape if retain else ""}{s[index + len(escape)]}'
        if not retain:
            escape_marker.add(len(escape_str) - 1)
        cursor = index + len(escape) + 1
    if cursor != 0:
        s = escape_str

    rc = re.compile(left + '|' + right)
    cursor = s
    pos_list = []
    while True:
        m = rc.search(cursor)
        if m:
            span = m.span()
            if len(s) - len(cursor) + span[0] not in escape_marker:
                pos_list.append((bool(rcl.fullmatch(m.group())), [len(s) - len(cursor) + x for x in span]))
