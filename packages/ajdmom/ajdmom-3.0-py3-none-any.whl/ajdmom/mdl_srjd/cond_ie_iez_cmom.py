r"""
Conditional Central Moment of :math:`E[IE_t^{n_1} \overline{IEZ}_t^{n_2}|v_0]`
"""
import math
from fractions import Fraction

from ajdmom.poly import Poly
from ajdmom.mdl_srjd.iez_cmom import cmoment_IEZ
...
def int_e_poly(c, tp, m, poly):
    poln = Poly()
    poln.set_keyfor(['e^{kt}', 'k^{-}', 'v0-E[v]', 'E[v]', 'sigma', 'lmbd', 'mu_v'])
    for k, v in poly.items():
...
def recursive_ie(n, IE, IE_IEZ):
    poly = Poly()
    poly.set_keyfor(['e^{kt}', 'k^{-}', 'v0-E[v]', 'E[v]', 'sigma', 'lmbd', 'mu_v'])
    if n >= 2:
        c = Fraction(n * (n - 1), 2)
...
def moment_ie(n):
    ...
    IE_IEZ = {}
    #
    IE_IEZ[(0, 1)] = iez_to_ie_iez(cmoment_IEZ(1))
    #
    IE_IEZ[(1, 1)] = P0
    #
    # n >= 3: typical cases
    ...
    if n > 3:
        ...
        for i in range(3, n):
            if i > 3: IE_IEZ[(i - 2, 1)] = cmoment_ie_iez(i - 2, 1)
            ...
            del IE_IEZ[(i - 2, 1)]
    # the last one
    if n > 3: IE_IEZ[(n - 2, 1)] = cmoment_ie_iez(n - 2, 1)
...
def recursive_ie_iez(n1, n2, IE_IEZ):
    ...
    if n1 >= 2 and n2 >= 1:
        for i in range(n2 + 1):
            bino = math.comb(n2, i)
            pol1 = cmoment_IEZ(n2 - i)  # ['e^{kt}', 'e^{ks}', 'k^{-}', 'lmbd', 'mu_v']
    ...
    if n1 == 0:  # works even n2 = 0
        poly = iez_to_ie_iez(cmoment_IEZ(n2))
    return poly

def cmoment_ie_iez(n1, n2):
    r"""Conditional Central Moment of :math:`E[IE_t^{n_1} \overline{IEZ}_t^{n_2}|v_0]`

    :param integer n1: order of :math:`IE_t`
    :param integer n2: order of :math:`\overline{IEZ}_t`
    :return: poly with ``keyfor`` =
    ...
    IE_IEZ[(1, 1)] = P0
    IE_IEZ[(0, 2)] = iez_to_ie_iez(cmoment_IEZ(2))
    #
    if n1 + n2 <= 2:
        return IE_IEZ[(n1, n2)]
    #
...
def poly2num(poly, par):
    ...
    #
    meanv = theta + lmbd * mu_v / k
    #
    f = 0
    for K, V in poly.items():
        val = math.exp(K[0] * k * h) / (k ** K[1])
        val *= ((v0-meanv) ** K[2]) * (meanv ** K[3]) * (sigma ** K[4])
...
def cm_ie_iez(n1, n2, par):
    moment = cmoment_ie_iez(n1, n2)
    value = poly2num(moment, par)
    return value
...
if __name__ == '__main__':
    ...
    n1 = 3
    n2 = 2
    print(f"\ncmoment_ie_iez(n1, n2) returns a poly with keyfor = \n{kf}\n")
    print(f'cmoment_ie_iez({n1}, {n2}) = ')
    pprint(cmoment_ie_iez(n1, n2))
