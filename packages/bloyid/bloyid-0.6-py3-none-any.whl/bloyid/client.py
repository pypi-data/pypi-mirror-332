...
from bloyid.context import Context
from bloyid.channel import Channel
from bloyid.member import Member, ServerMember, ClientMember
from bloyid.server import Server
from bloyid.roles import Role
from bloyid.post import Post
from bloyid.status import Status
from bloyid.buttoninteraction import ButtonInteraction
...
class Client:
    ...
    def get_user(self, user_id: str, cache_fallback: bool = True) -> Member:
        ...
        api_url = f"https://server.bloyid.com/api/users/{user_id}"
        ...
        def decorator(func: Callable):
            ...
            async def async_wrapper(ctx, *args, **kwargs):
                if not isinstance(ctx, Context):
                    raise TypeError(f"Error: Expected bloyid.Context, got {type(ctx)}")
    ...
    def run(self, debug_mode: bool=False, restart_always: bool=False) -> None:
        ...
        async def main():
            ...
            while True:
                try:
                    async with websockets.connect("wss://server.bloyid.com/socket.io/?EIO=4&transport=websocket") as websocket:
