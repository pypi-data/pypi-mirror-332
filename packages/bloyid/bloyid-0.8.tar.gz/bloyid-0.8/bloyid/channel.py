...
class Channel():
    ...
    async def _get_messages_raw(self, amount: int) -> str:
        ...
        api_endpoint = f"https://server.bloyid.com/api/channels/{self.id}/messages?limit={amount}"
    ...
    async def set_permissions(self, permission_integer: int, role: Role) -> 'Channel':
        ...
        """
        api_url = f"https://server.bloyid.com/api/servers/{self.server_id}/channels/{self.id}/permissions/{role.id}"
        headers = {
            "Authorization": GlobalClientInformation.TOKEN,
            "Content-Type": "application/json",
        }
