...
from jingzhi.constants import (GIT_ATTRIBUTES_CONTENT, 
                                REPO_TYPE_DATASET,
                                GIT_HIDDEN_DIR,
                                GIT_ATTRIBUTES_FILE,
                                DEFAULT_JINGZHI_ENDPOINT)
from jingzhi.utils import (build_jz_headers,
...
class Repository:
    def __init__(
        ...
        work_dir: Optional[str] = "/tmp/jingzhi",
    ...
    def repo_exists(self):
        action_endpoint = DEFAULT_JINGZHI_ENDPOINT
        url = f"{action_endpoint}/api/v1/{self.repo_url_prefix}/{self.repo_id}/branches"
        headers = build_jz_headers(token=self.token, headers={
    ...
    def generate_repo_clone_url(self) -> str:
        clone_endpoint = DEFAULT_JINGZHI_ENDPOINT
    ...
    def git_commit(
        self, 
        work_dir: str, 
        commit_message: str = "commit files to Jingzhi"
    ) -> subprocess.CompletedProcess:
        try:
