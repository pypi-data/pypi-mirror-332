...
from jingzhi.constants import REPO_TYPE_DATASET, DEFAULT_JINGZHI_ENDPOINT
from jingzhi.constants import DEFAULT_REVISION, DEFAULT_MAX_RETRIES,DEFAULT_TIMEOUT,DEFAULT_CHUNK_SIZE

from jingzhi.utils import get_cache_dir, model_id_to_group_owner_name ,filter_repo_objects,get_file_download_url, build_jz_headers, pack_repo_file_info
from jingzhi.errors import InvalidParameter, FileDownloadError
...
def file_download(
    ...
    if cache_dir is None:
        cache_dir = get_cache_dir(repo_type=repo_type, repo_id=repo_id)
...
def http_get(*,
    ...
    get_headers = build_jz_headers(token=token, headers=headers)
    total_content_length = 0
    with tempfile_mgr() as temp_file:
        # retry sleep 0.5s, 1s, 2s, 4s
        retry = Retry(total=DEFAULT_JINGZHI_ENDPOINT, backoff_factor=1, allowed_methods=['GET'])
        while True:
            try:
