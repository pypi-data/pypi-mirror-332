...
def train_spackle(adata, device, save_path, prediction_layer, lr, train, get_performance, load_ckpt_path, args_dict, optimizer = "Adam", max_steps = 10000):
    ...
    device = "cuda" if torch.cuda.is_available() else "cpu"
    train_loader = DataLoader(train_data, batch_size=args_dict["batch_size"], shuffle=args_dict["shuffle"], pin_memory=False, drop_last=True, num_workers=args_dict["num_workers"], generator=torch.Generator(device=device))
    val_loader = DataLoader(val_data, batch_size=args_dict["batch_size"], shuffle=args_dict["shuffle"], pin_memory=False, drop_last=True, num_workers=args_dict["num_workers"], generator=torch.Generator(device=device))
