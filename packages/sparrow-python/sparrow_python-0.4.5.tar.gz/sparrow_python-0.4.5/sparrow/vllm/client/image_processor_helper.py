...
class ImageProcessor:
    ...
    # 异步方法（需要session）
    async def encode_from_url(self, url: str, return_with_mime: bool = True) -> str:
    ...
    async def process_content(self, content: Dict[str, Any], inplace: bool = True, use_cache: bool = True, cache_dir: str = DEFAULT_CACHE_DIR, **kwargs) -> Dict[str, Any]:
        """递归处理内容中的图像URL
        ...
        """
        session = await self._ensure_session()
        result = await process_content_recursive(content, session, inplace=inplace, use_cache=use_cache, cache_dir=cache_dir, **kwargs)
        return result
    
    async def preprocess_messages(self, messages: List[Dict[str, Any]], inplace: bool = False, use_cache: bool = False, cache_dir: str = DEFAULT_CACHE_DIR, **kwargs) -> List[Dict[str, Any]]:
        """预处理消息列表中的图像URL
        ...
        """
        session = await self._ensure_session()
        result = await messages_preprocess(messages, inplace=inplace, use_cache=use_cache, cache_dir=cache_dir, **kwargs)
        return result
...
if __name__ == "__main__":
    ...
    async def async_example():
        ...
        async with ImageProcessor() as processor:
            ...
            processed_messages = await processor.preprocess_messages(messages, use_cache=True, cache_dir="./cache")
    ...
    def sync_example():
        ...
        try:
            ...
            base64_data = run_async(processor.encode_from_url("https://example.com/image.jpg"))
