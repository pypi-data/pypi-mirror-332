...
class ModelRestApi(BaseApi):
    ...
    async def _generate_info_schema(
        ...
        schema.add_schema = self._generate_jsonforms_schema(
            self.add_obj_schema, schema.filters
        )
        schema.edit_schema = self._generate_jsonforms_schema(
            self.edit_obj_schema, schema.filters
        )

        # Add `required` for edit_schema, we want the API itself to be optional, but not the data itself, hence we use the edit_columns to determine the required fields
        if "required" not in schema.edit_schema:
            schema.edit_schema["required"] = [
                x.name for x in schema.edit_columns if x.required
            ]
        ...
        for key, value in jsonforms_schema["properties"].items():
            ...
            if self.datamodel.is_boolean(key):
                ...
                if self.datamodel.is_relation_one_to_many(
                    ...
                    value["items"] = {"oneOf": oneOf}
                elif self.datamodel.is_relation_one_to_one(
                    key
