...
def parse_structure(
    ...
    if len(docs) > len(annotations):
        warnings.warn_explicit(
            f"More documented items than annotations in {function.__qualname__}, using only the first {len(annotations)}.",
    ...
    for index, (annotation, decorator, doc) in enumerate(
        ...
        try:
            data_type = parser.parse(annotation, feature)
        except TypeError:
            raise TypeError(
                f"Unable to identify SiLA type from annotation '{annotation.__name__}' in {function.__qualname__}."
            ) from None
        ...
        element = sila.data_types.structure.Element(
    ...
    return factory(elements=elements)
