...
@click.command()
def sync(repo_url: str, token: str, branch: str, ci: bool, dry_run: bool):
    ...
    if dry_run:
        subprocess.run(["cruft", "diff"])
        return

    if ci or (branch != DEFAULT_BRANCH and not ci):
        subprocess.run(["git", "switch", "-c", branch], check=True)

    subprocess.run(["poetry", "run", "cruft", "update", "--skip-apply-ask"], check=True)
...
def push_to_gitlab(branch: str, repo_url: str, auth: str) -> None:
    ...
    subprocess.run(["git", "commit", "-m", f'"update to {commit_sha}"'])
