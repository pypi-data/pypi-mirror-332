...
def find_factory() -> str:
    ...
    for root, dirs, files in walk(pathlib.Path.cwd()):
        dirs[:] = [
            ...
            for dir in dirs
            if any(
                path in itertools.chain([pathlib.Path(root) / dir], (pathlib.Path(root) / dir).parents)
...
def walk(self, top_down=True, on_error=None) -> typing.Iterable[tuple[str, list[str], list[str]]]:
