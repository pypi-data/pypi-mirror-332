...
@dataclasses.dataclass
class Parameters(sila.data_types.Structure):
    """
    ...
    def from_signature(
        ...
        for i, parameter in enumerate(cls.__get_parameters(signature)):
            if parameter.annotation is inspect.Parameter.empty:
                raise TypeError(f"Missing type annotation for parameter '{parameter.name}' in {function.__qualname__}.")
    ...
    def get_mapping(self, function: typing.Callable):
        """
        Maps structure elements to function parameters based on their names.
        ...
        return {
            ...
            for element, parameter in zip(self.elements, self.__get_parameters(signature))
        }
