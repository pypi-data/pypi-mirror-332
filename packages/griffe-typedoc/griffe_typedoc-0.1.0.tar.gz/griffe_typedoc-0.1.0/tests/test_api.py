...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    """All public objects in the internal API are exposed under `griffe_typedoc`."""
    not_exposed = [
        obj.path
        for obj in modulelevel_internal_objects
        if obj.name not in griffe_typedoc.__all__ or not hasattr(griffe_typedoc, obj.name)
    ]
...
def test_single_locations(public_api: griffe.Module) -> None:
    ...
    multiple_locations = {}
    for obj_name in griffe_typedoc.__all__:
...
def test_inventory_matches_api(
    ...
    public_api_paths.add("griffe_typedoc")
