...
def analyze_multivariate(
    df: pd.DataFrame,
    columns: Optional[List[str]] = None,
    method: Literal['pca', 'tsne', 'both'] = 'both',
    n_components: int = 2,
    scale: bool = True,
    ...
    """
    Perform multivariate analysis on the specified columns using PCA and/or t-SNE.
    ...
    method : Literal['pca', 'tsne', 'both'], default='both'
        The dimensionality reduction method to use.
    n_components : int, default=2
        The number of components to extract.
    scale : bool, default=True
        Whether to standardize the data before analysis.
