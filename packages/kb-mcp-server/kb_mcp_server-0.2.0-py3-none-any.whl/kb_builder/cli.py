...
def build_main():
    """
    Entry point for kb-build command.
    
    This function modifies sys.argv to insert the 'build' subcommand
    and then calls the main function.
    """
    import sys
    # Save original command name
    original_command = sys.argv[0]
    # Insert 'build' subcommand if not already present
    if len(sys.argv) == 1 or sys.argv[1] != 'build':
        sys.argv.insert(1, 'build')
    # Update command name for help text
    sys.argv[0] = 'kb-build'
    main()

def retrieve_main():
    """
    Entry point for kb-search command.
    
    This function modifies sys.argv to insert the 'retrieve' subcommand
    and then calls the main function.
    """
    import sys
    # Save original command name
    original_command = sys.argv[0]
    # Insert 'retrieve' subcommand if not already present
    if len(sys.argv) == 1 or sys.argv[1] != 'retrieve':
        sys.argv.insert(1, 'retrieve')
    # Update command name for help text
    sys.argv[0] = 'kb-search'
    main()
