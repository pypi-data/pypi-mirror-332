...
def load_config_and_secrets():
    try:
        ...
        os.environ["BEEKEEPER_ORGANIZATION_ID"] = config.get("BEEKEEPER_ORGANIZATION_ID")
        email_address = config.get("BEEKEEPER_USERNAME")
        os.environ["BEEKEEPER_ENVIRONMENT"] = config.get("BEEKEEPER_ENVIRONMENT", "prod")
        ...
        if not config.get("BEEKEEPER_PROJECT_ID") or not config.get("BEEKEEPER_ORGANIZATION_ID"):
            raise ValueError("Missing BEEKEEPER_PROJECT_ID or BEEKEEPER_ORGANIZATION_ID in config.yaml.")
        print(f"Found user {email_address} from organization {config.get("BEEKEEPER_ORGANIZATION_ID")} on project {config.get("BEEKEEPER_PROJECT_ID")}.")
        ...
        if not secret_key_b64:
            raise ValueError("Missing CONTENT_ENCRYPTION_KEY_BASE64 environment variable")
        if not project_private_key_b64:
            raise ValueError("Missing PROJECT_PRIVATE_KEY_BASE64 environment variable")
...
def encrypt_and_upload(secrets):
    ...
    subprocess.run(["rsync", "-av", "--exclude", ".git", "--exclude", "temp", ".", folder_path], check=True)
    ...
    subprocess.run([
        ...
        "--algorithm_type", secrets["ALGORITHM_TYPE"],
        "--algorithm_name", secrets["ALGORITHM_NAME"],
        "--algorithm_description", secrets["ALGORITHM_DESCRIPTION"],
        "--version_description", secrets["VERSION_DESCRIPTION"],
        "--username", secrets["BEEKEEPER_USERNAME"]
    ], check=True)
...
def cleanup(converted_scripts):
    ...
    for i in converted_scripts: os.remove(i)
