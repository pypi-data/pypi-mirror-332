...
def main():
    ...
    print("nsys2prv v0.5.0-dev2503071")
    ...
    if t_graphs and graphs_mode == "Node": compute_threads_with.append(graphs_df[["Pid", "Tid"]])
    ...
    if t_graphs and graphs_mode == "Node":
        graphs_df["thread"] = graphs_df["Tid"].map(threads.set_index('Tid')["thread"])
        graphs_df["task"] = graphs_df["Tid"].map(threads.set_index('Tid')["task"])
...
14    Gradient 14
    ...
    with open(trace_name+".prv", "w") as prv_file:
        ...
        if t_graphs and graphs_mode == "Node":
            types = [event_type_kernels] + event_types_block_grid_values + [event_type_registers_thread, event_type_correlation, CUDAGraphsSemantic.cuda_graph_node_id]
            ewr(prv_file, kernels_df, "Kernels", lambda r:
                    (create_combined_events_record(r.iloc[0], r.iloc[1], int(r["thread"]), int(r["task"]), types, [r["event_value"]] + [int(r['GrdX']), int(r['GrdY']), int(r['GrdZ']), int(r['BlkX']), int(r['BlkY']), int(r['BlkZ']), int(r['Reg/Trd']), r["CorrID"], r["graphNodeId"]])))
        else:
        ...
        if t_nccl:
            ...
            types = [event_type_nccl_kernels] + event_types_block_grid_values + [event_type_registers_thread, event_type_correlation]
            ewr(prv_file, nccl_df, "NCCL Kernels", lambda r:
                (create_combined_events_record(r.iloc[0], r.iloc[1], int(r["thread"]), int(r["task"]), types, [r["event_value"]] + [int(r['GrdX']), int(r['GrdY']), int(r['GrdZ']), int(r['BlkX']), int(r['BlkY']), int(r['BlkZ']), int(r['Reg/Trd']), r["CorrID"]])))

        if t_graphs and graphs_mode == "Node":
            types_mem = [event_type_kernels, event_type_memcopy_size, event_type_correlation, CUDAGraphsSemantic.cuda_graph_node_id]
