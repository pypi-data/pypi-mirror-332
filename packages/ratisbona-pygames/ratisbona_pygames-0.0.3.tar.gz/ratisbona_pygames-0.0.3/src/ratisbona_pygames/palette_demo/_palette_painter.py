...
from ratisbona_utils.colors.palette_generators import (
    ...
    hsv_based_rgb_palette,
    terrain,
    viridis,
)


class PalettePainter(GameElement):
...
def scene1(game: Game, quit_scene_action: Callable):
    game.game_elements.clear()
    game.game_elements.append(Background(game, solarized_base03))
    ...
    game.game_elements.extend(painters)

    ret_key = Debounce(game, pygame.K_RETURN, quit_scene_action, 10)
    game.game_elements.append(ret_key)
...
def palette_scene(
    ...
    for row in range(n_rows):
        for col in range(n_cols):
            ...
            game.game_elements.append(palette_painter)
