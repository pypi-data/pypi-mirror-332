...
def mk_pval_file(df: pd.DataFrame, p: list, n: list|int, rsid: list, filename: str):
    """
    ...
    n = n if isinstance(n, int) else df[[col for col in col_names if col.lower().strip() in n][0]]
    df2use = pd.DataFrame({"rsid": df[rsid_col], "p": df[p_col], "n": n})
...
def mk_loc_file(df: pd.DataFrame, rsid: list,  chromosome: list, position: list, filename: str):
    """
