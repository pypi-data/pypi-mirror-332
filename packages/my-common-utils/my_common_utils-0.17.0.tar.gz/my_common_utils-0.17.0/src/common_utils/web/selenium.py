    ...
    def run_actions(self, actions: list[DriverAction], driver: webdriver.Chrome | None = None):
        ...
        for action in actions:
            match action.action:
                ...
                case "get_class":
                    result = _driver.find_element(By.CSS_SELECTOR, action.identifier).get_attribute("class")
                    results.append(result)
                    self.log_msg(f"Storing class: {result} at idx {len(results)-1}", level='debug')
                case "get_classes":
                    result = [element.get_attribute("class") for element in _driver.find_elements(By.CSS_SELECTOR, action.identifier)]
                    results.append(result)
                    self.log_msg(f"Storing classes: {result} at idx {len(results)-1}", level='debug')
                case _:
