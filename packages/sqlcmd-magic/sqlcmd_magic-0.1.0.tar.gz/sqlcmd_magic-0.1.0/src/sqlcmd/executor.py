...
class SQLExecutor:
    ...
    def execute_sql(self, sql_content: str, debug: bool = False, return_df: bool = False) -> Optional[pd.DataFrame]:
        """
        ...
        Returns:
            DataFrame if return_df is True and results can be parsed, None otherwise
        """
        ...
        try:
            for i, batch in enumerate(batches):
                ...
                # Add pipe formatting options if we want to return as DataFrame
                if return_df:
                    # Ensure the sqlcmd command includes -s'|' for pipe delimiter formatting
                    if not any(arg.startswith('-s') for arg in command):
                        command.insert(1, "-s|")
                    # Leave any -h header options intact
                ...
                batch_output = []  # Collect output for this batch
        ...
        return self.last_dataframe if return_df else None
