...
class HchGetServiceDetailsInfo(BaseModel):
    service: str = Field(
        title="Home clean home(hch) service.",
        description=f"Home clean home(hch) service. Available services: [{', '.join(service_options)}]",
    )
...
def get_service_details_info(service: str) -> str:
    """Get details info with respect to the selected cleaning service.

    Args:
        service (str): Home clean home(hch) residential/commercial service.
...
def get_service_price_info(service: str) -> str:
...
async def handle_list_tools() -> list[Tool]:
    ...
    return [
        Tool(
            ...
            description="get_greetings() - Get greetings message when user sends a greeting/requests a price quotation/inquires about booking/sends an image without context.",
            inputSchema=HchGetGreetings.model_json_schema(),
        ),
        Tool(
            name=HchTools.RESIDENTIAL_SERVICES,
            description="get_residential_services() - Get all residential cleaning services.",
            inputSchema=HchGetResidentialServices.model_json_schema(),
        ),
        Tool(
            name=HchTools.SERVICE_DETAILS_INFO,
            description="get_service_details_info() - Get details info with respect to the selected cleaning service.",
            inputSchema=HchGetServiceDetailsInfo.model_json_schema(),
        ),
        Tool(
            name=HchTools.SERVICE_PRICE_INFO,
            description="get_service_price_info() - Get price info with respect to the selected cleaning service.",
