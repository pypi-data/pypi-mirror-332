...
class TrdType(IntEnum):
    ...
    def __str__(self):
        ...
        return {
            TrdTypeEnum.UNKNOWN: f"未知({self.name}={char_str})",
            TrdTypeEnum.BUY: f"主动买({self.name}={char_str})",
            TrdTypeEnum.SELL: f"主动卖({self.name}={char_str})",
            TrdTypeEnum.UNKNOWN_N: f"未知N({self.name}={char_str})",
            TrdTypeEnum.CANCEL: f"撤单({self.name}={char_str})",
            TrdTypeEnum.DEAL: f"成交({self.name}={char_str})"
        }[self]
...
class OrdType(IntEnum):
    """订单类别"""
    UNKNOWN = OrdTypeEnum.TORT_UNKNOWN        # 未知类型
    MARKET = OrdTypeEnum.TORT_Market          # SZ市价 ('1'的ASCII码)
    LIMIT = OrdTypeEnum.TORT_Limit            # SZ限价 ('2'的ASCII码)
    MARKET_SELF = OrdTypeEnum.TORT_MarketSelf # SZ本方最优 ('U'的ASCII码)
    ADD = OrdTypeEnum.TORT_Add                # SH增加委托订单 ('A'的ASCII码)
    DELETE = OrdTypeEnum.TORT_Delete          # SH删除委托订单 ('D'的ASCII码)
    STATUS = OrdTypeEnum.TORT_Status          # 产品状态订单 ('S'的ASCII码)
    ...
    def __str__(self):
        ...
        return {
            OrdType.UNKNOWN: f"未知({self.name}={char_str})",
            OrdType.MARKET: f"市价委托({self.name}={char_str})",
            OrdType.LIMIT: f"限价委托({self.name}={char_str})",
            OrdType.MARKET_SELF: f"本方最优({self.name}={char_str})",
            OrdType.ADD: f"增加订单({self.name}={char_str})",
            OrdType.DELETE: f"删除订单({self.name}={char_str})",
            OrdType.STATUS: f"产品状态订单({self.name}={char_str})"
        }[self]
