...
class InputConnector:
    ...
    def forward(self):
        try:
            ...
            self._log.info(
                f"The input could not be forwarded from cuid={self._conn.inbound_chan.cuid} to process pid={self._proc.pid}"
            )
...
class AvailableLocalCUIDS:
    ...
    def next(self):
        ...
        if len(self._active) == AvailableLocalCUIDS.AVAILABLE_PER_NODE:
            raise RuntimeError(f"Ran out of Process Local CUIDs")
    ...
    def reserve(self, cuid):
        if not self.is_available(cuid):
            raise RuntimeError(f"Attempted reservation for local cuid {cuid} which is already in use.")
        self._active.add(cuid)
...
class AvailableLocalMUIDS:
    ...
    def next(self):
        ...
        if len(self._active) == dfacts.RANGE_LS_MUIDS_PER_NODE:
            raise RuntimeError(f"Ran out of Process Local MUIDs")
    ...
    def reserve(self, muid):
        if not self.is_available(muid):
            raise RuntimeError(f"Attempted reservation for local pool {muid} which is already in use.")
        self._active.add(muid)
...
class LocalServer:
    ...
    def make_local_pool(self, msg):
        ...
        name = f"{msg.name}{os.getpid()}"
        log.debug(f"Creating local memory pool with {muid=} for process with {msg.puid=} and {name=}")
    ...
    def __str__(self):
        with self.apt_lock:
            plist = [f"\t{p_uid}:{pid} {self.apt[pid]!s}" for p_uid, pid in self.puid2pid.items()]
            ...
            if died_pid:
                ...
                for muid in proc.props.local_muids:
                    try:
                        ...
                        log.info(f"Destroyed process local pool with {muid=} upon process exit.")
                    except Exception as ex:
                        log.info(f"Could not destroy process local pool on process exit. Error:{repr(ex)}")

                    try:
                        del self.pools[muid]
                    except Exception as ex:
                        log.info(f"Could not remove process local pool on process exit. Error:{repr(ex)}")
