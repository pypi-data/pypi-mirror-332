...
class MessageTypes(enum.Enum):
    ...
    DD_MANAGER_GET_STATE = enum.auto()  #:
    DD_MANAGER_GET_STATE_RESPONSE = enum.auto()  #:
    DD_MARK_DRAINED_MANAGER = enum.auto()  #:
    DD_MARK_DRAINED_MANAGER_RESPONSE = enum.auto()  #:
...
class DDManagerSync(CapNProtoMsg):
    ...
    def __init__(self, tag, fullManagerFLI, respFLI):
        super().__init__(tag)
        self._fullManagerFLI = fullManagerFLI
        self._respFLI = respFLI

    def get_sdict(self):
        rv = super().get_sdict()
        rv["fullManagerFLI"] = self._fullManagerFLI
        rv["respFLI"] = self._respFLI
        return rv
    ...
    def builder(self):
        cap_msg = super().builder()
        client_msg = cap_msg.init(self.capnp_name)
        client_msg.fullManagerFLI = self._fullManagerFLI
        client_msg.respFLI = self._respFLI
        return cap_msg

    @property
    def fullManagerFLI(self):
        return self._fullManagerFLI
...
class DDManagerGetState(CapNProtoMsg):

    _tc = MessageTypes.DD_MANAGER_GET_STATE
...
class DDManagerGetStateResponse(CapNProtoResponseMsg):

    _tc = MessageTypes.DD_MANAGER_GET_STATE_RESPONSE

    def __init__(self, tag, ref, err, errInfo="", state=""):
        super().__init__(tag, ref, err, errInfo)
        self._state = state
...
class DDMarkDrainedManager(CapNProtoMsg):

    _tc = MessageTypes.DD_MARK_DRAINED_MANAGER

    def __init__(self, tag, respFLI, managerID):
        super().__init__(tag)
        self._respFLI = respFLI
        self._managerID = managerID
...
class DDMarkDrainedManagerResponse(CapNProtoResponseMsg):

    _tc = MessageTypes.DD_MARK_DRAINED_MANAGER_RESPONSE
