...
OPENAI_MESSAGE_CLASSES = {
    PromptRole.SYSTEM: ChatCompletionSystemMessageParam,
    PromptRole.USER: ChatCompletionUserMessageParam,
    PromptRole.ASSISTANT: ChatCompletionAssistantMessageParam,
}
...
class Prompt(base_models._Prompt):
    ...
    def to_provider_content_blocks(
        ...
        """Convert prompt content to provider-specific content blocks"""
    ...
    def to_provider_message_param(
        self,
        provider: Literal[Provider.ANTHROPIC],
        role: Literal[PromptRole.SYSTEM],
        **variables: Any,
    ) -> TextBlockParam: ...
    ...
    def to_provider_message_param(
        self,
        provider: Literal[Provider.ANTHROPIC],
        role: Literal[PromptRole.USER, PromptRole.ASSISTANT],
        **variables: Any,
    ) -> MessageParam: ...
    ...
    def to_provider_message_param(
        ...
        **variables: Any,
        ...
        """
        Convert prompt to provider-specific message format
        ...
        """
        # Use provided role or fall back to the prompt's role
        ...
        if provider == Provider.ANTHROPIC:
            ...
            if effective_role == PromptRole.SYSTEM:
                # Ensure we only have text blocks for system prompts
    ...
    def from_provider_response(
        ...
        """Create a Prompt from a provider response"""

        parsed_response = unpack_llm_response_content(content, provider)
        if not parsed_response.content:
            raise ValueError("Cannot create prompt from empty response content")
