...
@dataclass
class PollingConfig:
    """Configuration for polling tasks and requests."""

    interval: float
    tasks_to_track: list[str]
    requests_to_track: list[str]
    max_retries: int = 3
...
class PollingManager:
    def __init__(
        ...
        fetch_task: Callable,
        fetch_request: Callable,
        store_task: Callable,
        store_request: Callable,
    ...
    async def _check_request_updates(self, request_id: str) -> None:
        """Check for new versions of a request."""
        try:
            # TODO: In future, first check latest version_id via API
            ...
            await self._update_last_polled(request_id, "request", datetime.utcnow())

            logger.info(f"Successfully checked request {request_id} for updates")
        except Exception as e:
            logger.error(f"Error checking updates for request {request_id}: {e}")
            raise
    ...
    async def _poll_loop(self) -> None:
        ...
        while not self._should_stop:
            try:
                ...
                for request_id in self.config.requests_to_track:
                    await self._check_request_updates(request_id)
