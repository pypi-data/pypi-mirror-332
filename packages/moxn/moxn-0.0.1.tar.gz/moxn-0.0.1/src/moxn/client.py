...
from moxn.models.request import Request, RequestInstance
from moxn.models.schema import RequestSchemas, SchemaRequestType, CodegenResponse
...
class MoxnClient(TelemetryTransportBackend):
    ...
    async def start_polling(self, config: PollingConfig) -> None:
        """Start polling for new versions of tasks and requests."""
        ...
        self._polling_manager = PollingManager(
            ...
            store_request=self.storage.store_request,
    ...
    async def fetch_request(
        self, request_id: str, version_id: str | None = None
    ) -> Request:
        """
        ...
        response = await self.get(f"/requests/{request_id}", params=params)
        response.raise_for_status()

        return Request.model_validate(response.json())

    async def get_request(
        self, request_id: str, request_version_id: str | None
    ) -> Request:
        """
        Get a request version from storage, fetching from API if not found.
        """
        try:
            return await self.storage.get_request(request_id, request_version_id)
        except KeyError:
            # If not in storage, fetch from API and store
            request = await self.fetch_request(request_id, request_version_id)
            await self.storage.store_request(request)
            return request
    ...
    async def fetch_request_schemas(
        self,
        request_id: str,
        version_id: str,
        schema_request_type: SchemaRequestType = SchemaRequestType.ALL,
    ) -> RequestSchemas:
        """
        Fetch schemas for a specific request version.
        Returns RequestSchemas containing SchemaWithMetadata objects.
        """
        params = {"version_id": version_id, "type": schema_request_type.value}

        response = await self.get(
            f"/requests/{request_id}/schemas",
            params=params,
            headers=self.get_headers(),
        )

        if response.status_code == 404:
            raise KeyError(f"Request {request_id} or version {version_id} not found")

        response.raise_for_status()

        try:
            schemas = response.json()
            return RequestSchemas.model_validate(schemas)
        except Exception as e:
            logger.error(
                f"Error fetching request schemas for {request_id} version {version_id}: {e}",
                exc_info=True,
            )
    ...
    async def get_code_stubs(
        ...
        try:
            params = {
                "version_id": version_id,
                "schema_type": schema_request_type.value,
            }

            response = await self.get(
                f"/requests/{request_id}/codegen",
                params=params,
                headers=self.get_headers(),
            ...
            if response.status_code != 200:
                logger.error(f"Codegen request failed: {response.status_code}")
                ...
                raise RuntimeError(f"Codegen request failed: {error_message}")
            ...
            raise RuntimeError("Code stub request timed out") from e
        except Exception as e:
            logger.error(f"Code stub request failed: {e}", exc_info=True)
            raise RuntimeError("Code stub request failed") from e
    ...
    async def generate_code_stubs(
        ...
        """
        Generate Python type stubs from request schemas.

        Args:
            request_id: The request ID
            version_id: The version ID
            schema_request_type: Type of schemas to generate (input, output, or all)
        ...
        Raises:
            ...
            RuntimeError: If the codegen request fails
            IOError: If file operations fail
        """
        logger.info(
            f"Generating code stubs for request {request_id} version {version_id}"
        )
        ...
        try:
            ...
            codegen_response = await self.get_code_stubs(
                request_id=request_id,
                version_id=version_id,
                schema_request_type=schema_request_type,
            )
            logger.info(f"Generated {len(codegen_response.files)} files")
            ...
            logger.error(f"Codegen request timed out: {e}")
    ...
    async def create_telemetry_log(
        self,
        request: SpanLogRequest | SpanEventLogRequest,
    ) -> TelemetryLogResponse:
        """Send telemetry log to the API."""
        logger.debug(f"Sending telemetry log: {request}")
        ...
        try:
            ...
            logger.error(f"Telemetry request timed out: {e}")
            raise RuntimeError("Telemetry request timed out") from e
        except Exception as e:
            logger.error(f"Telemetry request failed: {e}", exc_info=True)
            raise RuntimeError("Failed to send telemetry log") from e

    async def create_request_instance(
        self,
        request_id: str,
        ...
        **variables,
        ...
        """
        Create a new RequestInstance for managing LLM interactions.

        Args:
            request_id: The base request ID
            version_id: Optional specific version
            prompt_names: Optional list of prompt names to include
            **variables: Variables to use in prompt rendering
        """
        request = await self.get_request(request_id, version_id)
        return request.create_instance(prompt_names=prompt_names, **variables)
    ...
    async def span(
        ...
        Example:
            async with client.span("agent_task", request_id=req_id, request_version_id=ver_id, kind="agent"):
