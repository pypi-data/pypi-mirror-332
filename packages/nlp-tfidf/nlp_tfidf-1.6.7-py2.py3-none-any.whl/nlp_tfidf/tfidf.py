...
class Tfidf:
    ...
    def load_stop_words(self):
        if self.stopWordsList is not None:
    ...
    def save_model(
        ...
        try:
            ...
            self.save_answer(
                answer_list=answer_list, filepath="%s/%s_answer.json" % (dirname, file)
            )
            # save question
            self.save_question(
    ...
    def load_dictionary(self, file):
        if self.loadDict is None:
            self.loadDict = corpora.Dictionary.load("%s.dict" % (file,))
        return self.loadDict

    # 加载 tf-idf model
    def load_tf_model(self, file):
        if self.loadTfModel is None:
            self.loadTfModel = models.TfidfModel.load("%s.model" % (file,))
        return self.loadTfModel

    # load index 加载索引
    def load_index(self, file):
        if self.loadIndex is None:
            self.loadIndex = similarities.MatrixSimilarity.load("%s.index" % (file,))
        return self.loadIndex

    # load answer json
    def load_answer(self, file):
        if self.loadAnswer is None:
            with open("%s_answer.json" % (file,), mode="r", encoding="utf-8") as f:
                data = f.read()
            data = json.loads(data)
            self.loadAnswer = data
        return self.loadAnswer

    # load question json
    def load_question(self, file):
        if self.loadQuestion is None:
            with open("%s_question.json" % (file,), mode="r", encoding="utf-8") as f:
                data = f.read()
            data = json.loads(data)
            self.loadQuestion = data
    ...
    def quickRun(self, originQuestions=[], matchQuestion=""):
        ...
        # 对结果进行排序和过滤
        results = sorted(
            [(i, sim) for i, sim in enumerate(sims)], key=lambda x: x[1], reverse=True
        )
        filtered_results = [
            result for result in results if result[1] > self.DIFFERENCE_SIMS_NUM
        ]

        return [
            {"index": index, "sims": sim, "text": originQuestions[index]}
            for index, sim in filtered_results
        ]
