...
from app.services.api_services import API, SessionMode
    ...
    async def rest(self, req: Request):
        try:
            ...
            target_query = await self.api.db_pools.get_pool(defaultPool).db_dao.get_framework_query(
                query, self.api.db_pools.get_pool(defaultPool).db_type
            )

            api = await self.api.db_pools.get_pool(defaultPool).db_dao.get(target_query, context)
            rows = api.get("rows")
