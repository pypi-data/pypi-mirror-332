...
from agox_extra.symmetry import SymmetryGenerator, SymmetryRattleGenerator


@pytest.fixture(scope="module")
def gofee_slab_symmetry(tmp_path_factory) -> Database:
    ...
    model = GPR(descriptor=descriptor, kernel=kernel, database=database, prior=Repulsive(), use_ray=False)
    ...
    random_generator = SymmetryGenerator(**environment.get_confinement(), sym_type="slab")
...
def test_has_candidates(gofee_slab_symmetry: Database) -> None:
    assert len(gofee_slab_symmetry.get_all_candidates()) != 0
