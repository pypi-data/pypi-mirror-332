...
from agox_extra.symmetry import (
    SymmetryGenerator,
...
def generator_factory(environment: Environment) -> SymmetryGenerator:
...
def rattle_generator_factory(environment: Environment) -> SymmetryRattleGenerator:
    rattle_generator = SymmetryRattleGenerator(
        **environment.get_confinement(), let_lose_sym=1.0
    )
    return rattle_generator


def permutation_generator_factory(environment: Environment) -> SymmetryPermutationGenerator:
    permutation_generator = SymmetryPermutationGenerator(
        **environment.get_confinement()
    )
    return permutation_generator


def test_symmetry_generator() -> None:
    np.random.seed(1)
...
def test_symmetry_rattle_generator() -> None:
    np.random.seed(1)
    environment = environment_factory()
    generator = generator_factory(environment)
...
def test_symmetry_permutation_generator() -> None:
    np.random.seed(1)
    environment = environment_factory()
    generator = generator_factory(environment)
