...
class client:
    ...
    def _login_via_sso(client_id : str, open_id_config : Dict, SERVER_URL : str,
        ...
        user_settings.token = token
        try:
            user_settings.user_sub = client.__session.get(f"{SERVER_URL}{PREFIX}/token/userinfo").json()['sub']
    ...
    def __handle_request(url, method, data = None, json_data=None, params=None, headers = None, api_version = "v2"):
        ...
        try:
            response = method(f'{client.url(api_version)}{url}', data=data, json=json_data, params=params, headers=headers, timeout=10)
    ...
    if response.status_code >=400:
        try:
            ...
            if "error" in json_response:
                http_exception_handler(json_response)
            else:
