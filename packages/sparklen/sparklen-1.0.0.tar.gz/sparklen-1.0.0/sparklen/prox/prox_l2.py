...
from sparklen.prox.build.prox import ProxL2 as CppProxL2
...
class ProxL2(Prox):
    """
    Proximal operator class for the L2 squared norm. 
    
    This class implements the proximal operator of the :math:`\ell_2` norm, in 
    the case of Ridge (Tikhonov) regularization.
    ...
    def apply(self, x, step_size):
        ...
        self._cpp_prox = CppProxL2(self._pen_const, self._start, self._end, self._positive)
        
        return self._cpp_prox.apply(x, step_size)
        
    def print_info(self):
        """ Display information about the instantiated model object. """
        table = [["Proximal operator", "Shrinkage operator"],
                 ["Induced regularization", "Ridge"],
