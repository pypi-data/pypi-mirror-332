...
// Constructor
ProxZero::ProxZero(double pen_const, size_t st, size_t ed, bool pos){
...
double ProxZero::apply_single(const double coeff, const double step_size) {
	if (coeff < 0 && positive) {
		return 0;
	} else {
		return coeff;
...
void ProxZero::apply(SharedArrayDouble2D &x, const double step_size) {
