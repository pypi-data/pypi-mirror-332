...
class ValkeyDictJSONEncoder(json.JSONEncoder):
    """
...
class ValkeyDictJSONDecoder(json.JSONDecoder):
    """
    JSON decoder leveraging ValkeyDict existing type conversion system.

    Works with ValkeyDictJSONEncoder to reconstruct Python objects from JSON using
    ValkeyDict decoding_registry.
...
def encode_json(obj: Any) -> str:
    ...
    """
    return json.dumps(obj, cls=ValkeyDictJSONEncoder)
...
def decode_json(s: str) -> Any:
    ...
    """
    return json.loads(s, cls=ValkeyDictJSONDecoder)
