...
class MinMaxPriorityQueue(AbstractQueue):
    ...
    *Performance notes:*
    ...
    class Builder:
        ...
        Type `<B>`: the upper bound on the eventual type that can be produced by this builder (for
            example, a `Builder<Number>` can produce a `Queue<Number>` or `Queue<Integer>` but not a `Queue<Object>`).
