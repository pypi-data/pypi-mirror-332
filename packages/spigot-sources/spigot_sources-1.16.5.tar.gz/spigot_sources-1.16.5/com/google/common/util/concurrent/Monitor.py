...
class Monitor:
    ...
    a condition becomes True (no "signaling storms" due to use of java.util.concurrent.locks.Condition.signalAll Condition.signalAll) and that no signals are lost
    ...
    *left*. Only one thread may occupy a given monitor at any moment. A monitor is also
    ...
    the current thread leaves the monitor cleanly: ```   `if (monitor.tryEnter()) {
        try {
          // do things while occupying the monitor` finally {
          monitor.leave();
        }
    ...
    logical conditions being awaited. ```   `public class SafeBox<V> {
        private V value;
    ...
    benefit. ```   `public class SafeBox<V> {
    ...
    remember to use `while` instead of `if`. ```   `public class SafeBox<V> {
    ...
    def newGuard(self, isSatisfied: "BooleanSupplier") -> "Guard":
        ...
        Unknown Tags
        - isSatisfied The guards boolean condition.  See Guard.isSatisfied.
        """
        ...
