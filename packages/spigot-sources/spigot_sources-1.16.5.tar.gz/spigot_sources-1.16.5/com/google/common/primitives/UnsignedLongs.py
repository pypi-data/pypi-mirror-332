...
class UnsignedLongs:
    ...
    *unsigned* (that is, any negative value `x` is treated as the positive value
    `2^64 + x`). The methods for which signedness is not an issue are in Longs, as well
    ...
    def parseUnsignedLong(string: str) -> int:
        ...
        - NumberFormatException: if the string does not contain a valid unsigned `long`
            value
        - NullPointerException: if `string` is null (in contrast to
    ...
    def parseUnsignedLong(string: str, radix: int) -> int:
        ...
        - NumberFormatException: if the string does not contain a valid unsigned `long` with
            the given radix, or if `radix` is not between Character.MIN_RADIX and
            Character.MAX_RADIX.
        - NullPointerException: if `string` is null (in contrast to
            Long.parseLong(String))
        """
