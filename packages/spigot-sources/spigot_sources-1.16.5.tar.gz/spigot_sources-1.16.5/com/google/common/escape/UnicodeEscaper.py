...
class UnicodeEscaper(Escaper):
    ...
    For example, an XML escaper would convert the literal string `"Foo<Bar>"` into `"Foo&lt;Bar&gt;"` to prevent `"<Bar>"` from being confused with an
    XML tag. When the resulting XML document is parsed, the parser API will return this text as the
    original literal string `"Foo<Bar>"`.
    ...
    **Note:** This class is similar to CharEscaper but with one very important
    ...
    create your own escapers extend this class and implement the .escape(int) method.
    ...
    def escape(self, string: str) -> str:
        ...
        method will throw IllegalArgumentException. You should ensure your input is valid
