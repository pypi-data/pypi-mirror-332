...
from com.google.common.collect import TreeTraverser
...
class MoreFiles:
    ...
    @staticmethod
    def directoryTreeTraverser() -> "TreeTraverser"["Path"]:
        """
        Returns a TreeTraverser for traversing a directory tree. The returned traverser
        ...
        Note that if the Path passed to one of the traversal methods does not exist, no
        exception will be thrown and the returned Iterable will contain a single element: that
        path.
        
        DirectoryIteratorException  may be thrown when iterating Iterable instances
        created by this traverser if an IOException is thrown by a call to
        .listFiles(Path).
        """
        ...
    ...
    def isDirectory(*options: Tuple["LinkOption", ...]) -> "Predicate"["Path"]:
        """
        Returns a predicate that returns the result of Files.isDirectory(Path, LinkOption...)
    ...
    def deleteRecursively(path: "Path", *options: Tuple["RecursiveDeleteOption", ...]) -> None:
        ...
        SecureDirectoryStream, it is possible for a recursive delete to delete files and
        directories that are *outside* the directory being deleted. This can happen if, after
        ...
        anyway, pass RecursiveDeleteOption.ALLOW_INSECURE to this method to override that
        behavior.

        Raises
        - NoSuchFileException: if `path` does not exist *(optional specific
            exception)*
    ...
    def deleteDirectoryContents(path: "Path", *options: Tuple["RecursiveDeleteOption", ...]) -> None:
        """
        Deletes all files within the directory at the given `path`
        .deleteRecursively recursively. Does not delete the directory itself. Deletes
        symbolic links, not their targets (subject to the caveat below). If `path` itself is
