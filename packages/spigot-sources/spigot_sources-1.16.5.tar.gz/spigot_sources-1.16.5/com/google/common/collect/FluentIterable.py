...
class FluentIterable(Iterable):
    ...
    - chaining methods which return a new `FluentIterable` based in some way on the contents
    ...
    - element extraction methods which facilitate the retrieval of certain elements (for example
        .last)
    - query methods which answer questions about the `FluentIterable`'s contents (for example
        .anyMatch)
    - conversion methods which copy the `FluentIterable`'s contents into a new collection or
    ...
    - A stream is *single-use*; it becomes invalid as soon as any "terminal operation" such as
        ...
        all the right method *signatures* to implement Iterable, it does not actually do
        so, to avoid implying repeat-iterability.) `FluentIterable`, on the other hand, is
    ...
    - Streams include primitive-specialized variants such as `IntStream`, the use of which is
    ...
    def from(iterable: Iterable["E"]) -> "FluentIterable"["E"]:
        ...
        **`Stream` equivalent:** Collection.stream if `iterable` is a
        Collection; Streams.stream(Iterable) otherwise.
        """
        ...
    ...
    @staticmethod
    def of(elements: list["E"]) -> "FluentIterable"["E"]:
        """
        Returns a fluent iterable containing `elements` in the specified order.
        ...
        **`Stream` equivalent:** Stream.of(T...).
        ...
        Deprecated
        - Use .from(E[]) instead (but note the differences in mutability). This
    ...
    def cycle(self) -> "FluentIterable"["E"]:
        ...
        this fluent iterable. The iterator's `hasNext()` method returns `True` until this
    ...
    def last(self) -> "Optional"["E"]:
        ...
        iterable is empty, `Optional.absent()` is returned. If the underlying `iterable`
        is a List with java.util.RandomAccess support, then this operation is
