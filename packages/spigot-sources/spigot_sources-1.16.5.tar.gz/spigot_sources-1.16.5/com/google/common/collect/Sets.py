...
class Sets:
    ...
    def newHashSet() -> set["E"]:
        ...
        **Note:** if mutability is not required, use ImmutableSet.of() instead. If
        `E` is an Enum type, use EnumSet.noneOf instead. Otherwise, strongly
    ...
    def newConcurrentHashSet(elements: Iterable["E"]) -> set["E"]:
        ...
        elements. The set is backed by a ConcurrentHashMap instance, and
        ...
        Raises
        - NullPointerException: if `elements` or any of its contents is
             null
    ...
    def complementOf(collection: Iterable["E"]) -> "EnumSet"["E"]:
        ...
        method has the same behavior as EnumSet.complementOf. Otherwise,
        ...
        Returns
        - a new, modifiable `EnumSet` containing all values of the enum
    ...
    def complementOf(collection: Iterable["E"], type: type["E"]) -> "EnumSet"["E"]:
        ...
        EnumSet.complementOf, but can act on any input collection, as long
        ...
        Returns
        - a new, modifiable `EnumSet` initially containing all the
    ...
    def newSetFromMap(map: dict["E", "Boolean"]) -> set["E"]:
        ...
        corresponding Set implementation (such as java.util.HashMap
        ...
        in the following code fragment: ```  `Set<Object> identityHashSet = Sets.newSetFromMap(
        ...
        Deprecated
        - Use Collections.newSetFromMap instead. This method
    ...
    def intersection(set1: set["E"], set2: set[Any]) -> "SetView"["E"]:
        ...
        **Note:** The returned view performs slightly better when `set1` is the smaller of the two sets. If you have reason to believe one of
        ...
        you to make a cast, for example: ```   `Set<Object> aFewBadObjects = ...
    ...
    def filter(unfiltered: "SortedSet"["E"], predicate: "Predicate"["E"]) -> "SortedSet"["E"]:
        ...
        **Warning:** `predicate` must be *consistent with equals*,
        as documented at Predicate.apply. Do not provide a predicate such as
    ...
    def cartesianProduct(sets: list[set["B"]]) -> set[list["B"]]:
        ...
        product</a>" of the sets. For example: ```   `Sets.cartesianProduct(ImmutableList.of(
              ImmutableSet.of(1, 2),
        ...
        *Performance notes:* while the cartesian product of sets of size
        ...
        Arguments
        - sets: the sets to choose elements from, in the order that
            the elements chosen from those sets should appear in the resulting
            lists

        Returns
        - the Cartesian product, as an immutable set containing immutable
            lists
    ...
    def cartesianProduct(*sets: Tuple[set["B"], ...]) -> set[list["B"]]:
        ...
        from each of the given sets in order; the "n-ary
        <a href="http://en.wikipedia.org/wiki/Cartesian_product">Cartesian
        product</a>" of the sets. For example: ```   `Sets.cartesianProduct(
              ImmutableSet.of(1, 2),
              ImmutableSet.of("A", "B", "C"))````
    ...
    def powerSet(set: set["E"]) -> set[set["E"]]:
        ...
        *Performance notes:* while the power set of a set with size `n` is of size `2^n`, its memory usage is only `O(n)`. When the
        ...
        Raises
        - IllegalArgumentException: if `set` has more than 30 unique
            elements (causing the power set size to exceed the `int` range)
        - NullPointerException: if `set` is or contains `null`

        See
        - <a href="http://en.wikipedia.org/wiki/Power_set">Power set article at
             Wikipedia</a>
    ...
    def synchronizedNavigableSet(navigableSet: "NavigableSet"["E"]) -> "NavigableSet"["E"]:
        ...
        **all** access to the backing navigable set is accomplished
        ...
        `subSet`, `headSet`, or `tailSet` views. ```   `NavigableSet<E> set = synchronizedNavigableSet(new TreeSet<E>());
           ...
          synchronized (set) {
        ...
        or: ```   `NavigableSet<E> set = synchronizedNavigableSet(new TreeSet<E>());
    ...
    def subSet(set: "NavigableSet"["K"], range: "Range"["K"]) -> "NavigableSet"["K"]:
        ...
        This method delegates to the appropriate methods of NavigableSet (namely
        NavigableSet.subSet(Object, boolean, Object, boolean) subSet(),
        NavigableSet.tailSet(Object, boolean) tailSet(), and
        NavigableSet.headSet(Object, boolean) headSet()) to actually construct the view.
        ...
        **Warning:** `Range`s always represent a range of values using the values' natural
        ordering. `NavigableSet` on the other hand can specify a custom ordering via a
        Comparator, which can violate the natural ordering. Using this method (or in general
        using `Range`) with unnaturally-ordered sets can lead to unexpected and undefined
        behavior.
