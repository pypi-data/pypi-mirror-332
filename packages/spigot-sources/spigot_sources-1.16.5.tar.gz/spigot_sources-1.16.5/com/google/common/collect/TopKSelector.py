...
class TopKSelector:
    ...
    If your input data is available as an Iterable or Iterator, prefer
    Ordering.leastOf(Iterable, int), which provides the same implementation with an
    ...
    .offer and a call to .topK, with O(k) memory. In comparison, quickselect has the
    same asymptotics but requires O(n) memory, and a `PriorityQueue` implementation takes O(n
    ...
    def offer(self, elem: "T") -> None:
        """
        Adds `elem` as a candidate for the top `k` elements. This operation takes
        amortized O(1) time.
        """
        ...
    ...
    def offerAll(self, elements: Iterable["T"]) -> None:
        ...
        If all input data to this `TopKSelector` is in a single `Iterable`,
        prefer Ordering.leastOf(Iterable, int), which provides a simpler API for that use
        case.
        """
        ...
    ...
    def offerAll(self, elements: Iterator["T"]) -> None:
        ...
        If all input data to this `TopKSelector` is in a single `Iterator`,
        prefer Ordering.leastOf(Iterator, int), which provides a simpler API for that use
        case.
        """
        ...
