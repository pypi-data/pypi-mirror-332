...
class Quantiles:
    ...
    If any values in the input are Double.NaN NaN then all values returned are
    Double.NaN NaN. (This is the one occasion when the behaviour is not the same as you'd get
    from sorting with java.util.Arrays.sort(double[]) Arrays.sort(double[]) or
    java.util.Collections.sort(java.util.List) Collections.sort(List&lt;Double&gt;) and
    selecting the required value(s). Those methods would sort Double.NaN NaN as if it is
    ...
    Otherwise, Double.NEGATIVE_INFINITY NEGATIVE_INFINITY and
    Double.POSITIVE_INFINITY POSITIVE_INFINITY sort to the beginning and the end of the
    ...
    ScaleAndIndex.computeInPlace computeInPlace (in
    ScaleAndIndexes.computeInPlace either form), only the overhead is required. The
    ...
    class ScaleAndIndexes:
        ...
        def compute(self, dataset: Iterable["Number"]) -> dict["Integer", "Double"]:
            ...
            Returns
            - an unmodifiable map of results: the keys will be the specified quantile indexes, and
