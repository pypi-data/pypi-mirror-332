...
class HashFunction:
    ...
    - **block of data:** the input for a hash function is always, in concept, an ordered byte
        ...
        Hasher), but this is merely a convenience; these are always translated into raw byte
        sequences under the covers.
    
    - **hash code:** each hash function always yields hash codes of the same fixed bit length
        (given by .bits). For example, Hashing.sha1 produces a 160-bit number, while
        Hashing.murmur3_32() yields only 32 bits. Because a `long` value is clearly
    ...
    - **collision-averse:** while it can't be helped that a hash function will sometimes produce
        the same hash code for distinct inputs (a "collision"), every hash function strives to
        *some* degree to make this unlikely. (Without this condition, a function that always
    ...
    *often*." This is the most important characteristic of all hash functions.
    ...
    - **bit-dispersing:** masking out any *single bit* from a hash code should yield only
        the expected *twofold* increase to all collision rates. Informally, the "information" in
        the hash code should be as evenly "spread out" through the hash code's bits as possible. The
        result is that, for example, when choosing a bucket in a hash table of size 2^8, *any*
    ...
    Hasher. Obtain a new hasher from the hash function using .newHasher, "push" the
    relevant data into it using methods like Hasher.putBytes(byte[]), and finally ask for the
    `HashCode` when finished using Hasher.hash. (See an .newHasher example
    of this.)
    ...
    Hasher accepts primitive data types, but can also accept any Object of type `T` provided that you implement a Funnel Funnel<T> to specify how to "feed" data from that
    object into the function. (See Hasher.putObject an example of this.)
    ...
    *little-endian* order. That is, hashing the byte array `{0x01, 0x02, 0x03, 0x04`} is
    ...
    def newHasher(self) -> "Hasher":
        """
        Begins a new hash code computation by returning an initialized, stateful `Hasher` instance that is ready to receive data. Example: ```   `HashFunction hf = Hashing.md5();
