...
class ImmutableCollection(AbstractCollection, Serializable):
    ...
    **Warning:** avoid *direct* usage of ImmutableCollection as a type (just as
    ...
    - **Shallow immutability.** Elements can never be added, removed or replaced in this
        ...
        Collections.unmodifiableCollection, whose contents change whenever the wrapped
        collection is modified.
    - **Null-hostility.** This collection will never contain a null element.
    - **Deterministic iteration.** The iteration order is always well-defined, depending on how
        the collection was created (see the appropriate factory method for details). View collections
        such as ImmutableMultiset.elementSet iterate in the same order as the parent, except
        as noted.
    ...
    Each public class, such as ImmutableSet, is a *type* offering meaningful
    behavioral guarantees -- not merely a specific *implementation* as in the case of, say,
    ArrayList. You should treat them as interfaces in every important sense of the word.
    ...
    - The `copyOf` methods will sometimes recognize that the actual copy operation is
        unnecessary; for example, `copyOf(copyOf(anArrayList))` should copy the data only once.
    ...
    - **Warning:** a view collection such as ImmutableMap.keySet or ImmutableList.subList may retain a reference to the entire data set, preventing it from
        ...
        constitutes a memory leak. Pass the view collection to the appropriate `copyOf` method
        to obtain a correctly-sized copy.
    - The performance of using the associated `Builder` class can be assumed to be
    ...
    class Builder:
        ...
        def build(self) -> "ImmutableCollection"["E"]:
            ...
            of `ImmutableCollection` from this method.
            """
            ...
