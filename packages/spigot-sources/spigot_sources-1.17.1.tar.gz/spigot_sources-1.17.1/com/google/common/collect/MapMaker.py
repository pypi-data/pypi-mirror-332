...
class MapMaker:
    ...
    wrapped in WeakReference weak references.
    
    Usage example: ```   `ConcurrentMap<Request, Stopwatch> timers = new MapMaker()
          .concurrencyLevel(4)
          .weakKeys()
          .makeMap();````
    ...
    **Note:** by default, the returned map uses equality comparisons (the Object.equals
    ...
    specified, the map uses identity (`==`) comparisons instead for keys. Likewise, if
    ...
    modification or on occasional map accesses; such entries may be counted by Map.size, but
    will never be visible to read or write operations. A partially-reclaimed entry is never exposed
    to the user. Any java.util.Map.Entry instance retrieved from the map's
    Map.entrySet entry set is a snapshot of that entry's state at the time of retrieval;
    such entries do, however, support java.util.Map.Entry.setValue, which simply calls
    Map.put on the entry's key.
    ...
    `new MapMaker().weakKeys().makeMap()` is a recommended replacement for
    java.util.WeakHashMap, but note that it compares keys using object identity whereas
    ...
    def makeMap(self) -> "ConcurrentMap"["K", "V"]:
        ...
        `containsValue` are implemented as bulk read operations, and thus may fail to observe
        concurrent writes.
