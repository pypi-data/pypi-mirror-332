...
class AtomicLongMap(Serializable):
    ...
    documented. Exceptions to this are .containsKey, .size, .isEmpty,
    .asMap, and .toString.
    ...
    **Note:** If your values are always positive and less than 2^31, you may wish to use a
    com.google.common.collect.Multiset such as
    ...
    **Warning:** Unlike `Multiset`, entries whose values are zero are not automatically
    ...
    def updateAndGet(self, key: "K", updaterFunction: "LongUnaryOperator") -> int:
        """
        Updates the value currently associated with `key` with the specified function,
        and returns the new value.  If there is not currently a value associated with `key`,
        the function is applied to `0L`.
    ...
    def getAndUpdate(self, key: "K", updaterFunction: "LongUnaryOperator") -> int:
        """
        Updates the value currently associated with `key` with the specified function,
        and returns the old value.  If there is not currently a value associated with `key`,
        the function is applied to `0L`.
    ...
    def accumulateAndGet(self, key: "K", x: int, accumulatorFunction: "LongBinaryOperator") -> int:
        """
        Updates the value currently associated with `key` by combining it with `x`
        via the specified accumulator function, returning the new value.  The previous value
        associated with `key` (or zero, if there is none) is passed as the first argument
        to `accumulatorFunction`, and `x` is passed as the second argument.
