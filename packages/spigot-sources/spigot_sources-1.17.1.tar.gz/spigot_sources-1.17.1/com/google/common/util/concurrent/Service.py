...
class Service:
    ...
    def stopAsync(self) -> "Service":
        ...
        State.NEW new, it is State.TERMINATED terminated without having been
    ...
    def awaitRunning(self) -> None:
        ...
        - IllegalStateException: if the service reaches a state from which it is not possible to
            enter the State.RUNNING state. e.g. if the `state` is
    ...
    def awaitRunning(self, timeout: int, unit: "TimeUnit") -> None:
        ...
        - IllegalStateException: if the service reaches a state from which it is not possible to
            enter the State.RUNNING RUNNING state. e.g. if the `state` is
            `State.TERMINATED` when this method is called then this will throw an
            IllegalStateException.
