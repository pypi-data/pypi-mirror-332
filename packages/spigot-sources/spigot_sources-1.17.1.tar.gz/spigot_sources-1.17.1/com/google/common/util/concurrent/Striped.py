...
class Striped:
    ...
    *stripes*, increasing the granularity of a single lock and allowing independent operations
    ...
    <strong>not</strong> equal to `key2`, it is <strong>not</strong> guaranteed that
    `striped.get(key1) != striped.get(key2)`; the elements might nevertheless be mapped to the
    ...
    `Striped<ReadWriteLock>`. For each type, two implementations are offered:
    .lock(int) strong and .lazyWeakLock(int) weak `Striped<Lock>`,
    .semaphore(int, int) strong and .lazyWeakSemaphore(int, int) weak
    `Striped<Semaphore>`, and .readWriteLock(int) strong and
    .lazyWeakReadWriteLock(int) weak `Striped<ReadWriteLock>`. *Strong* means
    that all stripes (locks/semaphores) are initialized eagerly, and are not reclaimed unless
    `Striped` itself is reclaimable. *Weak* means that locks/semaphores are created
    lazily, and they are allowed to be reclaimed if nobody is holding on to them. This is useful, for
    example, if one wants to create a `Striped<Lock>` of many locks, but worries that in most cases only a small portion of these would
    be in use.
    ...
    def bulkGet(self, keys: Iterable[Any]) -> Iterable["L"]:
        """
        Returns the stripes that correspond to the passed objects, in ascending (as per
        .getAt(int)) order. Thus, threads that use the stripes in the order returned by this
        ...
        `bulkGet(keys)` with a relative large number of keys can cause an excessive number of
        ...
        `bulkGet(keys)` operation. See <a href="http://www.mathpages.com/home/kmath199.htm">Balls
