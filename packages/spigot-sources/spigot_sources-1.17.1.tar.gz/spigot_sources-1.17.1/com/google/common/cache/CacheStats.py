...
class CacheStats:
    ...
    - After successfully loading an entry `missCount` and `loadSuccessCount` are
        incremented, and the total loading time, in nanoseconds, is added to `totalLoadTime`.
    - When an exception is thrown while loading an entry, `missCount` and `loadExceptionCount` are incremented, and the total loading time, in nanoseconds, is added to
        `totalLoadTime`.
    - Cache lookups that encounter a missing cache entry that is still loading will wait for
        loading to complete (whether successful or not) and then increment `missCount`.
    ...
    - No stats are modified by operations invoked on the Cache.asMap asMap view of the
        cache.
    
    
    A lookup is specifically defined as an invocation of one of the methods
    LoadingCache.get(Object), LoadingCache.getUnchecked(Object),
    Cache.get(Object, Callable), or LoadingCache.getAll(Iterable).
    ...
    def hitRate(self) -> float:
        ...
        `hitCount / requestCount`, or `1.0` when `requestCount == 0`. Note that
    ...
    def missRate(self) -> float:
        ...
        `missCount / requestCount`, or `0.0` when `requestCount == 0`. Note that
        `hitRate + missRate =~ 1.0`. Cache misses include all requests which weren't cache hits,
        ...
        `missCount &gt;= loadSuccessCount + loadExceptionCount`. Multiple concurrent misses for
    ...
    def loadSuccessCount(self) -> int:
        ...
        single load operation. This may be incremented not in conjunction with `missCount` if the
        load occurs as a result of a refresh or if the cache loader returned more items than was
        requested. `missCount` may also be incremented not in conjunction with this (nor
        .loadExceptionCount) on calls to `getIfPresent`.
        """
        ...
    ...
    def loadExceptionCount(self) -> int:
        ...
        value. This is usually incremented in conjunction with `missCount`, though
        `missCount` is also incremented when cache loading completes successfully (see
        .loadSuccessCount). Multiple concurrent misses for the same key will result in a single
        load operation. This may be incremented not in conjunction with `missCount` if the load
        ...
        .loadSuccessCount) on calls to `getIfPresent`.
        """
        ...
    ...
    def plus(self, other: "CacheStats") -> "CacheStats":
        """
        Returns a new `CacheStats` representing the sum of this `CacheStats` and
        `other`.
