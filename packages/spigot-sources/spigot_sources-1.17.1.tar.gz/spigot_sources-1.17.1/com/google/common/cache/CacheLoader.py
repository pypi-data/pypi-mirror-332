...
class CacheLoader:
    ...
    Usage example: ```   `CacheLoader<Key, Graph> loader = new CacheLoader<Key, Graph>() {
    ...
    def from(function: "Function"["K", "V"]) -> "CacheLoader"["K", "V"]:
        ...
        to create a *new* function just to pass it in here; just subclass `CacheLoader` and
    ...
    def asyncReloading(loader: "CacheLoader"["K", "V"], executor: "Executor") -> "CacheLoader"["K", "V"]:
        """
        Returns a `CacheLoader` which wraps `loader`, executing calls to
        CacheLoader.reload using `executor`.
