...
class FinalizableReferenceQueue(Closeable):
    ...
    java.net.ServerSocket ServerSocket, and you would like to ensure that the
    `ServerSocket` is closed even if the `MyServer` object is garbage-collected without
    calling its `close` method. You *could* use a finalizer to accomplish this, but that
