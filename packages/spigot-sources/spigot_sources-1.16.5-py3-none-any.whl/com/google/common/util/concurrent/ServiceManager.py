...
class ServiceManager:
    ...
    def addListener(self, listener: "Listener") -> None:
        """
        Registers a Listener to be run when this ServiceManager changes state. The
        listener will not have previous state changes replayed, so it is suggested that listeners are
        added before any of the managed services are Service.startAsync started.
    ...
    def awaitStopped(self) -> None:
        ...
        services will either be Service.State.TERMINATED terminated or
    ...
    class Listener:
        ...
        Service service starts, or terminates), should attach Service.Listener
    ...
    Author(s)
        ...
        def healthy(self) -> None:
            ...
            State.RUNNING running state. If any services fail during start up or
            State.FAILED fail/State.TERMINATED terminate before all other
            services have started State.RUNNING running then this method will not be called.
            """
            ...
