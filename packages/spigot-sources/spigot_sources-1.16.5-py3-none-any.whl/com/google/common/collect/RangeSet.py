...
class RangeSet:
    ...
    Range.isConnected(Range) disconnected ranges of type `C`.
    
    Implementations that choose to support the .add(Range) operation are required to
    ignore empty ranges and coalesce connected ranges.  For example:  ```   `RangeSet<Integer> rangeSet = TreeRangeSet.create();
    ...
    Note that the behavior of Range.isEmpty() and Range.isConnected(Range) may
    ...
    def enclosesAll(self, other: "RangeSet"["C"]) -> bool:
        ...
        this range set which Range.encloses encloses it. It follows that
    ...
    def asDescendingSetOfRanges(self) -> set["Range"["C"]]:
        """
        Returns a descending view of the Range.isConnected disconnected ranges that
        make up this range set. The returned set may be empty. The iterators returned by its
        Iterable.iterator method return the ranges in decreasing order of lower bound
        (equivalently, of upper bound).
    ...
    def subRangeSet(self, view: "Range"["C"]) -> "RangeSet"["C"]:
        ...
        .add(Range) add any range not Range.encloses(Range) enclosed by
        `view`.
        """
        ...
    ...
    def add(self, range: "Range"["C"]) -> None:
        ...
        the range set that are Range.isConnected(Range) connected with it.  Moreover,
    ...
    def addAll(self, other: "RangeSet"["C"]) -> None:
        ...
        .enclosesAll(RangeSet) encloses both the original range set and `other`.
    ...
    def toString(self) -> str:
        ...
        `RangeSet` consisted of `Range.closed(1, 3)` and `Range.greaterThan(4)`,
