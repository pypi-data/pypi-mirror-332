...
class ImmutableSortedMapFauxverideShim(ImmutableMap):
    """
    "Overrides" the ImmutableMap static methods that lack
    ImmutableSortedMap equivalents with deprecated, exception-throwing
    versions. See ImmutableSortedSetFauxverideShim for details.
    ...
    def toImmutableMap(keyFunction: "Function"["T", "K"], valueFunction: "Function"["T", "V"]) -> "Collector"["T", Any, "ImmutableMap"["K", "V"]]:
        """
        Not supported. Use ImmutableSortedMap.toImmutableSortedMap, which offers
        better type-safety, instead. This method exists only to hide
        ImmutableMap.toImmutableMap from consumers of `ImmutableSortedMap`.
    ...
    def builder() -> "ImmutableSortedMap.Builder"["K", "V"]:
        """
        Not supported. Use ImmutableSortedMap.naturalOrder, which offers
        better type-safety, instead. This method exists only to hide
        ImmutableMap.builder from consumers of `ImmutableSortedMap`.

        Raises
        - UnsupportedOperationException: always

        Deprecated
        - Use ImmutableSortedMap.naturalOrder, which offers
            better type-safety.
        """
        ...
    ...
    def of(k1: "K", v1: "V") -> "ImmutableSortedMap"["K", "V"]:
        """
        Not supported. **You are attempting to create a map that may contain a
        non-`Comparable` key.** Proper calls will resolve to the version in
        `ImmutableSortedMap`, not this dummy version.
    ...
    def of(k1: "K", v1: "V", k2: "K", v2: "V") -> "ImmutableSortedMap"["K", "V"]:
        """
        Not supported. **You are attempting to create a map that may contain
        non-`Comparable` keys.** Proper calls will resolve to the version
        in `ImmutableSortedMap`, not this dummy version.
    ...
    def of(k1: "K", v1: "V", k2: "K", v2: "V", k3: "K", v3: "V") -> "ImmutableSortedMap"["K", "V"]:
        """
        Not supported. **You are attempting to create a map that may contain
        non-`Comparable` keys.** Proper calls to will resolve to the
        version in `ImmutableSortedMap`, not this dummy version.
