...
class Iterables:
    ...
    *Performance notes:* Unless otherwise noted, all of the iterables produced in this class
    ...
    @staticmethod
    def contains(iterable: Iterable[Any], element: "Object") -> bool:
        """
    ...
    def removeAll(removeFrom: Iterable[Any], elementsToRemove: Iterable[Any]) -> bool:
        ...
        This method calls Collection.removeAll if `iterable` is a
    ...
    def retainAll(removeFrom: Iterable[Any], elementsToRetain: Iterable[Any]) -> bool:
        ...
        This method calls Collection.retainAll if `iterable` is a
    ...
    def removeIf(removeFrom: Iterable["T"], predicate: "Predicate"["T"]) -> bool:
        ...
        `predicate` is dependent on `removeFrom`.
        
        **Java 8 users:** if `removeFrom` is a Collection,
        use `removeFrom.removeIf(predicate)` instead.

        Arguments
        - removeFrom: the iterable to (potentially) remove elements from
        - predicate: a predicate that determines whether an element should
            be removed
        ...
        Raises
        - UnsupportedOperationException: if the iterable does not support
            `remove()`.
    ...
    def toString(iterable: Iterable[Any]) -> str:
        """
        Returns a string representation of `iterable`, with the format `[e1, e2, ..., en]` (that is, identical to java.util.Arrays
        Arrays`.toString(Iterables.toArray(iterable))`). Note that for
        *most* implementations of Collection, `collection.toString()` also gives the same result, but that behavior is not
    ...
    def toArray(iterable: Iterable["T"], type: type["T"]) -> list["T"]:
        ...
        Returns
        - a newly-allocated array into which all the elements of the iterable
            have been copied
        """
        ...
    ...
    def find(iterable: Iterable["T"], predicate: "Predicate"["T"]) -> "T":
        ...
        it is possible that *no* element will match, use .tryFind or
        .find(Iterable, Predicate, Object) instead.
    ...
    def find(iterable: Iterable["T"], predicate: "Predicate"["T"], defaultValue: "T") -> "T":
        """
        Returns the first element in `iterable` that satisfies the given
        predicate, or `defaultValue` if none found. Note that this can
    ...
    def tryFind(iterable: Iterable["T"], predicate: "Predicate"["T"]) -> "Optional"["T"]:
        ...
        **Warning:** avoid using a `predicate` that matches `null`. If `null` is matched in `iterable`, a
    ...
    def indexOf(iterable: Iterable["T"], predicate: "Predicate"["T"]) -> int:
        ...
        `predicate.apply(Iterables.get(iterable, i))` returns `True`,
    ...
    def transform(fromIterable: Iterable["F"], function: "Function"["F", "T"]) -> Iterable["T"]:
        """
        Returns a view containing the result of applying `function` to each
        element of `fromIterable`.
        
        The returned iterable's iterator supports `remove()` if `fromIterable`'s iterator does. After a successful `remove()` call,
        `fromIterable` no longer contains the corresponding element.
        
        If the input `Iterable` is known to be a `List` or other
        `Collection`, consider Lists.transform and Collections2.transform.
    ...
    def get(iterable: Iterable["T"], position: int, defaultValue: "T") -> "T":
        ...
        **`Stream` equivalent:**
        `stream.skip(position).findFirst().orElse(defaultValue)`
        (returns the default value if the index is out of bounds)

        Arguments
        - position: position of the element to return
        - defaultValue: the default value to return if `position` is
            greater than or equal to the size of the iterable

        Returns
        - the element at the specified position in `iterable` or
            `defaultValue` if `iterable` contains fewer than
            `position + 1` elements.
    ...
    def getLast(iterable: Iterable["T"]) -> "T":
        """
        Returns the last element of `iterable`. If `iterable` is a List with
        RandomAccess support, then this operation is guaranteed to be `O(1)`.
    ...
    def getLast(iterable: Iterable["T"], defaultValue: "T") -> "T":
        """
        Returns the last element of `iterable` or `defaultValue` if
        the iterable is empty. If `iterable` is a List with
        RandomAccess support, then this operation is guaranteed to be `O(1)`.
    ...
    def limit(iterable: Iterable["T"], limitSize: int) -> Iterable["T"]:
        """
        Returns a view of `iterable` containing its first `limitSize`
        elements. If `iterable` contains fewer than `limitSize`
        elements, the returned view contains all of its elements. The returned
        iterable's iterator supports `remove()` if `iterable`'s
        iterator does.
    ...
    def consumingIterable(iterable: Iterable["T"]) -> Iterable["T"]:
        ...
        Note: If `iterable` is a Queue, the returned iterable will
        get entries from Queue.remove() since Queue's iteration
        order is undefined.  Calling Iterator.hasNext() on a generated
        ...
        - a view of the supplied iterable that wraps each generated iterator
            through Iterators.consumingIterator(Iterator); for queues,
    ...
    def mergeSorted(iterables: Iterable[Iterable["T"]], comparator: "Comparator"["T"]) -> Iterable["T"]:
        ...
        `iterables`. Equivalent entries will not be de-duplicated.
        
        Callers must ensure that the source `iterables` are in
        non-descending order as this method does not sort its input.
        
        For any equivalent elements across all `iterables`, it is
