...
from javax.annotation import CheckReturnValue
...
class CacheBuilder:
    ...
    - values automatically wrapped in WeakReference weak or SoftReference
        soft references
    ...
    Usage example: ```   `LoadingCache<Key, Graph> graphs = CacheBuilder.newBuilder()
          .maximumSize(10000)
    ...
    Or equivalently, ```   `// In real life this would come from a command-line flag or config file
    ...
    **Note:** by default, the returned cache uses equality comparisons (the
    Object.equals equals method) to determine equality for keys or values. However, if
    ...
    cache accesses, or on calls to Cache.cleanUp. Expired entries may be counted by
    Cache.size, but will never be visible to read or write operations.
    
    If .weakKeys weakKeys, .weakValues weakValues, or
    .softValues softValues are requested, it is possible for a key or value present in
    ...
    .expireAfterAccess expireAfterAccess, .weakKeys weakKeys,
    .weakValues weakValues, or .softValues softValues perform periodic
    maintenance.
    ...
    *not* include cache contents, but only configuration.
    ...
    Type `<K>`: the base key type for all caches created by this builder
    
    Type `<V>`: the base value type for all caches created by this builder
    ...
    def maximumSize(self, size: int) -> "CacheBuilder"["K", "V"]:
        """
    ...
    def maximumWeight(self, weight: int) -> "CacheBuilder"["K", "V"]:
        """
    ...
    def softValues(self) -> "CacheBuilder"["K", "V"]:
        ...
        **Warning:** in most circumstances it is better to set a per-cache
        .maximumSize(long) maximum size instead of using soft references. You should only
