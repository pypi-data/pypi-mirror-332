...
class Range(RangeGwtSerializationDependencies, Predicate, Serializable):
    """
    ...
    *x* such that *statement*.")
    ...
    <h3>Warnings</h3>
      ...
      - Your value type's comparison method should be Comparable consistent with
    ...
    href="https://github.com/google/guava/wiki/RangesExplained">`Range`</a>.
    ...
    def singleton(value: "C") -> "Range"["C"]:
        """
        Returns a range that Range.contains(Comparable) contains only the given value. The
        returned range is BoundType.CLOSED closed on both ends.
    ...
    def encloseAll(values: Iterable["C"]) -> "Range"["C"]:
        """
        Returns the minimal range that Range.contains(Comparable) contains all of the
        given values. The returned range is BoundType.CLOSED closed on both ends.
    ...
    def gap(self, otherRange: "Range"["C"]) -> "Range"["C"]:
        """
