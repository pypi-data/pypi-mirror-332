...
class TypeToken(TypeCapture, Serializable):
    ...
    - Capture a generic type with a (usually anonymous) subclass. For example: ```   `new TypeToken<List<String>>() {`}```
    ...
    - Capture a generic type with a (usually anonymous) subclass and resolve it against a context
        class that knows what the type parameters are. For example: ```   `abstract class IKnowMyType<T> {
    ...
    def getRawType(self) -> type["T"]:
        ...
        java.lang.reflect.Method.getGenericReturnType, the raw type is what's returned by
    ...
    def where(self, typeParam: "TypeParameter"["X"], typeArg: "TypeToken"["X"]) -> "TypeToken"["T"]:
        ...
        any `K` and `V` type: ```   `static <K, V> TypeToken<Map<K, V>> mapOf(
    ...
    def resolveType(self, type: "Type") -> "TypeToken"[Any]:
        ...
        example: ```   `new TypeToken<List<String>>() {`.resolveType(
    ...
    def getSupertype(self, superclass: type["T"]) -> "TypeToken"["T"]:
        """
        Returns the generic form of `superclass`. For example, if this is
