...
class TypeResolver:
    ...
    def where(self, formal: "Type", actual: "Type") -> "TypeResolver":
        ...
        For example, if `formal` is a `TypeVariable T`, and `actual` is `String.class`, then `new TypeResolver().where(formal, actual)` will
        .resolveType resolve `ParameterizedType List<T>` to `List<String>`,
        and resolve `Map<T, Something>` to `Map<String, Something>` etc. Similarly,
        `formal` and `actual` can be `Map<K, V>` and `Map<String, Integer>`
        respectively, or they can be `E[]` and `String[]` respectively, or even any
