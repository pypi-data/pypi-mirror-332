...
class Splitter:
    ...
    For example, this expression: ```   `Splitter.on(',').split("foo,bar,qux")````
    ...
    expression: ```   `Splitter.on(',').split(" foo,,,  bar ,")````
    ...
    behavior: ```   `private static final Splitter MY_SPLITTER = Splitter.on(',')
          .trimResults()
          .omitEmptyStrings();````
    ...
    **Warning:** Splitter instances are immutable. Invoking a configuration method has no
    ...
    instead. ```   `// Do NOT do this
      Splitter splitter = Splitter.on('/');
      splitter.trimResults(); // does nothing!
      return splitter.split("wrong / wrong / wrong");````
    
    For separator-based splitters that do not use `omitEmptyStrings`, an input string
    containing `n` occurrences of the separator naturally yields an iterable of size
    `n + 1`. So if the separator does not occur anywhere in the input, a single substring is
    ...
    def fixedLength(length: int) -> "Splitter":
        ...
        `["ab", "cd", "e"]`. The last piece can be smaller than `length` but will never be
        empty.
        
        **Exception:** for consistency with separator-based splitters, `split("")` does not yield an empty iterable, but an iterable containing `""`. This is the
        only case in which `Iterables.size(split(input))` does not equal `IntMath.divide(input.length(), length, CEILING)`. To avoid this behavior, use
        `omitEmptyStrings`.
    ...
    def limit(self, limit: int) -> "Splitter":
        """
        ...
        `Splitter.on(',').limit(3).trimResults().split(" a , b , c , d ")` results in
        `["a", "b", "c , d"]`.
    ...
    def trimResults(self) -> "Splitter":
        ...
        substring; equivalent to `trimResults(CharMatcher.whitespace())`. For example, `Splitter.on(',').trimResults().split(" a, b ,c ")` returns an iterable containing
        `["a", "b", "c"]`.
    ...
    def split(self, sequence: "CharSequence") -> Iterable[str]:
        ...
        Iterator, which may be lazily evaluated. If you want an eagerly computed List,
        use .splitToList(CharSequence).
    ...
    class MapSplitter:
        ...
        `Splitter`, it is thread-safe and immutable.
        ...
        def split(self, sequence: "CharSequence") -> dict[str, str]:
            ...
            `Splitter.on(';').trimResults().withKeyValueSeparator("=>").split("a=>b ; c=>b")` will
            return a mapping from `"a"` to `"b"` and `"c"` to `"b"`.
