...
class Ascii:
    ...
    - CharMatcher.ascii matches ASCII characters and provides text processing methods which
    ...
    def truncate(seq: "CharSequence", maxLength: int, truncationIndicator: str) -> str:
        ...
        `maxLength` chars in length and will end with the given `truncationIndicator`.
    ...
    def equalsIgnoreCase(s1: "CharSequence", s2: "CharSequence") -> bool:
        ...
        String.equalsIgnoreCase). However in almost all cases that ASCII strings are used, the
        author probably wanted the behavior provided by this method rather than the subtle and
        sometimes surprising behavior of `toUpperCase()` and `toLowerCase()`.
