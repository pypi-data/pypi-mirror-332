...
class IntMath:
    ...
    `int` values, see com.google.common.primitives.Ints.
    ...
    def ceilingPowerOfTwo(x: int) -> int:
        ...
        - ArithmeticException: of the next-higher power of two is not representable as an
                `int`, i.e. when `x > 2^30`
    ...
    def floorPowerOfTwo(x: int) -> int:
        """
        Returns the largest power of two less than or equal to `x`.  This is equivalent to
    ...
    def sqrt(x: int, mode: "RoundingMode") -> int:
        ...
        Raises
        - IllegalArgumentException: if `x < 0`
        - ArithmeticException: if `mode` is RoundingMode.UNNECESSARY and
    ...
    def mod(x: int, m: int) -> int:
        """
        Returns `x mod m`, a non-negative value less than `m`. This differs from
        `x % m`, which might be negative.
        
        For example:``` `mod(7, 4) == 3
        mod(-7, 4) == 1
        mod(-1, 4) == 3
        mod(-8, 4) == 0
        mod(8, 4) == 0````
    ...
    def checkedPow(b: int, k: int) -> int:
        ...
        Raises
        - ArithmeticException: if `b` to the `k`th power overflows in signed
            `int` arithmetic
        """
        ...
    ...
    def isPrime(n: int) -> bool:
        ...
        <a href="http://mathworld.wolfram.com/PrimeNumber.html">prime number</a>: an integer *greater
