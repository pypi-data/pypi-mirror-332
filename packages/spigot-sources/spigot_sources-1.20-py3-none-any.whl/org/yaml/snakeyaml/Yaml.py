...
class Yaml:
    ...
    def __init__(self, constructor: "BaseConstructor", representer: "Representer", dumperOptions: "DumperOptions"):
        ...
        Arguments
        - constructor: BaseConstructor to construct incoming documents. Its LoaderOptions will be
               used everywhere
        - representer: Representer to emit outgoing objects
        - dumperOptions: DumperOptions to configure outgoing objects
        """
    ...
    def represent(self, data: "Object") -> "Node":
        ...
        See
        - <a href="http://yaml.org/spec/1.1/.id859333">Figure 3.1. Processing Overview</a>
        """
        ...
    ...
    def dumpAs(self, data: "Object", rootTag: "Tag", flowStyle: "FlowStyle") -> str:
        ...
        This method is similar to `Yaml.dump(data)` except that the root tag for the whole
        document is replaced with the given tag. This has two main uses.
        
        
        
        First, if the root tag is replaced with a standard YAML tag, such as `Tag.MAP`, then
        the object will be dumped as a map. The root tag will appear as `!!map`, or blank
        (implicit !!map).
        ...
        Arguments
        - data: Java object to be serialized to YAML
        - rootTag: the tag for the whole YAML document. The tag should be Tag.MAP for a JavaBean to
               make the tag disappear (to use implicit tag !!map). If `null` is provided
               then the standard tag with the full class name is used.
        - flowStyle: flow style for the whole document. See Chapter 10. Collection Styles
               http://yaml.org/spec/1.1/#id930798. If `null` is provided then the flow style
    ...
    def dumpAsMap(self, data: "Object") -> str:
        ...
        This method is similar to `Yaml.dump(data)` except that the root tag for the whole
        document is replaced with `Tag.MAP` tag (implicit !!map).
    ...
    def loadAll(self, yaml: "InputStream") -> Iterable["Object"]:
        ...
        Arguments
        - yaml: YAML data to load from (BOM is respected to detect encoding and removed from the
               data)
    ...
    def addImplicitResolver(self, tag: "Tag", regexp: "Pattern", first: str, limit: int) -> None:
        """
        Add an implicit scalar detector. If an implicit scalar value matches the given regexp, the
        corresponding tag is assigned to the scalar.
