...
class StyleProcessor(BaseProcessor):
    ...
    def _apply_styles(self, xml: str) -> str:
        ...
        try:
            ...
            open('hahahaxml.xml', 'wb').write(xml.encode())
            ...
            for selector, style_str in self.styles.items():
                ...
                for elem in matched_elements:
                    ...
                    if current_style:
                        current_styles = dict(item.split(':') for item in current_style.split(';') if ':' in item)
                        new_styles = dict(item.split(':') for item in style_str.split(';') if ':' in item)
                        current_styles.update(new_styles)
                        merged_style = ';'.join(f"{k}:{v}" for k, v in current_styles.items())
