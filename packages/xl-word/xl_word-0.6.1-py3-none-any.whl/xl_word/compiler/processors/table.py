...
class TableProcessor(BaseProcessor):
    ...
    def decompile_tbl(cls, xml: str) -> str:
        ...
        # 使用正则表达式匹配w:tbl标签及其内容
        pattern = r'<w:tbl>.*?</w:tbl>'
        result = re.sub(pattern, process_word_table, xml, flags=re.DOTALL)
        return result
    ...
    def decompile_tr(cls, xml: str) -> str:
        def process_word_tr(match):
            ...
            content_match = re.search(r'<w:tr[^>]*>(?:<w:trPr>.*?</w:trPr>)?(.*?)</w:tr>', full_tr, re.DOTALL)
            content = content_match.group(1) if content_match else ""
        ...
        pattern = r'<w:tr[^>]*>.*?</w:tr>'
        result = re.sub(pattern, process_word_tr, xml, flags=re.DOTALL)
        return result

    @classmethod
    def decompile_tc(cls, xml: str) -> str:
        ...
        # 使用正则表达式匹配w:tc标签及其内容
        pattern = r'<w:tc>.*?</w:tc>'
        result = re.sub(pattern, process_word_tc, xml, flags=re.DOTALL)
        return result
