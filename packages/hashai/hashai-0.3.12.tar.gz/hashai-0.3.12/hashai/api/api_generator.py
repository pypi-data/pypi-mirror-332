        ...
        async def websocket_tool_status(websocket: WebSocket, task_id: str):
            ...
            try:
                ...
                if not task_id.startswith("tool_"):
                    await websocket.send_json({"status": "error", "message": "Invalid task ID format"})
                    return
                ...
                if tool_name == "WebBrowserAsync":
                    ...
                    tool = next((t for t in self.assistant.tools if t.name == "WebBrowserAsync"), None)
                    if not tool:
                        await websocket.send_json({"status": "error", "message": "WebBrowserAsync tool not found"})
                        return
                    
                    # Send initial status
                    status = tool.get_task_status(uuid_part)
                    # Log debug info for the WebSocket connection
                    logger.info(f"WebSocket connected for task: {task_id}, UUID: {uuid_part}")
                    ...
                    while True:
                        ...
                        # Get updated status
                        current_status = tool.get_task_status(uuid_part)
                    ...
                    while True:
                        if task_id not in self._background_tasks:
                            await websocket.send_json({"status": "error", "message": f"Task ID {task_id} not found"})
                            break
        ...
        async def debug_tasks():
            ...
            if browser_tool:
                ...
                try:
                    ...
                    # Get instance tasks
                    instance_tasks = {}
                    if hasattr(browser_tool, "_tasks"):
                        instance_tasks = {k: v.get("status", "unknown") for k, v in browser_tool._tasks.items()}
