            ...
            for i, tool in enumerate(self.tools):
                if tool.name == "WebBrowser":
                    try:
                        ...
                        self.logger.info("Replacing WebBrowser tool with WebBrowserAsync adapter for API compatibility")
                        self.tools[i] = async_tool
                    except ImportError:
                        self.logger.warning("WebBrowserAsyncAdapter not found. The WebBrowser tool may block the API server.")
