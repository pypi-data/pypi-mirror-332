...
async def converse_with_agent(agent_name: str, sub_log_id: str, first_message: str, contextual_info: str, exit_criteria: str, context=None):
    ...
    my_sub_context = ChatContext(service_manager, command_manager, user=context.username)
    ...
    while not finished_conversation:
        ...
        async with asyncio.timeout(1200.0):
            [_, replies] = await send_message_to_agent(sub_log_id, first_message, user=context.username)
        ...
        print("End of loop")
        sub_context = ChatContext(service_manager, command_manager, user=context.username)
