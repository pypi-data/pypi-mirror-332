...
from revo_snd._adpcm.adpcm import (
    AdpcmParamSet,
...
try:
    ...
    print_warning('BRSTM >> Could not find internal "revo_snd_adpcm" module')
    print_warning('BRSTM >> Decoding will switch to Python fallback function')
    print_warning('BRSTM >> Encoding is not available')
...
class BRSTM(RevFile):
    ...
    def __get_trk_pcm_samples(self, trk_id: int) -> bytes:
        ...
        import numpy as np
        left_pcm = np.array(pcm_left, dtype=np.int16)
        right_pcm = np.array(pcm_right, dtype=np.int16)
    ...
    def decode(self, out_path: str, *, one_file: bool = False) -> None:
        ...
        if one_file:
            ...
            with wave.open(f'{path}{os.sep}{os_path.stem}.wav', 'wb') as trk_out:
            ...
            for track_id in range(self.n_tracks):
                with wave.open(f'{path}{os.sep}{os_path.stem}_track_{track_id}.wav', 'wb') as trk_out:
    ...
    @classmethod
    def from_file(cls, data: (str | BinaryIO)) -> Self:
        if data is None:
