...
from revo_snd.rsar.info import (
    ...
    _GroupTableEntry,
    _FileEntry,
    _FilePositionEntry, SoundDataEntry, Sound3DParam,
)
...
class BRSAR(RevFile):
    ...
    def __get_files_of_group(self, group_name: str, file_type: FileTag) -> list:
        ...
        for file in set([group.file, group.audio_file] + group.files + group.audio_files):
            if self._file[file].file_type == file_type:
                entry = (file, f'B{file_type}_id_{file}')
    ...
    def get_group_brwar(self, group_name: str) -> dict[str, list]:
        return {
            'brwar': self.__get_files_of_group(group_name, FileTag.BRWAR)
        }

    def get_group_brbnk(self, group_name: str) -> dict[str, list]:
        return {
            'brbnk': self.__get_files_of_group(group_name, FileTag.BRBNK)
        }

    def get_group_brwsd(self, group_name: str) -> dict[str, list]:
        return {
            'brwsd': self.__get_files_of_group(group_name, FileTag.BRWSD)
        }

    def get_group_brseq(self, group_name: str) -> dict[str, list]:
        return {
            'brseq': self.__get_files_of_group(group_name, FileTag.BRSEQ)
        }

    def get_group_strm_snd(self, group_name: str = None) -> dict[str, list]:
        if group_name is None:
            ...
            for snd in self._info.snd_data:
                if snd.snd_type == _SoundType.STRM:
                    name = self._symbol.names[snd.file_name_idx]
                    sounds.append((name, snd, self._snd_map[name]))
            return {
                'brstm': sounds
            }
    ...
    def get_group_wav_snd(self, name: str) -> dict[str, list]:
        sounds = []
        brwsd = []
        brwar = []
        ...
        for snd in self._info.snd_data:
            if snd.snd_type == _SoundType.WAVE:
                ...
                if self._symbol.names[parent_grp.file_name_idx] == name:
                    sounds.append((self._symbol.names[snd.file_name_idx], snd))

                    brwsd_file = self._file.offset_to_id(parent_grp.grp_file_off + grp_entry.file_data_off)
                    brwsd_file_entry = (brwsd_file, f'BRWSD_id_{brwsd_file}')

                    brwar_file = self._file.offset_to_id(parent_grp.grp_audio_off)
                    brwar_file_entry = (brwar_file, f'BRWAR_id_{brwar_file}')

                    if brwsd_file_entry not in brwsd:
                        brwsd.append(brwsd_file_entry)

                    if brwar_file_entry not in brwar:
                        brwar.append(brwar_file_entry)
    ...
    def get_group_seq_snd(self, name: str) -> dict[str, list]:
        sounds = []
        brseq = []
        brbnk = []
        brwar = []
            ...
            if snd.snd_type == _SoundType.SEQ:
                ...
                if self._symbol.names[parent_grp.file_name_idx] == name:
                    ...
                    if brseq_file is not None:
                        ...
                        brseq_file_entry = (file_id, f'BRSEQ_id_{file_id}', snd_info.brwav)
                    else:
                        brseq_file_entry = (-1, f'UNKNOWN_BRSEQ', None)

                    brseq.append(brseq_file_entry)

                    file_id = self._file.offset_to_id(brbnk_file.base_offset)
                    brbnk_file_entry = (file_id, f'BRBNK_id_{file_id}')

                    if brbnk_file_entry not in brbnk:
                        brbnk.append(brbnk_file_entry)
    ...
    def __bytes__(self) -> bytes:
        ...
        buffer.write(struct.pack('>4sHHIHHIIIIII',
                                 ...
                                 0x40, 0,  # DATA offset always at 0x40
                                 0, 0,
                                 0, 0))

        buffer.write('Generated by REVO_SND'.encode('ascii'))
        ...
        buffer.seek(0x14)
        buffer.write(struct.pack('>IIIII', len(symbol), info_off, info_size, file_off, file_size))
    ...
    def from_file(cls, data: (str | BinaryIO), *args, **kwargs) -> Self:
        ...
        if len(args) != 1:
            raise RSARError('Missing brstm_path for reading BRSAR')

        return cls(data, args[0])
