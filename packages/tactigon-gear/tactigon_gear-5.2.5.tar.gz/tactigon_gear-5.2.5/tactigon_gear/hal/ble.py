...
from ..models import  TBleConnectionStatus, TBleSelector, Hand, Angle, Acceleration, Touch, Gyro, OneFingerGesture, TwoFingerGesture, TouchConfig, BLECommands, BLECommand
...
class Ble(Thread):
    ...
    def __init__(self, address: str, hand: Hand, touch_config: Optional[TouchConfig] = None, sleep_timeout_s: float = 2, debug: bool = False):
        Thread.__init__(self, daemon=True)
        self.address = address
        self.hand = hand
        self._update_time_s = sleep_timeout_s
    ...
    async def task(self):
        ...
        while not self._stop_event.is_set():
            ...
            if not await self._start_notify(self.TOUCHPAD_UUID, self.handle_touchpad):
            ...
            while self.connected:
                ...
                if self._last_update:
                    last_update_delta = (datetime.now() - self._last_update).total_seconds()
                    if self.sleep:
                        if last_update_delta < self._update_time_s:
                            logging.debug("[BLE] Clearing sleep")
                            self._sleep.clear()
                    else:
                        if last_update_delta > self._update_time_s:
                            logging.debug("[BLE] Setting sleep after %f seconds", last_update_delta)
                            self._sleep.set()
