...
from rstms_bcc.browser import InitFailed, Session
...
def test_session_reset(session, account):
    status = session.status(account)
    assert isinstance(status, dict)
    assert status["login"] == "success"
...
def test_session_initialized(session, account):
    with pytest.raises(InitFailed) as e:
        session.initialize(account)
    assert isinstance(e.value, InitFailed)
    assert "initialize: already initialized" in e.value.args
