...
class API:
    ...
    def status(self) -> Dict[str, str]:
        response = StatusResponse(**self._get("bcc/status/"))
        return response.status

    @validate_call
    def initialize(self) -> Dict[str, str]:
        return self._post("bcc/initialize/")

    @validate_call
    def reset(self) -> Dict[str, str]:
        return self._post("bcc/reset/")

    @validate_call
    def users(self) -> List[User]:
        response = UsersResponse(**self._get("bcc/users/"))
    ...
    def add_user(self, username: str, displayname: str, password: str) -> User:
        ...
        response = AddUserResponse(**self._post("bcc/user/", data=request.model_dump_json()))
        return response.user

    @validate_call
    def delete_user(self, username: str) -> Dict[str, str]:
        request = DeleteUserRequest(username=username)
        return self._delete("bcc/user/", data=request.model_dump_json())

    @validate_call
    def books(self, username: str | None = None) -> List[Book]:
        if username:
            path = f"bcc/books/{username}/"
        else:
            path = "bcc/books/"
    ...
    def add_book(self, username: str, bookname: str, description: str) -> Book:
        ...
        response = AddBookResponse(**self._post("bcc/book/", data=request.model_dump_json()))
        return response.book

    @validate_call
    def delete_book(self, username: str, token: str) -> Dict[str, str]:
        request = DeleteBookRequest(username=username, token=token)
        return self._delete("bcc/book/", data=request.model_dump_json())

    def shutdown(self):
        return self._post("bcc/shutdown/")

    def uptime(self):
        return self._get("bcc/uptime/")
