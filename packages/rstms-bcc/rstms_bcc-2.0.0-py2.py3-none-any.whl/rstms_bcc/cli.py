...
@click.option("--passwd-file", help="passwd filename")
@click.option(
    "--shell-completion",
    is_flag=False,
    flag_value="[auto]",
...
@click.pass_context
def bcc(
    ctx, debug, username, password, caldav_url, api_url, cert, key, api_key, log_level, passwd_file, shell_completion
):
    ...
    if passwd_file is not None:
        settings.PASSWD_FILE = passwd_file
