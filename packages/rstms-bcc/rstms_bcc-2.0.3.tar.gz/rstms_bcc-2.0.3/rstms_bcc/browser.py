...
TITLE_ALREADY_INITIALIZED = "Installation was already completed."
...
class Session:
    ...
    def login(self, admin: Account):
        ...
        if dom.title.text.strip() == TITLE_MAINTENANCE:
    ...
    def initialize(self, admin: Account) -> Dict[str, str]:
        ...
        if len(strings) == 1 and strings[0].startswith(TITLE_ALREADY_INITIALIZED):
        ...
        while True:
            ...
            if JUMBOTRON_AUTHENTICATION in jumbotron:
                if str(self.driver.response.url).endswith("/admin/"):
                    message = "configured successfully"
                    self.logger.info(message)
                    return dict(message=message)
                else:
                    raise InitFailed("unexpected url after start button: {self.driver.response.url}k")
                ...
                raise InitFailed(f"unrecognzized jumbotron text: {jumbotron}")
    ...
    def status(self, admin: Account) -> Dict[str, str]:
        ...
        state = dict(
            ...
            reset=self.reset_time.humanize() if self.reset_time else None,
