...
class TradeClient:
    ...
    #     # Calculate percentage of ADV (Average Daily Volume)
    #     # In a real implementation, this would be based on actual market data
    #     example_adv = 1000000  # Example ADV for the stock
    #     payload["percent_of_adv"] = round((payload["quantity"] / example_adv) * 100, 2)
        
    #     # Handle venue information
    #     # Default venues if not specified
    #     if "venues" not in payload:
    #         payload["venues"] = ["NYSE", "NASDAQ", "LSE"]#, "ARCA"]
    ...
    #         # Create execution times varying by 1-3 minutes from each other
    ...
    #             "latency" : f"{random.randint(10, 50)}ms" # Random latency between 1 and 10 ms
    #         }
    ...
    #         "estimated_total_slippage": (notional * (3.14 if strategy == "Our SOR" else (25.2 if strategy == "Market Only" else 21.8))) / 10000,  # Convert bps to dollar value
    ...
    #     delay = random.uniform(2, 5.0)  # Random delay between 2 and 5 seconds
    #     time.sleep(delay)
