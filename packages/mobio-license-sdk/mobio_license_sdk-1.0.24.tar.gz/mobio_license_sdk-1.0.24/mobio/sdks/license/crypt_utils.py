...
class CryptUtil:
    ...
    @classmethod
    def get_license_info(cls, license_key, merchant_id, use_cache=True):
        license_info = None
        try:
            ...
            # print("license_sdk:: get_license_info merchant_id: {}".format(merchant_id))
            if use_cache:
                license_encrypt = cls.get_license_encrypt(merchant_id)
            else:
                license_encrypt = cls.get_license_encrypt_no_cache(merchant_id)
            if license_encrypt:
    ...
    @classmethod
    @lru_redis_cache.add()
    def get_license_encrypt(cls, merchant_id):
        license_encrypt = cls.get_license_encrypt_no_cache(merchant_id)
        return license_encrypt

    @classmethod
    def get_license_encrypt_no_cache(cls, merchant_id):
        license_encrypt = ""
        try:
            if Mobio.VM_K8S and Mobio.VM_K8S == "1":
                file_path = cls.get_file_path_license(merchant_id)
                license_encrypt = cls.get_content_from_file(file_path)
            else:
                license_encrypt = cls.get_file_license_from_admin(merchant_id)
        except Exception as ex:
