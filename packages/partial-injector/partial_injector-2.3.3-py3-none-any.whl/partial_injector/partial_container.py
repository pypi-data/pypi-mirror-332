...
class Container: # TODO: Add validation and proper error handling
    ...
    def __build_dependency(self, registration_key: ContainerKey) -> None | ContainerKey: # TODO: Add circular dependency tracking
        ...
        if multiple_registrations:
            built_key = list[registration_key.__args__[0]]
        else:
            built_key = registration_key

        if built_key in self.__built:
            return built_key
        ...
        if len(built_dependencies) == 0 and any(not r.condition_ignore_not_satisfied for r in registrations):
            raise TerminationException(f"No objects with key {registration_key} were built because built conditions have not been met for any of the registrations.")
        ...
        if multiple_registrations:
            ...
            if len(built_dependencies) == 1:
                self.__built[built_key.__args__[0]] = built_dependencies[0]
        else:
    ...
    def __build_partial(self, func: Callable, inject_returns: bool) -> Callable:
        ...
        for param_name, param in sig.parameters.items():
            reg_container_type = Container.RegistrationContainer[param.annotation.__args__[0]] \
                if param.annotation and (hasattr(param.annotation, '__origin__') and param.annotation.__origin__ is list) \
                else Container.RegistrationContainer[param.annotation] if param.annotation else None
            ...
            if not reg_dep_key is None:
                if last_not_registered_name is not None:
                    raise TerminationException(f"Cannot build partial function without registered parameter {last_not_registered_name}:{last_not_registered_annotation}")
