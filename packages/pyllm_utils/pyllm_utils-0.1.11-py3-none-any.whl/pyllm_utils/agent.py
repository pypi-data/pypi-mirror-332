...
class Agent:
    def __init__(self,
        ...
        # Define a postprocessor for the "calculate" field:
        #    This callback will actually add the numbers in the "calculate" array
        def add_numbers_callback(value):
            \"""
            Expects value to be a list of two numbers, e.g. [4, 6].
            Returns a dict with role/content to append to conversation or None.
            \"""
            if isinstance(value, list) and len(value) == 2:
                total = sum(value)
                # Return a message that gets appended to the conversation
                return {
                    "role": "assistant",
                    "content": f"The result of adding {value[0]} and {value[1]} is: {total}"
                }
            else:
                # If the LLM output is malformed, respond accordingly
                return {
                    "role": "assistant",
                    "content": "Could not parse the array for addition."
                }

        async def main():
            # Create an Agent instance with two output fields: "chat" and "calculate" (array).
            agent = Agent(
                model="gpt-4o",  # or your model of choice
                template_source="You are a helpful assistant who can do arithmetic. \
                                When numbers are provided in the 'calculate' field, \
                                you should perform the calculation and then inform the user of the result. \
                                Hello, {username}! \
                                If I receive numbers to calculate, \
                                I'll do the math and let you know the answer.",
                max_messages=10,
                output_format={
                    "chat": {
                        "type": "string",
                    ...
                    "calculate": {
                        "type": "array",  # <--- The LLM must produce an array of [x, y]
                        "description": "List of two numbers to be added together"
                    }
                },
                # Provide a postprocessor dict that maps field "calculate" to our add_numbers_callback
                postprocessor={
                    "calculate": add_numbers_callback
                    # If you had others, you could list them here as well
                }
            )
            # Add a user message. The LLM sees this message right before the final "JSON format" reminder.
            agent.messages.add("user", "Please add 4 and 6. I'd appreciate your help.")

            # Run the agent in streaming mode
            stream_iterator = await agent.run(stream=True, stream_keys=["chat"], username="Alice")
            async for chunk in stream_iterator:
                # Each chunk is either:
                #   (partial_stream_text, None)          for incremental streaming tokens, or
                #   (None, processed_final_response_dict) for the final structured response
                if chunk is not None:
                    key, value = chunk

                    if key is not None:
                        # This is a partial chunk from the LLM (streaming text)
                        print("STREAMING PARTIAL TEXT KEY AND VALUE:", key, value)
                    else:
                        # final_dict is the final JSON parsed from the LLM, minus any fields
                        # consumed by postprocessor. For example, if "calculate" is handled
                        # by the callback, the final_dict might only have "chat" left.
                        print("FINAL RESPONSE DICT:", value)

                        # The callback's return (an assistant message about the sum)
                        # has already been appended to the conversation. 
                        # If you want to see the entire conversation:
                        print("\n=== Current Conversation Messages ===")
                        for msg in agent.messages.get_messages():
        ...
        if __name__ == "__main__":
            asyncio.run(main())
        """
        self.messages = AgentMessages(template_source, max_messages, trim_strategy, output_format, output_fields)
    ...
    async def run(self, stream=False, stream_keys=None, **kwargs):
        ...
        if stream:
            ...
            response = self.llm.request_messages(messages, json_mode=True)
    ...
    async def _process_response(self, response: dict) -> dict:
        ...
        if isinstance(response, dict):
            ...
            for field, result in response.items():
                ...
                if self.postprocessor is not None:
                    callback = self.postprocessor.get(field, None)
                    if callable(callback):
                        # Create a task for each callback
                        async def process_callback(f, r):
                            return f, await callback(r) if inspect.iscoroutinefunction(callback) else callback(r)
...
class AgentMessages:
    def __init__(self, 
        ...
        if output_format is None and output_fields is None:
            output_fields = {
        ...
        # JSONフォーマットを指定する最後のユーザーメッセージを追加（必須）
    ...
    def _generate_json_format_prompt(self, output_format):
        ...
        prompt += "\n}\n\n上記のuser,assistantの履歴を元に上記のJSONフォーマットのみで応答してください。"
    ...
    def update_json_format(self, output_format=None, output_fields=None):
        """
    ...
    def clear(self):
        ...
        # Always keep the JSON format user message at the end
        if json_format_msg:
            self.messages.append(json_format_msg)
        else:
            # JSONフォーマットメッセージがなければデフォルトを追加
            default_format = {
