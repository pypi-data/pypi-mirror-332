...
region_datetime_formats = {
    ...
    "AU": {"date": "%d/%m/%Y", "time": "%I:%M:%S %p"},  # Australia (12-hour format)
    "AT": {"date": "%d.%m.%Y", "time": "%H:%M:%S"},  # Austria
    "BR": {"date": "%d/%m/%Y", "time": "%H:%M:%S"},  # Brazil
    "CA": {"date": "%m/%d/%Y", "time": "%I:%M:%S %p"},  # Canada (12-hour format)
    "CN": {"date": "%Y-%m-%d", "time": "%H:%M:%S"},  # China
    "DE": {"date": "%d.%m.%Y", "time": "%H:%M:%S"},  # Germany
    "EG": {"date": "%d/%m/%Y", "time": "%I:%M:%S %p"},  # Egypt (12-hour format)
    "FR": {"date": "%d/%m/%Y", "time": "%H:%M:%S"},  # France
    "IN": {"date": "%d/%m/%Y", "time": "%I:%M:%S %p"},  # India (12-hour format)
    ...
    "NZ": {"date": "%d/%m/%Y", "time": "%I:%M:%S %p"},  # New Zealand (12-hour format)
    "PK": {"date": "%d/%m/%Y", "time": "%I:%M:%S %p"},  # Pakistan (12-hour format)
    "PL": {"date": "%d.%m.%Y", "time": "%H:%M:%S"},  # Poland
    "RU": {"date": "%d.%m.%Y", "time": "%H:%M:%S"},  # Russia
    "SA": {"date": "%d/%m/%Y", "time": "%I:%M:%S %p"},  # Saudi Arabia (12-hour format)
    "SE": {"date": "%Y-%m-%d", "time": "%H:%M:%S"},  # Sweden
    "GB": {"date": "%d/%m/%Y", "time": "%H:%M:%S"},  # United Kingdom
    "US": {"date": "%m/%d/%Y", "time": "%I:%M:%S %p"},  # United States (12-hour format)
}
...
def get_connected_wifi():
    ...
    output = check_output("netsh wlan show interfaces", shell=True, universal_newlines=True)
...
__all__ = ["aliaFolder", "cycle_list", "human_readable_size", "get_files", "list_files_in_dir", "default",
           ...
           "region_datetime_formats", "get_country_code", "get_connected_wifi", "is_dark_mode", "Class"]
