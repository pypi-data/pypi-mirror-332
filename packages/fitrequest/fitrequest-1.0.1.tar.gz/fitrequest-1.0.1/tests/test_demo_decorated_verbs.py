...
from tests.demo_decorator_verbs import client_decorated_verbs as client
from tests.demo_decorator_verbs import RestApiClient
...
def test_methods():
    for method in ('get_item', 'put_item', 'post_item', 'delete_item'):
        ...
        assert getattr(getattr(client, method), 'fit_method') == True
...
def test_authenticate_with_init_username_password():
    ...
    TestCase().assertDictEqual(vars(rest_client.session.synchronous.auth), vars(expected))
    TestCase().assertDictEqual(vars(rest_client.session.asynchronous.auth), vars(expected))
