...
def test_methods_binding_with_default_docstring_and_docstring_but_no_variables(
    ...
    value = config_with_default_docstring_and_variables.fit_class().async_get_with_no_docstring_variable_but_dosctring_set.__doc__
    assert value == expected
...
async def test_methods_binding_with_save_method_without_params(
    ...
    mock_file_stream = mock.MagicMock(read=lambda *args, **kwargs: next(file_chunks_iter))
    default_open_args = dict(buffering=-1, encoding=None, errors=None, newline=None, closefd=True, opener=None)
