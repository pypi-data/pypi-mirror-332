...
async def test_decorated_async_get_and_save_items(client):
    ...
    mock_file_stream = MagicMock(read=lambda *args, **kwargs: next(iter(json_data)))
...
def test_bad_decorator_type():
    with pytest.raises(ValidationError) as err:
        ...
        assert 'InvalidMethodDecoratorError' in err.value.errors()[0]['ctx']['error']
