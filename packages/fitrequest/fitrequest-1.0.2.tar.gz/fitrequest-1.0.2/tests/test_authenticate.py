...
def test_authenticate_with_init_username_password(config_with_url):
    ...
    TestCase().assertDictEqual(vars(client.session.synchronous.auth), vars(expected))
    TestCase().assertDictEqual(vars(client.session.asynchronous.auth), vars(expected))
...
def test_authenticate_on_init(config_with_url):
    ...
    assert config.session.synchronous.auth == None
    assert config.session.asynchronous.auth == None
    ...
    assert new_class.session.synchronous.auth == None
    assert new_class.session.asynchronous.auth == None
...
def test_authenticate_with_multiple_auth_methods():
    with pytest.raises(ValidationError) as err:
        ...
        assert 'MultipleAuthenticationError' in err.value.errors()[0]['ctx']['error']
...
class TestSKCRUtilsSecret(TestCase):
    ...
    def test_get_ssm_username(self):
        ...
        self.assertEqual(aws_username, ssm_value)
    ...
    def test_get_secretsmanager_secret(self):
        ...
        self.assertEqual(aws_password, secretsmanager_value)
