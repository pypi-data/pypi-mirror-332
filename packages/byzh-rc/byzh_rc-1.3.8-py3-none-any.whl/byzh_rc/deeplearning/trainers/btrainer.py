...
class BTrainer(Byzh):
    ...
    def set_writer(self, path: Path, mode: Literal["a", "w"] = "a"):
        ...
        请在训练前设置set_writer
        '''
        self._writer = BWriter(path, ifTime=True)
        if mode == 'a':
            pass
        if mode == 'w':
            self._writer.clearFile()

        self._writer.toFile("[dataset] -> " + self.train_loader.dataset.__class__.__name__, ifTime=False)
        self._writer.toFile("[batch_size] -> " + str(self.train_loader.batch_size), ifTime=False)
        self._writer.toFile("[lr] -> " + str(self.optimizer.param_groups[0]['lr']), ifTime=False)
        self._writer.toFile("[criterion] -> " + str(self.criterion), ifTime=False)
        self._writer.toFile("[optimizer] -> " + str(self.optimizer), ifTime=False)
        if self.lrScheduler is not None:
            self._writer.toFile("[lrScheduler] -> " + str(self.lrScheduler), ifTime=False)
        self._writer.toFile("[model] -> " + str(self.model), ifTime=False)
