...
class BTrainer(Byzh):
        ...
        for epoch in range(epochs):
            ...
            if self.__stop_by_acc is not None:
                if self.__stop_by_acc(train_loss):
                    info = f'[stop] 模型在连续{self.__stop_by_acc.rounds}个epoch内停滞, 触发stop_by_acc'
                    self.__print_and_toWriter(info)
                    info = "[stop] " + str(self.__stop_by_byzh.cnt_list)
                    self.__print_and_toWriter(info, if_print=False)
                    break
            # 早停
            if self.__stop_by_acc_delta is not None:
                if self.__stop_by_acc_delta(val_acc):
                    info = f'[stop] 模型在连续{self.__stop_by_acc_delta.rounds}个epoch内过拟合, 触发stop_by_acc_delta'
                    self.__print_and_toWriter(info)
                    info = "[stop] " + str(self.__stop_by_byzh.cnt_list)
                    self.__print_and_toWriter(info, if_print=False)
                    break
            ...
            if self.__stop_by_loss is not None:
                if self.__stop_by_loss(train_loss):
                    info = f'[stop] 模型在连续{self.__stop_by_loss.rounds}个epoch内停滞, 触发stop_by_loss'
                    self.__print_and_toWriter(info)
                    info = "[stop] " + str(self.__stop_by_byzh.cnt_list)
                    self.__print_and_toWriter(info, if_print=False)
                    break
            ...
            if self.__stop_by_loss_delta is not None:
                if self.__stop_by_loss_delta(train_loss):
                    info = f'[stop] 模型在连续{self.__stop_by_loss_delta.rounds}个epoch内停滞, 触发stop_by_loss_delta'
                    self.__print_and_toWriter(info)
                    info = "[stop] " + str(self.__stop_by_byzh.cnt_list)
                    self.__print_and_toWriter(info, if_print=False)
                    break
            ...
            if self.__stop_by_byzh is not None:
                if self.__stop_by_byzh(train_loss, train_acc, val_acc):
                    info = f'[stop] 模型在连续{self.__stop_by_byzh.rounds}个epoch内停滞, 触发stop_by_byzh'
                    self.__print_and_toWriter(info)
                    info = "[stop] " + str(self.__stop_by_byzh.flags)
                    self.__print_and_toWriter(info, if_print=False)
                    break
            # 过拟合
            if self.__stop_by_overfitting is not None:
                if self.__stop_by_overfitting(train_acc, val_acc):
                    info = f'[stop] 模型在连续{self.__stop_by_overfitting.rounds}个epoch内过拟合, 触发stop_by_overfitting'
                    self.__print_and_toWriter(info)
                    info = "[stop] " + str(self.__stop_by_byzh.cnt_list)
                    self.__print_and_toWriter(info, if_print=False)
                    break
    ...
    def set_writer(self, path: Path, mode: Literal["a", "w"] = "a"):
        ...
        请在训练前设置set_writer
        '''
        self.__writer = BWriter(path, ifTime=True)
        if mode == 'a':
            pass
        if mode == 'w':
    ...
    def set_save_during_train(self, path: Path, rounds=10):
        ...
        self.__print_and_toWriter(f"[set] save_during_train")
    def set_stop_by_acc(self, rounds=10, delta=0.01):
        ...
        self.__print_and_toWriter(f"[set] stop_by_acc")
    def set_stop_by_overfitting(self, rounds=10, delta=0.1):
        ...
        self.__print_and_toWriter(f"[set] stop_by_overfitting")
    def set_stop_by_acc_delta(self, rounds=10, delta=0.003):
        ...
        self.__print_and_toWriter(f"[set] stop_by_acc_delta")
    def set_stop_by_loss(self, rounds=10, delta=0.01):
        ...
        self.__print_and_toWriter(f"[set] stop_by_loss")
    def set_stop_by_loss_delta(self, rounds=10, delta=0.002):
        ...
        self.__print_and_toWriter(f"[set] stop_by_loss_delta")
    def set_stop_by_byzh(self, rounds=10, loss_delta=0.002, acc_delta=0.003):
        ...
        self.__print_and_toWriter(f"[set] stop_by_byzh")
    def set_reload_by_loss(self, max_reload_count=5, reload_rounds=10, delta=0.01):
        ...
        self.__print_and_toWriter(f"[set] reload_by_loss")
    ...
    def __print_and_toWriter(self, info: str, if_print=True):
        if if_print:
            print(info)
        if self.__writer is not None:
