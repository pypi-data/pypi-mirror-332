...
class BStopByBYZH:
    ...
    def __call__(self, train_loss, train_acc, val_acc):
        ...
        self.flags[0] = self.stop_by_loss(train_loss)
        self.flags[1] = self.stop_by_acc(val_acc)

        self.flags[2] = self.stop_by_loss_delta(train_loss)
        self.flags[3] = self.stop_by_acc_delta(val_acc)

        self.flags[4] = self.stop_by_overfitting(train_acc, val_acc)
