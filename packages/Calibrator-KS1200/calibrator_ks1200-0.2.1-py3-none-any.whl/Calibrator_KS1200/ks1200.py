            ...
            if ish_set[0][0].find(sensor[0][4])>=0:                             # если выбранный датчик является эталонным
                ...
                for i in range(len(coef_set)):
                    ...
                    if coef_data:
                        ...
                        self.itm_send_command(f"W,{coef_set_itm[i]}={str(coef_data)}", f"W,{coef_set_itm[i]}-OK", f'Коэффициент {coef_set_itm[i]} установлен в значение {str(coef_data)}', f'Невозможно установить коэффициент {coef_set_itm[i]} установлен в значение {str(coef_data)}')
            else:
