...
class ScanResult:
    ...
    def add_issue(
        ...
        logger.log(
            logging.ERROR if severity == IssueSeverity.ERROR else 
            logging.WARNING if severity == IssueSeverity.WARNING else 
            logging.INFO,
            str(issue)
        )
    ...
    def summary(self) -> str:
        ...
        result.append(f"Scanned {self.bytes_scanned} bytes with scanner '{self.scanner_name}'")
        result.append(f"Found {len(self.issues)} issues ({error_count} errors, {warning_count} warnings, {info_count} info)")
...
class BaseScanner(ABC):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.chunk_size = self.config.get("chunk_size", 10 * 1024 * 1024)  # Default: 10MB chunks
