...
SUSPICIOUS_CONFIG_PROPERTIES = [
    "function", "module", "code", "eval", "exec", "import", 
...
class KerasH5Scanner(BaseScanner):
                ...
                if "training_config" in f.attrs:
                    ...
                    if "metrics" in training_config:
                        for metric in training_config["metrics"]:
                            if isinstance(metric, dict) and metric.get("class_name") not in ["Accuracy", "CategoricalAccuracy", "BinaryAccuracy"]:
                                result.add_issue(
    ...
    def _scan_model_config(self, model_config: Dict[str, Any], result: ScanResult) -> None:
        ...
        for layer in layers:
            ...
            self._check_config_for_suspicious_strings(layer.get("config", {}), result, layer_class)
    ...
    def _check_config_for_suspicious_strings(self, config: Dict[str, Any], result: ScanResult, context: str = "") -> None:
        ...
        for key, value in config.items():
            if isinstance(value, str):
                ...
                self._check_config_for_suspicious_strings(value, result, f"{context}.{key}")
            elif isinstance(value, list):
                # Check each item in the list
                for i, item in enumerate(value):
                    if isinstance(item, dict):
                        self._check_config_for_suspicious_strings(item, result, f"{context}.{key}[{i}]")
