...
country_state_city = {
    'USA': {'states': ['NY', 'CA', 'TX'], 'cities': ['NYC', 'San Francisco', 'Pasadena']},
...
def calculate_discount(sales, promo_code, category, sales_channel, customer_loyalty):
    ...
    """
    base_discount = {
        'FREE10': 0.10, 'PARTY10': 0.10, 'WELCOMEBACK15': 0.15,
        'WELCOMEBACK20': 0.20, 'THANKYOU20': 0.20, 'NO_CODE': 0.0
    }
    channel_multipliers = {
    ...
    discount_rate = (base_discount[promo_code] *
...
def calculate_fulfillment_cost(quantity, sales_channel, territory, product_weight):
    ...
    """
    channel_factors = {'OnlineStore:Mobile App': 1.2, 'RetailOutlet:Mall Store': 0.8}
    territory_factors = {'NA': 1.0, 'EMEA': 1.1, 'APAC': 1.3}
    per_unit_cost = 2.0
    fixed_cost = 5.0
    weight_factor = max(1.0, product_weight / 5)
    return round((per_unit_cost * quantity + fixed_cost) *
    ...
    for date in date_range:
        ...
        for (brand, hierarchy), product_code in product_code_mapping.items():
            ...
            for _ in range(num_orders):
                ...
                mapped_category = category_mapping.get(category, category)  # Default to category if not mapped
                price_range = price_ranges[mapped_category]
                price = round(np.random.lognormal(mean=np.log((price_range[0] + price_range[1]) / 2), sigma=0.5), 2)
                ...
                discount = calculate_discount(sales, promo_code, mapped_category, sales_channel, customer_loyalty)
                ...
                fulfillment_cost = calculate_fulfillment_cost(quantity, sales_channel, territory, product_weight)
                marketing_cost = round(np.random.uniform(1, 5) * (1.5 if mapped_category == 'Beauty & Personal Care' else 1.0), 2)
                return_cost = round(net_sales * return_rates[mapped_category], 2)
                cost_of_goods_sold = round(unit_cost * quantity, 2)
                shipping_revenue = 0.0 if sales >= shipping_threshold else round(shipping_fee_per_unit * quantity, 2)
                profit = round(
                    net_sales - fulfillment_cost - marketing_cost - return_cost - cost_of_goods_sold + shipping_revenue, 2
                )
...
def inject_anomalies_by_date(df, anomaly_schedule):
    ...
    for date_str, (anomaly_type, severity, root_cause, scope) in anomaly_schedule.items():
        ...
        if not df[mask].empty:
            ...
            if anomaly_type == 'ExcessiveDiscount':
                ...
                df.loc[mask, 'DISCOUNT'] = np.minimum(new_discount, df.loc[mask, 'SALES'])
                df.loc[mask, 'NET_SALES'] = (df.loc[mask, 'SALES'] - df.loc[mask, 'DISCOUNT']).round(2)
                category = scope if scope in price_ranges else df.loc[mask, 'MERCHANDISE_HIERARCHY'].str.split('.').str[0].iloc[0]
                df.loc[mask, 'RETURN_COST'] = (df.loc[mask, 'NET_SALES'] * return_rates[category]).round(2)
            
            elif anomaly_type == 'COGSOverstatement':
                df.loc[mask, 'UNIT_COST'] = (df.loc[mask, 'UNIT_COST'] * (1 + severity)).round(2)
                df.loc[mask, 'COST_OF_GOODS_SOLD'] = (df.loc[mask, 'UNIT_COST'] * df.loc[mask, 'QUANTITYORDERED']).round(2)
            
            elif anomaly_type == 'FulfillmentSpike':
                df.loc[mask, 'FULFILLMENT_COST'] = (df.loc[mask, 'FULFILLMENT_COST'] * (1 + severity)).round(2)
            
            elif anomaly_type == 'ShippingDisruption':
                df.loc[mask, 'SHIPPING_REVENUE'] = (-df.loc[mask, 'SHIPPING_REVENUE'] * severity).round(2)
            
            elif anomaly_type == 'ReturnSurge':
                df.loc[mask, 'RETURN_COST'] = (df.loc[mask, 'RETURN_COST'] * (1 + severity)).round(2)
            
            # Recalculate PROFIT and PROFIT_MARGIN for all affected rows
            df.loc[mask, 'PROFIT'] = (
                df.loc[mask, 'NET_SALES'] - df.loc[mask, 'FULFILLMENT_COST'] - df.loc[mask, 'MARKETING_COST'] -
                df.loc[mask, 'RETURN_COST'] - df.loc[mask, 'COST_OF_GOODS_SOLD'] + df.loc[mask, 'SHIPPING_REVENUE']
            ).round(2)
            df.loc[mask, 'PROFIT_MARGIN'] = np.where(
                df.loc[mask, 'NET_SALES'] > 0,
                (df.loc[mask, 'PROFIT'] / df.loc[mask, 'NET_SALES'] * 100).round(2), 0
            )
