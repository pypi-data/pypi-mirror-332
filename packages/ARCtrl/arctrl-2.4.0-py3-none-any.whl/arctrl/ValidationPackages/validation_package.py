...
class ValidationPackage:
    ...
    def __str__(self, __unit: None=None) -> str:
        this: ValidationPackage = self
        def _arrow2905(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow2904(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow2903(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow2902(__unit: None=None) -> IEnumerable_1[str]:
                        return singleton("}")

                    return append(singleton((" Version = " + value_1(this.Version)) + "") if (this.version is not None) else empty(), delay(_arrow2902))

                return append(singleton((" Name = " + this.Name) + ""), delay(_arrow2903))

            return append(singleton("{"), delay(_arrow2904))

        return join("\n", to_list(delay(_arrow2905)))
...
ValidationPackage_reflection = _expr2913
