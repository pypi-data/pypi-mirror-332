...
def get_protocol_names(process_sequence: FSharpList[Process]) -> FSharpList[str]:
    ...
    return List_distinct(choose(chooser, process_sequence), ObjectExpr688())
...
def get_protocols(process_sequence: FSharpList[Process]) -> FSharpList[Protocol]:
    ...
    return List_distinct(choose(chooser, process_sequence), ObjectExpr689())
...
def filter_by_protocol_name(protocol_name: str, process_sequence: FSharpList[Process]) -> FSharpList[Process]:
    def _arrow690(p: Protocol, protocol_name: Any=protocol_name, process_sequence: Any=process_sequence) -> bool:
        return equals(p.Name, protocol_name)

    return filter_by_protocol_by(_arrow690, process_sequence)
...
def get_parameters(process_sequence: FSharpList[Process]) -> FSharpList[ProtocolParameter]:
    ...
    return List_distinct(collect(mapping, process_sequence), ObjectExpr691())
...
def get_characteristics(process_sequence: FSharpList[Process]) -> FSharpList[MaterialAttribute]:
    ...
    return List_distinct(collect(mapping, process_sequence), ObjectExpr692())
...
def get_factors(process_sequence: FSharpList[Process]) -> FSharpList[Factor]:
    ...
    return List_distinct(collect(mapping, process_sequence), ObjectExpr693())
...
def get_root_inputs(process_sequence: FSharpList[Process]) -> FSharpList[ProcessInput]:
    ...
    outputs: Any = of_list(collect(mapping_2, process_sequence), ObjectExpr694())
...
def get_final_outputs(process_sequence: FSharpList[Process]) -> FSharpList[ProcessOutput]:
    ...
    inputs: Any = of_list(collect(mapping_1, process_sequence), ObjectExpr695())
...
def get_root_input_of(process_sequence: FSharpList[Process], sample: str) -> FSharpList[str]:
    ...
    def mapping_2(p: Process, process_sequence: Any=process_sequence, sample: Any=sample) -> FSharpList[tuple[str, str]]:
        ...
        return List_distinct(zip(map(mapping, value_4(p.Outputs)), map(mapping_1, value_4(p.Inputs))), ObjectExpr696())

    class ObjectExpr698:
        @property
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow697(x_1: str, y_1: str) -> bool:
                return x_1 == y_1
    ...
    mappings: Any = of_list_1(map(mapping_4, List_groupBy(projection, collect(mapping_2, process_sequence), ObjectExpr698())), ObjectExpr699())
...
def get_final_outputs_of(process_sequence: FSharpList[Process], sample: str) -> FSharpList[str]:
    ...
    def mapping_2(p: Process, process_sequence: Any=process_sequence, sample: Any=sample) -> FSharpList[tuple[str, str]]:
        ...
        return List_distinct(zip(map(mapping, value_4(p.Inputs)), map(mapping_1, value_4(p.Outputs))), ObjectExpr700())

    class ObjectExpr702:
        @property
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow701(x_1: str, y_1: str) -> bool:
                return x_1 == y_1
    ...
    mappings: Any = of_list_1(map(mapping_4, List_groupBy(projection, collect(mapping_2, process_sequence), ObjectExpr702())), ObjectExpr703())
    def loop(last_state_mut: FSharpList[str], state_mut: FSharpList[str], process_sequence: Any=process_sequence, sample: Any=sample) -> FSharpList[str]:
        while True:
            (last_state, state) = (last_state_mut, state_mut)
            if equals(last_state, state):
...
def get_units(process_sequence: FSharpList[Process]) -> FSharpList[OntologyAnnotation]:
    def _arrow704(p: Process, process_sequence: Any=process_sequence) -> FSharpList[OntologyAnnotation]:
    ...
    return List_distinct(collect(_arrow704, process_sequence), ObjectExpr705())
...
def get_data(process_sequence: FSharpList[Process]) -> FSharpList[Data]:
    ...
    return List_distinct(collect(mapping, process_sequence), ObjectExpr706())
