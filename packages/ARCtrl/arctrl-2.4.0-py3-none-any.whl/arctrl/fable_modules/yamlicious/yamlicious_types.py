...
def Config__get_WhitespaceString(this: Config) -> str:
    def _arrow337(_arg: int, this: Any=this) -> str:
        return " "

    return initialize(this.Level * this.Whitespace, _arrow337)
...
Preprocessor_reflection = _expr338
...
class PreprocessorElement(Union):
    ...
    def __str__(self, __unit: None=None) -> str:
        ...
        def innerprint(next_1: PreprocessorElement, level: int) -> None:
            def _arrow339(_arg: int, next_1: Any=next_1, level: Any=level) -> str:
                return " "

            indent: str = initialize(level * 2, _arrow339)
            if next_1.tag == 1:
...
PreprocessorElement_reflection = _expr340
...
YAMLContent_reflection = _expr341
...
YAMLElement_reflection = _expr342
