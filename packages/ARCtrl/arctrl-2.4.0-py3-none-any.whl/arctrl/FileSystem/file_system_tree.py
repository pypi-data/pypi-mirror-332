...
class FileSystemTree(Union):
    ...
    def try_get_child_by_name(name: str) -> Callable[[FileSystemTree], FileSystemTree | None]:
        def _arrow429(fst: FileSystemTree) -> FileSystemTree | None:
    ...
    def contains_child_with_name(name: str) -> Callable[[FileSystemTree], bool]:
        def _arrow432(fst: FileSystemTree) -> bool:
    ...
    def AddFile(self, path: str) -> FileSystemTree:
        ...
        def _arrow434(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow433(__unit: None=None) -> IEnumerable_1[str]:
                return existing_paths

            return append(singleton(path), delay(_arrow433))

        file_paths: Array[str] = to_array(delay(_arrow434))
        return FileSystemTree.from_file_paths(file_paths)

    @staticmethod
    def add_file(path: str) -> Callable[[FileSystemTree], FileSystemTree]:
        def _arrow435(tree: FileSystemTree) -> FileSystemTree:
    ...
    def from_file_paths(paths: Array[str]) -> FileSystemTree:
        def loop(paths_1: Array[Array[str]], parent: FileSystemTree) -> FileSystemTree:
            ...
            class ObjectExpr437:
                ...
                def Equals(self) -> Callable[[str, str], bool]:
                    def _arrow436(x_3: str, y_2: str) -> bool:
                        return x_3 == y_2
            ...
            folders: Array[FileSystemTree] = map(mapping_2, Array_groupBy(projection, filter(predicate_1, paths_1), ObjectExpr437()), None)
            if parent.tag == 1:
                def _arrow440(__unit: None=None, paths_1: Any=paths_1, parent: Any=parent) -> IEnumerable_1[FileSystemTree]:
                    def _arrow439(__unit: None=None) -> IEnumerable_1[FileSystemTree]:
                        return folders

                    return append(files, delay(_arrow439))

                return FileSystemTree.create_folder(parent.fields[0], to_array(delay(_arrow440)))
        ...
        class ObjectExpr444:
            ...
            def Equals(self) -> Callable[[Array[str], Array[str]], bool]:
                def _arrow443(x: Array[str], y: Array[str]) -> bool:
                    def _arrow442(x_1: str, y_1: str) -> bool:
                        return x_1 == y_1

                    return equals_with(_arrow442, x, y)
        ...
        return loop(Array_distinct(map(split, paths, None), ObjectExpr444()), FileSystemTree.create_folder(FileSystemTree.ROOT_NAME(), []))
    ...
    def filter(predicate: Callable[[str], bool]) -> Callable[[FileSystemTree], FileSystemTree | None]:
        def _arrow455(tree: FileSystemTree) -> FileSystemTree | None:
    ...
    def Union(self, other_fst: FileSystemTree) -> FileSystemTree:
        this: FileSystemTree = self
        def _arrow456(__unit: None=None) -> Array[str]:
        ...
        paths: Array[str] = Array_distinct(_arrow456(), ObjectExpr458())
