...
def directory_exists_async(path: str) -> Async[bool]:
    def _arrow3216(__unit: None=None, path: Any=path) -> Async[bool]:
...
def create_directory_async(path: str) -> Async[None]:
    def _arrow3217(__unit: None=None, path: Any=path) -> Async[None]:
...
def ensure_directory_async(path: str) -> Async[None]:
    def _arrow3219(__unit: None=None, path: Any=path) -> Async[None]:
        def _arrow3218(_arg: bool) -> Async[None]:
            return singleton.ReturnFrom(create_directory_async(path)) if (not _arg) else singleton.Zero()

        return singleton.Bind(directory_exists_async(path), _arrow3218)

    return singleton.Delay(_arrow3219)


def ensure_directory_of_file_async(file_path: str) -> Async[None]:
    def _arrow3220(__unit: None=None, file_path: Any=file_path) -> Async[None]:
...
def file_exists_async(path: str) -> Async[bool]:
    def _arrow3221(__unit: None=None, path: Any=path) -> Async[bool]:
        return singleton.Return(Path(path).is_file())

    return singleton.Delay(_arrow3221)


def read_file_text_async(path: str) -> Async[str]:
    def _arrow3223(__unit: None=None, path: Any=path) -> Async[str]:
        def _arrow3222(__unit: None=None) -> str:
            with open(path, 'r', encoding='utf-8') as f: return f.read()

        return singleton.Return(_arrow3222())

    return singleton.Delay(_arrow3223)


def read_file_binary_async(path: str) -> Async[bytearray]:
    def _arrow3225(__unit: None=None, path: Any=path) -> Async[bytearray]:
        def _arrow3224(__unit: None=None) -> bytearray:
            with open(path, 'rb') as f: return f.read()

        return singleton.Return(_arrow3224())

    return singleton.Delay(_arrow3225)


def read_file_xlsx_async(path: str) -> Async[FsWorkbook]:
    def _arrow3226(__unit: None=None, path: Any=path) -> Async[FsWorkbook]:
...
def move_file_async(old_path: str, new_path: str) -> Async[None]:
    def _arrow3227(__unit: None=None, old_path: Any=old_path, new_path: Any=new_path) -> Async[None]:
...
def delete_file_async(path: str) -> Async[None]:
    def _arrow3228(__unit: None=None, path: Any=path) -> Async[None]:
        try:
            os.remove(path)
...
def delete_directory_async(path: str) -> Async[None]:
    def _arrow3229(__unit: None=None, path: Any=path) -> Async[None]:
...
def write_file_text_async(path: str, text: str) -> Async[None]:
    def _arrow3230(__unit: None=None, path: Any=path, text: Any=text) -> Async[None]:
...
def write_file_binary_async(path: str, bytes: bytearray) -> Async[None]:
    def _arrow3231(__unit: None=None, path: Any=path, bytes: Any=bytes) -> Async[None]:
...
def write_file_xlsx_async(path: str, wb: FsWorkbook) -> Async[None]:
    def _arrow3232(__unit: None=None, path: Any=path, wb: Any=wb) -> Async[None]:
        FsSpreadsheet_FsWorkbook__FsWorkbook_toXlsxFile_Static(path, wb)
        return singleton.Zero()

    return singleton.Delay(_arrow3232)
...
def get_sub_directories_async(path: str) -> Async[Array[str]]:
    def _arrow3233(__unit: None=None, path: Any=path) -> Async[Array[str]]:
    ...
    return singleton.Delay(_arrow3233)


def get_sub_files_async(path: str) -> Async[Array[str]]:
    def _arrow3234(__unit: None=None, path: Any=path) -> Async[Array[str]]:
        paths: Array[str] = [str(entry) for entry in Path(path).iterdir() if entry.is_file()]
        return singleton.Return(map(standardize_slashes, paths, None))

    return singleton.Delay(_arrow3234)


def get_all_file_paths_async(directory_path: str) -> Async[Array[str]]:
    directory_path_1: str = standardize_slashes(directory_path)
    def _arrow3240(__unit: None=None, directory_path: Any=directory_path) -> Async[Array[str]]:
        def all_files(dirs: IEnumerable_1[str]) -> Async[IEnumerable_1[str]]:
            def _arrow3238(__unit: None=None, dirs: Any=dirs) -> Async[IEnumerable_1[str]]:
                def _arrow3237(_arg: Array[Array[str]]) -> Async[IEnumerable_1[str]]:
                    sub_files_1: IEnumerable_1[str] = concat(_arg)
                    def _arrow3236(_arg_1: Array[Array[str]]) -> Async[IEnumerable_1[str]]:
                        def _arrow3235(_arg_2: Array[IEnumerable_1[str]]) -> Async[IEnumerable_1[str]]:
                            sub_dir_contents_1: IEnumerable_1[str] = concat(_arg_2)
                            return singleton.Return(append(sub_dir_contents_1, sub_files_1))

                        return singleton.Bind(all(map_1(all_files, _arg_1)), _arrow3235)

                    return singleton.Bind(all(map_1(get_sub_directories_async, dirs)), _arrow3236)

                return singleton.Return(empty()) if is_empty(dirs) else singleton.Bind(all(map_1(get_sub_files_async, dirs)), _arrow3237)

            return singleton.Delay(_arrow3238)

        def _arrow3239(_arg_3: IEnumerable_1[str]) -> Async[Array[str]]:
        ...
        return singleton.Bind(all_files(to_enumerable([directory_path_1])), _arrow3239)

    return singleton.Delay(_arrow3240)


def rename_file_or_directory_async(old_path: str, new_path: str) -> Async[None]:
    def _arrow3243(__unit: None=None, old_path: Any=old_path, new_path: Any=new_path) -> Async[None]:
        def _arrow3242(_arg: bool) -> Async[None]:
            ...
            return singleton.Bind(directory_exists_async(old_path), _arrow3241)

        return singleton.Bind(file_exists_async(old_path), _arrow3242)

    return singleton.Delay(_arrow3243)


def delete_file_or_directory_async(path: str) -> Async[None]:
    def _arrow3246(__unit: None=None, path: Any=path) -> Async[None]:
        def _arrow3245(_arg: bool) -> Async[None]:
            def _arrow3244(_arg_1: bool) -> Async[None]:
                if _arg:
            ...
            return singleton.Bind(directory_exists_async(path), _arrow3244)

        return singleton.Bind(file_exists_async(path), _arrow3245)

    return singleton.Delay(_arrow3246)
