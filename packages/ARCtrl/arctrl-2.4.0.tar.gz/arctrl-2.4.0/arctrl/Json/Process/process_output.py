...
def _arrow2366(Item: Sample) -> ProcessOutput:
    return ProcessOutput(0, Item)


def _arrow2369(Item_1: Data) -> ProcessOutput:
    return ProcessOutput(1, Item_1)


def _arrow2371(Item_2: Material) -> ProcessOutput:
    return ProcessOutput(2, Item_2)


ROCrate_decoder: Decoder_1[ProcessOutput] = one_of(of_array([map(_arrow2366, ROCrate_decoder_1), map(_arrow2369, ROCrate_decoder_2), map(_arrow2371, ROCrate_decoder_3)]))
...
def _arrow2374(Item: Sample) -> ProcessOutput:
    return ProcessOutput(0, Item)


def _arrow2375(Item_1: Data) -> ProcessOutput:
    return ProcessOutput(1, Item_1)


def _arrow2376(Item_2: Material) -> ProcessOutput:
    return ProcessOutput(2, Item_2)


ISAJson_decoder: Decoder_1[ProcessOutput] = one_of(of_array([map(_arrow2374, ISAJson_decoder_1), map(_arrow2375, ISAJson_decoder_2), map(_arrow2376, ISAJson_decoder_3)]))
