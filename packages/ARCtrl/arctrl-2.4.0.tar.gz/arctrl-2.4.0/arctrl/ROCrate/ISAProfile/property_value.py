...
def _expr1399() -> TypeInfo:
    return class_type("ARCtrl.ROCrate.PropertyValue", None, PropertyValue, LDObject_reflection())


class PropertyValue(LDObject):
    ...
    def get_name() -> Callable[[PropertyValue], str]:
        def _arrow1397(lp: PropertyValue) -> str:
            return lp.GetName()
    ...
    def get_value() -> Callable[[PropertyValue], str]:
        def _arrow1398(lp: PropertyValue) -> str:
            return lp.GetValue()

        return _arrow1398


PropertyValue_reflection = _expr1399
