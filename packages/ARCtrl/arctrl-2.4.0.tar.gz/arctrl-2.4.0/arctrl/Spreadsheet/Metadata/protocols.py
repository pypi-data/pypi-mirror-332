...
def from_sparse_table(matrix: SparseTable) -> FSharpList[Protocol]:
    if (length(matrix.CommentKeys) != 0) if (matrix.ColumnCount == 0) else False:
        ...
        def _arrow1169(i: int, matrix: Any=matrix) -> Protocol:
        ...
        return initialize(matrix.ColumnCount, _arrow1169)
...
def to_sparse_table(protocols: FSharpList[Protocol]) -> SparseTable:
    ...
    class ObjectExpr1171:
        @property
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow1170(x: str, y: str) -> bool:
                return x == y
    ...
    return SparseTable(matrix.Matrix, matrix.Keys, reverse(List_distinct(comment_keys, ObjectExpr1171())), matrix.ColumnCount)
