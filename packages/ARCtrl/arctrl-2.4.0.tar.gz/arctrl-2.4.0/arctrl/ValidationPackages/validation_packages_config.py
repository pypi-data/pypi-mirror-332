...
class ValidationPackagesConfig:
    ...
    def __str__(self, __unit: None=None) -> str:
        this: ValidationPackagesConfig = self
        def _arrow3044(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow3043(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow3042(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow3041(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow3040(__unit: None=None) -> IEnumerable_1[str]:
                            def _arrow3039(__unit: None=None) -> IEnumerable_1[str]:
                                return singleton("}")

                            return append(singleton("]"), delay(_arrow3039))

                        return append(singleton(join((";" + "\n") + "", map(to_string, this.ValidationPackages))), delay(_arrow3040))

                    return append(singleton(" ValidationPackages = ["), delay(_arrow3041))

                return append(singleton((" ARCSpecification = " + value(this.ARCSpecification)) + "") if (this.ARCSpecification is not None) else empty(), delay(_arrow3042))

            return append(singleton("{"), delay(_arrow3043))

        return join("\n", to_list(delay(_arrow3044)))
    ...
    def StructurallyEquals(self, other: ValidationPackagesConfig) -> bool:
        ...
        if equals(this.ARCSpecification, other.ARCSpecification):
            ...
            def _arrow3047(__unit: None=None) -> IEnumerable_1[bool]:
                def _arrow3046(i: int) -> bool:
                    return equals(item(i, a), item(i, b))

                return map(_arrow3046, range_big_int(0, 1, length(a) - 1))

            return fold(folder, True, to_list(delay(_arrow3047))) if (length(a) == length(b)) else False
...
ValidationPackagesConfig_reflection = _expr3051
