...
class FsWorkbook(IDisposable):
    ...
    def RemoveWorksheet(self, name: str) -> None:
        self_1: FsWorkbook = self
        def _arrow194(__unit: None=None) -> FsWorksheet:
            try: 
                def _arrow193(ws: FsWorksheet) -> bool:
                    return ws.Name == name

                return default_arg(try_find_1(_arrow193, self_1._worksheets), None)
        ...
        ignore(remove_in_place(_arrow194(), self_1._worksheets, ObjectExpr195()))
...
FsWorkbook_reflection = _expr196
