    ...
    def create_at(index: int, cells: FsCellsCollection) -> FsColumn:
        def get_index_by(f: Callable[[Callable[[FsCell], int], IEnumerable_1[FsCell]], FsCell]) -> int:
            if length(FsCellsCollection__GetCellsInColumn_Z524259A4(cells, index)) == 0:
                ...
                def _arrow174(c: FsCell, f: Any=f) -> int:
                    return FsAddress__get_RowNumber(c.Address)

                return FsAddress__get_RowNumber(f(_arrow174)(FsCellsCollection__GetCellsInColumn_Z524259A4(cells, index)).Address)


        def _arrow177(projection: Callable[[FsCell], int]) -> Callable[[IEnumerable_1[FsCell]], FsCell]:
            def _arrow176(source_1: IEnumerable_1[FsCell]) -> FsCell:
                class ObjectExpr175:
                    @property
                    def Compare(self) -> Callable[[int, int], int]:
                        return compare_primitives

                return min_by(projection, source_1, ObjectExpr175())

            return _arrow176

        min_row_index: int = get_index_by(_arrow177) or 0
        def _arrow180(projection_1: Callable[[FsCell], int]) -> Callable[[IEnumerable_1[FsCell]], FsCell]:
            def _arrow179(source_2: IEnumerable_1[FsCell]) -> FsCell:
                class ObjectExpr178:
                    @property
                    def Compare(self) -> Callable[[int, int], int]:
                        return compare_primitives

                return max_by(projection_1, source_2, ObjectExpr178())

            return _arrow179

        max_row_index: int = get_index_by(_arrow180) or 0
