...
class FRU:
  ...
  def membership_values(self, 
    ...
    if not complete:
      ...
      Writearray(self.df[features], alpha, 'full', method=similarity, target=self.df[target]).sim_array(h5file = h5file, group = group, hide_progress = hide_progress)
    ...
    for s_attr in columns:
      ...
      dataset = self.df[features].drop(s_attr, axis=1) # remove protected
