...
The function takes the following parameters:
- ```params``` (Dict[str, Tuple[str, List[Tuple[Union[None, str, re.Pattern], str, int]]], optional): A dictionary containing modifications to input files. **Format:** {filename: (id_col, [(id, col, value)])}
...
Parameters:
- ```params``` (List[Dict[str, Tuple[str, List[Tuple[Union[None, str, re.Pattern], str, int]]]]): A list of dictionaries containing modifications to input files. **Format:** [{filename: (id_col, [(id, col, value)])}]
...
```py
from pySWATPlus import FileReader
...
```py
from pySWATPlus import SWATProblem
