...
def test_capture_with_object_fit(sample_html_file, tmp_path):
    """Test that the object-fit parameter works correctly"""
    output_file = tmp_path / "object_fit_output.png"
    runner = CliRunner()
    app = typer.Typer()
    app.command()(capture)
    result = runner.invoke(app, [str(sample_html_file), "--output", str(output_file), "--width", "800", "--height", "600", "--object-fit", "cover"])
    assert result.exit_code == 0
    assert output_file.exists()
    assert "Image successfully saved" in result.stdout
