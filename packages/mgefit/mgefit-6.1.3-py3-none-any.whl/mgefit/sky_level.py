...
def biweight_sigma(y, zero=False):
    ...
    if mad == 0:   # weird distribution
        print("Weird distribution returning np.std")
        return np.std(y)
...
def biweight_mean(y, itmax=10):
    ...
    if mad == 0:   # weird distribution
        print("Weird distribution returning np.mean")
        return np.mean(y)
...
def sky_level(img, plot=False, itmax=5):
    ...
    statistics (via biweight_mean and biweight_sigma). A histogram of the image
    ...
    """
    # Initial estimates
    mean0 = biweight_mean(img)
    sig0 = biweight_sigma(img)
    ...
    for j in range(itmax):  # Do at most itmax iterations
        ...
        gau = lambda peak, x0, sig: peak*np.exp(-(x0 - xgood)**2/(2*sig**2)) 
        resid = lambda pars: gau(*pars) - ygood
        p0 = [ymax0, mean0, sig0]
        sol = capfit(resid, p0)

        if abs(sol.x[1] - mean0) < sol.x_err[1]:
            break
