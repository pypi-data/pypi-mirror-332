...
def test_test_rank():
    ...
    U, _ = jnp.linalg.qr(jax.random.normal(key, (shape[0], shape[0])))  # Orthogonal matrix
    V, _ = jnp.linalg.qr(jax.random.normal(key, (shape[1], shape[1])))  # Orthogonal matrix
    
    # Construct matrix with controlled singular values
    S = jnp.zeros((shape[0], shape[1]))
    S = S.at[:len(singular_values), :len(singular_values)].set(jnp.diag(singular_values))
