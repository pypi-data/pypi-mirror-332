...
from .blocks import WeatherSearchBlock,RandomGirlVideoBlock,RandomCosplayBlock,RandomEmojiBlock,SougouImageSearchBlock,CutOffPrefix
...
class ApiCollectionPlugin(Plugin):
    ...
    def on_load(self):
        ...
        try:
            ...
            self.block_registry.registerexample("random_emoji", "image", RandomEmojiBlock)
        ...
        try:
            ...
            with importlib.resources.path('scheduler', '__init__.py') as p:
                ...
                for yaml in yaml_files:
                    logger.info(yaml)
                    self.workflow_registry.register("api", yaml.stem, WorkflowBuilder.load_from_yaml(os.path.join(example_dir, yaml), self.container))
        except Exception as e:
            try:
