...
def epic_xspec_mcmc_fit(xspec, model_name,
                        ...
                        gauss_priors=[],
                        gauss_prior_parameters={'dummy':(0., 1.)},  # mean and average for Gaussian prior
                        burn=0, runLength=0, walkers=10, save_xcm=True, compute_errors=True, run_chain=True,
                        statistics='cstat', plt_type='euf del', reset_constants=True,
                        min_jeffreys=1e-8, max_jeffreys=1e8):
    '''
    ...
    :param gauss_priors: a list of names of parameters with gaussian priors (note that 'norm' will apply to all parameters
                            with norm in the name
    :param gauss_prior_parameters: a dictionary of the parameters for Gaussian priors, it must be xspec specific
    :param burn: the length of the MCMC burning phase, if zero it is derived from parameter number
    :param runLength: the length of the actual chain, if zero it is derived from parameter number
    :param walkers: number of walkers per free parameter in the GW algorithm
    ...
    ;param min_jeffreys values of the minimum allowed for jeffreys prior, if left to the default by Xspec
    ;param max_jeffreys values of the maximum allowed for jeffreys prior, if left to the default by Xspec

    :return: chain_name, fit_results, chain (Xspec object)
    '''
    ...
    if set_priors:
        ...
        for i,mm in enumerate(models):
            ...
            nPars = mm.nParameters
            for cc in comp_names:
                comp = getattr(mm, cc)
                for par_name in comp.parameterNames:
                    par = getattr(comp, par_name)
                    par_index = par.index
                    par_name_complete='%s__%02d' %(par_name, par_index+i*nPars)

                    if (par.frozen or par.link != '') and (par_name_complete not in gauss_priors):
                        print(par_name, ' linked or frozen ')
                        continue

                    if par_name not in jeffreys_priors and par_name_complete not in gauss_priors:
                        par.prior = 'cons'
                        print('uniform prior for %s' % (par_name_complete))
                    elif par_name in jeffreys_priors:
                        par.prior = 'jeffreys'
                        print('jeffreys prior for %s' % (par_name_complete))
                        if par.values[2] == 0:
                            par.values[2] = min_jeffreys
                            print('set par[2] to  %e' % par.values[2])
                        if par.values[3] == 0:
                            par.values[3] = min_jeffreys
                            print('set par[3] to  %e' % par.values[3])
                        if par.values[4] >= 1e20:
                            par.values[4] = max_jeffreys
                            print('set par[4] to  %e' % par.values[4])
                        if par.values[5] >= 1e20:
                            par.values[5] = max_jeffreys
                            print('set par[5] to  %e' % par.values[5])
                        
                    elif par_name_complete in gauss_priors:
                        par.frozen = False
                        par.prior = 'gauss %e %e' % (gauss_prior_parameters[par_name_complete][0], \
                                                            gauss_prior_parameters[par_name_complete][1])
                        
                        print('Gauss prior for %s %s with par  %e %e' % (cc, par_name_complete,
                                                                                gauss_prior_parameters[par_name_complete][0],
                                                                                gauss_prior_parameters[par_name_complete][1]))
                        print(par.prior)
                    else:
            ...
            for i,mm in enumerate(models):
                ...
                for cc in comp_names:
                    ...
                    for par_name in comp.parameterNames:
                        ...
                        if (par.frozen or par.link != '') and (par_name_complete not in gauss_priors):
                            print(par_name, ' linked or frozen, skip diagonal ')
                            continue
                        control_list.append(par.sigma)
                        if par_name_complete in gauss_priors:
                            diagonal_covariance.append(3*par.prior[2])
                        elif par.values[1] > 0:
                            tmp_val = np.abs(5 * par.values[1])
                            if tmp_val <1e-3:
                                tmp_val=1e-3
                            if tmp_val < 0.1 * np.abs(par.values[0]):
                                tmp_val = 0.1 * np.abs(par.values[0])
                            diagonal_covariance.append(tmp_val)
                        else:
                            x1 = par.values[2]
                            x2 = par.values[4]
                            factor = 0.1
                            if par.prior[0].lower() == 'cons':
                                diagonal_covariance.append(factor * (x2-x1) )
                            else:
                                if x1 > 0:
                                    diagonal_covariance.append(10**(factor * np.log10(x2/x1)) )
                                else:
                                    diagonal_covariance.append(10**(factor * np.log10(x2)) )
                
            # if not perform_fit or np.sum(np.array(control_list) <= 0):
            diagonal_covariance_str = " ".join( ['%e' % k for k in diagonal_covariance])
            proposal = 'gaussian diagonal %s' % diagonal_covariance_str
            # else:
            #     #print diagonal
            #     count=0
            #     masked_i = 0
            #     covariance = list(xspec.Fit.covariance)
            #     for i in range(xspec.Fit.nVarPars):
            #         #print(i, masked_i, count+i, xspec.Fit.nVarPars, len(covariance), control_list[i])
            #         if control_list[i] > 0:
            #             covariance[masked_i+count] = diagonal_covariance[i]
            #             count+=masked_i+1
            #             masked_i+=1
            #     proposal = 'gaussian matrix %s' % (" ".join(['%e' % k for k in covariance]))
            
            if os.path.isfile(chain_name):
                os.remove(chain_name)
                print("Removed chain " + chain_name)
            
            actual_walkers = len(diagonal_covariance) * walkers
            if runLength == 0:
                actual_runLength = 1000*actual_walkers
            else:
                actual_runLength = runLength
            if burn == 0:
                actual_burn = 100 * actual_walkers
            else:
                actual_burn = burn

            chain1 = xspec.Chain(chain_name, algorithm='gw', burn=actual_burn, runLength=actual_runLength, walkers=actual_walkers, 
                                 proposal=proposal)
    ...
    return chain_name, fit_results, chain1
...
#    for kk in to_be_dropped:
    chain_df.drop(to_be_dropped, axis=1, inplace=True)
    # except:
    #     pass
...
def dump_latex_table(fit_by_bin, latex_label_dict=default_latex_label_dict, to_skip=[], flux_norm=1e7, get_unit=None,
    ...
    def default_get_unit(par, kk, ff, par_name):
        ...
        if 'kT' in par or 'peak' in par or 'E' in par or 'g1_center' in par or 'g1_sigma' in par or 'poly_c1' in par \
                or 'gg1_siggma' in par or 'e_turn' in par or 'Sigma' in par or 'Strength' in par:
            unit = 'keV'
...
'''
        for kk in fit_by_bin.keys():
            ...
            for par in params:
                ...
                if 'cstat' in par:
                    format_str = ' & %%.%df &/ %%d & \\\\\n' % int(np.ceil(np.log10(ff[1])))
                    out_str += par_name + format_str % (ff[0] / ff[1], ff[1])
                ...
                elif ff[1] == 0.0 or np.log10(np.abs(ff[1])) < (np.log10(np.abs((ff[0]+ff[2])/2)) -2):
                    #upper limit
                    format_str = get_format_string(ff[2], ff[2] - ff[0], ff[2] - ff[0])
                    output_str = "%s & $<$ &" + format_str + " & %s\\\\\n"
                    out_str += output_str % (par_name, ff[2] *2, expo+ ' '+ unit)
                else:
        ...
        for par in all_parameters:
            ...
            for kk in fit_by_bin.keys():
                if par in fit_by_bin[kk].keys():
                    ...
                    expo, unit, ff, par_name = get_unit(par, kk, ff, par_name)
                    if 'cstat' in par:
                        format_str = ' %%.%df &/%%d & ' % int(np.ceil(np.log10(ff[1])))
                        row += format_str % (ff[0] / ff[1], ff[1])
                    elif ff[0] == ff[1] and ff[0] == ff[2]:
                        if np.floor(ff[0]) == ff[0]:
                            output_str = "%d & & "    
                        else:
                        ...
                        if expo == '':
                            ...
                            output_str = "$<$ & " + format_str + " %s & " % expo
                        row += output_str % (ff[2] * 2)
                    else:
                        format_str = get_format_string(ff[0], ff[2] - ff[0],
                                                       ff[0] - ff[1])
