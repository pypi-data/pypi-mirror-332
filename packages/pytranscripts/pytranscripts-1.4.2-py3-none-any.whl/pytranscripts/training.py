...
class TranscriptTrainer:
    ...
    def train_and_classify(self):
        ...
        self.bert_checkpoint = bert_trainer.state.best_model_checkpoint or os.path.join(self.destination_path, 'bert_weights')
        self.electra_checkpoint = electra_trainer.state.best_model_checkpoint or os.path.join(self.destination_path, 'electra_weights')

        bert_sheet = self.classify_sheet_with_model(self.bert_checkpoint, self.full_data)
        electra_sheet = self.classify_sheet_with_model(self.electra_checkpoint, self.full_data)
    ...
    def inference_documents(self, input_folder: str, output_folder: str, threshold: float = 0.15, model_type: str = 'bert'):
        """
        ...
        if  self.bert_checkpoint is None and self.electra_checkpoint is None:
            return ValueError("You haven't trained the models yet , Please make sure to run the train_and_classify method first")
        
        def generate_lower_colors(n):
            """Generate n distinct highlight colors from WD_COLOR_INDEX"""
            all_colors = [
                WD_COLOR_INDEX.BLUE, WD_COLOR_INDEX.BRIGHT_GREEN, WD_COLOR_INDEX.DARK_BLUE,
                WD_COLOR_INDEX.DARK_RED, WD_COLOR_INDEX.DARK_YELLOW, WD_COLOR_INDEX.GRAY_25,
                WD_COLOR_INDEX.GRAY_50, WD_COLOR_INDEX.GREEN, WD_COLOR_INDEX.PINK,
                WD_COLOR_INDEX.RED, WD_COLOR_INDEX.TEAL, WD_COLOR_INDEX.TURQUOISE,
                WD_COLOR_INDEX.VIOLET, WD_COLOR_INDEX.YELLOW
            ]
            if n <= len(all_colors):
                return all_colors[:n]
            else:
        ...
        def apply_high_highlight(run, label):
            ...
            if not (hasattr(color, 'rgb') or isinstance(color, type(RGBColor(0, 0, 0)))):
                ...
                if re.match(r'^\s*interviewee\s*:', paragraph.text, re.IGNORECASE):
                    ...
                    if len(interview_text.split()) > 10:
                        ...
                        if prediction['score'] > threshold:
                            ...
                            # Clear and rewrite paragraph without coloring the label prefix
                            for run in paragraph.runs:
                                run.clear()
                            
                            paragraph.add_run("Interviewee: ")  # static text without highlighting
                            run = paragraph.add_run(interview_text)  # text to be highlighted
