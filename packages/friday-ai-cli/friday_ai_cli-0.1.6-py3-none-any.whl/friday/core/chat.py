...
from .engine import sampling_loop, DEV_MODE
...
def _render_message(
    ...
    if is_tool_result:
        ...
        if message["type"] == "text":
            ...
            print_panel(
                f"Using tool: {message['name']}\nInput: ```json\n{message['input']}\n```",
                styles[Sender.TOOL],
                prefixes[Sender.TOOL],
            )
    else:
    ...
    while True:
        try:
            ...
            if user_input.lower() in {"exit", "quit"}:
                print(f"{Fore.YELLOW}Exiting the application.{Style.RESET_ALL}")
                break
            ...
            print(f"\n{Fore.YELLOW}Exiting the application.{Style.RESET_ALL}")
            break
