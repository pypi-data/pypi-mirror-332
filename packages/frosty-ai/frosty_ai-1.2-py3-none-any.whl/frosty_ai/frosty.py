...
class Frosty:
    ...
    def chat_fallback(self, prompt, trace_id=None, wait_for_log=False):
    ...
    def chat(self, prompt, rule=None, thinking= None, wait_for_log=False):
        ...
        try:
            if primary_provider == 'OpenAI':
                return self.openai_chat(prompt, provider_key, model, wait_for_log=wait_for_log)
            elif primary_provider == 'MistralAI':
                return self.mistralai_chat(prompt, provider_key, model, wait_for_log=wait_for_log)
            elif primary_provider == 'Anthropic':
                return self.anthropic_chat(prompt, provider_key, model, thinking=thinking, wait_for_log=wait_for_log)
            elif primary_provider == 'Meta':
                return self.meta_chat(prompt, model, wait_for_log=wait_for_log)
            else:
    ...
    def openai_chat(self, prompt, api_key, model, trace_id=None, fallback=False, wait_for_log=False):
        ...
        try:
            ...
            if wait_for_log:
                ...
                executor.submit(self.set_best_model)  # keep async
    ...
    def mistralai_chat(self, prompt, api_key, model, trace_id=None, fallback=False, wait_for_log=False):
        trace_id = trace_id if trace_id else uuid.uuid4()
    ...
    def anthropic_chat(self, prompt, api_key, model, trace_id=None, fallback=False, thinking=None, wait_for_log=False):
        trace_id = trace_id if trace_id else uuid.uuid4()
        try:
    ...
    def meta_chat(self, prompt, model, trace_id=None, fallback=False, wait_for_log=False):
        trace_id = trace_id if trace_id else uuid.uuid4()
    ...
    def embedding_fallback(self, prompt, trace_id = None, wait_for_log=False):
    ...
    def embeddings(self, prompt, rule=None, wait_for_log=False):
        self.rule = rule
    ...
    def openai_embeddings(self, prompt, api_key, model, trace_id=None, fallback=False, wait_for_log=False):
        trace_id = trace_id if trace_id else uuid.uuid4()
    ...
    def mistralai_embeddings(self, prompt, api_key, model,trace_id=None, fallback=False, wait_for_log=False):
        trace_id = trace_id if trace_id else uuid.uuid4()
