...
from ...types.transforms import result_export_params, result_update_params
from ...types.transforms.result_export_response import ResultExportResponse
from ...types.transforms.result_update_response import ResultUpdateResponse
...
class ResultsResource(SyncAPIResource):
    ...
    def update(
        ...
        *,
        ...
        - HTTPException: If the user does not have access to the transformation with the
          given transform_id.
        ...
        return self._patch(
            f"/v1/transforms/{transform_id}/results/{result_id}",
            body=maybe_transform(body, result_update_params.ResultUpdateParams),
            options=make_request_options(
                extra_headers=extra_headers, extra_query=extra_query, extra_body=extra_body, timeout=timeout
            ),
            cast_to=ResultUpdateResponse,
        )
    ...
    def export(
        ...
        transform_id: str,
        *,
        file_type: Literal["excel", "csv", "json"],
        ...
        if not transform_id:
            raise ValueError(f"Expected a non-empty value for `transform_id` but received {transform_id!r}")
        return self._post(
            f"/v1/transforms/{transform_id}/results/export",
    ...
    async def update(
        ...
        *,
        ...
        return await self._patch(
            f"/v1/transforms/{transform_id}/results/{result_id}",
            body=await async_maybe_transform(body, result_update_params.ResultUpdateParams),
            options=make_request_options(
                extra_headers=extra_headers, extra_query=extra_query, extra_body=extra_body, timeout=timeout
            ),
            cast_to=ResultUpdateResponse,
