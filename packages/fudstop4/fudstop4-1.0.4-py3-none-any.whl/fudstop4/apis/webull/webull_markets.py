...
class WebullMarkets(DatabaseManager):
    ...
    async def new_tc_summaries(
        ...
        """
        Retrieve technical summary tickers with flexible parameters.
        
        Args:
            headers (dict, optional): HTTP headers for the request. Defaults to {}.
            rank_types (list of str, optional): List of rank types. Defaults to ['long', 'intermediate', 'short'].
            region_id (int, optional): Region ID for the API call. Defaults to 6.
            req_type (int, optional): Type parameter for the API call. Defaults to 1.
            size (int, optional): Size parameter for the API call. Defaults to 30.
            
        Returns:
            pd.DataFrame: A DataFrame containing the rank type and associated bearish and bullish tickers.
        """
        # Set default values if parameters are not provided
        if headers is None:
            headers = {}
        if rank_types is None:
            rank_types = ['long', 'intermediate', 'short']
        ...
        async with aiohttp.ClientSession(headers=headers) as session:
            for rank_type in rank_types:
                payload = {
                    ...
                    "type": req_type,
                    "size": size,
                    "rankType": f"technicalSummary.{rank_type}"
                }
                async with session.post(url, json=payload) as resp:
                    data = await resp.json()
                    group = data.get('group', [])
                    
                    # Extract bearish and bullish sections from each item in group
                    bearish = [item.get('bearish') for item in group if item.get('bearish')]
                    bullish = [item.get('bullish') for item in group if item.get('bullish')]
                    
                    # Extract the tickers from bearish and bullish data
                    bear_tickers = [entry.get('tickers') for entry in bearish if entry is not None]
                    bull_tickers = [entry.get('tickers') for entry in bullish if entry is not None]
