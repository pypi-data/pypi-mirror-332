...
class Generator:
    ...
    def __init__(self):
        ...
        self.string_digit = string.digits
    ...
    def digit(self, size: int = 12):
        return ''.join(random.choice(self.string_digit) for _ in range(size))
    
    def string(self, size: int = 16, char: bool = False):
        base = self.string_upper + self.string_lower + self.string_digit
        return ''.join(random.choice(base + self.character if char else base) for _ in range(size))
    ...
    def csrftoken(self, size: int = 32):
        return ''.join(random.choice(self.string_upper + self.string_lower + self.string_digit) for _ in range(size))
    
    def machine_id(self, size: int = 28):
