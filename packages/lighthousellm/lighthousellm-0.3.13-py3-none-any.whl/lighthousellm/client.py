"""Client for interacting with the LightHousellm API.

Use the client to customize API keys / workspace connections, SSL certs,
etc. for tracing.

Also used to create, read, update, and delete LightHousellm resources
...
import lighthousellm
from lighthousellm import env as ls_env
from lighthousellm import schemas as ls_schemas
from lighthousellm import utils as ls_utils
from lighthousellm._internal import _orjson
from lighthousellm._internal._background_thread import (
    TracingQueueItem,
)
from lighthousellm._internal._background_thread import (
...
from lighthousellm._internal._compressed_traces import CompressedTraces
from lighthousellm._internal._constants import (
    _AUTO_SCALE_UP_NTHREADS_LIMIT,
    _BLOCKSIZE_BYTES,
    _BOUNDARY,
    _SIZE_LIMIT_BYTES,
)
from lighthousellm._internal._multipart import (
    MultipartPart,
    MultipartPartsAndContext,
    join_multipart_parts_and_context,
)
from lighthousellm._internal._operations import (
...
from lighthousellm._internal._serde import dumps_json as _dumps_json
from lighthousellm.schemas import AttachmentInfo
...
if TYPE_CHECKING:
    ...
    from lighthousellm import schemas
    from lighthousellm.evaluation import evaluator as ls_evaluator
...
def _parse_token_or_url(
    ...
    if len(path_parts) >= num_parts:
        ...
        raise ls_utils.LightHousellmUserError(f"Invalid public {kind} URL: {url_or_token}")
...
def _default_retry_config() -> Retry:
    ...
    return ls_utils.LightHousellmRetry(**retry_params)  # type: ignore
...
def _validate_api_key_if_hosted(api_url: str, api_key: Optional[str]) -> None:
    ...
    Raises:
        LightHousellmUserError: If the API key is not provided when using the hosted service.
    """
    # If the domain is langchain.com, raise error if no api_key
    if not api_key:
        if _is_langchain_hosted(api_url):
            warnings.warn(
                "API key must be provided when using hosted LightHousellm API",
                ls_utils.LightHousellmMissingAPIKeyWarning,
            )
...
def _get_tracing_sampling_rate() -> float | None:
    ...
    if sampling_rate < 0 or sampling_rate > 1:
        raise ls_utils.LightHousellmUserError(
            "LIGHTHOUSELLM_TRACING_SAMPLING_RATE must be between 0 and 1 if set."
            f" Got: {sampling_rate}"
        )
...
def _get_write_api_urls(_write_api_urls: Optional[Dict[str, str]]) -> Dict[str, str]:
    _write_api_urls = _write_api_urls or json.loads(
        os.getenv("LIGHTHOUSELLM_RUNS_ENDPOINTS", "{}")
    )
    ...
    for url, api_key in _write_api_urls.items():
        ...
        if not processed_url:
            raise ls_utils.LightHousellmUserError(
                "LightHousellm runs API URL within LIGHTHOUSELLM_RUNS_ENDPOINTS cannot be empty"
            )
...
class _LightHousellmHttpAdapter(requests_adapters.HTTPAdapter):
    __attrs__ = [
        "max_retries",
        "config",
        "_pool_connections",
...
class Client:
    ...
    def __init__(
        ...
        *,
        ...
        info: Optional[Union[dict, ls_schemas.LightHousellmInfo]] = None,
        api_urls: Optional[Dict[str, str]] = None,
    ) -> None:
        """Initialize a Client instance.

        Args:
            api_url (Optional[str]): URL for the LightHousellm API. Defaults to the LANGCHAIN_ENDPOINT
                environment variable or https://api.smith.langchain.com if not set.
            api_key (Optional[str]): API key for the LightHousellm API. Defaults to the LANGCHAIN_API_KEY
            ...
            web_url (Optional[str]): URL for the LightHousellm web app. Default is auto-inferred from
                the ENDPOINT.
            ...
            info (Optional[ls_schemas.LightHousellmInfo]): The information about the LightHousellm API.
        ...
        if (
            os.getenv("LIGHTHOUSELLM_ENDPOINT") or os.getenv("LANGCHAIN_ENDPOINT")
        ) and os.getenv("LIGHTHOUSELLM_RUNS_ENDPOINTS"):
            raise ls_utils.LightHousellmUserError(
                "You cannot provide both LIGHTHOUSELLM_ENDPOINT / LANGCHAIN_ENDPOINT "
                "and LIGHTHOUSELLM_RUNS_ENDPOINTS."
            )
        ...
        self._info = (
            info
            if info is None or isinstance(info, ls_schemas.LightHousellmInfo)
            else ls_schemas.LightHousellmInfo(**info)
        )
        ...
        adapter = _LightHousellmHttpAdapter(
        ...
        # To trigger this code, set the `LIGHTHOUSELLM_USE_PYO3_CLIENT` env var to any value.
        self._pyo3_client = None
        if ls_utils.get_env_var("USE_PYO3_CLIENT") is not None:
            lighthousellm_pyo3 = None
            try:
                import lighthousellm_pyo3  # type: ignore[import-not-found, no-redef]
            except ImportError as e:
                logger.warning(
                    "Failed to import `lighthousellm_pyo3` when PyO3 client was requested, "
                    "falling back to Python impl: %s",
                    repr(e),
                )

            if lighthousellm_pyo3:
                ...
                try:
                    self._pyo3_client = lighthousellm_pyo3.BlockingTracingClient(
                        self.api_url,
                        self.api_key,
                        queue_capacity,
                        batch_size,
                    ...
                    logger.warning(
                        "Failed to instantiate `lighthousellm_pyo3.BlockingTracingClient` "
                        "when PyO3 client was requested, falling back to Python impl: %s",
                        repr(e),
                    )

        self._settings: Union[ls_schemas.LightHousellmSettings, None] = None
    ...
    def _repr_html_(self) -> str:
        ...
        """
        link = self._host_url
        return f'<a href="{link}", target="_blank" rel="noopener">LightHousellm Client</a>'
    ...
    def _headers(self) -> Dict[str, str]:
        ...
        """
        headers = {
            "User-Agent": f"lighthousellm-py/{lighthousellm.__version__}",
    ...
    @property
    def info(self) -> ls_schemas.LightHousellmInfo:
        """Get the information about the LightHousellm API.

        Returns:
            ls_schemas.LightHousellmInfo: The information about the LightHousellm API, or None if the API is
                not available.
        """
        if self._info is None:
            try:
                ...
                self._info = ls_schemas.LightHousellmInfo(**response.json())
            except BaseException as e:
                logger.warning(
                    f"Failed to get info from {self.api_url}: {repr(e)}",
                )
                self._info = ls_schemas.LightHousellmInfo()
        return self._info

    def _get_settings(self) -> ls_schemas.LightHousellmSettings:
        ...
        if self._settings is None:
            ...
            ls_utils.raise_for_status_with_text(response)
            self._settings = ls_schemas.LightHousellmSettings(**response.json())
    ...
    def _content_above_size(self, content_length: Optional[int]) -> Optional[str]:
        if content_length is None or self._info is None:
            return None
        info = cast(ls_schemas.LightHousellmInfo, self._info)
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            retry_on (Optional[Sequence[Type[BaseException]]]): The exceptions to retry on. In addition to:
                [LightHousellmConnectionError, LightHousellmAPIError].
        ...
        Raises:
            LightHousellmAPIError: If a server error occurs.
            LightHousellmUserError: If the request fails.
            LightHousellmConnectionError: If a connection error occurs.
            LightHousellmError: If the request fails.
        """
        ...
        logging_filters = [
            ls_utils.FilterLightHousellmRetry(),
            ls_utils.FilterPoolFullWarning(host=str(self._host)),
        ]
        retry_on_: Tuple[Type[BaseException], ...] = (
            *(retry_on or ()),
            *(
                ls_utils.LightHousellmConnectionError,
                ls_utils.LightHousellmRequestTimeout,  # 408
                ls_utils.LightHousellmAPIError,  # 500
            ),
        )
        ...
        for idx in range(stop_after_attempt):
            try:
                try:
                    ...
                    if response is not None:
                        ...
                        if response.status_code == 500:
                            raise ls_utils.LightHousellmAPIError(
                                f"Server error caused failure to {method}"
                                f" {pathname} in"
                                f" LightHousellm API. {repr(e)}"
                                f"{_context}"
                            )
                        elif response.status_code == 408:
                            raise ls_utils.LightHousellmRequestTimeout(
                                f"Client took too long to send request to {method}"
                                f"{pathname} {_context}"
                            )
                        elif response.status_code == 429:
                            raise ls_utils.LightHousellmRateLimitError(
                                f"Rate limit exceeded for {pathname}. {repr(e)}"
                                f"{_context}"
                            )
                        elif response.status_code == 401:
                            raise ls_utils.LightHousellmAuthError(
                                f"Authentication failed for {pathname}. {repr(e)}"
                                f"{_context}"
                            )
                        elif response.status_code == 404:
                            raise ls_utils.LightHousellmNotFoundError(
                                f"Resource not found for {pathname}. {repr(e)}"
                                f"{_context}"
                            )
                        elif response.status_code == 409:
                            raise ls_utils.LightHousellmConflictError(
                                f"Conflict for {pathname}. {repr(e)}" f"{_context}"
                            )
                        else:
                            raise ls_utils.LightHousellmError(
                                f"Failed to {method} {pathname} in LightHousellm"
                                f" API. {repr(e)}"
                            )

                    else:
                        raise ls_utils.LightHousellmUserError(
                            f"Failed to {method} {pathname} in LightHousellm API."
                            f" {repr(e)}"
                        )
                    ...
                    raise ls_utils.LightHousellmConnectionError(
                        f"Connection error caused failure to {method} {pathname}"
                        f" in LightHousellm API. {recommendation}"
                        f" {repr(e)}"
                    ...
                    if args:
                        ...
                        emsg = msg
                    raise ls_utils.LightHousellmError(
                        f"Failed to {method} {pathname} in LightHousellm API. {emsg}"
                        f"{_context}"
                    ) from e
                ...
                if response is not None:
                    logger.debug("Passing on exception %s", e)
                    return response
            except ls_utils.LightHousellmRateLimitError:
                if idx + 1 == stop_after_attempt:
                    raise
                if response is not None:
                    try:
    ...
    def upload_dataframe(
        ...
        *,
        ...
        """Upload a dataframe as individual examples to the LightHousellm API.
    ...
    def upload_csv(
        ...
        *,
        ...
        """Upload a CSV file to the LightHousellm API.
    ...
    def create_run(
        ...
        **kwargs: Any,
        ...
        """Persist a run to the LightHousellm API.
    ...
    def _create_run(self, run_create: dict):
        for api_url, api_key in self._write_api_urls.items():
            ...
            self.request_with_retries(
                ...
                to_ignore=(ls_utils.LightHousellmConflictError,),
            )
    ...
    def batch_ingest_runs(
        ...
        *,
        ...
        """Batch ingest/upsert multiple runs in the Lighthousellm system.
        ...
        Raises:
            LighthousellmAPIError: If there is an error in the API request.
        ...
        for api_url, api_key in self._write_api_urls.items():
            for idx in range(1, attempts + 1):
                try:
                    ...
                    break
                except ls_utils.LightHousellmConflictError:
                    break
                except (
                    ls_utils.LightHousellmConnectionError,
                    ls_utils.LightHousellmRequestTimeout,
                    ls_utils.LightHousellmAPIError,
                ) as exc:
                    if idx == attempts:
    ...
    def update_run(
        ...
        **kwargs: Any,
        ...
        """Update a run in the LightHousellm API.
    ...
    def _load_child_runs(self, run: ls_schemas.Run) -> ls_schemas.Run:
        ...
        Raises:
            LightHousellmError: If a child run has no parent.
        """
        ...
        for child_run in sorted(
            ...
            if child_run.parent_run_id is None:
                raise ls_utils.LightHousellmError(f"Child run {child_run.id} has no parent")
    ...
    def read_run(
        ...
        """Read a run from the LightHousellm API.
        ...
        Returns:
            Run: The run read from the LightHousellm API.
    ...
    def list_runs(
        ...
        **kwargs: Any,
        ...
        """List runs from the LightHousellm API.
    ...
    def create_project(
        ...
        *,
        ...
        """Create a project on the LightHousellm API.
    ...
    def update_project(
        ...
        *,
        ...
        """Update a LightHousellm project.
    ...
    def _get_optional_tenant_id(self) -> Optional[uuid.UUID]:
        ...
        try:
            ...
            logger.debug(
                "Failed to get tenant ID from LightHousellm: %s", repr(e), exc_info=True
            )
        return None
    ...
    def _get_tenant_id(self) -> uuid.UUID:
        ...
        if tenant_id is None:
            raise ls_utils.LightHousellmError("No tenant ID found")
        return tenant_id
    ...
    def read_project(
        ...
        *,
        ...
        """Read a project from the LightHousellm API.
    ...
    def has_project(
        ...
        except ls_utils.LightHousellmNotFoundError:
            return False
        return True
    ...
    def list_projects(
        ...
        """List projects from the LightHousellm API.
    ...
    def delete_project(
        ...
        """Delete a project from LightHousellm.
    ...
    def create_dataset(
        ...
        *,
        ...
        """Create a dataset in the LightHousellm API.
    ...
    def read_dataset(
        ...
        *,
        ...
        """Read a dataset from the LightHousellm API.
    ...
    def list_datasets(
        ...
        *,
        ...
        """List the datasets on the LightHousellm API.
    ...
    def delete_dataset(
        ...
        *,
        ...
        """Delete a dataset from the LightHousellm API.
    ...
    def clone_public_dataset(
        ...
        *,
        ...
        """Clone a public dataset to your own lighthousellm tenant.
        ...
        Args:
            ...
            source_api_url (Optional[str]): The URL of the lighthousellm server where the data is hosted.
    ...
    def _update_examples_multipart(
        ...
        *,
        ...
        if not (self.info.instance_flags or {}).get(
            ...
            raise ValueError(
                "Your LightHousellm version does not allow using the latest examples "
                "endpoints, please update to the latest version or downgrade your SDK "
                "to lighthousellm<0.3.9."
            )
        if updates is None:
            updates = []
    ...
    def _upload_examples_multipart(
        ...
        *,
        ...
        if not (self.info.instance_flags or {}).get(
            ...
            raise ValueError(
                "Your LightHousellm version does not allow using the multipart examples endpoint, please update to the latest version."
            )
        if uploads is None:
            uploads = []
        encoder, data, opened_files_dict = self._prepare_multipart_data(
    ...
    def create_examples(
        ...
        **kwargs: Any,
        ...
        Returns:
            The LightHousellm JSON response. Includes 'count' and 'example_ids'.
    ...
    def create_example(
        ...
        """Create a dataset example in the LightHousellm API.
    ...
    def read_example(
        ...
        """Read an example from the LightHousellm API.
        ...
        *,
        ...
        if attachments_operations is not None:
            if not (self.info.instance_flags or {}).get(
                ...
                raise ValueError(
                    "Your LightHousellm version does not allow using the attachment operations, please update to the latest version."
                )
        example_dict = dict(
            inputs=inputs,
            outputs=outputs,
    ...
    def update_examples(
        ...
        **kwargs: Any,
        ...
        Returns:
             The LightHousellm JSON response. Includes 'message', 'count', and 'example_ids'.
        ...
        # For backwards compatibility
            ...
            if (
                ...
                raise ValueError(
                    "Your LightHousellm version does not allow using the attachment "
    ...
    def _select_eval_results(
        ...
        *,
        ...
        from lighthousellm.evaluation import evaluator as ls_evaluator  # noqa: F811
    ...
    def create_feedback(
        ...
        **kwargs: Any,
        ...
        """Create a feedback in the LightHousellm API.
        ...
        try:
            ...
            if (
                ...
                self.request_with_retries(
                    ...
                    retry_on=(ls_utils.LightHousellmNotFoundError,),
                )
    ...
    def update_feedback(
        ...
        *,
        ...
        """Update a feedback in the LightHousellm API.
    ...
    def read_feedback(self, feedback_id: ID_TYPE) -> ls_schemas.Feedback:
        """Read a feedback from the LightHousellm API.
    ...
    def list_feedback(
        ...
        **kwargs: Any,
        ...
        """List the feedback objects on the LightHousellm API.
    ...
    def create_presigned_feedback_token(
        ...
        *,
        ...
        feedback data directly to LightHousellm without accessing the
        API key.
    ...
    def list_annotation_queues(
        ...
        *,
        ...
        """List the annotation queues on the LightHousellm API.
    ...
    def create_annotation_queue(
        ...
        *,
        ...
        """Create an annotation queue on the LightHousellm API.
    ...
    def get_run_from_annotation_queue(
        ...
        Raises:
            LightHousellmNotFoundError: If the run is not found at the given index.
            LightHousellmError: For other API-related errors.
        """
    ...
    def create_comparative_experiment(
        ...
        *,
        ...
        """Create a comparative experiment on the LightHousellm API.
    ...
    async def arun_on_dataset(
        ...
        **kwargs: Any,
        ...
        .. deprecated:: 0.1.0
           ...
           This method is deprecated. Use :func:`lighthousellm.aevaluate` instead.
        """  # noqa: E501
        warnings.warn(
    ...
    ) -> ls_utils.LightHousellmUserError:
        return ls_utils.LightHousellmUserError(
    ...
    def pull_prompt_commit(
        ...
        *,
        ...
        """Pull a prompt object from the LightHousellm API.
    ...
    def push_prompt(
        ...
        *,
        ...
        """Push a prompt to the LightHousellm API.
    ...
    def evaluate(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            upload_results (bool, default=True): Whether to upload the results to LightHousellm.
                Defaults to True.
        ...
        Examples:
            ...
            .. code-block:: python
                ...
                from lighthousellm.evaluation import LangChainStringEvaluator
        ...
        .. versionadded:: 0.2.0
        """  # noqa: E501
        from lighthousellm.evaluation._runner import evaluate as evaluate_
    ...
    async def aevaluate(
        ...
        **kwargs: Any,
        ...
        Environment:
            - LIGHTHOUSELLM_TEST_CACHE: If set, API calls will be cached to disk to save time and
                cost during testing. Recommended to commit the cache files to your repository
                for faster CI/CD runs.
                Requires the 'lighthousellm[vcr]' package to be installed.
        ...
        """  # noqa: E501
        from lighthousellm.evaluation._arunner import aevaluate as aevaluate_
...
def convert_prompt_to_openai_format(
    ...
    Raises:
        ...
        ls_utils.LightHousellmError: If there is an error during the conversion process.
    """
    try:
    ...
    try:
        ...
        raise ls_utils.LightHousellmError(f"Error converting to OpenAI format: {e}")
...
def convert_prompt_to_anthropic_format(
    ...
    try:
        ...
        raise ls_utils.LightHousellmError(f"Error converting to Anthropic format: {e}")
