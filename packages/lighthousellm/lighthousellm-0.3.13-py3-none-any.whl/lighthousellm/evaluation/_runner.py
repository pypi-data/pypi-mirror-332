...
from lighthousellm._internal._beta_decorator import _warn_once
...
from lighthousellm.evaluation.integrations import LangChainStringEvaluator
...
def evaluate(
    ...
    client: Optional[lighthousellm.Client] = None,
    blocking: bool = True,
    ...
    **kwargs: Any,
    ...
    Args:
        ...
        max_concurrency (int | None): The maximum number of concurrent
            ...
            Defaults to 0.
        client (lighthousellm.Client | None): The LightHousellm client to use.
            Defaults to None.
    ...
    Examples:
        ...
        >>> from lighthousellm.schemas import Example, Run
        >>> client = Client()
    ...
    if isinstance(target, (str, uuid.UUID, schemas.TracerSession)):
        ...
        msg = (
            ...
            "from lighthousellm import aevaluate\n\n"
            "await aevaluate(\n"
...
def evaluate_existing(
    ...
    Args:
        ...
        max_concurrency (int | None): The maximum number of concurrent
            evaluations to run. If None then no limit is set. If 0 then no concurrency.
            Defaults to 0.
        client (Optional[lighthousellm.Client]): Optional Lighthousellm client to use for evaluation.
    ...
    Examples:
        >>> from lighthousellm.evaluation import evaluate, evaluate_existing
        >>> dataset_name = "Evaluate Examples"
        >>> def predict(inputs: dict) -> dict:
        ...     # This can be any function or just an API call to your app.
        ...     return {"output": "Yes"}
...
def evaluate_comparative(
    ...
    Args:
        ...
        max_concurrency (int): The maximum number of concurrent evaluations to run.
            Defaults to 5.
        client (Optional[lighthousellm.Client]): The LightHousellm client to use.
            Defaults to None.
    ...
    Examples:
        ...
        >>> import json
        >>> from lighthousellm.evaluation import evaluate_comparative
        >>> from lighthousellm import schemas
...
def _load_experiment(
    project: EXPERIMENT_T, client: lighthousellm.Client
...
def _load_examples_map(
    client: lighthousellm.Client, project: schemas.TracerSession
...
class _ExperimentManager(_ExperimentManagerMixin):
    ...
    Args:
        ...
        client (Optional[lighthousellm.Client]): The Lighthousellm client used for
             the experiment.
...
def _forward(
    fn: rh.SupportsLighthousellmExtra,
    example: schemas.Example,
    experiment_name: str,
    metadata: dict,
    client: lighthousellm.Client,
    ...
    with rh.tracing_context(enabled="local" if not upload_results else True):
        ...
        lighthousellm_extra = rh.LightHousellmExtra(
...
def _ensure_traceable(
    target: TARGET_T | rh.SupportsLighthousellmExtra[[dict], dict] | Runnable,
) -> rh.SupportsLighthousellmExtra[[dict], dict]:
    ...
    if rh.is_traceable_function(target):
        fn: rh.SupportsLighthousellmExtra[[dict], dict] = target
    else:
...
def _get_random_name() -> str:
    from lighthousellm.evaluation._name_generation import random_name  # noqa: F401
