...
from lighthousellm import client as ls_client
from lighthousellm import run_trees, schemas, utils
from lighthousellm._internal import _aiter as aitertools
from lighthousellm.env import _runtime_env
    ...
    **kwargs: Any,
    ...
    Args:
        ...
        parent: The parent run to use for the context. Can be a Run/RunTree object,
            ...
            Defaults to None.
        client: The client to use for logging the run to LightHousellm. Defaults to None,
...
def is_traceable_function(func: Any) -> TypeGuard[SupportsLighthousellmExtra[P, R]]:
...
) -> SupportsLighthousellmExtra[P, R]:
...
class LightHousellmExtra(TypedDict, total=False):
...
@runtime_checkable
class SupportsLighthousellmExtra(Protocol, Generic[P, R]):
    """Implementations of this Protoc accept an optional lighthousellm_extra parameter.

    Args:
        *args: Variable length arguments.
        lighthousellm_extra (Optional[LightHousellmExtra): Optional dictionary of
            additional parameters for Lighthousellm.
    ...
    def __call__(  # type: ignore[valid-type]
        self,
        *args: P.args,
        lighthousellm_extra: Optional[LightHousellmExtra] = None,
        **kwargs: P.kwargs,
    ) -> R:
        ...
        Args:
            *args: Variable length argument list.
            lighthousellm_extra: Optional dictionary containing additional
                parameters specific to Lighthousellm.
...
) -> Callable[[Callable[P, R]], SupportsLighthousellmExtra[P, R]]: ...
...
def traceable(
    ...
    **kwargs: Any,
    ...
    """Trace a function with lighthousellm.
    ...
    Args:
        ...
        metadata: The metadata to add to the run. Defaults to None.
        tags: The tags to add to the run. Defaults to None.
        client: The client to use for logging the run to LightHousellm. Defaults to
        ...
        dangerously_allow_filesystem: Whether to allow filesystem access for attachments.
            ...
            Traces that reference local filepaths will be uploaded to LightHousellm.
    ...
    Note:
            - Requires that LIGHTHOUSELLM_TRACING_V2 be set to 'true' in the environment.
    ...
    Examples:
        ...
        Manually passing lighthousellm_extra:
        ...
        .. code-block:: python
            ...
            manual_extra_function(5, lighthousellm_extra={"metadata": {"version": "1.0"}})
    """
    run_type = cast(
    ...
    def decorator(func: Callable):
        ...
        async def async_wrapper(
            ...
            **kwargs: Any,
            ...
            run_container = await aitertools.aio_to_thread(
                ...
                lighthousellm_extra=lighthousellm_extra,
                args=args,
                kwargs=kwargs,
            )
        ...
        async def async_generator_wrapper(
            *args: Any, lighthousellm_extra: Optional[LightHousellmExtra] = None, **kwargs: Any
        ) -> AsyncGenerator:
            if not func_accepts_config:
                kwargs.pop("config", None)
            run_container = await aitertools.aio_to_thread(
        ...
        setattr(selected_wrapper, "__lighthousellm_traceable__", True)
...
class trace:
    """Manage a LightHousellm run in context.
    ...
    Args:
        ...
        client (Optional[ls_client.Client], optional): LightHousellm client for custom settings. Defaults to None.
        ...
        extra (Optional[Dict], optional): Extra data to send to LightHousellm. Use 'metadata' instead. Defaults to None.
...
def as_runnable(traceable_fn: Callable) -> Runnable:
    """Convert a function wrapped by the LightHousellm @traceable decorator to a Runnable.

    Args:
        traceable_fn (Callable): The function wrapped by the @traceable decorator.

    Returns:
        Runnable: A Runnable object that maintains a consistent LightHousellm
            tracing context.
    ...
    if not is_traceable_function(traceable_fn):
        ...
        raise ValueError(
            f"as_runnable expects a function wrapped by the LightHousellm"
    ...
    class RunnableTraceable(RunnableLambda):
        ...
        This helps maintain a consistent LightHousellm tracing context.
        """
        ...
        def _wrap_sync(
            ...
            def wrap_traceable(inputs: dict, config: RunnableConfig) -> Any:
                ...
                return func(**inputs, lighthousellm_extra={"run_tree": run_tree})
        ...
        def _wrap_async(
            ...
            if not is_traceable_function(afunc):
                raise ValueError(
                    "RunnableTraceable expects a function wrapped by the LightHousellm"
            ...
            async def awrap_traceable(inputs: dict, config: RunnableConfig) -> Any:
                run_tree = run_trees.RunTree.from_runnable_config(cast(dict, config))
                return await afunc_(**inputs, lighthousellm_extra={"run_tree": run_tree})
...
def _collect_extra(extra_outer: dict, lighthousellm_extra: LightHousellmExtra) -> dict:
    run_extra = lighthousellm_extra.get("run_extra", None)
    if run_extra:
...
def _get_parent_run(
    lighthousellm_extra: LightHousellmExtra,
    config: Optional[dict] = None,
) -> Optional[run_trees.RunTree]:
    parent = lighthousellm_extra.get("parent")
    if parent == "ignore":
        return None
    ...
    if isinstance(parent, dict):
        return run_trees.RunTree.from_headers(
            parent,
            client=lighthousellm_extra.get("client"),
            # Precedence: headers -> cvar -> explicit -> env var
            project_name=_get_project_name(lighthousellm_extra.get("project_name")),
        )
    if isinstance(parent, str):
        ...
        return dort
    run_tree = lighthousellm_extra.get("run_tree")
    if run_tree:
        return run_tree
    ...
    if _runtime_env.get_langchain_core_version() is not None:
        if rt := run_trees.RunTree.from_runnable_config(
            config, client=lighthousellm_extra.get("client")
        ):
            ...
            if (
                ...
                # If the LangChain dotted order is more nested than the LightHousellm
...
def _setup_run(
    ...
    lighthousellm_extra = lighthousellm_extra or LightHousellmExtra()
    name = lighthousellm_extra.get("name") or container_input.get("name")
    client_ = lighthousellm_extra.get("client", client) or _CLIENT.get()
    parent_run_ = _get_parent_run(
        {**lighthousellm_extra, "client": client_}, kwargs.get("config")
    )
    ...
    selected_project = (
        ...
        or lighthousellm_extra.get("project_name")  # at invocation time
        or container_input["project_name"]  # at decorator time
        or utils.get_tracer_project()  # default
    )
    reference_example_id = lighthousellm_extra.get("reference_example_id")
    id_ = lighthousellm_extra.get("run_id")
    if not parent_run_ and not utils.tracing_is_enabled():
        utils.log_once(
            logging.DEBUG,
            "LightHousellm tracing is not enabled, returning original function.",
        )
        return _TraceableContainer(
            ...
            outer_metadata=None,
            outer_tags=None,
            on_end=lighthousellm_extra.get("on_end"),
    ...
    extra_inner = _collect_extra(extra_outer, lighthousellm_extra)
    outer_metadata = _METADATA.get()
    outer_tags = _TAGS.get()
    context = copy_context()
    metadata_ = {
        **(lighthousellm_extra.get("metadata") or {}),
        **(outer_metadata or {}),
    }
    ...
    tags_ = (lighthousellm_extra.get("tags") or []) + (outer_tags or [])
...
def _is_traceable_function(func: Any) -> bool:
    return getattr(func, "__lighthousellm_traceable__", False)
