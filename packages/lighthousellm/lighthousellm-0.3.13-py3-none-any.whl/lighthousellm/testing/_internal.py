...
from lighthousellm import run_helpers as rh
from lighthousellm import run_trees
from lighthousellm import run_trees as rt
...
from lighthousellm._internal._serde import dumps_json
from lighthousellm.client import ID_TYPE
...
def test(*args: Any, **kwargs: Any) -> Callable:
    """Trace a pytest test case in LightHousellm.

    This decorator is used to trace a pytest test to LightHousellm. It ensures
    ...
    recorded and reported by LightHousellm.
    ...
    Args:
        ...
        - client (Optional[ls_client.Client]): An instance of the LightHousellm client
            to be used for communication with the LightHousellm service. If not provided,
    ...
    Environment:
        - LIGHTHOUSELLM_TEST_CACHE: If set, API calls will be cached to disk to
        ...
        - LIGHTHOUSELLM_TEST_TRACKING: Set this variable to the path of a directory
            to enable caching of test results. This is useful for re-running tests
             without re-executing the code. Requires the 'lighthousellm[vcr]' package.

    Example:
        For basic usage, simply decorate a test function with `@pytest.mark.lighthousellm`.
        ...
        .. code-block:: python
            ...
            # from lighthousellm import test
            # @test
            @pytest.mark.lighthousellm
            def test_addition():
                assert 3 + 4 == 7
        ...
        `LIGHTHOUSELLM_TEST_CACHE=path/to/cache`. Check in these files to speed up
        CI/CD pipelines, so your results only change when your prompt or requested
        model changes.

        Note that this will require that you install lighthousellm with the `vcr` extra:

        `pip install -U "lighthousellm[vcr]"`
        ...
        .. code-block:: python
            ...
            # os.environ["LIGHTHOUSELLM_TEST_CACHE"] = "tests/cassettes"
            import openai
            import pytest
        ...
        LLMs are stochastic. Naive assertions are flakey. You can use lighthousellm's
        `expect` to score and make approximate assertions on your results.

        .. code-block:: python

            import pytest
            from lighthousellm import expect
            ...
            def test_output_semantically_close():
                ...
                # The embedding_distance call logs the embedding distance to LightHousellm
                expect.embedding_distance(
                    prediction=response.choices[0].message.content,
                    reference="Hello!",
                    # The following optional assertion logs a
                    # pass/fail score to LightHousellm
                ...
                expect.edit_distance(
                    prediction=response.choices[0].message.content,
                    reference="Hello!",
                    # And then log a pass/fail score to LightHousellm
                ).to_be_less_than(1.0)

        The `@test` decorator works natively with pytest fixtures.
        The values will populate the "inputs" of the corresponding example in LightHousellm.
        ...
        .. code-block:: python
            ...
            @pytest.mark.lighthousellm(output_keys=["expected"])
        ...
        .. code-block:: python
            ...
            @pytest.mark.lighthousellm(id=str(example_id))
        ...
        .. code-block:: python
            ...
            @pytest.mark.lighthousellm(output_keys=["expected_output"])
    ...
    if disable_tracking:
        logger.info(
            "LIGHTHOUSELLM_TEST_TRACKING is set to 'false'."
            " Skipping LightHousellm test tracking."
        )
...
def _get_experiment_name(test_suite_name: str) -> str:
    ...
    if os.environ.get("LIGHTHOUSELLM_EXPERIMENT"):
        prefix = os.environ["LIGHTHOUSELLM_EXPERIMENT"]
    else:
...
def _get_test_suite_name(func: Callable) -> str:
    ...
    raise ValueError("Please set the LIGHTHOUSELLM_TEST_SUITE environment variable.")
...
def _end_tests(test_suite: _LightHousellmTestSuite):
...
class _TestCase:
    def __init__(
        self,
        test_suite: _LightHousellmTestSuite,
        example_id: uuid.UUID,
        run_id: uuid.UUID,
        pytest_plugin: Any = None,
        pytest_nodeid: Any = None,
...
def _create_test_case(
    ...
    **kwargs: Any,
    ...
    test_suite = _LightHousellmTestSuite.from_test(
    ...
    pytest_plugin = (
        pytest_request.config.pluginmanager.get_plugin("lighthousellm_output_plugin")
        if pytest_request
        else None
    )
...
def log_inputs(inputs: dict, /) -> None:
    ...
    Should only be used in pytest tests decorated with @pytest.mark.lighthousellm.
    ...
    Example:
        .. code-block:: python
            ...
            from lighthousellm import testing as t
    ...
    if ls_utils.test_tracking_is_disabled():
        logger.info(
            "LIGHTHOUSELLM_TEST_TRACKING is set to 'false'." " Skipping log_inputs."
        )
        return
    ...
    if not run_tree or not test_case:
        msg = (
            ...
            "@pytest.mark.lighthousellm, and with tracing enabled (by setting the "
            "LIGHTHOUSELLM_TRACING environment variable to 'true')."
        )
        raise ValueError(msg)
...
def log_outputs(outputs: dict, /) -> None:
    ...
    if ls_utils.test_tracking_is_disabled():
        logger.info(
            "LIGHTHOUSELLM_TEST_TRACKING is set to 'false'." " Skipping log_outputs."
        )
        return
    run_tree = rh.get_current_run_tree()
    test_case = _TEST_CASE.get()
...
def log_reference_outputs(reference_outputs: dict, /) -> None:
    ...
    Example:
        .. code-block:: python
            ...
            from lighthousellm import testing
    ...
    if not test_case:
        msg = (
            ...
            "decorated with @pytest.mark.lighthousellm."
        )
        raise ValueError(msg)
...
def log_feedback(
    ...
    **kwargs: Any,
    ...
    if ls_utils.test_tracking_is_disabled():
        logger.info(
            "LIGHTHOUSELLM_TEST_TRACKING is set to 'false'." " Skipping log_feedback."
        )
        return
