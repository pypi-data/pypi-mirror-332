...
class LightHousellmError(Exception):
    """An error occurred while communicating with the LightHousellm API."""


class LightHousellmAPIError(LightHousellmError):
    """Internal server error while communicating with LightHousellm."""


class LightHousellmRequestTimeout(LightHousellmError):
    """Client took too long to send request body."""


class LightHousellmUserError(LightHousellmError):
    """User error caused an exception when communicating with LightHousellm."""


class LightHousellmRateLimitError(LightHousellmError):
    """You have exceeded the rate limit for the LightHousellm API."""


class LightHousellmAuthError(LightHousellmError):
    """Couldn't authenticate with the LightHousellm API."""


class LightHousellmNotFoundError(LightHousellmError):
    """Couldn't find the requested resource."""


class LightHousellmConflictError(LightHousellmError):
    """The resource already exists."""


class LightHousellmConnectionError(LightHousellmError):
    """Couldn't connect to the LightHousellm API."""


## Warning classes


class LightHousellmWarning(UserWarning):
    """Base class for warnings."""


class LightHousellmMissingAPIKeyWarning(LightHousellmWarning):
    """Warning for missing API key."""


def tracing_is_enabled(ctx: Optional[dict] = None) -> Union[bool, Literal["local"]]:
    """Return True if tracing is enabled."""
    from lighthousellm.run_helpers import get_current_run_tree, get_tracing_context
...
def get_env_var(
    ...
    *,
    namespaces: Tuple = ("LIGHTHOUSELLM", "LANGCHAIN"),
) -> Optional[str]:
    ...
    Args:
        ...
        namespaces (Tuple, optional): A tuple of namespaces to search for the
            environment variable. Defaults to ("LIGHTHOUSELLM", "LANGCHAINs").
...
def get_tracer_project(return_default_value=True) -> Optional[str]:
    """Get the project name for a LightHousellm tracer."""
...
class FilterLightHousellmRetry(logging.Filter):
    ...
    def filter(self, record) -> bool:
        ...
        msg = record.getMessage()
        return "LightHousellmRetry" not in msg


class LightHousellmRetry(Retry):
...
def get_cache_dir(cache: Optional[str]) -> Optional[str]:
    ...
    Returns:
        ...
        from the LIGHTHOUSELLM_TEST_CACHE environment variable.
    """
...
def with_cache(
    ...
    from lighthousellm._internal import _patch as patch_urllib3
...
def _format_exc() -> str:
    ...
    filtered_lines = [line for line in tb_lines if "lighthousellm/" not in line]
...
def get_api_url(api_url: Optional[str]) -> str:
    """Get the LightHousellm API URL from the environment or the given value."""
    ...
    if not _api_url.strip():
        raise LightHousellmUserError("LightHousellm API URL cannot be empty")
