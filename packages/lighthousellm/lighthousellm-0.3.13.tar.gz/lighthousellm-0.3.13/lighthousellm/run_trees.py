"""Schemas for the LightHousellm API."""
...
from lighthousellm import utils
from lighthousellm.client import ID_TYPE, RUN_TYPE_T, Client, _dumps_json, _ensure_uuid

logger = logging.getLogger(__name__)

LIGHTHOUSELLM_PREFIX = "lighthousellm-"
LIGHTHOUSELLM_DOTTED_ORDER = sys.intern(f"{LIGHTHOUSELLM_PREFIX}trace")
LIGHTHOUSELLM_DOTTED_ORDER_BYTES = LIGHTHOUSELLM_DOTTED_ORDER.encode("utf-8")
LIGHTHOUSELLM_METADATA = sys.intern(f"{LIGHTHOUSELLM_PREFIX}metadata")
LIGHTHOUSELLM_TAGS = sys.intern(f"{LIGHTHOUSELLM_PREFIX}tags")
LIGHTHOUSELLM_PROJECT = sys.intern(f"{LIGHTHOUSELLM_PREFIX}project")
...
class RunTree(ls_schemas.RunBase):
    ...
    def from_dotted_order(
        ...
        **kwargs: Any,
        ...
        """
        headers = {
            LIGHTHOUSELLM_DOTTED_ORDER: dotted_order,
        }
    ...
    def from_headers(
        ...
        lighthousellm_trace = cast(Optional[str], headers.get(LIGHTHOUSELLM_DOTTED_ORDER))
        if not lighthousellm_trace:
            lighthousellm_trace_bytes = cast(
                Optional[bytes], headers.get(LIGHTHOUSELLM_DOTTED_ORDER_BYTES)
            )
            if not lighthousellm_trace_bytes:
                return  # type: ignore[return-value]
            lighthousellm_trace = lighthousellm_trace_bytes.decode("utf-8")

        parent_dotted_order = lighthousellm_trace.strip()
    ...
    def to_headers(self) -> Dict[str, str]:
        ...
        headers = {}
        if self.trace_id:
            headers[f"{LIGHTHOUSELLM_DOTTED_ORDER}"] = self.dotted_order
        baggage = _Baggage(
...
class _Baggage:
    ...
    def from_header(cls, header_value: Optional[str]) -> _Baggage:
        ...
        try:
            for item in header_value.split(","):
                ...
                if key == LIGHTHOUSELLM_METADATA:
                    metadata = json.loads(urllib.parse.unquote(value))
                elif key == LIGHTHOUSELLM_TAGS:
                    tags = urllib.parse.unquote(value).split(",")
                elif key == LIGHTHOUSELLM_PROJECT:
    ...
    def to_header(self) -> str:
        ...
        if self.metadata:
            ...
            items.append(
                f"{LIGHTHOUSELLM_PREFIX}metadata={urllib.parse.quote(serialized_metadata)}"
            )
        if self.tags:
            serialized_tags = ",".join(self.tags)
            items.append(
                f"{LIGHTHOUSELLM_PREFIX}tags={urllib.parse.quote(serialized_tags)}"
            )
        if self.project_name:
            items.append(
                f"{LIGHTHOUSELLM_PREFIX}project={urllib.parse.quote(self.project_name)}"
            )
        return ",".join(items)
