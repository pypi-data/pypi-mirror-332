...
class PyFohhnTextDevice:
    ...
    def load_preset(self, preset_nr):
        ...
        """
        _response = self.communicator.send_text_command(f"SET PRESET {self.id} {preset_nr}\r\n")
    ...
    def set_volume(self, channel, vol, on, invert):
        ...
        """
        _response = self.communicator.send_text_command(f"SET VOL {self.id} {channel} {int(vol * 10)} {1 if on else 0} {1 if invert else 0}\r\n")
    ...
    def set_relative_volume(self, channel, rel_vol):
        ...
        """
        _response = self.communicator.send_text_command(f"SET RVOL {self.id} {channel} {int(rel_vol * 10)}\r\n")

    def get_volume(self, channel):
        """
        Get a channels volume
        """
        response = self.communicator.send_text_command(f"GET VOL {self.id} {channel}\r\n")
    ...
    def set_routing_volume(self, channel_out, channel_in, vol, on, invert):
        ...
        """
        _response = self.communicator.send_text_command(f"SET ROUTING {self.id} {channel_out} {channel_in} {int(vol * 10)} {1 if on else 0} {1 if invert else 0}\r\n")

    def get_routing_volume(self, channel_out, channel_in):
        """
        Get the routing volume from a channel to a channel
        """
        response = self.communicator.send_text_command(f"GET ROUTING {self.id} {channel_out} {channel_in}\r\n")
        match = re.match(r"([\-]?\d+)\s(\d+)\s(\d+)", response)
        return float(match.group(1)) / 10, match.group(2) == "1", match.group(3) == "1"
    ...
    def set_mute(self, channel, on):
        ...
        """
        _response = self.communicator.send_text_command(f"SET MUTE {self.id} {channel} {1 if on else 0}\r\n")

    def get_mute(self, channel):
        """
        Returns if a channel is turned on
        """
        response = self.communicator.send_text_command(f"GET MUTE {self.id} {channel}\r\n")
        return response == "1"
    ...
    def set_standby(self, on):
        ...
        """
        _response = self.communicator.send_text_command(f"SET STANDBY {self.id} {1 if on else 0}\r\n")
    ...
    def get_info(self):
        ...
        return int(match.group(1), 16), int(match.group(2)), int(match.group(3)), int(match.group(4))
