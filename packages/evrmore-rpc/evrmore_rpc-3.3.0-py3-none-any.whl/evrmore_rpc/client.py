...
import inspect
import uuid
from evrmore_rpc.utils import detect_async_context, connection_pool
import traceback
...
class EvrmoreRPCError(Exception):
    """Exception raised for errors in the RPC response."""
    
    def __init__(self, message: str, code: Optional[int] = None, data: Optional[Any] = None):
        """Initialize the exception with a message and optional error code and data.
        
        Args:
            message: The error message
            code: The error code from the RPC server
            data: Additional error data
        """
        self.code = code
        self.data = data
        
        # Add troubleshooting hints based on the error code or message
        troubleshooting = self._get_troubleshooting_hint(message, code)
        if troubleshooting:
            message = f"{message}\n\nTroubleshooting hint: {troubleshooting}"
        
        super().__init__(message)
    
    def _get_troubleshooting_hint(self, message: str, code: Optional[int]) -> Optional[str]:
        """Get a troubleshooting hint based on the error message or code.
        ...
        Returns:
            A troubleshooting hint or None if no specific hint is available
        """
        # Authentication errors
        if code == 401 or "Unauthorized" in message or "Authentication failed" in message:
            return "Check your RPC username and password in evrmore.conf or in your client configuration."
        
        # Connection errors
        if "Connection refused" in message:
            return "Make sure the Evrmore daemon (evrmored) is running and the RPC server is enabled."
        
        # Method not found
        if code == -32601 or "Method not found" in message:
            return "The RPC method you're trying to call does not exist or is not exposed by the server."
        
        # Invalid parameters
        if code == -32602 or "Invalid params" in message or "incorrect parameters" in message:
            return "Check the parameters passed to the RPC method. The number or type of parameters may be incorrect."
        
        # Wallet errors
        if "wallet" in message.lower():
            if "not found" in message.lower():
                return "Make sure your wallet is loaded and accessible by the Evrmore daemon."
            if "locked" in message.lower():
                return "Your wallet is locked. Use 'walletpassphrase' to unlock it first."
        
        # Asset errors
        if "asset" in message.lower() and "not found" in message.lower():
            return "The specified asset does not exist or has not been issued on the blockchain."
        
        # Transaction errors
        if "transaction" in message.lower():
            if "not found" in message.lower():
                return "The transaction may not be in the blockchain or mempool. If it's recent, it might not be propagated yet."
            if "already in block chain" in message.lower():
                return "The transaction has already been included in the blockchain."
            if "rejected" in message.lower():
                return "The transaction was rejected. It might violate consensus rules or have insufficient fee."
        
        # Block errors
        if "block" in message.lower() and "not found" in message.lower():
            return "The specified block does not exist or is not in the main chain."
        
        # Generic RPC error
        if code and -32099 <= code <= -32000:
            return "This is a server-side error. Check the Evrmore daemon logs for more information."
        
        # Generic connection error
        if "timeout" in message.lower():
            return "The connection to the RPC server timed out. The server might be under heavy load or unreachable."
...
class EvrmoreConfig:
    ...
    def get_rpc_credentials(self) -> Tuple[Optional[str], Optional[str]]:
        """Get RPC credentials from configuration."""
        return self.get('rpcuser'), self.get('rpcpassword')
...
class EvrmoreClient:
    ...
    def _handle_response(self, response_data: Dict[str, Any]) -> Any:
        ...
        if "error" in response_data and response_data["error"] is not None:
            ...
            if isinstance(error, dict):
                ...
                raise EvrmoreRPCError(f"RPC error ({code}): {message}", code, error)
            else:
                raise EvrmoreRPCError(f"RPC error: {error}", -1, error)
        
        if "result" not in response_data:
            raise EvrmoreRPCError("No result in response", -1, None)
    ...
    def execute_command_sync(self, command: str, *args: Any) -> Any:
        ...
        if self.sync_session is None:
            raise EvrmoreRPCError("Session not initialized", -1, None)
        ...
        try:
            ...
            if response.status_code != 200:
                raise EvrmoreRPCError(f"HTTP error {response.status_code}: {response.text}", response.status_code, None)
            
            response_data = response.json()
            return self._handle_response(response_data)
        except requests.RequestException as e:
            raise EvrmoreRPCError(f"Request failed: {str(e)}", -1, e)
        except json.JSONDecodeError:
            raise EvrmoreRPCError("Invalid JSON response", -1, None)
    ...
    async def close(self):
        ...
        # Close the connection pool sessions
        ...
        try:
            async with self.async_session.post(
                ...
                if response.status != 200:
                    ...
                    raise EvrmoreRPCError(f"HTTP error {response.status}: {text}", response.status, None)
            ...
            raise EvrmoreRPCError(f"Request timed out after {self.timeout} seconds", -1, None)
        except json.JSONDecodeError:
            raise EvrmoreRPCError("Invalid JSON response", -1, None)
    ...
    def stress_test_sync(self, num_calls: int = 100, command: str = "getblockcount", concurrency: int = 10) -> Dict[str, Any]:
        ...
        if not valid_results:
            raise EvrmoreRPCError("All stress test calls failed", -1, None)
    ...
    def start_batch(self):
        """Start batch mode for collecting multiple RPC calls to execute together.
        
        In batch mode, RPC commands are collected but not executed until
        `execute_batch()` or `execute_batch_async()` is called.
        """
    ...
    def execute_batch(self) -> List[Dict[str, Any]]:
        """Execute all collected batch commands synchronously.
        
        Returns:
            A list of results corresponding to each command in the batch
        
        Raises:
            ValueError: If not in batch mode or no commands have been batched
        """
        if not self._batch_mode:
            raise ValueError("Not in batch mode. Call start_batch() first.")
        
        if not self._batch_requests:
            raise ValueError("No commands have been batched.")
        ...
        for i, result in enumerate(results):
            if "error" in result and result["error"] is not None:
                error = result["error"]
                raise EvrmoreRPCError(f"RPC Error in batch command {i}: {error['code']}, {error['message']}", error['code'], error)
        
        # Return just the results
        return [r["result"] for r in results]
    
    async def execute_batch_async(self) -> List[Dict[str, Any]]:
        """Execute all collected batch commands asynchronously.
        ...
        # Execute the batched requests asynchronously
        url = self._get_rpc_url()
        headers = self._get_auth_headers()
    ...
    def _call_rpc(self, method: str, *args) -> Dict[str, Any]:
        """Internal method for making a synchronous RPC call.
        
        If in batch mode, adds the command to the batch instead of executing it.
        """
        payload = {
            "jsonrpc": "2.0",
            "id": str(uuid.uuid4()),
        ...
        # If in batch mode, add to batch and return None
        if self._batch_mode:
        ...
        # Use the connection pool to get a session
        session = connection_pool.get_sync_session()
        ...
        if "error" in data and data["error"] is not None:
            error = data["error"]
            raise EvrmoreRPCError(f"RPC Error: {error['code']}, {error['message']}", error['code'], error)
        
        # Return the result
        return data["result"]
    
    async def _call_rpc_async(self, method: str, *args) -> Dict[str, Any]:
        """Internal method for making an asynchronous RPC call.
    ...
    def __del__(self):
        """Ensure resources are cleaned up when the client is garbage collected."""
        if hasattr(self, "_is_async") and not self._is_async:
            connection_pool.close_sync_session()
        # We can't use await in __del__ so the async session will be closed by the pool's __del__ 