...
class IW4MWrapper:
    ...
    class Utils:
        ...
        def login(self, client_id: str, password: str):
            response = self.wrapper.session.get(f"{self.wrapper.base_url}/Action/Login/?clientId={client_id}&Password={password}")
            print(response.headers)

        def parse_request_login_token(self, message: str):
            print(re.search(r'"([^"]+)"', message).group(1))
            print(re.search(r'ID is (\d+)', message).group(1))
    ...
    class Server:
        ...
        def get_server_uptime(self):
            return self.wrapper.session.get(f"{self.wrapper.base_url}/Console/Execute?serverId=15253132414984&command=!uptime").text
