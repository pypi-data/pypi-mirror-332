...
def class_names_from_csv(class_map_csv_text):
    ...
    return [line.split(',')[0] for line in class_map_csv_text.splitlines()]
...
def predict_yamnet(file_path):
    ...
    inferred_class_idx = scores_np.mean(axis=0).argmax()

    # Load class names
    class_map_csv_bytes = tf.io.read_file(yamnet_model.class_map_path())
    class_names = class_names_from_csv(class_map_csv_bytes.numpy().decode('utf-8'))
