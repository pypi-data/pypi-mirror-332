...
class API:
  ...
  def init(endpoint='production'):
    copr = API()
    copr._initialize(endpoint)
    return copr

  ## INITIALIZE

  def _initialize(self, endpoint):
    ...
    # load scheme
    with open('copr/scheme.json', 'r') as file:
  ...
  def __query(self, query, _d=None, **kwargs):
    ...
    def _applyVocabulary(x, vocabulary):
      return objectForId(vocabulary, x) or next((y for value in vocabulary if 'children' in value and (y := _applyVocabulary(x, value['children'])) != None), None)
    ...
    def packIntoObject(x):
      ...
      if isinstance(name, dict):
        if x['class'] not in name:
          return x
        name = name[x['class']]
      return self.__classForName(name)(x, self._info)
  ...
  def __validParametersForQuery(self, query):
    ...
    if 'parameter' in query:
      ...
      for q in query['queries']:
        parameters += self.__validParametersForQuery(q)
    return parameters
  def __normalizeQuery(self, query, extend={}):
  ...
  def __elements(self, elementType, **kwargs):
    return self.__query(self.__normalizeQuery(elementType['query'], extend={'class': elementType['name']}), **kwargs)
  def __baseFunction(self, resultDescription):
    if resultDescription['resultType'] == 'dict':
      ...
      for key, query in resultDescription['query'].items():
        result[key] = self.__query(query)
      return result
    elif resultDescription['resultType'] == 'string':
      return self.__query(resultDescription['query'])
...
class COPRElement:
  ...
  def __getParameter(self, name):
    ...
    return self.__query(query, _d=self._d)
