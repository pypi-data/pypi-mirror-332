...
def prepare_unwrap(strat: "StrategyTutorialUniswapSwap") -> Optional[ActionBundle]:
    """
    ...
    WETH_ADDRESS = get_address_by_chain_and_network(
        chain=strat.chain, network=strat.network, contract_name="WETH"
    )
...
def validate_unwrap(strat: "StrategyTutorialUniswapSwap") -> bool:
    ...
    if len(unwrap_actions) != 1:
        raise ValueError(
            f"Validation failed (Unwrap): Expected 1 unwrap action, received: {len(unwrap_actions)}"
        )
