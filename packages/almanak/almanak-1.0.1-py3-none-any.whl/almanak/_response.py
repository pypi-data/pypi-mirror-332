...
from ._exceptions import AlmanakError, APIResponseValidationError
    ...
    def _parse(
        ...
        *,
        ...
        if (
            ...
            raise TypeError(
                "Pydantic models must subclass our base model type, e.g. `from almanak import BaseModel`"
            )
        ...
        data = response.json()
        if isinstance(unpack_by_keys, str):
            unpack_by_keys = [unpack_by_keys]
        for unpack_by_key in unpack_by_keys:
            if unpack_by_key in data:
                data = data[unpack_by_key]
        if page_key:
            if page_key in data:
                data["data"] = data.pop(page_key)
                data["object"] = "list"
...
class APIResponse(BaseAPIResponse[R]):
    ...
    def parse(
        ...
        *,
        ...
        parsed = self._parse(to=to, unpack_by_keys=unpack_by_keys, page_key=page_key)
...
class AsyncAPIResponse(BaseAPIResponse[R]):
    ...
    async def parse(
        self, *, to: type[_T] | None = None, unpack_by_keys: str | List[str] = "data"
    ) -> R | _T:
        """Returns the rich python representation of this response's data.
        ...
        parsed = self._parse(to=to, unpack_by_keys=unpack_by_keys)
        if is_given(self._options.post_parser):
            parsed = self._options.post_parser(parsed)
...
class MissingStreamClassError(TypeError):
    def __init__(self) -> None:
        super().__init__(
            "The `stream` argument was set to `True` but the `stream_cls` argument was not given. See `almanak._streaming` for reference",
        )


class StreamAlreadyConsumed(AlmanakError):
    """
