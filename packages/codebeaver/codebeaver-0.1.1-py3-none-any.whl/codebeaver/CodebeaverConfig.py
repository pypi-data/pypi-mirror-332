...
class UnitTestConfig:
  ...
  def __init__(self, **kwargs):
    ...
    if kwargs.get('from'):
      ...
      template_name = kwargs['from']  # Use direct dictionary access instead of get()
...
class CodeBeaverConfig:
  ...
  def __init__(self, **kwargs):
    ...
    if hasattr(self, 'unit') and self.unit is not None and isinstance(self.unit, dict):
    ...
    if template_name and hasattr(self, 'unit'):
      ...
      if template_config.max_files_to_test is not None and self.unit.max_files_to_test is None:
        self.unit.max_files_to_test = template_config.max_files_to_test
      
      if template_config.single_file_test_commands is not None and self.unit.single_file_test_commands is None:
        self.unit.single_file_test_commands = template_config.single_file_test_commands
      
      if template_config.setup_commands is not None and self.unit.setup_commands is None:
        self.unit.setup_commands = template_config.setup_commands
      
      if template_config.test_commands is not None and self.unit.test_commands is None:
      ...
      if template_config.ignore is not None and self.unit.ignore is None:
        self.unit.ignore = template_config.ignore

  @staticmethod
  def from_yaml(yaml_content: dict, workspace_name: str | None = None) -> "CodeBeaverConfig":
  ...
  def template_dir() -> Path:
    ...
    if template_dir.exists():
      return template_dir
    raise ValueError("Templates directory not found. Please ensure CodeBeaver is installed correctly.")
  ...
  def parse_template(template_name: str) -> UnitTestConfig:
    ...
    try:
        ...
        if not isinstance(parsed_file, dict):
            logger.error(f"Invalid template format in {template_path} - expected a dictionary")
            sys.exit(1)
        ...
        logger.error(f"Unexpected error reading template at {template_path}: {str(e)}")
        sys.exit(1)
