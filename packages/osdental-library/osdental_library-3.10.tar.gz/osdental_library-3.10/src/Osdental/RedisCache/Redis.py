...
class RedisCacheAsync:
    ...
    def __init__(self, redis_url:str, db:int = 1):
    ...
    async def close(self):
        ...
        if self.client:
            await self.client.close()  # Cierra el cliente de Redis
        if self.pool:
            await self.pool.disconnect(inuse_connections=True)  # Desconecta el pool, incluso conexiones en uso
