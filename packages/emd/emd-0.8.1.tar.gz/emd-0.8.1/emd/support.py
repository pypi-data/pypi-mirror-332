...
def create_readthedocs_requirements():
    import tomllib
    with open(pathlib.Path(get_install_dir()).parent / 'pyproject.toml', "rb") as f:
        data = tomllib.load(f)

    with open(pathlib.Path(get_install_dir()).parent / 'requirements_rtd.txt', 'w') as f:
        for dep in data['project']['dependencies']:
            f.write(dep + '\n')
        for opt in data['project']['optional-dependencies']:
            if opt == 'full':
                continue
            for dep in data['project']['optional-dependencies'][opt]:
                f.write(dep + '\n')
        f.write('-e .\n')
...
def ensure_equal_dims(to_check, names, func_name, dim=None):
    ...
    if np.all(check) == False:  # noqa: E712
