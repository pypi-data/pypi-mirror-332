...
class Client:
    ...
    def save_autolog_cells(self, iteration_id: str, cells: list[dict[str, str]], prefix: str | None = None):
        return IterationApi(self._gql_client, self.auth).save_autolog_cells(iteration_id, cells, prefix)
    ...
    def register_dataset_from_source(
        ...
        dataset_register_input = DatasetRegisterInput(
            ...
            datasetInputs=[s for s in derived_from if s.startswith("DTV-")],
            modelInputs=[s for s in derived_from if s.startswith("MDV-")],
