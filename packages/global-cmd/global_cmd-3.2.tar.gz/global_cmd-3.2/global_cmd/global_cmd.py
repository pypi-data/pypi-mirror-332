...
def call_bedrock_api(image_path, party_name):
   ...
   - Make sure that the date is always in the format DD.MM.YYYY, and year is always in 4 digit format."""
        ...
        "4. 'Supplier Name' - the supplier's name, which must never be 'M S VIMALSON JEWELLERS' if u encounter this name dont consider it to be the name of the supplier and look for other supplier names usually they will be after Party: in the case of encountering 'M S VIMALSON JEWELLERS' .\n\n"
       "5. 'Supplier Name' - The supplier's name must never be '" + party_name + "'. "
        "If you encounter this name, do not consider it as the supplier. "
        "Instead, look for another supplier name, which is usually found after 'Party:' or similar keywords just not the same as party name ie not something u have already discarded. "
        "Ensure the extracted supplier name is different from '" + party_name + "' and is a valid entity."
