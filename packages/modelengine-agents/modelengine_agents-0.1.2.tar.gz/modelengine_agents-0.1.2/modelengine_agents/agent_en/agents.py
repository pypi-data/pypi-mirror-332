...
{str(additional_args)}."""
    ...
    def save(self, output_dir: str, relative_path: Optional[str] = None):
        ...
        app_template = textwrap.dedent("""
            import yaml
            import os
            from modelengine_agents.agent_en import GradioUI, {{ class_name }}, {{ agent_dict['model']['class'] }}
    ...
    def from_folder(cls, folder: Union[str, Path], **kwargs):
        ...
        for managed_agent_name, managed_agent_class in agent_dict["managed_agents"].items():
            agent_cls = getattr(importlib.import_module("modelengine_agents.agent_en.agents"), managed_agent_class)
        ...
        model_class: Model = getattr(importlib.import_module("modelengine_agents.agent_en.models"), agent_dict["model"]["class"])
    ...
    def push_to_hub(
        ...
        metadata_update(
            repo_id,
            {"tags": ["modelengine_agents.agent_en", "agent"]},
            repo_type="space",
            token=token,
            overwrite=True,
        )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        prompt_templates = prompt_templates or yaml.safe_load(
            importlib.resources.files("modelengine_agents.agent_en.prompts").joinpath("toolcalling_agent.yaml").read_text()
        )
        super().__init__(
            tools=tools,
            model=model,
...
class CodeAgent(MultiStepAgent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        prompt_templates = prompt_templates or yaml.safe_load(
            importlib.resources.files("modelengine_agents.agent_en.prompts").joinpath("code_agent.yaml").read_text()
        )
        super().__init__(
            tools=tools,
            model=model,
