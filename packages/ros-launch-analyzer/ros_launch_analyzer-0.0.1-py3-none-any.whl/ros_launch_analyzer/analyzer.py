...
class LaunchAnalyzer:
    ...
    def _get_cluster_id(self, launch_file: str) -> str:
        ...
        if launch_file not in self.cluster_id_cache:
            ...
            print(f"   🔑 新規クラスタID生成: {os.path.basename(launch_file)} -> {self.cluster_id_cache[launch_file]}")
        else:
            print(f"   💾 キャッシュからクラスタID取得: {os.path.basename(launch_file)} -> {self.cluster_id_cache[launch_file]}")
        ...
        try:
            ...
            for i, include in enumerate(includes, 1):
                ...
                if file_path is None:
                    print(f"      ❌ file属性が見つかりません")
                    continue

                print(f"      📝 file属性の値: '{file_path}'")

                if not file_path:
                    print(f"      ⚠️ fileパスが空です")
                    continue
