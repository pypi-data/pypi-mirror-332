...
if TYPE_CHECKING:
    from databricks.rag_eval.clients.managedevals import ManagedEvalsClient
from databricks.sdk import WorkspaceClient


def _get_client() -> "ManagedEvalsClient":
    from databricks.rag_eval import context
    from databricks.rag_eval.clients.managedevals import (
...
class LabelingSession:
    ...
    def add_dataset(
        ...
        if record_ids:
            assert isinstance(record_ids, list), "`record_ids` must be a list"
            assert all(
    ...
    def add_traces(
        ...
        traces = [
            mlflow.entities.Trace.from_json(t) if isinstance(t, str) else t
            for t in traces
        ]
        ...
        for trace in traces:
            ...
            trace_ids.append(trace.info.request_id)
