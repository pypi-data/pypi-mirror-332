    ...
    def _handle_error(self, error: Exception, resource_type: str) -> None:
        ...
        if isinstance(error, ClientError):
            if error.response["Error"]["Code"] == "AccessDeniedException":
                ...
                logger.error(f"Error collecting {resource_type} in {self.region}: {error}", exc_info=True)
        else:
            logger.error(f"Unexpected error collecting {resource_type} in {self.region}: {error}", exc_info=True)
