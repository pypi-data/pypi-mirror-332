...
class GeneExpressionReordererOptimized:
    """
    Class for optimizing gene expression in spatial transcriptomics data using the alignment-based approach.
    The optimization is performed by iteratively swapping gene expression values between cells in a spatially coherent manner.
    The optimization can be performed on a single cluster of cells or on the entire dataset.

    args:
        adata1: AnnData object with gene expression data for the first dataset
        adata2: AnnData object with gene expression data for the second dataset
        adata3: AnnData object with gene expression data for the spatial transcriptomics dataset
        pi_13: Transport plan from the first dataset to the spatial transcriptomics dataset
        pi_23: Transport plan from the second dataset to the spatial transcriptomics dataset
        n_top_genes: Number of top genes to consider for optimization
        spatial_sigma: Sigma parameter for the spatial consistency term
        use_spatial_consistency: Whether to use the spatial consistency term
        spatial_weight: Weight of the spatial consistency term
        n_jobs: Number of parallel jobs to use for optimization
        k_neighbors: Number of neighbors to consider for spatial consistency term
