...
@pytest.mark.parametrize("directory", [activitytype_path, flowobject_path])
def test_files(directory):
    test_pass = True

    # Check bonsai tree file
    df_tree = assert_columns(directory / "tree_bonsai.csv", ["prefixed_id", "level"])
    if df_tree["prefixed_id"].isna().any():
        logger.warning(f"Empty 'prefixed_id' in {directory.name}/tree_bonsai.csv")
    level_0 = len(df_tree[df_tree.level == "0"])
    if level_0 > 6:
        logger.warning(
            f"{level_0} items have level 0 in {directory.name}/tree_bonsai.csv"
        )
    # Check if the auto-filled columns are filled
    for col in ["prefixed_id", "level"]:
        if df_tree[col].isna().any():
            logger.warning(f"Empty '{col}' in {directory.name}/tree_bonsai.csv")
            # test_pass = False
    ...
    for conc_file in conc_files:
        df_conc = assert_columns(
            conc_file,
            ["classification_from", "classification_to", "prefixed_id", "comment"],
        )
        # Check if the auto-filled columns are filled
        for col in ["prefixed_id", "comment"]:
...
@pytest.mark.parametrize("directory", [flow_path])
