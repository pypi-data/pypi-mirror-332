...
class CloneRetriever(WorkflowRule):
    ...
    def process_repo(
        ...
        if retrieved_repo.latest is not None:
            ...
            if clone_dir is not None:
                ...
                if self.storage_dir is not None:
                    self.store_clone(retrieved_repo.latest, self.storage_dir, clone_dir)
    ...
    def store_clone(
        self, latest_version: LatestVersion, storage_dir: str, clone_dir: Path
    ) -> None:
        """
        Compresses the clone directory and stores it in the given storage_dir.

        :param latest_version: The latest version for which to store the cloned dir
        ...
        m = hashlib.md5()
        m.update(clone_dir.name.encode("utf-8"))
        hsh = m.hexdigest()
        ...
        file_path = storage_dir / (hsh + ".tar.gz")
