...
class Credentials(ABC):
    ...
    class Carta:
        ...
        def read(self, path: file_t) -> "Credentials":
            ...
            try:
                # response = self.agent.get(f"files/Carta/file/{first.id}")
                # response.raise_for_status()
                # logger.info(f"Download success: {path}")
                # self.scope._content = response.content
        ...
        def write(self, path: file_t | None=None, *, overwrite: bool=False) -> "Credentials":
            ...
            kwargs = {
                ...
                "files": {"file": (str(path), self.scope._content, "application/octet-stream")},
            }
            try:
                first, *others = self.find(path)
                if len(others):
                ...
                kwargs["params"] = {"path": "/"}  # str(path.parent)}
                url = "files/Carta"
...
class TLSCredentials(ZippedCredentials):
    def __init__(self,
        ...
        --------
            ...
            credentials.folder.write("destdir/aimpf_local.zip")  # Writes credentials to a folder.
    ...
    def ca(self, path: str | Path) -> None:
        try:
            ...
            logger.info(f"Certificate Authority file: {path!s}")
            self._ca = str(path)
            if self.has_certs():
                self._update()
    ...
    def cert(self, path: str | Path) -> None:
        try:
            ...
            # logger.error(str(e))
            raise
        logger.info(f"Certificate file: {path!s}")
        self._cert = str(path)
        if self.has_certs():
            self._update()
    ...
    def authenticate(self, client: SyncClient | AsyncClient) -> SyncClient | AsyncClient:
        ...
        if isinstance(client, SyncClient):
            if self.has_certs() or self.ca:
                # Use credentials set manually
                client.tls_set(
                    ca_certs=self.ca,
                    certfile=self.cert,
                    keyfile=self.key,
                ...
                with self as entry:
                    try:
                        self.identify_certificates(entry.name, recursive=True)
                        client.tls_set(
                            ca_certs=self.ca,
                            certfile=self.cert,
                            keyfile=self.key,
            ...
            # aiomqtt.Client wraps a paho.mqtt.client.Client.
            # Authorize the wrapped client.
    ...
    def identify_certificates(self, folder: str | Path, *, password: str | None=None, recursive: bool=False) -> "TLSCredentials":
        ...
        for filename in folder.iterdir():
            ...
            # All files found
            if self.has_certs():
                logger.info(f"Found all certificate files in {folder!s}")
                self._update()
                break
        else:
