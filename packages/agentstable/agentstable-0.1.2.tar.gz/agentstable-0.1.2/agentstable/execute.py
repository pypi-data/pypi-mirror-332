...
def execute_openai(
    ...
    # Extract usage information from the response if available
    if hasattr(openai_response, "usage"):
        ...
        usage_tracker.add_record(
            ...
            flow_id=flow_response.get("flow_id"),
            metadata={"response_type": "execute"}
        )
    # Handle dictionary-style responses
    elif isinstance(openai_response, dict) and "usage" in openai_response:
        usage = openai_response["usage"]
...
def execute_anthropic(
    ...
    # Extract usage information from the response if available
    # Handle Anthropic object response
    if hasattr(anthropic_response, "usage"):
        usage = anthropic_response.usage
        model = getattr(anthropic_response, "model", "unknown")
