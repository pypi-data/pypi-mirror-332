...
class ChainClient(BaseClient):
    ...
    def _decode_error(self, error: Web3Exception) -> str:
        abi_errors = {
    ...
    def add_gas_limit(self, tx: TxParams) -> TxParams:
        ...
        try:
            ...
            self.logger.error(f"Failed to add gas limit: {self._decode_error(e)}")
            raise e
    ...
    def deposit_usde(
        ...
        try:
            ...
            self.logger.error(
                f"Failed to prepare deposit transaction: {self._decode_error(e)}"
            )
            raise e
