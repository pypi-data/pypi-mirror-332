...
def get_args():
    ...
    # init hf-command parser
    parser_init = subparsers.add_parser('init-hf', help='Initialize HF', parents=[root_parser])
    parser_init.add_argument(
        'project_name',
        help='Path to directory where project state will be initialized')
    parser_init.add_argument(
    ...
    parser_deploy.add_argument(
        ...
        help='GCP region')
    parser_deploy.add_argument(
        '-host', dest='host', default='https://app.aixblock.io',
        help='AIxBlock hostname')
    parser_deploy.add_argument(
        '-api-key', dest='api_key', required=True,
        help='AIxBlock API key')
...
def create_dir_hf(args):
    ...
    default_configs_dir = os.path.join(os.path.dirname(__file__), 'huggingface_configs')
    shutil.copytree(default_configs_dir, output_dir, ignore=shutil.ignore_patterns('*.tmpl'))
...
def deploy_to_gcp(args):
    ...
    subprocess.check_output(' '.join([
        ...
        "--source", output_dir,
        "--region", region,
        "--update-env-vars", f"ML_BACKEND_V2=1,HOSTNAME={args.host},API_KEY={args.api_key}"
