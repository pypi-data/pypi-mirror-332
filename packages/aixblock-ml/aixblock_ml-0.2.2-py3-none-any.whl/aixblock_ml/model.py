...
AIxBlockML_BACKEND_V2_DEFAULT = False
...
class AIxBlockMLManager(object):
    ...
    def has_active_model(cls, project):
        if not os.getenv('AIXBLOCK_ML_BACKEND_V2', default=AIxBlockML_BACKEND_V2_DEFAULT):
    ...
    def fetch(cls,  **kwargs):
        ...
        if not cls._current_model or model_version != cls._current_model.model_version:
            ...
            model_version = kwargs.get('model_version')
            model = cls.model_class( **kwargs)
            cls._current_model = ModelWrapper(model=model, model_version=model_version)
        return cls._current_model
    ...
    def action(
        ...
        m = cls.fetch(**kwargs)
        if not cls._current_model:
            raise ValueError(f'Model is not loaded for {cls.__class__.__name__}: run setup() before using action()')
        predictions = m.model.action(command, **kwargs)
        return predictions
    # @classmethod
    ...
    def model(cls, **kwargs):
        if not os.getenv('AIXBLOCK_ML_BACKEND_V2', default=AIxBlockML_BACKEND_V2_DEFAULT):
            ...
            if not m:
                raise FileNotFoundError('No model loaded. Specify "try_fetch=True" option.')
            predictions = m.model.model(**kwargs)
            return predictions
...
def get_all_classes_inherited_AIxBlockMLBase(script_file):
    ...
    try:
        ...
        print(Fore.RED + 'Can\'t import module "' + module_name + f'", reason: {e}.\n'
              ...
              Fore.LIGHTYELLOW_EX + 'https://app.aixblock.io/tutorials/dummy_model.html')
        module = None
        exit(-1)
