...
def start_vscode_service(
    ...
    if "WORKSPACE_API_KEY" in os.environ:
        ...
        raise RuntimeError("WORKSPACE_API_KEY is not set")
...
async def watch_heartbeat(child_process: Process, start_time: float, ttl_seconds: int, logger):
    if hasattr(child_process, "is_alive"):
        aliveness_check = child_process.is_alive
    else:
        aliveness_check = child_process.is_running

    while aliveness_check():
...
async def monitor_vscode(
    ...
    async for stop_event in merge(
        loop,
        remote.watch_workspace_instance(workspace_instance_name),
        watch_heartbeat(child_process, start_time, ttl_seconds, logger),
    ):
        if stop_event:
