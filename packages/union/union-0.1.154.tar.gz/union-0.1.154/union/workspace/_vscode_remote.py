...
from union.internal.workspace.workspace_instance_pb2 import WorkspaceInstanceIdentifier
...
class WorkspaceRemote:
    ...
    async def watch_workspace_instance(
        ...
        async for response in self.instance_async_client.WatchWorkspaceInstances(watch_request):
            logger.info(f"WatchWorkspaceInstances response {response}")
            yield response.stopping_event is not None
