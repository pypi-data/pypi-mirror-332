...
def create_hf_model_cache_workflow(
    ...
    default_resource = union.Resources(mem="2Gi", cpu="2")
    if requests is None:
        requests = default_resource
    if limits is None:
        limits = default_resource

    validate_repo_task = union.task(
        container_image=image,
        requests=requests,
        limits=limits,
        secret_requests=[Secret(key=hf_token_key)],
    ...
    cache_mode_task = union.task(
        ...
        limits=limits,
        secret_requests=[
            Secret(key=hf_token_key),
            Secret(key=union_api_key, env_var="UNION_API_KEY"),
        ],
    ...
    return imperative_wf, os.path.dirname(union.__path__[0])
