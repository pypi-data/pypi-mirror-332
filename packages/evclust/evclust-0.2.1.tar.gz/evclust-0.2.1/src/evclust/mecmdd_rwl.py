...
#---------------------- MECMdd-RWL------------------------------------------------
def mecmdd_rwl(Xlist, c, type='full', alpha=1, beta=1.5, delta=9, epsi=1e-4, 
    ...
    lambda_ = lambdaInit_local(weight, pmatrix, nF-1)
    ...
    while pasfini:
        ...
        for j in range(nF-1):
            ...
            for i in range(n):
                num = (card[j]**alpha * np.sum([D[l][i, j] * (lambda_[j, l])**s for l in range(pmatrix)]))**(-1/(beta-1))
                den = (np.sum([(card[k]**alpha * np.sum([D[l][i, k] * (lambda_[k, l])**s for l in range(pmatrix)]))**(-1/(beta-1)) for k in range(nF-1)]) + delta2**(-1/(beta-1)))
                m[i, j] = num / den
        ...
        if weight == 'sum':
            for j in range(nF-1):
                for l in range(pmatrix):
                    num = np.sum([s * (card[j]**alpha) * m[i, j]**beta * D[l][i, j] for i in range(n)])**(-1/(s-1))
                    den = np.sum([np.sum([s * (card[j]**alpha) * m[i, j]**beta * D[h][i, j] for i in range(n)])**(-1/(s-1)) for h in range(pmatrix)])
                    lambda_[j, l] = num / den

        if weight == 'prod':
            for j in range(nF-1):
                for l in range(pmatrix):
                    num = np.prod([np.sum([(card[j]**alpha) * m[i, j]**beta * D[h][i, j] for i in range(n)])**(1/pmatrix) for h in range(pmatrix)], axis=0)
                    den = np.sum([(card[j]**alpha) * m[i, j]**beta * D[l][i, j] for i in range(n)])
                    lambda_[j, l] = num / den
        ...
        for k in range(nF-1):
            ...
            if np.sum(fk) == 1:
                mk_beta = m[:, k]**beta
                for i in range(n):
                    V[i, k] = np.sum([np.sum([Xlist[l][i, :] * (lambda_[k, l])**s for l in range(pmatrix)]) * mk_beta[i]])
        ...
        J = np.sum([np.sum([card[j]**alpha * m[i, j]**beta * np.sum([Xlist[l][i, prototype[j]] * (lambda_[j, l])**s for l in range(pmatrix)]) for j in range(nF-1)]) for i in range(n)]) + delta2 * np.sum(mvide**beta)
    ...
    clus = extractMass(m, F, g=g, method="mecmdd", crit=J, 
                       param={'alpha': alpha, 'beta': beta, 'delta': delta, 'lambda': lambda_})
    return clus



#---------------------- Utils of MECMdd-RWL------------------------------------------------
def lambdaInit_local(weight, p, c):
    """
    Initialize lambda matrix.
    """
    lambda_ = np.zeros((c, p))
    for j in range(c):
        if weight == 'sum':
            lambda_[j, :] = np.full(p, 1/p)
        elif weight == 'prod':
            lambda_[j, :] = np.ones(p)
        else:
            raise ValueError("Weight must be 'sum' or 'prod'.")
    return lambda_
