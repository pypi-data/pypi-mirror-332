...
class CryptSM2(object):
    ...
    def __init__(self, private_key=None):
        ...
        logger.debug(f"SM2对象的公钥:{self.public_key}")

    def private_key_export_pubkey(self, private_key=None):
    ...
    def _convert_jacb_to_nor(self, point):  # Jacobian加重射影坐标转换成仿射坐标
        ...
        z_inv_squar = (z_inv * z_inv) % int(self.ecc_table['p'], base=16)
        z_inv_qube = (z_inv_squar * z_inv) % int(self.ecc_table['p'], base=16)
        x_new = (x * z_inv_squar) % int(self.ecc_table['p'], base=16)
        y_new = (y * z_inv_qube) % int(self.ecc_table['p'], base=16)
