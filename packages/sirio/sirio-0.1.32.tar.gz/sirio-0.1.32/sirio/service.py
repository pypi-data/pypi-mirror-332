...
class SirioService():
    ...
    def __init__(self, 
        ...
        if host_sirio_ep is not None:
            ...
            self._signal = '{host}/sirio/enginebackend/signals'
        else:
    ...
    def signal(self, domain: str, processId: str, businessKey: str, processInstanceId:str, signalName: str):
        ...
        try:
            ...
            responseAbort = requests.put(self._signal, json==data, headers=headers)
            if responseAbort.status_code == 200:
                idSync = responseAbort.text
            else:
                print('Problemi nella generazione del signal: status_code: {}'.format(str(responseAbort.status_code)))
        except Exception as ex:
