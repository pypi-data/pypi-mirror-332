...
const std::string Model_tentusscher2006epi_info = R"model(
tentusscher2006epi:
  name: Ten Tusscher et al. 2004 (Epi)
  description: ''
  dois:
  ...
  key: tentusscher2006epi
)model";
#endif

static const Size Model_tentusscher2006epi_id = UNIQUE_ID;
static const Size Model_tentusscher2006epi_Nv = 19;
static const Size Model_tentusscher2006epi_Np = 53;

#ifdef __OPENCL_VERSION__
void Model_tentusscher2006epi_step(
        Real* const params,
  ...
  *_new_r = r + dt*((transient_outward_current_r_gate_r_inf - r) / transient_outward_current_r_gate_tau_r);
  ...
  const Real transient_outward_current_s_gate_s_inf = 1.0 / (1.0 + exp((V + 20.0) / 5.0));
  const Real transient_outward_current_s_gate_tau_s = 85.0 * exp(-pow(V + 45.0, 2.0) / 320.0) + 5.0 / (1.0 + exp((V - 20.0) / 5.0)) + 3.0;
...
#ifdef __OPENCL_VERSION__
__kernel void Model_tentusscher2006epi_kernel(
        Size model_count,
  ...
  if (ix < states_old.Nx && iy < states_old.Ny && iz < states_old.Nz) {
    ...
    if (inhom_zyx > 0) {
      ...
      if (model_id == Model_tentusscher2006epi_id) {
        ...
        Model_tentusscher2006epi_step(params, w, u, u_, dt);
      }
    }
  }
}
