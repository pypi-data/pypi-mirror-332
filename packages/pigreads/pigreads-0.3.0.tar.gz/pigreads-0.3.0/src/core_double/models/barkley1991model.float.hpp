...
const std::string Model_barkley1991model_info = R"model(
barkley1991model:
  name: Barkley 1991
  description: A model for fast computer simulation of waves in excitable media
  dois:
...
static const Size Model_barkley1991model_id = UNIQUE_ID;
static const Size Model_barkley1991model_Nv = 2;
static const Size Model_barkley1991model_Np = 4;

#ifdef __OPENCL_VERSION__
void Model_barkley1991model_step(
        Real* const params,
  ...
  const Real a = params[1];
  const Real b = params[2];
...
#ifdef __OPENCL_VERSION__
__kernel void Model_barkley1991model_kernel(
        Size model_count,
  ...
  if (ix < states_old.Nx && iy < states_old.Ny && iz < states_old.Nz) {
    ...
    if (inhom_zyx > 0) {
      ...
      if (model_id == Model_barkley1991model_id) {
        ...
        Model_barkley1991model_step(params, w, u, u_, dt);
      }
    }
  }
}
