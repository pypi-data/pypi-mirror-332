...
void Model_paci2013atrial_step(
  ...
  const Real i_CaL_d_gate_d_infinity = 1.0f / (1.0f + native_exp(-(Vm * 1000.0f + 5.986f) / 7.0f));
  ...
  *_new_d = d + dt*((i_CaL_d_gate_d_infinity - d) / i_CaL_d_gate_tau_d);
  ...
  // i_CaL_f1_gate
  const Real i_CaL_f1_gate_f1_inf = 1.0f / (1.0f + native_exp((Vm * 1000.0f + 25.226f) / 3.0f));
  ...
  *_new_f1 = f1 + dt*((i_CaL_f1_gate_f1_inf - f1) / i_CaL_f1_gate_tau_f1);
  ...
  // i_CaL_f2_gate
  const Real i_CaL_f2_gate_f2_inf = 0.33f + 0.67f / (1.0f + native_exp((Vm * 1000.0f + 31.226f) / 4.0f));
  ...
  *_new_Xr1 = Xr1 + dt*((i_Kr_Xr1_gate_Xr1_inf - Xr1) / i_Kr_Xr1_gate_tau_Xr1);
  ...
  // i_Kr
  const Real i_Kr_i_Kr = i_Kr_g_Kr * ((Vm - electric_potentials_E_K) * (Xr1 * (Xr2 * native_sqrt(model_parameters_Ko / 5.4f))));

  // i_Ks
  const Real i_Ks_i_Ks = i_Ks_g_Ks * ((Vm - electric_potentials_E_Ks) * (pow(Xs, 2.0f) * (1.0f + 0.6f / (1.0f + pow(3.8f * 1e-05f / Cai, 1.4f)))));

  // i_Na
  const Real i_Na_i_Na = i_Na_g_Na * (pow(m, 3.0f) * (h * (j * (Vm - electric_potentials_E_Na))));
  ...
  *_new_Vm = Vm + dt*(-(i_K1_i_K1 + (i_to_i_to + (i_Kr_i_Kr + (i_Ks_i_Ks + (i_CaL_i_CaL + (i_NaK_i_NaK + (i_Na_i_Na + (i_NaCa_i_NaCa + (i_PCa_i_PCa + (i_f_i_f + (i_b_Na_i_b_Na + i_b_Ca_i_b_Ca))))))))))) + _diffuse_Vm);
  ...
  const Real calcium_dynamics_i_rel = (calcium_dynamics_c_rel + calcium_dynamics_a_rel * pow(Ca_SR, 2.0f) / (pow(calcium_dynamics_b_rel, 2.0f) + pow(Ca_SR, 2.0f))) * (d * (g * 0.0556f));
