...
void Model_paci2013ventricular_step(
  ...
  const Real i_CaL_d_gate_d_infinity = 1.0 / (1.0 + exp(-(Vm * 1000.0 + 9.1) / 7.0));
  ...
  *_new_d = d + dt*((i_CaL_d_gate_d_infinity - d) / i_CaL_d_gate_tau_d);
  ...
  // i_CaL_f1_gate
  const Real i_CaL_f1_gate_f1_inf = 1.0 / (1.0 + exp((Vm * 1000.0 + 26.0) / 3.0));
  ...
  *_new_f1 = f1 + dt*((i_CaL_f1_gate_f1_inf - f1) / i_CaL_f1_gate_tau_f1);
  ...
  // i_CaL_f2_gate
  const Real i_CaL_f2_gate_f2_inf = 0.33 + 0.67 / (1.0 + exp((Vm * 1000.0 + 35.0) / 4.0));
  ...
  *_new_Vm = Vm + dt*(-(i_K1_i_K1 + (i_to_i_to + (i_Kr_i_Kr + (i_Ks_i_Ks + (i_CaL_i_CaL + (i_NaK_i_NaK + (i_Na_i_Na + (i_NaCa_i_NaCa + (i_PCa_i_PCa + (i_f_i_f + (i_b_Na_i_b_Na + i_b_Ca_i_b_Ca)))))))))) - stim_mode_i_stim) + _diffuse_Vm);
  ...
  const Real calcium_dynamics_i_rel = (calcium_dynamics_c_rel + calcium_dynamics_a_rel * pow(Ca_SR, 2.0) / (pow(calcium_dynamics_b_rel, 2.0) + pow(Ca_SR, 2.0))) * (d * (g * 0.0411));
