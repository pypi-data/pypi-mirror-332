...
class Test_Misc:
    ...
    def test_run_memmap(self):
        with NamedTemporaryFile(suffix=".npy") as f:
            np.save(f, np.ones(shape=(0,)))
            f.seek(0)
            Simulation(**data_minimal).run(path=f.name)
...
class Test_Model:
    ...
    def test_check_empty(self):
        options: list[Any] = [
            ...
            {"aliev1996simple": {}},
            {"name": "aliev1996simple"},
            {"name": "aliev1996simple", "parameters": {}},
        ]
    ...
    def test_check_one(self):
        options: list[Any] = [
            {"aliev1996simple": {"k": 3}},
            {"name": "aliev1996simple", "k": 3},
            {"name": "aliev1996simple", "parameters": {"k": 3}},
        ]
        options = [*options, *([o] for o in options)]
        for sim in check_all_sims("models", options):
            sim.prepare_models()
    ...
    def test_check_two(self):
        options: list[Any] = [
            ...
            [{"marcotte2017dynamical": None}, {"aliev1996simple": {"k": 3}}],
            [{"name": "marcotte2017dynamical"}, {"name": "aliev1996simple", "k": 3}],
            [
                {"name": "marcotte2017dynamical"},
                {"name": "aliev1996simple", "parameters": {"k": 3}},
            ],
        ]
        for sim in check_all_sims("models", options):
...
class Test_Diffusivity:
    ...
    def test_file(self):
        with NamedTemporaryFile(suffix=".npy") as f:
            np.save(f, np.ones(shape=(1, 1, 1, 6)))
            f.seek(0)
            options: list[Any] = [
                f.name,
                {"file": f.name},
    ...
    def test_params(self):
        modify_sim(
            "diffusivity",
            {"f": [1, 2, 3], "n": [4, 5, 6], "Df": 1, "Ds": 0.1, "Dn": 0.01},
        ).diffusivity()
...
class Test_Inhom:
    def test_file(self):
        with NamedTemporaryFile(suffix=".npy") as f:
            np.save(f, np.ones(shape=(1, 1, 1)))
            f.seek(0)
            options: list[Any] = [
                f.name,
                {"file": f.name},
                [{"file": f.name}],
                [{"cmd": "file", "file": f.name}],
            ]
...
class Test_Init:
    ...
    def test_file_var(self):
        with NamedTemporaryFile(suffix=".npy") as f:
            np.save(f, np.ones(shape=(1, 1, 1)))
            f.seek(0)
            modify_sim("init", {"u": f.name}).run()

    def test_file(self):
        with NamedTemporaryFile(suffix=".npy") as f:
            np.save(f, np.ones(shape=(1, 1, 1, 2)))
            f.seek(0)
            modify_sim("init", f.name).run()
