...
@click.group()
def main() -> None:
    "Run and manage Pigreads simulations."
...
@click.option("-v", "--variable", type=str, default=None, help="variable name or index")
@click.option(
    "--kwargs",
    type=yaml.safe_load,
    default="{}",
...
def movie(
    ...
    array: np.ndarray[Any, Any] = np.lib.format.open_memmap(result, mode="r")  # type: ignore[no-untyped-call]  # pylint: disable=line-too-long
    ...
    pp.movie(
        ...
        **pp.imshow_defaults(
            ...
            dpi=dpi,
            fps=fps,
            vlabel=list(models[0]["variables"])[iv],
            **kwargs,
        ),
    )
