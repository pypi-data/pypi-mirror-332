...
def run_case(case_name: str, restart: bool=False, background: bool=False):
    ...
    if background:
        ...
        message('case '+ case_name + ' running on the background, stdout saved in "'+ case_name + '.out"', 'INFO')
    else :
        end=''
    try :
...
def plot(alpha: str, numeric: str, restart: str, case_folder='', save_fig=''):
    ...
    if len(cases) > 1:
        ...
        return cases[0]
#▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄
def plot_profile(alpha: str, nodes: list, time: float, restart: str, case_folder='', save_fig=''):

    """ Plots a profile at a particular time for the given variables

    :alpha: [str] type of variable to be plotted
    :nodes: [list] list with the numeric part of the variable
    :time: [float] the time at which you want to plot the profile
    :restart: [str] the restart code name
    :case_folder: [str or list] (optional) case or cases to be plotted
    :save_fig: [str] (optional) write 'pdf' or 'png' if you want to save a fig
    :returns: [ndarray] with the data extracted
    ...
    for i in range(0, len(case_folder)):
        ...
        if relap_scdap():
            ...
            run_strip('dummy_strip.i', restart, case_folder[i])
        cas = read_stripf(case_folder[i])
        time_column = cas['time0'][:]
        closest_index = np.argmin(np.abs(time_column - time))
        closest_time = time_column[closest_index]
        # Print warning if the closest time isn't an exact match
        if closest_time != time:
            message(f"Warning: Exact time {time}s not found. Using closest time {closest_time}s instead.", 'WARNING')
        # Extract variable values for the closest time
        variables = list(cas[closest_index])
        cases.append(variables[1:])
        # Plot the profile
        plt.plot(range(1, len(variables)), variables[1:], marker='o',
                 linestyle='-', label=case_folder[i])
    plt.xlabel('Node')
    if alpha in relap_labels:
       plt.ylabel(relap_labels.get(alpha))
    ...
    plt.tight_layout()
    if save_fig == 'pdf' or save_fig == 'png':
       fig_name = figures_loc() + alpha + '_profile.' + save_fig
       plt.savefig(fig_name, format=save_fig, dpi=300,bbox_inches='tight')
       print(67*'-')
       print("plot has been saved as:", figures_loc() + alpha + '_profile.' + save_fig)
       print(67*'-')
    if show_plot() : plt.show()
    plt.close()
    if len(cases) > 1:
