...
def plot(alpha: str, numeric: str, restart: str, case_folder='', save_fig=''):
    ...
    for item in case_folder:
        if not os.path.isfile('./' + item + '/' + restart + '.r'):
            message('File ./'+ item + '/' + restart + '.r does not exist.', 'ERROR')
        try :
            os.remove('./' + item + '/' + 'stripf')
        except OSError :
            pass
        ...
        what = alpha + numeric
        plt.plot(cas['time0'],cas[what],linewidth=0.75,label=item)
...
def plot_profile(alpha: str, nodes: list, time: float, restart: str, case_folder='', save_fig=''):
    ...
    for item in case_folder:
        ...
        # Print warning if the closest time isn't an exact match
            if closest_time != tim:
                message(f"Warning: Exact time {tim}s not found. Using closest time {closest_time}s instead.", 'WARNING')
