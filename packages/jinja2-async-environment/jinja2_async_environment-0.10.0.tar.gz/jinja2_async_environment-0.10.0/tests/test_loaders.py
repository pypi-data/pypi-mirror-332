...
class TestAsyncFileSystemLoader:
    ...
    def test_init(self) -> None:
        ...
        loader = AsyncFileSystemLoader(
            path, encoding="latin-1", followlinks=True
        )
    ...
    async def test_get_source_found(self, loader: AsyncFileSystemLoader) -> None:
        ...
        with patch.object(AsyncPath, "joinpath", return_value=template_path), \
             patch.object(AsyncPath, "is_file", return_value=AsyncMock(return_value=True)), \
            ...
            # Call the method
            source, filename, uptodate = await loader.get_source(AsyncPath("template.html"))
    ...
    async def test_uptodate(self, loader: AsyncFileSystemLoader) -> None:
        ...
        with patch.object(AsyncFileSystemLoader, "get_source") as mock_get_source:
            ...
            mock_get_source.return_value = ("content", "/path/to/templates/template.html", uptodate_func)
...
class TestAsyncPackageLoader:
    ...
    async def test_get_source_found(self) -> None:
        ...
        with patch("jinja2_async_environment.loaders.import_module") as mock_import:
            ...
            with patch.object(AsyncPath, "is_file", return_value=AsyncMock(return_value=True)), \
                 patch.object(AsyncPath, "open") as mock_open:
...
class TestAsyncFunctionLoader:
    ...
    @pytest.mark.asyncio
    async def test_get_source_found(self, loader: AsyncFunctionLoader, load_func: AsyncMock) -> None:
        """Test get_source when the template is found."""
        source, filename, uptodate = await loader.get_source(AsyncPath("index.html"))
    ...
    @pytest.mark.asyncio
    async def test_get_source_not_found(self, loader: AsyncFunctionLoader, load_func: AsyncMock) -> None:
        """Test get_source when the template is not found."""
        with pytest.raises(TemplateNotFound):
            await loader.get_source(AsyncPath("nonexistent.html"))
...
class TestAsyncChoiceLoader:
    ...
    def loaders(self) -> t.List[AsyncBaseLoader]:
        ...
        loader1.get_source = AsyncMock(side_effect=lambda t: 
            ("<h1>Loader 1</h1>", "/path1/template1.html", None) 
        ...
        loader2.get_source = AsyncMock(side_effect=lambda t: 
            ("<h1>Loader 2</h1>", "/path2/template2.html", None) 
    ...
    @pytest.mark.asyncio
    async def test_get_source_first_loader(self, loader: AsyncChoiceLoader, loaders: t.List[AsyncBaseLoader]) -> None:
        """Test get_source when the template is found in the first loader."""
        source, filename, uptodate = await loader.get_source(AsyncPath("template1.html"))
    ...
    @pytest.mark.asyncio
    async def test_get_source_second_loader(self, loader: AsyncChoiceLoader, loaders: t.List[AsyncBaseLoader]) -> None:
        ...
        loader.loaders[0].get_source = AsyncMock(side_effect=TemplateNotFound("template2.html"))
        loader.loaders[1].get_source = AsyncMock(return_value=(source_value, filename_value, uptodate_value))
        
        # Call the method
        source, filename, uptodate = await loader.get_source(AsyncPath("template2.html"))
    ...
    @pytest.mark.asyncio
    async def test_get_source_not_found(self, loader: AsyncChoiceLoader, loaders: t.List[AsyncBaseLoader]) -> None:
    ...
    @pytest.mark.asyncio
    async def test_list_templates(self, loader: AsyncChoiceLoader, loaders: t.List[AsyncBaseLoader]) -> None:
        """Test list_templates method."""
        templates = await loader.list_templates()
