...
class TextColor:
    ...
    def validate_color(color: str) -> None:
        ...
        """
        if color.startswith("#"):
            if not TextColor._is_valid_hex_color(color):
                raise ValueError("Invalid hex color code. Must be in format '#RRGGBB'")
        elif color.lower() not in TextColor.text_color_dict:
            raise ValueError(f"Invalid color name: {color}")

    @staticmethod
    def get_ansi_code(text_color: str) -> str:
        """Get ANSI escape code for a given text color.

        Args:
            text_color (str): The text color to get the ANSI code for.

        Returns:
            str: ANSI escape code for the color.
        """
        if text_color == "reset":
            return TextColor.text_color_dict["reset"]

        TextColor.validate_color(text_color)

        if text_color.startswith("#"):
            return TextColor._hex_to_ansi(text_color)
        else:
            return TextColor.get_ansi_code(TextColor.text_color_dict[text_color])
