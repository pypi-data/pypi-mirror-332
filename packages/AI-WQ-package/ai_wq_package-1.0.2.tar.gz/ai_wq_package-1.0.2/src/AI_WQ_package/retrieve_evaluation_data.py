...
def retrieve_weekly_obs(date,variable,password,local_destination=None):
    ...
    try:
        weekly_obs = xr.open_dataarray(local_filename).squeeze().drop_dims('bnds').drop_vars('time_bnds',errors='ignore').to_array().squeeze()
    except:
        weekly_obs = xr.open_dataarray(local_filename).squeeze()
    # return the single day climatology.
    try: 
