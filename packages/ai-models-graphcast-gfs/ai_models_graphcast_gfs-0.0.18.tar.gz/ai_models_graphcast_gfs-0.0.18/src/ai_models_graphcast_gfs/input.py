...
def forcing_variables_numpy_onedeg(sample, forcing_variables, dates):
    """Generate variables from earthkit-data
...
def create_training_xarray(
    *,
    ...
    with timer("Creating forcing variables"):
        if onedeg:
            forcing_numpy = forcing_variables_numpy_onedeg(fields_sfc, forcing_variables, all_datetimes)
        else:
