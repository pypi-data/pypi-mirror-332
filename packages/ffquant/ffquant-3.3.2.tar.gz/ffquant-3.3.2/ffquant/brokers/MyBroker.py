                ...
                if response.get('code') == "200":
                    for item in response['results']:
                        ...
                        if item['orderStatus'] == "pending" or item['orderStatus'] == "working":
                            ...
                            # 对于已提交的订单 需要记录创建的size和价格
                            order.created.size = item['qty']
                            order.created.price = item['allocationPrice']
                            if item['openTime'] is not None:
                                order.created.dt = bt.date2num(datetime.fromtimestamp(item['openTime'] / 1000.0))
                            ...
                            if order.executed.size == 0 and order.created.size > 0:
                                order.executed.size = order.created.size
                            order.executed.price = item['executePrice']
                            if item['closeTime'] is not None:
                                order.executed.dt = bt.date2num(datetime.fromtimestamp(item['closeTime'] / 1000.0))
