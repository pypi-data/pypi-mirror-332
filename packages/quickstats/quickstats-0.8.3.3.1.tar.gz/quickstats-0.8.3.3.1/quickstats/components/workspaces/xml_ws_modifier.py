...
class XMLWSModifier(XMLWSBase):
    ...
    def correct_formula_once(self,formulaVar):
        ...
        update_formula = (formula!=formula_bak)
        return formula, update_formula, actual_vars
    ...
    def create_modified_workspace(self, infile:Optional[str]=None, outfile:Optional[str]=None,
                                  import_class_code:bool=True, recreate:bool=True, float_all_nuis:bool=False):
        ...
        with timer() as t:
            ...
            for mc in model.model_configs.values():
            ...
            strict = self.config['strict'] and (len(model.model_configs) == 1)
            for mc in model.model_configs.values():
                mc_final = self.create_model_config(mc, ws_final, actual_rename_map,
                                                    float_all_nuis, strict=strict)
        ...
        for nuis_name in nuis_names:
            ...
            if nuis.isConstant() and nuis not in pois:
                if float_all_nuis:
                    ...
                    self.stdout.warning(f"The nuisace parameter {nuis_name} is initialized as a constant. Careful!")
