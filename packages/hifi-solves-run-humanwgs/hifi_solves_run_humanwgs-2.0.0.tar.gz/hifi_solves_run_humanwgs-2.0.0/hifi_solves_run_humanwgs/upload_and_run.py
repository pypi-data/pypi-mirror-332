...
def upload_files(
    ...
    Returns:
        ...
        total_file_size_bytes (int): Total size of all input bams
    """
    ...
    for sample_id, file_path in sample_info["hifi_reads"].explode().items():
        ...
        file_size_bytes = remote_size_bytes or local_size_bytes or src_remote_size_bytes
        file_info[file_path] = {
...
def _register_or_retrieve_workflow(workflow_name, workflow_version):
    """
...
def _register_workflow_version(workflow_id, workflow_version):
    """
...
def _register_workflow_defaults(
    workflow_id, workflow_version, backend, region, static_inputs
):
    """
    ...
    def _register_defaults(
        ...
        if default_info.returncode == 0:
            print("\t✓ Registered workflow defaults")
        else:
            raise SystemExit(
...
def _register_workflow_transformations(workflow_id, workflow_version):
    """
    ...
    def _register_transformation(
        ...
        if transformation_info.returncode == 0:
            print("\t✓ Registered workflow transformation")
        else:
            raise SystemExit(
...
def configure_workflow(workflow_name, workflow_version, backend, region, static_inputs):
    """
    ...
    """
    workflow_id = _register_or_retrieve_workflow(workflow_name, workflow_version)
    _register_workflow_version(workflow_id, workflow_version)
    _register_workflow_defaults(
        workflow_id, workflow_version, backend, region, static_inputs
    )
    _register_workflow_transformations(workflow_id, workflow_version)
...
def trigger_workflow_run(
    ...
    if len(previous_runs) > 0:
        ...
        if "SUCCEEDED" in run_states:
            status_symbol = "🟢"
        elif "RUNNING" in run_states:
            status_symbol = "🔵"
        ...
        if rerun is False:
            print(f"\t{status_symbol} {family_id}")
            return
        else:
            if "RUNNING" in run_states or "SUCCEEDED" in run_states:
                print(f"\t{status_symbol} {family_id}")
                return
            else:
                print(f"\tForce-rerunning failed family {family_id}")
...
def main():
    ...
    print("\t✓ Successfully configured workflow")

    if args.upload_only:
        print("✓ Successfully uploaded workflow input files")
    else:
        print("Configuring workbench")

        print("Triggering workflow runs")
