...
class ModelOrder(BaseModel):
    ...
    order_number: StrictStr = Field(description="The order number", alias="orderNumber")
    contact: ModelOrderContact
    order_date: datetime = Field(description="Needs to be provided as timestamp or dd.mm.yyyy", alias="orderDate")
    status: StrictInt = Field(description="Please have a look in       <a href='#tag/Order/Types-and-status-of-orders'>status of orders</a>      to see what the different status codes mean")
    header: StrictStr = Field(description="Normally consist of prefix plus the order number")
    ...
    address_country: ModelOrderAddressCountry = Field(alias="addressCountry")
    ...
    version: StrictInt = Field(description="Version of the order.<br>      Can be used if you have multiple drafts for the same order.<br>      Should start with 0")
    ...
    contact_person: ModelOrderContactPerson = Field(alias="contactPerson")
    ...
    tax_set: Optional[ModelOrderTaxSet] = Field(default=None, alias="taxSet")
    tax_text: StrictStr = Field(description="A common tax text would be 'Umsatzsteuer 19%'", alias="taxText")
    tax_type: StrictStr = Field(description="**Use this in sevdesk-Update 1.0 (instead of taxRule).**  Tax type of the order. There are four tax types: 1. default - Umsatzsteuer ausweisen 2. eu - Steuerfreie innergemeinschaftliche Lieferung (Europäische Union) 3. noteu - Steuerschuldnerschaft des Leistungsempfängers (außerhalb EU, z. B. Schweiz) 4. custom - Using custom tax set 5. ss - Not subject to VAT according to §19 1 UStG Tax rates are heavily connected to the tax type used.", alias="taxType")
    ...
    currency: StrictStr = Field(description="Currency used in the order. Needs to be currency code according to ISO-4217")
    ...
    __properties: ClassVar[List[str]] = ["id", "objectName", "mapAll", "create", "update", "orderNumber", "contact", "orderDate", "status", "header", "headText", "footText", "addressCountry", "deliveryTerms", "paymentTerms", "version", "smallSettlement", "contactPerson", "taxRate", "taxRule", "taxSet", "taxText", "taxType", "orderType", "sendDate", "address", "currency", "customerInternalNote", "showNet", "sendType", "origin"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "contact": ModelOrderContact.from_dict(obj["contact"]) if obj.get("contact") is not None else None,
            "orderDate": obj.get("orderDate"),
            "status": obj.get("status"),
            "header": obj.get("header"),
            "headText": obj.get("headText"),
            "footText": obj.get("footText"),
            "addressCountry": ModelOrderAddressCountry.from_dict(obj["addressCountry"]) if obj.get("addressCountry") is not None else None,
            "deliveryTerms": obj.get("deliveryTerms"),
            "paymentTerms": obj.get("paymentTerms"),
            "version": obj.get("version"),
            "smallSettlement": obj.get("smallSettlement"),
            "contactPerson": ModelOrderContactPerson.from_dict(obj["contactPerson"]) if obj.get("contactPerson") is not None else None,
            ...
            "taxSet": ModelOrderTaxSet.from_dict(obj["taxSet"]) if obj.get("taxSet") is not None else None,
