...
class ModelCreditNote(BaseModel):
    """
    creditNote model
    """ # noqa: E501
    id: Optional[StrictInt] = Field(default=None, description="The creditNote id. <span style='color:red'>Required</span> if you want to create/update an credit note position for an existing credit note\"")
    object_name: StrictStr = Field(description="The creditNote object name", alias="objectName")
    ...
    credit_note_number: StrictStr = Field(description="The creditNote number", alias="creditNoteNumber")
    contact: ModelCreditNoteContact
    credit_note_date: datetime = Field(description="Needs to be provided as timestamp or dd.mm.yyyy", alias="creditNoteDate")
    status: StrictStr = Field(description="Please have a look in       <a href='#tag/CreditNote/Status-of-credit-notes'>status of credit note</a>      to see what the different status codes mean")
    header: StrictStr = Field(description="Normally consist of prefix plus the creditNote number")
    ...
    address_country: Optional[ModelCreditNoteAddressCountry] = Field(alias="addressCountry")
    ...
    contact_person: ModelCreditNoteContactPerson = Field(alias="contactPerson")
    ...
    tax_type: StrictStr = Field(description="**Use this in sevdesk-Update 1.0 (instead of taxRule).**  Tax type of the creditNote. There are four tax types: 1. default - Umsatzsteuer ausweisen 2. eu - Steuerfreie innergemeinschaftliche Lieferung (Europäische Union) 3. noteu - Steuerschuldnerschaft des Leistungsempfängers (außerhalb EU, z. B. Schweiz) 4. custom - Using custom tax set 5. ss - Not subject to VAT according to §19 1 UStG Tax rates are heavily connected to the tax type used.", alias="taxType")
    ...
    booking_category: StrictStr = Field(description="Defines the booking category, for more information see the section [Credit note booking categories](#tag/CreditNote/Credit-note-booking-categories)  The booking category of the credit note.  **Must be <code>UNDERACHIEVEMENT</code> in sevdesk-Update 2.0.**", alias="bookingCategory")
    currency: StrictStr = Field(description="Currency used in the creditNote. Needs to be currency code according to ISO-4217")
    ...
    __properties: ClassVar[List[str]] = ["id", "objectName", "mapAll", "create", "update", "creditNoteNumber", "contact", "creditNoteDate", "status", "header", "headText", "footText", "addressCountry", "createUser", "sevClient", "smallSettlement", "contactPerson", "taxRule", "taxRate", "taxSet", "taxText", "taxType", "sendDate", "address", "bookingCategory", "currency", "sumNet", "sumTax", "sumGross", "sumDiscounts", "sumNetForeignCurrency", "sumTaxForeignCurrency", "sumGrossForeignCurrency", "sumDiscountsForeignCurrency", "customerInternalNote", "showNet", "sendType"]
    ...
    @field_validator('booking_category')
    def booking_category_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['PROVISION', 'ROYALTY_ASSIGNED', 'ROYALTY_UNASSIGNED', 'UNDERACHIEVEMENT', 'ACCOUNTING_TYPE']):
            raise ValueError("must be one of enum values ('PROVISION', 'ROYALTY_ASSIGNED', 'ROYALTY_UNASSIGNED', 'UNDERACHIEVEMENT', 'ACCOUNTING_TYPE')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "contact": ModelCreditNoteContact.from_dict(obj["contact"]) if obj.get("contact") is not None else None,
            ...
            "contactPerson": ModelCreditNoteContactPerson.from_dict(obj["contactPerson"]) if obj.get("contactPerson") is not None else None,
            ...
            "bookingCategory": obj.get("bookingCategory"),
