...
from pysensorsdata.tests.sqlalchemy_test_case import SqlAlchemyTestCase
from pysensorsdata.tests.sqlalchemy_test_case import with_engine_connection
...
class TestSqlAlchemyHive(unittest.TestCase, SqlAlchemyTestCase):
    ...
    def test_lots_of_types(self, engine, connection):
        ...
        cols.append(Column('hive_date', sqlalchemy.types.DATE))
        cols.append(Column('hive_decimal', sqlalchemy.types.DECIMAL))
        cols.append(Column('hive_timestamp', sqlalchemy.types.TIMESTAMP))
