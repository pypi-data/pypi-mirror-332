...
1. **DuckDB** with `kernel=duckdb` using `jupyter-duckdb` / `duckdb_kernel`
2. **SQLite** with `kernel=sqlite` after installing `jptest2[sqlite]` (This does not start an actual Jupyter Kernel but
   sends the queries directly to a connected database.)
