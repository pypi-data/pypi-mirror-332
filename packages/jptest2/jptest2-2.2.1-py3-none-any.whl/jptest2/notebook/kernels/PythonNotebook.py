...
class PythonNotebook(Notebook):
    ...
    async def __aenter__(self) -> "PythonNotebook":
        """
        initialize kernel
    ...
    def refs(self, *names: str) -> List[NotebookReference]:
        """
        ...
        """
        return [self.ref(n) for n in names]
    ...
    async def gets(self, *names: str) -> List[NotebookReference]:
        """
    ...
    async def stores(self, **kwargs) -> List[NotebookReference]:
        """
    ...
    async def execute_fun(self, fun: Callable) -> NotebookCell:
        ...
        loc = loc[1:]
        initial_indent = len(re.split(r'\S', loc[0], maxsplit=1)[0])
        ...
        # remove initial indent to allow comments in functions
        code = '\n'.join(l[initial_indent:] for l in loc)
