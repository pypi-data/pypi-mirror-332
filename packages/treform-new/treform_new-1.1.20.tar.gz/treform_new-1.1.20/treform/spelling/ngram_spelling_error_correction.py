...
class BKTree:
    ...
    :param distfn : Function to compute distances between words, like edit distance
    function.
    ...
    def _search_descendants(self, parent, n, distance, query_word):
        ...
        for i in range(dist_to_node - n, dist_to_node + n + 1):
            ...
            if child is not None:
                results.extend(
                    self._search_descendants(child, n, distance, query_word))
    ...
    def query(self, query_word: str, n: int, return_distances: bool = False):
        ...
        :param n : maximum edit distance allowed
        :param return_distances : whether to return distances (True means return
        distance)
        """
        self.visited_nodes = []
...
class PyNgramSpell:
    ...
    :param string_preprocessor_func : function used to pre-process the string,
    `lower` by default.
    ...
    :param bktree : True if bktree is used to search candidates. False means
    exhaustive search.
    """
    ...
    def _get_candidates_bktree(self, token: str, max_dist: int):
        """Return a list of candidate words from the vocabulary at most `max_dist`
        away from the input token.
    ...
    def _get_candidates_exhaustive(self, token: str, max_dist: int):
        """Return a list of candidate words from the vocabulary at most `max_dist`
        away from the input token.
        This version of the function is private and kept for benchmarking
        pourposes. This function computes the
        edit distance between the input token and all words in the vocabulary. Then
    ...
    def _correct_with_bigrams(self, tokenized_sentence: List[str]) -> List[str]:
        """Correct the words in the tokenized_sentence that are not part of the
        vocabulary
        ...
        def _probability_bigram(word1, word2):
            ...
            if self.bigram_freq_dict.get(bigram, 0) == 0:
                ...
                return (
                        self.bigram_freq_dict.get(bigram, 0) / self.unigram_freq_dict[
                    word1]
                )
    ...
    def fit(self, X: Iterable[str]):
        ...
        :param X : Iterable over strings containing the corpus used to train the
        spellcheker.
        """

        self.tokenize_func = self._build_tokenizer()
        X_tokenized = [self.tokenize_func(self.string_preprocessor_func(x)) for x in
                       X]
    ...
    def read_file(self, filename, position_idx):
        lines = []
        with open(filename, 'r', encoding='utf-8') as csvfile:
            ...
            for row in csvreader:
                lines.append(row[position_idx])
        return lines

if __name__ == '__main__':

    file_name = 'D:\\python_workspace\\treform\data\\news_articles_201901_202007.csv'
    model_path = "ngram_speller.model"
    ...
    """
    if mode == 'train':
        corrector = PyNgramSpell()
        position_idx = 3
        input_list = corrector.read_file(file_name,position_idx)
        print(input_list[0])

        corrector.fit(input_list)
        ...
        if os.path.exists(model_path):
            with open(model_path, 'rb') as model_file:
                try:
                    ...
                    text = "정경두 전작꿘 전환 후 미군 철수·연합사 해체 없요"
                    corrected_text = model.transform(text)
