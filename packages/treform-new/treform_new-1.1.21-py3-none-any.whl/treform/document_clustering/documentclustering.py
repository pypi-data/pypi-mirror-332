...
class DocumentClustering:
    ...
    def make_matrix(self, documents=None, n_components=-1, doc2vec_matrix=None):
        ...
        if (n_components != -1):
            if n_components > len(self.vectorizer.get_feature_names_out()):
                n_components = len(self.vectorizer.get_feature_names_out())
    ...
    def visualize(self):
        ...
        if self.name == 'spectral_cocluster':
            ...
            if self.doc2vec_matrix == False and not isinstance(self.X,np.ndarray):
