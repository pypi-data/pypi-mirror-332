...
class CooccurrenceWorker(BaseCooccurrence):
    ...
    def __call__(self, *args, **kwargs):
        ...
        result = zip(self.inst.get_feature_names_out(), np.array(sum_occ)[0].tolist())
...
class CooccurrenceExternalManager():
    ...
    def execute(self):
        ...
        print(self.program_path + "\\"+ executable + " " + program)
        #subprocess.call(executable + program, cwd=self.program_path, stderr=subprocess.STDOUT,
        #                stdout=open(self.stdout, 'w'), shell=True)

        returned_output = subprocess.check_output(executable + " " + program)
        subprocess.run(executable + " " + program, shell=True, check=True)
