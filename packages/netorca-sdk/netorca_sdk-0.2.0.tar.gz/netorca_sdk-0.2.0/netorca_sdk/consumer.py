...
from netorca_sdk.config import LIST_CONSUMER_SUBMISSIONS_ENDPOINT,SUBMIT_CONSUMER_SUBMISSION_ENDPOINT,VALIDATE_CONSUMER_SUBMISSION_ENDPOINT
...
class ConsumerSubmission:
    ...
    def get_submissions(
        self, limit: int = 10, ordering: str = "created"
    ) -> dict:
        ...
        request_url = (
            f"{self.auth.fqdn}{LIST_CONSUMER_SUBMISSIONS_ENDPOINT}?limit={limit}&ordering={ordering}"
        )

        response = self.auth.get(url=request_url, authentication_required=True)
