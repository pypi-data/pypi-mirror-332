...
def generate_hard_negatives(
    ...
    Optimized to process negatives per user to reduce redundant computations.
    Only computes hard negatives for users with more than a specified number of applies.
    Now supports mixed precision when configured and DataParallel models.
    """
    ...
    # Retrieve configuration parameters
    N = config['N']  # Number of hard negatives to select per user
    ...
    if use_mixed_precision:
        print("Using mixed precision for hard negative generation")
    ...
    def collate_fn(batch):
        # Batch is a list of user_data dictionaries
        return batch
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm_fn(dataloader, desc="Generating hard negatives")):
            ...
            for user in batch:
                ...
                # Combine user and negative item tokens
                input_ids = torch.cat([inputs_user['input_ids'].repeat(len(neg_item_texts), 1), inputs_negatives['input_ids']], dim=1)
                attention_mask = torch.cat([inputs_user['attention_mask'].repeat(len(neg_item_texts), 1), inputs_negatives['attention_mask']], dim=1)

                input_ids = input_ids.to(device)
                attention_mask = attention_mask.to(device)

                # Prepare features if using sparse features
                ...
                if use_sparse:
                    ...
                    # Prepare features
                    user_features_tensor = torch.tensor(user_features, dtype=torch.float).to(device).unsqueeze(0)
                    neg_item_features_tensor = torch.tensor(neg_item_features_list, dtype=torch.float).to(device)
                    features_tensor = torch.cat([user_features_tensor.repeat(len(neg_item_texts), 1), neg_item_features_tensor], dim=1)
                ...
                # Model inference - use mixed precision if enabled
                if debug:
                    inference_start_time = time.time()

                # Use autocast for mixed precision when enabled
                if use_mixed_precision:
                ...
                # Use selected indices to select negative item IDs and texts
                ...
                # Store hard negatives for this user
