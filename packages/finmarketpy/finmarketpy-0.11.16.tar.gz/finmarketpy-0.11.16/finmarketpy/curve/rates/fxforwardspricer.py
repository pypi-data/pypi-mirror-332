...
@guvectorize(['void(f8[:], f8[:], f8[:,:], f8[:,:], intp, f8[:])'],
             '(n),(n),(n,m),(n,m),()->(n)', cache=True, target="cpu",
             nopython=True)
def _forwards_interpolate_numba(spot_arr, spot_delivery_days_arr,
                                quoted_delivery_days_arr,
    ...
    for i, delivery_day in enumerate(spot_delivery_days_arr):
        for j in range(no_of_tenors):
            ...
            # Say if we're in between SP and 1W
                ...
                out[i] = spot_arr[i] + (forwards_points_arr[i, j] /
                                        quoted_delivery_days_arr[
                                            i, j]) * delivery_day

                break
            # Eg. if we're in between 1W and 2W
            elif delivery_day >= quoted_delivery_days_arr[
                i, j] and delivery_day <= quoted_delivery_days_arr[i, j + 1]:
                ...
                forward_points_per_day = (forwards_points_arr[i, j + 1] -
                                          forwards_points_arr[i, j]) \
                                         / (quoted_delivery_days_arr[
                                                i, j + 1] -
                                            quoted_delivery_days_arr[i, j])

                out[i] = spot_arr[i] + forward_points_per_day * delivery_day \
                         + forwards_points_arr[i, j] - (
                                     forward_points_per_day *
                                     quoted_delivery_days_arr[i, j])
...
@guvectorize(['void(f8[:], f8[:,:], f8[:,:], f8[:,:], f8, f8, intp, f8[:,:])'],
             '(n),(n,m),(n,m),(n,m),(),(),()->(n,m)', cache=True, target="cpu",
             nopython=True)
def _infer_base_currency_depo_numba(spot_arr, outright_forwards_arr, depo_arr,
                                    quoted_delivery_days_arr,
                                    base_conv, terms_conv, no_of_tenors, out):
    for i in range(len(out)):
        for j in range(no_of_tenors):
            out[i, j] = (((1 + depo_arr[i, j] * (
...
@guvectorize(['void(f8[:], f8[:,:], f8[:,:], f8[:,:], f8, f8, intp, f8[:,:])'],
             '(n),(n,m),(n,m),(n,m),(),(),()->(n,m)', cache=True, target="cpu",
             nopython=True)
def _infer_terms_currency_depo_numba(spot_arr, outright_forwards_arr, depo_arr,
                                     quoted_delivery_days_arr,
                                     base_conv, terms_conv, no_of_tenors, out):
    for i in range(len(out)):
        for j in range(no_of_tenors):
            out[i, j] = ((outright_forwards_arr[i, j] / spot_arr[i]) *
                         (1 + depo_arr[i, j] * (quoted_delivery_days_arr[
                                                    i, j] / base_conv)) - 1) \
                        / (quoted_delivery_days_arr[i, j] / terms_conv)


def _forwards_interpolate(spot_arr, spot_delivery_days_arr,
                          quoted_delivery_days_arr,
...
class FXForwardsPricer(AbstractPricer):
    ...
    def price_instrument(self, cross, horizon_date, delivery_date,
        ...
        if quoted_delivery_df is None:
            quoted_delivery_df = self.generate_quoted_delivery(cross,
                                                               market_df,
                                                               quoted_delivery_df,
        ...
        quoted_delivery_df = quoted_delivery_df[
            quoted_delivery_df.index.isin(horizon_date)]
        ...
        quoted_delivery_df, quoted_delivery_days_arr, forwards_points_arr, divisor = \
            self._setup_forwards_calculation(cross, spot_date, market_df,
                                             quoted_delivery_df,
                                             fx_forwards_tenor_for_interpolation)
        ...
        if return_as_df:
            ...
            interpolated_df[
                cross + '-interpolated-outright-forward.close'] = interpolated_outright_forwards_arr
    ...
    def _setup_forwards_calculation(self, cross, spot_date, market_df,
        ...
        quoted_delivery_df = self.generate_quoted_delivery(cross, market_df,
                                                           quoted_delivery_df,
                                                           fx_forwards_tenor,
                                                           cal)
        ...
        forwards_points_arr = market_df[
                                  [cross + x + ".close" for x in
                                   fx_forwards_tenor]].values / divisor
        ...
        for i, tenor in enumerate(fx_forwards_tenor):
            quoted_delivery_days_arr[:, i] = (
                    pd.DatetimeIndex(quoted_delivery_df[
                                         cross + tenor + ".delivery"]) - spot_date).days
    ...
    def generate_quoted_delivery(self, cross, market_df, quoted_delivery_df,
        ...
        if quoted_delivery_df is None:
            quoted_delivery_df = pd.DataFrame(index=market_df.index,
                                              columns=[
                                                  cross + tenor + ".delivery"
                                                  for tenor in
                                                  fx_forwards_tenor])

            for tenor in fx_forwards_tenor:
                quoted_delivery_df[cross + tenor + ".delivery"] = \
                    self._calendar.get_delivery_date_from_horizon_date(
                        quoted_delivery_df.index, tenor, cal=cal)

        return quoted_delivery_df

    def calculate_implied_depo(self, cross, implied_currency, market_df=None,
                               quoted_delivery_df=None,
        ...
        if implied_currency == cross[0:3]:
            ...
            depo_arr = market_df[[original_currency + d + '.close' for d in
                                  depo_tenor]].values / 100.0

            implied_depo_arr = _infer_base_currency_depo_numba(spot_arr,
                                                               outright_forwards_arr,
                                                               depo_arr,
                                                               quoted_delivery_days_arr,
                                                               base_conv,
                                                               terms_conv,
        ...
        if implied_currency == cross[3:6]:
            ...
            implied_depo_arr = _infer_terms_currency_depo_numba(spot_arr,
                                                                outright_forwards_arr,
                                                                depo_arr,
                                                                quoted_delivery_days_arr,
                                                                base_conv,
