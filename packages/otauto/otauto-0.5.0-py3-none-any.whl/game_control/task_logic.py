...
from communication.fd.docker_fdv1 import SyncGRPCTritonRunner
import numpy as np
...
from communication.fastapi.clent_ppocr import OCRClient

from communication.ini_file_operationv2 import INIFileHandler  # ini文件操作对象
from communication.mongoDB.mongodb_v2 import MongoDBHandler # mongodb操作对象
from communication.myemailv2 import EmailHandler # 邮件发送对象
from communication.redis.redis_main import RedisHashManager # redis操作对象
from communication.way_finding_common.coordinate_conversion import way_finding_node, coordinate_point_conversion
...
from vnc_tool.image_processing.image_matcher import ImageMatcher
from vnc_tool.image_processing.imagefinder_v4 import ImageFinder
...
class TaskLogic:
    ...
    def find_data_from_keys_list_click(self, key_text_list: list, dict_name, x3: int = 0, y3: int = 0, delay_time=0.1,
        ...
        if res:
            ...
            # 检查 details_res 的类型
            #{'65%': (720, 645, 0.999)}
            if isinstance(details_res, dict) and 'scope' in details_res:
                ...
                if points_list and isinstance(points_list, list) and len(points_list) > 0:
                    ...
                    return False
            elif isinstance(details_res, tuple) and len(details_res) == 3:
                # 直接从元组中提取坐标
    ...
    def redis_update_info(self,task_message: str=None, team_status: str =" -1", schedule: str = "-1", interactor: str = "-1"):
        """
        更新队伍信息，并在有变化时推送到数据库。
        :param task_message: 任务信息
        :param team_status: 组队状态, 默认值:-1 未组队:0 已组队:1 等待组队:2
        :param schedule: 进度器, 默认值:-1,"tld","fms","csg"
        :param interactor: 交互器,默认值:-1 未战斗:0 战斗中:1 跟随中:2 移动中:3 boss战中:4
        """
        ...
        current_team_info = {
            ...
            "交互器": self.redis_value["interactor"],
        }
    ...
    def data_info(self):
        ...
        self.map_position = data_pool["coordinate_info_dict"]["map_position"]  # 获取地图位置信息
        self.task_info = data_pool["task_info_dict"]  # 获取任务信息
