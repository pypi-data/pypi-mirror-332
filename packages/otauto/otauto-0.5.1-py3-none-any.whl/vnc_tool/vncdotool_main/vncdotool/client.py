...
class VNCDoToolClient(rfb.RFBClient):
    ...
    def captureScreenAsArray(self, incremental: bool = False) -> defer.Deferred:
        """Capture the current display and return it as a NumPy array."""
        d = self.refreshScreen(incremental)  # 先请求刷新屏幕
        d.addCallback(self._captureSaveAsArray)  # 刷新完成后处理
        return d  # 返回 Deferred 对象

    def captureRegionAsArray(self, x: int, y: int, w: int, h: int,incremental: bool = False) -> np.ndarray:
        """Capture a region of the current display and return it as a NumPy array."""
        d = self.refreshScreen()
        d.addCallback(self._captureSaveRegionAsArray, x, y, w, h)
        return d

    def _captureSaveAsArray(self, data: object) -> np.ndarray:
        """Save the current display as a NumPy array."""
        assert self.screen is not None
        # Convert the screen image to a NumPy array
        array = np.array(self.screen)
        return array

    def _captureSaveRegionAsArray(self, data: object, x: int, y: int, w: int, h: int) -> np.ndarray:
        """Save a region of the current display as a NumPy array."""
        assert self.screen is not None
        # Crop the screen to the specified region and convert to NumPy array
        capture = self.screen.crop((x, y, x + w, y + h))
        array = np.array(capture)
        return array
