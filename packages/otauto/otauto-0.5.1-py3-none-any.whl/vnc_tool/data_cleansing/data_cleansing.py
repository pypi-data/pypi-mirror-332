...
from communication.text_correctionv1 import correct_term
from resource.parameters_info.basic_parameter_info import target_name_ls


class DataCleansing:
    def __init__(self, parameter_data: dict=None,queue_handle=None,debug:bool = False):
        """
    ...
    def information_processing(self):
        ...
        #线程队列信息处理,避免没必要的信息识别
        if not self.queue_handle.empty(): #队列为空,返回 True,否则返回 False
            self.update_parameter=self.queue_handle.get()
            logger.success(f"收到线程队列信息:{self.update_parameter}")
    ...
    def run(self,original_data,numpy_data):
