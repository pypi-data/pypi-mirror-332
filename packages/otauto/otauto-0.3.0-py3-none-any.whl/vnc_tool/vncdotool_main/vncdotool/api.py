...
class ThreadedVNCClientProxy:
    ...
    def captureScreenAsArray(self) -> Any:
        """Capture the screen and return it as a NumPy array."""
        if self.protocol is None:
            raise RuntimeError("Protocol not initialized. Ensure you are connected.")

        def capture(protocol: VNCDoToolClient) -> np.ndarray:
            return protocol.captureScreenAsArray()

        d = maybeDeferred(capture, self.protocol)
        d.addBoth(lambda result: self.queue.put(result))
        try:
            result = self.queue.get(timeout=self.timeout)
        except queue.Empty:
            raise TimeoutError("Timeout while waiting for screen capture response")
    ...
    def captureRegionAsArray(self, x1, y1, x2, y2) -> Any:
        """Capture a specific region of the screen and return it as a NumPy array."""
        w = x2 - x1
        h = y2 - y1
        ...
        def capture(protocol: VNCDoToolClient) -> np.ndarray:
            return protocol.captureRegionAsArray(x1, y1, w, h)
        ...
        try:
            ...
            raise TimeoutError("Timeout while waiting for region capture response")
...
def connect(
    ...
    >>> from vnc_tool.vncdotool_main.debian.vncdotool import api
