...
def calculate_classification_metrics(
    ...
    for label, prec, rec in zip(unique_labels, class_precision, class_recall):
        # 如果提供了id2label映射，使用映射的标签名称，否则使用原始标签
        label_display = (
            str(id2label[label]) if id2label and label in id2label else str(label)
        )
...
def calculate_multilabel_metrics(
    ...
    jaccard_macro = jaccard_score(true_labels, pred_labels, average="macro")
    jaccard_micro = jaccard_score(true_labels, pred_labels, average="micro")
    ...
    if np.any(valid_samples):
        sample_f1[valid_samples] = (
            ...
            * correct_labels_per_sample[valid_samples]
            / (
                true_labels_per_sample[valid_samples]
                + pred_labels_per_sample[valid_samples]
            )
        )
    ...
    # ===== 4. 标签维度统计 =====
    # 计算各种标签组合情况
    true_positives = np.sum(true_labels & pred_labels, axis=0)  # TP: 预测为1，实际为1
    false_positives = np.sum(
        (~true_labels) & pred_labels, axis=0
    )  # FP: 预测为1，实际为0
    ...
    label_stats = {
        "true_positives": {
            get_label_name(i): int(true_positives[i]) for i in range(n_classes)
        },
        "false_positives": {
            get_label_name(i): int(false_positives[i]) for i in range(n_classes)
        },
        "false_negatives": {
            get_label_name(i): int(false_negatives[i]) for i in range(n_classes)
        },
        "true_negatives": {
            get_label_name(i): int(true_negatives[i]) for i in range(n_classes)
        },
        "frequencies": {
            get_label_name(i): float(label_frequencies[i]) for i in range(n_classes)
        },
    }
