...
class BibleSearchEngine:
    def __init__(self, model_file_path: None|str=None) -> None:
        '''
        Initializes the Bible search engine.
        '''
        # Preprocess Bible data (if needed).
        old_testament_path = 'src/bible_search_engine/bible_data/old_testament.jsonl'
        new_testament_path = 'src/bible_search_engine/bible_data/new_testament.jsonl'

        if not os.path.isfile(old_testament_path) or not os.path.isfile(new_testament_path):
            get_bible_data('src/bible_search_engine/kjv-text-files')

        # NLP tokenizer.
        nlp_tokenizer = NLPTokenizer()

        # Bible chapter index.
        bible_chapter_index_path = 'src/bible_search_engine/bible_index'
        ...
        # Bi-encoder ranker.
        encoded_chapters_path = 'src/bible_search_engine/bible_data/encoded_chapters.npy'
        chapter_ids_path = 'src/bible_search_engine/bible_data/chapterids.json'
        ...
        if model_file_path and os.path.isfile(model_file_path):
            self.l2r_ranker.lightgbm_ranker = joblib.load(model_file_path)
        else:
            self.l2r_ranker.train('src/bible_search_engine/bible_queries_relevances/train_queries_relevances.csv')
            if model_file_path:
                joblib.dump(self.l2r_ranker.lightgbm_ranker, model_file_path, True)
    ...
    def search(self, query: str) -> list[dict]:
        ...
        search_results = [{'chapterid': chapterid, 'score': score, 'chapter': self.chapter_titles[chapterid], 'verses': self.chapter_verses[chapterid]}
                          for chapterid, score in query_results]
        return search_results

def create_bible_search_engine(model_file_path: str|None=None) -> BibleSearchEngine:
    '''
    Creates the Bible search engine.

    Returns the created Bible search engine.
    '''
    return BibleSearchEngine(model_file_path)
