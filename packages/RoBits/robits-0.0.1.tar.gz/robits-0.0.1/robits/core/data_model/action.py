...
class CartesianAction:
    ...
    @classmethod
    def from_matrix(cls, pose: np.ndarray, hand_open=True) -> "CartesianAction":
        """
    ...
    @classmethod
    def parse(cls, action: Sequence[float]) -> "CartesianAction":
        """
...
class BimanualAction:
    ...
    def from_numpy(cls, action: np.ndarray) -> "BimanualAction":
        ...
        if action.shape[0] % 2 != 0:
            raise ValueError("The input action array must have an even number of elements.")

        right_action, left_action = np.split(action, 2)

        right_action = CartesianAction.from_numpy(right_action)
        left_action = CartesianAction.from_numpy(left_action)
