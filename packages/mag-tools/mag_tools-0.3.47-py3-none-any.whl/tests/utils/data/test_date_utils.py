...
class TestDateUtils(unittest.TestCase):
    ...
    def test_pick_datetime(self):
        # 测试从文本中提取日期、时间和日期时间
        text = "会议时间是2025-03-10 16:30:45，截止日期是2025/03/15，提醒设置在15:45。"
        expected_results = [
            datetime(2025, 3, 10, 16, 30, 45),
            date(2025, 3, 15),
            time(15, 45),
        ]
        self.assertEqual(DateUtils.pick_datetime(text), expected_results)

        text = "这是一个空字符串，没有日期或时间。"
        self.assertEqual(DateUtils.pick_datetime(text), [])
