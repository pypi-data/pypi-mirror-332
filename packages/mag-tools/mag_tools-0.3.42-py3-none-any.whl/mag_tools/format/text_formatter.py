...
from utils.data.date_utils import DateUtils
...
class TextFormatter:
    ...
    def to_lines(self, values: List[Any], padding_length: int = None) -> List[str]:
        """
        将数值数组转为按指定格式的字符串
        :param values: 数值数组
        :param padding_length: 字段显示的最大长度,为None时则按格式中定义的长度
        """
        if padding_length is None:
            padding_length = self.pad_length
        ...
        for value in values:
            ...
            if isinstance(value, int) or isinstance(value, float):
                ...
                lines.append(value.strftime('%Y-%m-%d'))
            elif isinstance(value, time):
                lines.append(value.strftime('%H:%M:%S'))
            elif isinstance(value, datetime):
                lines.append(value.strftime('%Y-%m-%d %H:%M:%S'))
            else:
                lines.append(str(value) if value is not None else self.none_default)
    ...
    def to_blocks(self, values: List[List[Any]]) -> List[str]:
        ...
        lines = []
        for value in values:
            lines.extend(self.to_lines(value, padding_length))
        return lines
