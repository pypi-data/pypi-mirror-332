...
class StringFormat:
    ...
    @staticmethod
    def pad_text(strings: List[str], padding_length: Optional[int] = None, justify: JustifyType = JustifyType.RIGHT,
                 number_per_line: Optional[int] = None, at_header: Optional[str] = None, pad_char: Optional[str] = None) -> List[str]:
        """
        将字符串数组按指定格式拼接为文本
        :param strings: 字符串数组
        ...
        lines = [at_header + pad_char.join(block) for block in blocks]
        return lines
