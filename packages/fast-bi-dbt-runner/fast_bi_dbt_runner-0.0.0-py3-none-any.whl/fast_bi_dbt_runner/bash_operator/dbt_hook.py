...
class DbtCliHook(BaseHook):
    ...
    def __init__(self,
        ...
        self.worker_id = socket.gethostname()  # Add worker identification
    ...
    def _check_packages_dir(self):
        ...
        self.log.info(f"[Worker: {self.worker_id}] Checking dbt packages in {packages_dir}")

        if not os.path.exists(packages_dir):
            self.log.info(f"[Worker: {self.worker_id}] dbt_packages not found, initiating installation")
            ...
            try:
                with lock:
                    ...
                    if not os.path.exists(packages_dir):
                        ...
                        if self.verbose:
                            self.log.info(f"[Worker: {self.worker_id}] Running: {' '.join(wrapped_cmd)}")
                        ...
                        self.log.info(f"[Worker: {self.worker_id}] Successfully installed dbt packages")
                    
            except Exception as e:
                self.log.error(f"[Worker: {self.worker_id}] Error during package installation: {str(e)}")
    ...
    def run_cli(self, *command):
        ...
        :type command: str
        """
        # Check packages before running command
        ...
        if self.target == 'test' or self.target == 'e2e' or self.target == 'e2e_test':
            cmd_prefix = f"export GIT_BRANCH={self.git_branch} && "
        else:
            cmd_prefix = ""
        ...
        if self.verbose:
            self.log.info(" ".join(wrapped_cmd))
