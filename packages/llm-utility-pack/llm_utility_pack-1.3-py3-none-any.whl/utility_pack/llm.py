...
question_classifier_tokenizer = Tokenizer.from_file(str(importlib.resources.files('megautils').joinpath('resources/question-classifier/tokenizer.json')))
question_classifier_session = ort.InferenceSession(str(importlib.resources.files('megautils').joinpath('resources/question-classifier/model.onnx')))

reranker_tokenizer = Tokenizer.from_file(str(importlib.resources.files('megautils').joinpath('resources/reranker/tokenizer.json')))
reranker_session = ort.InferenceSession(str(importlib.resources.files('megautils').joinpath('resources/reranker/model.onnx')))
...
def classify_question_generic_or_directed(question):
    if count_tokens(question) > 500:
        question = compress_text(question, target_token_count=500)
...
def rerank(question, passages, normalize_scores=True):
    ...
    token_type_ids = [[0] * len(enc.ids) for enc in encoded_inputs]
