...
class EMLParser(EmailParser):
    def __init__(self, email_data, filename="unknown.eml", ignore_errors=False, exclude_attachment_extensions=None,
                 include_attachment_extensions=None, use_cids_as_names=False):
        super(EMLParser, self).__init__(email_data,
                                        ...
                                        use_cids_as_names=use_cids_as_names)
    ...
    def get_raw_headers(self):
        return u"\n".join([u"{}: {}".format(unicode(h[0], "utf-8", "ignore"), unicode(str(h[1]), "utf-8", "ignore")) for h in self.msg.items() or []])

    def get_headers_json(self):
        json_dict = {}
        for h in self.msg.items():
            json_dict[unicode(h[0], "utf-8", "ignore")] = unicode(str(h[1]), "utf-8", "ignore")
        return json.dumps(json_dict, cls=HeaderEncoder)
    ...
    def get_attachments(self):
        ...
        for raw_attachment in raw_attachments:
            if isinstance(raw_attachment[1], StringTypes):
                attach_data = raw_attachment[1].encode()
            else:
                attach_data = raw_attachment[1]

            filename = self._get_attachment_filename(raw_attachment[0], hashlib.md5(attach_data).hexdigest(), use_cids_as_names=self.use_cids_as_names)
    ...
    @staticmethod
    def _get_attachment_filename(attachment_headers, fallback_name, use_subject=True, use_cids_as_names=False):
        """
        ...
        if "content-location" in attachment_headers:
            filename = str(attachment_headers["content-location"])
            parsed = True
        elif 'subject' in attachment_headers and use_subject:
            filename = "{}.eml".format(str(attachment_headers['subject']))
            parsed = True
        elif 'content-id' in attachment_headers and use_cids_as_names:
            filename = str(attachment_headers["content-id"]).lstrip('<').rstrip('>')
            parsed = True
        elif "content-disposition" in attachment_headers:
            content = str(attachment_headers["content-disposition"]).split(";")
            for disp in content:
                disp = disp.strip()
        ...
        if "content-type" in attachment_headers and not parsed:
            properties = str(attachment_headers["content-type"]).split(";")

            if str(attachment_headers.get("content-type")) == "message/rfc822":
    ...
    def _is_attachment(self, headers):
        ...
        if "content-disposition" in headers:
            if str(headers["content-disposition"]).startswith("attachment"):
                return True
        if "content-transfer-encoding" in headers:
            trans_enc = str(headers["content-transfer-encoding"]).lower()
            if trans_enc in ("quoted-printable", "7bit", "8bit"):
                return False
            else:
                temp_name = str(uuid.uuid4())  # TODO Refactor this?
                fn = self._get_attachment_filename(headers, temp_name, use_subject=False, use_cids_as_names=self.use_cids_as_names)
    ...
    def _extract_attachments(self, eml_obj):
        ...
        if hash(eml_obj) in self.processed_attachment_hashes:
            ...
            if isinstance(payload, list) and len(payload) == 1 and \
                    self._get_attachment_filename(eml_obj, str(uuid.uuid4()), use_subject=False, use_cids_as_names=self.use_cids_as_names).lower().endswith(".eml"):
                ...
                if match:
                    ...
                    try:
                        ...
                        eml_obj._payload = attached_eml.as_bytes().decode('utf-8', errors="ignore")
                    ...
                    if isinstance(attachment, email.message.Message):
                        ...
                        if self.get_id() is not msg_id and msg_id is not None:
                            ...
                            if isinstance(attachment.get_payload(), StringTypes) and not found_txt_body:
                                ...
                                fn = self._get_attachment_filename(attachment, temp_name, use_subject=False, use_cids_as_names=self.use_cids_as_names)
    ...
    def _decode_payload(attachment, additional_headers=None):
        ...
        def check_all_headers(header_key):
            if header_key in attachment:
                return str(attachment[header_key])
            if header_key in additional_headers:
                return str(additional_headers[header_key])
