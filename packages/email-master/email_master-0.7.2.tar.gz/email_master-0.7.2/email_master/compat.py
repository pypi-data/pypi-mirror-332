...
"""
ATTENTION: Currently there is a bug in python's builtin email package.
This file overrides the generator.py with a small fix that can be found:
https://github.com/python/cpython/pull/22797/files

And is released in 3.8

A custom as_string() function is used in the eml/parser.py file
...
def override_as_string(msg_obj, unixfrom=False, maxheaderlen=0, policy=None):
    ...
    policy = msg_obj.policy if policy is None else policy
    fp = StringIO()
    g = Generator(fp,
                  mangle_from_=False,
