...
class EmailParser(object):
    def __init__(self, email_data, filename="unknown", ignore_errors=False, exclude_attachment_extensions=None,
        ...
        self.use_cids_as_names = use_cids_as_names
        ...
        self.exclude_attachment_extensions = tuple(
            map(to_lower, exclude_attachment_extensions)) if exclude_attachment_extensions else None
        self.include_attachment_extensions = tuple(
            map(to_lower, include_attachment_extensions)) if include_attachment_extensions else None
    ...
    def _try_date_format(self, date_data):
        try:
            ...
            dt = pendulum.parse(date_data, strict=False)
    ...
    def parse(self):
        ...
        headers_list = [ EmailUtil.try_decode(h[0] + ": " + str(h[1])) for h in self.get_headers() ]
