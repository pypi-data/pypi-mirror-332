...
from au.tools import dict_from_csv
...
class Roster:
    def __init__(self, source: Path|List):
        if isinstance(source, Path):
            if source and source.exists():
                ...
                self.login_student_map: Dict[str,str] = dict_from_csv(source, 'github_username', 'identifier')
    ...
    @staticmethod
    def _get_dirs_from_source(dir_source: Path|List[any], sort_by_size = False) -> List[str]:
    ...
    def get_login_dir_map(self, dir_source: Path|List[str], include_unmapped = False) -> Dict[str,str|None]:
        '''
    ...
    def get_dir_student_map(self, dir_source: Path|List[str]) -> Dict[str,str|None]:
        '''
