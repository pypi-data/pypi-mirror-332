...
class ServerRegistry:
    ...
    def save_config(self, path: Path | str) -> None:
        ...
        config = {"servers": {name: settings.model_dump(exclude_none=True) for name, settings in self.registry.items()}}
    ...
    def from_config(cls, path: Path | str) -> "ServerRegistry":
        ...
        if "servers" not in config:
            raise KeyError("Config file must have a 'servers' section")

        servers = {name: MCPServerSettings(**settings) for name, settings in config["servers"].items()}
        return cls(servers)
...
class MCPAggregator:
    ...
    async def call_tool(
        ...
        if actual_server_name not in self.server_names:
            return CallToolResult(isError=True, message=f"Server '{actual_server_name}' not found or not enabled")
