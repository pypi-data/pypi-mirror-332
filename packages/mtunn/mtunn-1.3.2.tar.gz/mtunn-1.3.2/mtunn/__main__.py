...
class Client(SockRunable):
    ...
    def _run(self):
        ...
        try:
            ...
            self.stop()
        while self._running and self._package is not None:
            data = read_package(self._sock)
            if data:
    ...
    if args.update:
        ...
        try:
            ...
            if not Path(str(args.config)).exists():
                printf(f"\033[01;31mfail\033[0m")
                printf(f"\033[01;36m"+str(time.strftime("%H:%M:%S"))+f"\033[0m [\033[01;31mERROR\033[0m] no config file found")
