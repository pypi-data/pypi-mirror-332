...
class Sweeper:
    ...
    def _log_params(self, type='voltage') -> None:
        ...
        with open(f"{log_filename}.txt", 'a') as file:
            ...
            if self.is_2d_sweep:
                file.write(f"{'Y Swept Gates: ':>16} {self.y_label_2d} \n")
                file.write(f"{'Start Voltage Y: ':>16} {self.Y_voltage:>24.16f} \n")
                file.write(f"{'End Voltage Y: ':>16} {self.Y_end_voltage:>24.16f} \n")
                file.write(f"{'Step Size Y: ':>16} {self.Y_step:24.16f} \n")
            if type == 'time':
    ...
    def sweep1D(
        ...
        if not self.is_2d_sweep:
            ...
            self.ax2.set_ylabel(f"{self.y_label} [{self.current_unit}]")
        ...
        print(
            f"[INFO] Start sweeping {self.x_label} from {self.start_voltage} [V] to {self.end_voltage} [V].")
