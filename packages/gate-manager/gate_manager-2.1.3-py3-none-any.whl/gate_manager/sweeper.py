...
class Sweeper:
    ...
    def _set_filename(self, prefix):
        ...
        if prefix == '1D':
            base_filename = f"{datetime.strftime('%Y%m%d')}_{self.temperature}_[{self.z_label}]_vs_[{self.x_label}]"
        elif prefix == '2D':
            base_filename = f"{datetime.strftime('%Y%m%d')}_{self.temperature}_[{self.z_label}]_vs_[{self.x_label}]_[{self.y_label}]"
        if self.comments:
