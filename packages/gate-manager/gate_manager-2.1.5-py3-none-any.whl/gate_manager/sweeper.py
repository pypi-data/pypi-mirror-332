...
class Sweeper:
    ...
    def sweep2D(self, 
        ...
        plt.rc('legend', fontsize=12, framealpha = 0.9)
        plt.rc('xtick', labelsize=14, color='#2C3E50') 
        plt.rc('ytick', labelsize=14, color='#2C3E50')
        ...
        while True:
            ...
            barticks = np.linspace(np.nanmin(data), np.nanmax(data), 5)  # Generate bar ticks
            barticks = np.around(barticks, 2)        # Round to 2 decimal places
            barticks_labels = [str(label) for label in barticks]
            
            cbar = self.fig.colorbar(self.img, ticks=barticks, ax=self.ax1, pad=0.005, extend='both')
