...
class MockNanonis:
    ...
    def Signals_ValsGet(self, read_indices, _bool):
        ...
        voltage = self.read_voltages.get(read_index, 0.0)
...
def test_set_volt_success(gate, mock_nanonis):
    ...
    """
    gate.set_volt(1.23)
    assert mock_nanonis.set_voltages[gate.source.write_index] == 1.23
...
def test_voltage_set_and_wait(gate, mock_nanonis):
    ...
    mock_nanonis.read_voltages[gate.source.read_index] = 0.0

    def simulate_voltage_reach():
        mock_nanonis.read_voltages[gate.source.read_index] = 1.0
    ...
    try:
        gate.voltage(1.0, is_wait=True)
        # Check that the final read voltage is 1.0
        assert gate._voltage == 1.0
        # Also verify that the voltage was set in the mock
        assert mock_nanonis.set_voltages[gate.source.write_index] == 1.0
    finally:
...
def test_turn_off(gate, mock_nanonis):
    ...
    assert mock_nanonis.set_voltages[gate.source.write_index] == 0.0
...
def test_is_at_target_voltage_true(gate, mock_nanonis):
    ...
    mock_nanonis.read_voltages[gate.source.read_index] = 1.0001
    assert gate.is_at_target_voltage(1.0, tolerance=1e-5)
...
def test_is_at_target_voltage_false(gate, mock_nanonis):
    ...
    mock_nanonis.read_voltages[gate.source.read_index] = 1.01
    assert not gate.is_at_target_voltage(1.0, tolerance=1e-6)
...
def test_read_current(gate, mock_nanonis):
    ...
    assert abs(currents - -5E-7) < 1E-10
