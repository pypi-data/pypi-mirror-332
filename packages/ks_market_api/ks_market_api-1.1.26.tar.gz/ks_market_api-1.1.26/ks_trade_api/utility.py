...
def get_interval_seconds(interval: Interval):
    n, period = re.search(r'(\d*)([^\d]+)', interval.value).groups()
    if not n:
        n = 1
    else:
        n = int(n)
...
# todo 兼容币安
def get_trading_time(exchange: Exchange, input_time: datetime = None, extended_time: bool = False):
    ...
    start_time = tz.localize(datetime.combine(exchange_date, start_time.value)).astimezone(CHINA_TZ)
    end_time = tz.localize(datetime.combine(exchange_date, end_time.value)).astimezone(CHINA_TZ)
