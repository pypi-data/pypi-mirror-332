...
from ks_trade_api.utility import TEMP_DIR
import sys
...
def process_b(bar: BarData, close_shared_dict: dict, datetime_shared_dict: dict, indicator_shared_queue: multiprocessing.Queue):
    ...
    if datetime_shared_dict[vt_symbol][interval][-1] < bar.datetime:
...
class KsMarketApiServer(ZmqPublisher, ZmqSubscriber, Logger):
    ...
    def __init__(self, setting: dict):
        ...
        # todo!!! remove
        # from ks_trade_api.constant import Exchange, Interval, SubscribeType
        # from ks_trade_api.utility import datetimes
        # from ks_utility.datetimes import get_dt_int
        # self.subscribe(['00700.SEHK'], [SubscribeType.K_MINUTE])
        # while True:
        #     self.market_api.on_bar_rt(BarData(
        #         gateway_name=self.gateway_name,
        #         symbol='00700',
        #         exchange=Exchange.SEHK,
        #         datetime=get_dt_int(interval=60),
    ...
    def update_shared_dict(self, vt_symbol: str, interval: Interval):
        ...
        if not interval in self.close_shared_dict[vt_symbol]:
        ...
        if not interval in self.datetime_shared_dict[vt_symbol]:
