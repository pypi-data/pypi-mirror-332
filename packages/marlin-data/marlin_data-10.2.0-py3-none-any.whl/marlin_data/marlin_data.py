...
class MarlinDerivedData(object):
    ...
    def build_derived_data(self, simulation_data : SimulationData = None, f_min : int = 0, f_max : int = 1000, indexing = True):
        """
        ...
        while current_idx < len(raw_data):
            ...
            # print (elapsed_s)
            # print (f_power_vector)
            # print ('---')
            # print (self.librosa_time_disrete_bins)
            # print (self.librosa_f_discrete_bins)
            
            # plt.plot(self.librosa_f_discrete_bins[5:],f_power_vector[5:],color='green', linewidth=0.5, markersize=0.5)
            # plt.xlabel('f (Hz)')
            # plt.ylabel('Amplitude')
            # plt.ylim(0,1)
            # plt.title("Waveform")
            # plt.savefig(f'test_plot{discrete_number}.png')
            # plt.close() 
