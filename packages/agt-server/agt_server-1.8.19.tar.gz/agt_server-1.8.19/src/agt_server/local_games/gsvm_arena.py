...
class GSVM9Arena(LocalArena):
    ...
    def run_game(self, group):
        ...
        """
        self._log_or_print(f"{group[0].name} (National) vs. {group[1].name} (R1) vs. {group[2].name} (R2) vs. {group[3].name} (R3)")
        group[0].is_national_bidder = True
        group[1].is_national_bidder = False
        group[2].is_national_bidder = False
        group[3].is_national_bidder = False
        ...
        self._log_or_print(f"Winners: {allocation}")
        self._log_or_print(f"Prices: {payments}")
        for player in group:
