...
class GSVM9Game(Game):
    ...
    def __init__(self, num_rounds=10, player_data=[], player_types=[], permissions_map={},
        ...
        self.max_valuations_national = {"A":15, "B":15, "C":30, "D":30, "E":15, "F":15, "G":0, "H":0, "I":0}
    ...
    async def run_game(self):
        """
        Runs the GSVM-9 auction game asynchronously.
        For each round:
          1. Each player's valuations are generated based on their role (national or regional).
          2. A "send_preround_data" message is sent to each player.
          3. Bids are requested from each player.
          4. The auction outcome is computed (using kth-price rule).
          5. Round utilities are computed using the synergy formula.
          6. A "prepare_next_round" message is sent to each player.
        """
