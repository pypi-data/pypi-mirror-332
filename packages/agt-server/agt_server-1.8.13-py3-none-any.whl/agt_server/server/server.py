...
class Server:
    ...
    async def run_game(self):
        ...
        if self.players_per_game == -1:
            # Throw all players into a single game.
            addresses = list(self.player_data.keys())
            # If any player is disconnected, you might want to skip forming the game.
            if any(self.player_data[address]["disconnected"] for address in addresses):
                # Optionally, handle disconnected players here (e.g., remove them or log a message).
                pass
                ...
                # Use the number of players for the type configuration if "all" is specified.
                ...
                for player_types in type_configs:
                    ...
                    # Skip if any player has no client.
