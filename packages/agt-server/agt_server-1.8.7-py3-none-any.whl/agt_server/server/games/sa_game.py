...
class SAGame(Game):
    ...
    def __init__(self, server_config, num_goods = -1, good_names=None, valuation_type="additive", bid_range=(0, 100),
        ...
        if good_names is not None:
            ...
            self._goods_to_index = SAGame._name_goods(num_goods)
    ...
    def _name_goods(size, alphabet="ABCDEFGHIJKLMNOPQRSTUVWXYZ"):
        alphabet_generator = SimultaneousAuctionGame._generate_sequence(alphabet)
        map_dict = {}
    ...
    def compute_auction_result(self, bids):
        ...
        payments = {data['address']: 0 for data in self.player_data}
        prices = {}
            ...
            for addr, bid_bundle in bids.items():
                if bid_bundle is not None:
                    ...
                    if bid_value is not None and bid_value > 0:
                        if highest_bid is None or bid_value > highest_bid:
                            highest_bid = bid_value
                            winner = addr
            allocation[good] = winner
            prices[good] = highest_bid
            if winner is not None:
                payments[winner] += highest_bid
    ...
    async def run_game(self):
        ...
        for round in range(self.num_rounds):
            ...
            # --------------------
            # Announce outcome to players.
            # --------------------
            outcome_message = {
                "message": "auction_outcome",
            ...
            for data in self.player_data:
                ...
                if not self.game_reports[data['address']]['disconnected']:
                    try:
                        writer.write(json.dumps(outcome_message).encode())
                        await writer.drain()
                    except Exception as e:
                        self.game_reports[data['address']]['disqualification_message'] = (
                            f"{data['name']} error during outcome announcement: {type(e).__name__}: {e}"
                        )
                        self.game_reports[data['address']]['disconnected'] = True
            ...
            for data in self.player_data:
                ...
                won_goods = [good for good, winner in allocation.items() if winner == data['address']]
                base_sum = sum(valuations.get(good, 0) for good in won_goods)
                total_payment = payments.get(data['address'], 0)
                n = len(won_goods)
        ...
        # --------------------
        # End-of-game: notify players to prepare for the next game.
        # --------------------
        final_message = {"message": "prepare_next_game"}
        for data in self.player_data:
            if not self.game_reports[data['address']]['disconnected']:
                try:
                    writer, reader = data['client']
                    writer.write(json.dumps(final_message).encode())
                    await writer.drain()
                    resp = await asyncio.wait_for(reader.read(1024), timeout=self.kick_time)
                    resp = json.loads(resp)
                    ...
                    self.game_reports[data['address']]['disqualification_message'] = (
                        f"{data['name']} Disqualified: Timed out on prepare_next_game"
                    )
                    self.game_reports[data['address']]['disconnected'] = True
                except Exception as e:
                    stack_trace = traceback.format_exc()
                    self.game_reports[data['address']]['disqualification_message'] = (
                        f"{data['name']} error on prepare_next_game: {type(e).__name__}: {e}\n{stack_trace}"
                    )
                    self.game_reports[data['address']]['disconnected'] = True
