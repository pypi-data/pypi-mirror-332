# File generated from our OpenAPI spec by Stainless. See CONTRIBUTING.md for details.

from __future__ import annotations

import os
from typing import Any, cast

import pytest

from pyopenwebui import Pyopenwebui, AsyncPyopenwebui
from tests.utils import assert_matches_type

base_url = os.environ.get("TEST_API_BASE_URL", "http://127.0.0.1:4010")


class TestChat:
    parametrize = pytest.mark.parametrize("client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    def test_method_generate_completion(self, client: Pyopenwebui) -> None:
        chat = client.openai.chat.generate_completion(
            body={},
        )
        assert_matches_type(object, chat, path=["response"])

    @parametrize
    def test_method_generate_completion_with_all_params(self, client: Pyopenwebui) -> None:
        chat = client.openai.chat.generate_completion(
            body={},
            bypass_filter=True,
        )
        assert_matches_type(object, chat, path=["response"])

    @parametrize
    def test_raw_response_generate_completion(self, client: Pyopenwebui) -> None:
        response = client.openai.chat.with_raw_response.generate_completion(
            body={},
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        chat = response.parse()
        assert_matches_type(object, chat, path=["response"])

    @parametrize
    def test_streaming_response_generate_completion(self, client: Pyopenwebui) -> None:
        with client.openai.chat.with_streaming_response.generate_completion(
            body={},
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            chat = response.parse()
            assert_matches_type(object, chat, path=["response"])

        assert cast(Any, response.is_closed) is True


class TestAsyncChat:
    parametrize = pytest.mark.parametrize("async_client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    async def test_method_generate_completion(self, async_client: AsyncPyopenwebui) -> None:
        chat = await async_client.openai.chat.generate_completion(
            body={},
        )
        assert_matches_type(object, chat, path=["response"])

    @parametrize
    async def test_method_generate_completion_with_all_params(self, async_client: AsyncPyopenwebui) -> None:
        chat = await async_client.openai.chat.generate_completion(
            body={},
            bypass_filter=True,
        )
        assert_matches_type(object, chat, path=["response"])

    @parametrize
    async def test_raw_response_generate_completion(self, async_client: AsyncPyopenwebui) -> None:
        response = await async_client.openai.chat.with_raw_response.generate_completion(
            body={},
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        chat = await response.parse()
        assert_matches_type(object, chat, path=["response"])

    @parametrize
    async def test_streaming_response_generate_completion(self, async_client: AsyncPyopenwebui) -> None:
        async with async_client.openai.chat.with_streaming_response.generate_completion(
            body={},
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            chat = await response.parse()
            assert_matches_type(object, chat, path=["response"])

        assert cast(Any, response.is_closed) is True
