...
class TestTags:
    ...
    def test_method_generate(self, client: Pyopenwebui) -> None:
        tag = client.api.v1.tasks.tags.generate(
            body={},
        )
    ...
    def test_raw_response_generate(self, client: Pyopenwebui) -> None:
        response = client.api.v1.tasks.tags.with_raw_response.generate(
            body={},
        )
    ...
    def test_streaming_response_generate(self, client: Pyopenwebui) -> None:
        with client.api.v1.tasks.tags.with_streaming_response.generate(
            body={},
        ) as response:
...
class TestAsyncTags:
    ...
    async def test_method_generate(self, async_client: AsyncPyopenwebui) -> None:
        tag = await async_client.api.v1.tasks.tags.generate(
            body={},
        )
        assert_matches_type(object, tag, path=["response"])
    ...
    async def test_raw_response_generate(self, async_client: AsyncPyopenwebui) -> None:
        response = await async_client.api.v1.tasks.tags.with_raw_response.generate(
            body={},
        )
    ...
    async def test_streaming_response_generate(self, async_client: AsyncPyopenwebui) -> None:
        async with async_client.api.v1.tasks.tags.with_streaming_response.generate(
            body={},
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
