# File generated from our OpenAPI spec by Stainless. See CONTRIBUTING.md for details.

from __future__ import annotations

import os
from typing import Any, cast

import pytest

from pyopenwebui import Pyopenwebui, AsyncPyopenwebui
from tests.utils import assert_matches_type

base_url = os.environ.get("TEST_API_BASE_URL", "http://127.0.0.1:4010")


class TestOllama:
    parametrize = pytest.mark.parametrize("client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    def test_method_get_status(self, client: Pyopenwebui) -> None:
        ollama = client.ollama.get_status()
        assert_matches_type(object, ollama, path=["response"])

    @parametrize
    def test_raw_response_get_status(self, client: Pyopenwebui) -> None:
        response = client.ollama.with_raw_response.get_status()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        ollama = response.parse()
        assert_matches_type(object, ollama, path=["response"])

    @parametrize
    def test_streaming_response_get_status(self, client: Pyopenwebui) -> None:
        with client.ollama.with_streaming_response.get_status() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            ollama = response.parse()
            assert_matches_type(object, ollama, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    def test_method_verify_connection(self, client: Pyopenwebui) -> None:
        ollama = client.ollama.verify_connection(
            url="url",
        )
        assert_matches_type(object, ollama, path=["response"])

    @parametrize
    def test_method_verify_connection_with_all_params(self, client: Pyopenwebui) -> None:
        ollama = client.ollama.verify_connection(
            url="url",
            key="key",
        )
        assert_matches_type(object, ollama, path=["response"])

    @parametrize
    def test_raw_response_verify_connection(self, client: Pyopenwebui) -> None:
        response = client.ollama.with_raw_response.verify_connection(
            url="url",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        ollama = response.parse()
        assert_matches_type(object, ollama, path=["response"])

    @parametrize
    def test_streaming_response_verify_connection(self, client: Pyopenwebui) -> None:
        with client.ollama.with_streaming_response.verify_connection(
            url="url",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            ollama = response.parse()
            assert_matches_type(object, ollama, path=["response"])

        assert cast(Any, response.is_closed) is True


class TestAsyncOllama:
    parametrize = pytest.mark.parametrize("async_client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    async def test_method_get_status(self, async_client: AsyncPyopenwebui) -> None:
        ollama = await async_client.ollama.get_status()
        assert_matches_type(object, ollama, path=["response"])

    @parametrize
    async def test_raw_response_get_status(self, async_client: AsyncPyopenwebui) -> None:
        response = await async_client.ollama.with_raw_response.get_status()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        ollama = await response.parse()
        assert_matches_type(object, ollama, path=["response"])

    @parametrize
    async def test_streaming_response_get_status(self, async_client: AsyncPyopenwebui) -> None:
        async with async_client.ollama.with_streaming_response.get_status() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            ollama = await response.parse()
            assert_matches_type(object, ollama, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    async def test_method_verify_connection(self, async_client: AsyncPyopenwebui) -> None:
        ollama = await async_client.ollama.verify_connection(
            url="url",
        )
        assert_matches_type(object, ollama, path=["response"])

    @parametrize
    async def test_method_verify_connection_with_all_params(self, async_client: AsyncPyopenwebui) -> None:
        ollama = await async_client.ollama.verify_connection(
            url="url",
            key="key",
        )
        assert_matches_type(object, ollama, path=["response"])

    @parametrize
    async def test_raw_response_verify_connection(self, async_client: AsyncPyopenwebui) -> None:
        response = await async_client.ollama.with_raw_response.verify_connection(
            url="url",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        ollama = await response.parse()
        assert_matches_type(object, ollama, path=["response"])

    @parametrize
    async def test_streaming_response_verify_connection(self, async_client: AsyncPyopenwebui) -> None:
        async with async_client.ollama.with_streaming_response.verify_connection(
            url="url",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            ollama = await response.parse()
            assert_matches_type(object, ollama, path=["response"])

        assert cast(Any, response.is_closed) is True
