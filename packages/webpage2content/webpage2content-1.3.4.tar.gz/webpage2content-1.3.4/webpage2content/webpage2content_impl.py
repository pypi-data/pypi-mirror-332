...
def _get_html_content_via_selenium(
    ...
    NUM_SELENIUM_ATTEMPTS = 3
    last_exception = None
    is_first_attempt = True

    for _ in range(NUM_SELENIUM_ATTEMPTS):
        if is_first_attempt:
            is_first_attempt = False
        else:
            LOGGER.debug(f"Backing off and trying again to fetch {url} with Selenium. ")
            time.sleep(10)
        try:
            driver.get(url)
            ...
            last_exception = e
            LOGGER.warning(
                f"Recoverable exception while attempting to fetch {url} with Selenium. "
                f"Error: {e}"
            )
