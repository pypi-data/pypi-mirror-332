...
def pandas_query(
    ...
    # Make sure we have the column names, in case we need
    # to produce an empty framework as returned value.
...
def query_count(query: QueryType, session: Session):
    ...
    query = normalize_query(query)
    return session.execute(text(f"SELECT COUNT(*) FROM ({query}) AS Q_COUNT")).first()[0]  # noqa: S608
