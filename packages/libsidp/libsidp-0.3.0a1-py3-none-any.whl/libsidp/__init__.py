...
class SIDPClient:
    def __init__(self, base_url: str, config: SidpConfigurator):
        ...
        self._config = config
        self._GITLAB_HEADERS = {'PRIVATE-TOKEN': os.environ.get("CI_JOB_TOKEN")}
    ...
    class TestInfo(object):
        ...
        def format_json_error(self, is_err: bool, message: str) -> str:
            if is_err:
                status = "error"
            else:
                status = "success"
        ...
        def get_provisioning_job_status(self) -> dict:
            """
            ...
            try:
                branch = self.get_deploy_branch_name
            except Exception as e:
                return self.format_json_error(True, e)

            # Get Vcluster UUID
            uuid = self.get_vcluster_uuid(branch)

            # Get pipeline ID
            pipeline_id = self.get_last_pipeline_id(branch)
            if not pipeline_id:
                return self.format_json_error(True, "pipeline not found or pipeline too old.")

            # Wait for pipeline to be finished
            success, reason = self.wait_pipeline(pipeline_id)
            if not success:
                return self.format_json_error(True, reason)

            # Get job ID
            job_id = self.get_provisioning_job_id(pipeline_id)
            if not job_id:
                return self.format_json_error(True, f"job provisioning not found in pipeline {pipeline_id}")

            # Get artifact and return results
            result = self.get_job_result_json(job_id, uuid)
            return self.format_json_error(not result["success"], str(result["errors"]))

        def get_last_pipeline_id(self, branch: str) -> str:
            pipelines = requests.get(f"https://gitlab.com/api/v4/projects/{self.DEPLOY_PROJECT_ID}/pipelines", headers = self._GITLAB_HEADERS)
            pipeline_id = str()
        ...
        def get_provisioning_job_id(self, pipeline_id: str) -> str:
            ...
            jobs = requests.get(f"https://gitlab.com/api/v4/projects/{self.DEPLOY_PROJECT_ID}/pipelines/{pipeline_id}/jobs", headers = self._GITLAB_HEADERS)
            for j in jobs.json():
        ...
        def get_job_result_json(self, job_id: str, uuid: str) -> dict:
            ...
            return [x for x in result.keys() if uuid + ".json" in x][0]

        def get_raw_artifact(self, job_id: str) -> str:
            data = requests.get(f"https://gitlab.com/api/v4/projects/{self.DEPLOY_PROJECT_ID}/jobs/{job_id}/artifacts", headers = self._GITLAB_HEADERS)
            return data.content

        def get_vcluster_uuid(self, branch: str) -> str:
            response = requests.get(f"https://gitlab.com/api/v4/projects/{self.DEPLOY_PROJECT_ID}/variables/VCLUSTER_UUID?filter[environment_scope]={branch}", headers = self._GITLAB_HEADERS)
        ...
        def wait_pipeline(self, pipeline_id: str) -> tuple[bool, str]:
            ...
            while timeout > 0:
                timeout = timeout - 5
                response = requests.get(f"https://gitlab.com/api/v4/projects/{self.DEPLOY_PROJECT_ID}/pipelines/{pipeline_id}", headers = self._GITLAB_HEADERS)
        ...
        def get_deploy_branch_name(self) -> str:
            resp = self.get_cluster_info()
            return resp["DeployRepoURL"].split("/")[-1]
        ...
        def extract_gitlab_artifacts(self, zip_content):
            ...
            if not isinstance(zip_content, bytes):
                if isinstance(zip_content, str) and (zip_content.startswith("b'") or zip_content.startswith('b"')):
