class VCry:
    ...
    def crypt_all(self, text, key="keyword", substitution_key="qwertyuiopasdfghjklzxcvbnm"): #параметры по умолчанию для ключей
        """
        Шифрует текст, используя все доступные методы шифрования последовательно.
        """
        encrypted_text = text
        for method in self.cipher_methods:
            if method == "keyword":
                encrypted_text = self.crypt(encrypted_text, method, key=key) #нужно передать ключ для ключевого слова
            elif method == "substitution":
                 encrypted_text = self.crypt(encrypted_text, method, key=substitution_key) # нужно передать ключ для substitution
            else:
                encrypted_text = self.crypt(encrypted_text, method)
            if encrypted_text is None: # Если какой-то метод вернул None, прерываем
                return None
        return encrypted_text

    def deall(self, text, key="keyword", substitution_key="qwertyuiopasdfghjklzxcvbnm"):
        """
        Дешифрует текст, используя все доступные методы шифрования в обратном порядке.
        """
        decrypted_text = text
        methods = list(self.cipher_methods.keys())
        methods.reverse() # Важно дешифровать в обратном порядке

        for method in methods:
            if method == "keyword":
                decrypted_text = self.uncrypt(decrypted_text, method, key=key) #нужно передать ключ для ключевого слова
            elif method == "substitution":
                 decrypted_text = self.uncrypt(decrypted_text, method, key=substitution_key) # нужно передать ключ для substitution
            else:
                decrypted_text = self.uncrypt(decrypted_text, method)
            if decrypted_text is None: # Если какой-то метод вернул None, прерываем
                return None
        ...
        for char in text:
            if char.isalpha():
                ...
                if encrypt:
                    ...
                    index = key.find(char) #индексы для алфавита который передал пользователь
                    if index != -1:
                        new_char = alphabet[index]
                    else:
                        new_char = char
