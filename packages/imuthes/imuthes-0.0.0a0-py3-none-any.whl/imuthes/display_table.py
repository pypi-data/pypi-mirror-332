...
class DisplayColumn:
    ...
    def __init__(self, name: str, format: str = "", header: str = None, is_key: bool = False, show_none: bool = False):
        self.name = name
        self.format = format
...
class DisplayTable:
    ...
    def append(self, value):
        ...
        if hasattr(value, "__getitem__"):
            f = lambda x: value.get(x.name, "")
        else:  # expecting to get an object, but here the name might be nested...
            f = lambda x: attr_getter_factory(x.name)(value)
    ...
    def ascii(self) -> str:
        ...
        columns = {
            i.name: [i.format, len(i.header) + 2 if i.is_key else len(i.header), i.show_none] for i in self._columns
        }
        logger.trace(*columns.items())
        rows = []
        ...
        formatter = "║ {:" + "} │ {:".join([f'{columns[i][0] or "<"}{str(columns[i][1])}' for i in columns]) + "} ║"

        result = [
            "╔═" + "═╤═".join(["═" * columns[i][1] for i in columns.keys()]) + "═╗",
            formatter.format(*[f"*{i.header}*" if i.is_key else i.header for i in self._columns]),
            "╟─" + "─┼─".join(["─" * columns[i][1] for i in columns.keys()]) + "─╢",
        ]
        result.extend([formatter.format(*i) for i in rows])
        result.append("╚═" + "═╧═".join(["═" * columns[i][1] for i in columns.keys()]) + "═╝")
    ...
    def markdown(self) -> str:
        ...
        for row in self._rows:
            ...
            for n, c in enumerate(columns.keys()):
                if isinstance(row[n], bool):
                    ...
                    row[n] = "&check;" if row[n] else ""  # convert to display_table chan
                elif isinstance(row[n], (int, float)):
                    if not columns[c][0]:
                        columns[c][0] = ">"
                        columns[c][3] = ":"
        ...
        result = [
            formatter.format(*[f"**{i.header}**" if i.is_key else i.header for i in self._columns]),
            "|" + "|".join([columns[i][2] + "-" * columns[i][1] + columns[i][3] for i in columns.keys()]) + "|",
        ]
        result.extend([formatter.format(*i) for i in rows])
