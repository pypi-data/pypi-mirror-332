...
def create_virtual_environment(env_name: str):
    ...
    old_env_path = sys.prefix 
    logger.info(f"Current environment: {old_env_path}")

    site_packages_path = os.path.join(old_env_path, "Lib", "site-packages") 
    if not os.path.exists(site_packages_path):  
        site_packages_path = os.path.join(old_env_path, "lib", "python3.x", "site-packages")
    ...
    logger.info(f"Using old environment packages from: {site_packages_path}")
...
def run_client_in_env(env_name: str, client_script: str, input_path: str, output_path: str, client_port: int):
    ...
    cwd = os.path.dirname(os.path.dirname(os.path.dirname(client_script)))
    ...
    try:
        ...
        return False, e.stdout, error_message
