import os
import pyspark
from pyspark.sql.types import StringType, LongType, FloatType, DateType, BooleanType, TimestampType, IntegerType
        ...
        try:
            ...
            for c in df.schema.fields:
                if c.dataType == BooleanType():
                    ...
                    cast_string.append(f"{c.name.lower()} VARCHAR(200)")
                elif c.dataType == IntegerType():
                    cast_string.append(f"{c.name.lower()} INTEGER")
