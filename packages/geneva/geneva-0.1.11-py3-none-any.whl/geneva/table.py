...
class Table(LanceTable):
    ...
    def add_columns(
        ...
        **kwargs,
        ...
        data_type = schema_to_json(pa.schema({"col": udf.data_type}))["fields"][0][
            "type"
        ]
        req_data = {
            "new_columns": [
                {
                    ...
                    "virtual_column": {
                        ...
                        "udf_spec": {
                            "name": udf_spec.name,
                            "backend": udf_spec.backend,
                            "version": udf.version,
                            "udf_payload": base64.b64encode(
                                udf_spec.udf_payload
                            ).decode("utf-8"),
                            "runner_payload": base64.b64encode(
                                udf_spec.runner_payload
                            ).decode("utf-8"),
                        },
                    },
                }
    ...
    def alter_columns(self, *alterations: Iterable[dict[str, Any]]) -> None:
        ...
        for alter in alterations:
            if "virtual_column" in alter:
                ...
                virtual_column_alterations.append(
                    {
                        "path": col_name,
                        "virtual_column": {
                            "input_columns": udf.input_columns,
        ...
        if len(virtual_column_alterations) > 0:
            client = self._conn._client
            req_data = {"alterations": virtual_column_alterations}
            result = client.post(
                f"v1/table/{self.name}/alter_columns/",
                data=req_data,
                content_type="application/json",
            )
            _LOG.debug("Alter virtual columns result: %s", result)
