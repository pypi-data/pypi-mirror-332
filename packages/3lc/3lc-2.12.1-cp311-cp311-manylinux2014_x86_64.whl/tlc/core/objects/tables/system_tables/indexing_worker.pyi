...
class _ScanUrlHandler:
    ...
    def write_index_file_timestamp(self, new_timestamp: datetime.datetime) -> bool:
        """Update the timestamp file with the given timestamp content.

        :param new_timestamp: The timestamp to write to the file
        :return: True if timestamp was successfully written, False otherwise
        """
    @staticmethod
    def compare_entries(previous_entries: Sequence[UrlAdapterDirEntry] | None, current_entries: Sequence[UrlAdapterDirEntry]) -> bool:
        """
        Compare two partial index results to determine if they differ.
        Returns True if the results are different, False if they are the same.

        Returns:
            bool: True if the scans differ, False if they are identical
        """
