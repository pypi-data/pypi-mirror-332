...
from typing import Any, Dict, List, MutableMapping, MutableSequence, Optional, Sequence, Tuple, Type
...
class AlchemyMixIn(object):
    ...
    def gen_schema(schema_cls: Type[Schema], class_suffix: str = "", table_suffix: str = "",
                   ...
                   schema_fields: Optional[Sequence[str]] = None) -> Type[Schema]:
        """
            ...
            if query_key.get("id"):
                if isinstance(query_key["id"], MutableMapping):
                    query_key["_id"] = {key: [ObjectId(item) for item in items] if isinstance(
                        items, Sequence) else ObjectId(items) for key, items in query_key.pop("id").items()}
                else:
