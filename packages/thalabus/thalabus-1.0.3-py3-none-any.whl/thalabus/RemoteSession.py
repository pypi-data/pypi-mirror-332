...
class RemoteSession:
    ...
    async def create(self, endpoint_base:str, session_id:str, token:str) -> ContainerMessage:
        ...
        log(INFO, f"Connected to the remote session: {self.rs_session.s_id}")
        return msg

    def disconnect(self) -> None:
        log(INFO, f"Disconnecting from the session: {self.rs_session.s_id if self.rs_session else None}")
        self.rs_session = None
        ...
        if self.rs_session:
            try:
                async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=BLAZOR_SSL_CERTIFICATE_VERIFICATION)) as client:
                    ...
                    log(INFO, f"Session '{self.rs_session.s_id}' was deleted from the server.")
            except aiohttp.ClientError as e:
