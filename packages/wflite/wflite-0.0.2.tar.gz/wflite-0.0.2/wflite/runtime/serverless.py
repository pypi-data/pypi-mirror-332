...
def trigger_event(request: EventRequest, config_json: Optional[Dict[str, Any]] = None):
    """
    ...
    """
    try:
        # Convert request to EventRequest if it's a dict
...
# Example of Azure Function handler
def azure_function_handler(req):
    """
    Handler for Azure Functions integration.
    ...
    try:
        # Get request body
        request_data = req.get_json() if hasattr(req, 'get_json') else req
        
        # Get configuration from request if available
        # This is now optional since we can use KeyVault and env vars instead
        config = request_data.get('config') if isinstance(request_data, dict) else None
        
        # Remove config from request data if present
        if isinstance(request_data, dict) and 'config' in request_data:
            request_data = {k: v for k, v in request_data.items() if k != 'config'}
        
        # Process the event
        return trigger_event(request_data, config)
        
    except Exception as e:
        logger.error(f"Azure function error: {str(e)}")
        return {
            "success": False,
            "error": str(e)
        }
