...
# def get_links(url,xpath=None,proxy=None,article_nums=None,required_fields=None,base_url=None):
#     def filter_by_fields(urls, required_fields):
#         """
#         根据必须包含的字段筛选 URL
#         :param urls: URL 列表
...
#             return urls
#         return [url for url in urls if any(field in url for field in required_fields)]
#
#
#
#     """
...
#             if required_fields:
#                 urls = filter_by_fields(urls, required_fields)
#             if len(urls) > 0:
...
def get_links(url, xpath=None, proxy=None, article_nums=None, required_fields=None, base_url=None, regex=None,html=None):
    ...
    try:
        ...
        if html:
            ...
            if response is None:  # 检查响应是否为 None
...
def get_article(url, proxy=None, parsing_mode='lx', headers=headers, Filter=False,url_filter=None):
    def lx_response(res):
        lx = LxParse()
    ...
    try:
        ...
        if Filter:
            if response.status_code==200:
                if not url_filter.is_url_new(url) :
                    logger.info(f'requests已存在，跳过: {url}')
                    return None
        ...
        # dit['publish_time']=result_gne.get('publish_time','')
