    ...
    def app(cls):
        ...
        if cls._app is None:
            ...
            if 'mode' in uwsgi.opt and uwsgi.opt['mode'] == b'test':
                ...
                # route tasks for celery

                if celeryc.app is not None and _conf_content.get("default_task_queue") is not None:

                    include = []
                    task_routes = {}
                    DEFAULT_QUEUE = _conf_content.get("default_task_queue")

                    if _package_list:
                        include.extend(_package_list)
                        for package in _package_list:
                            module_name = f"{package}.tasks"
                            if importlib.util.find_spec(module_name):
                                include.append(module_name)
                                task_routes[f"{module_name}.*"] = {"queue": DEFAULT_QUEUE}

                    elif _modules_list:
                        include.extend(_modules_list)

                    if include:
                        celeryc.app.autodiscover_tasks(include)

                    if task_routes:
                        celeryc.app.conf.task_routes = task_routes
