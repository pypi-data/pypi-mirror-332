    ...
    def analysis_channel_blockage_json(content, args):
        ...
        for key, value in content.items():
            if key == "single_object_dict":
                for rectangular_key, rectangular_value in value.items():
                    for objectRectangle_key, objectRectangle in rectangular_value.values():
                        ltx = objectRectangle.get("objectRectangle").get("ltX")
                        lty = objectRectangle.get("objectRectangle").get("ltY")
                        rbx = objectRectangle.get("objectRectangle").get("rbX")
                        rby = objectRectangle.get("objectRectangle").get("rbY")
            ...
            if key == "polygon_vertex_dict" and args.choose_polygon:
                for polygon_key, polygon_value in value.items():
                    ...
                    for polygon_point in polygon_value.get('polygonVertexList')[0].get('polygonVertex'):
                        ...
                        point.append(polygon_point.get('y'))
                        polygon_points.append(point)
                    shape = {"label": polygon_key, "points": polygon_points, "group_id": None, "shape_type": "polygon", "flags": {}, 'text': content.get("data").get("alarm_message")}
                    labelme_data.get('shapes').append(shape)
        return labelme_data
