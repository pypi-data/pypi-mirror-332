...
class Instance:
    ...
    def writeFileJsonv2(self, filename: str, compressed: bool = True) -> None:
        """Write the instance to a JSON file using schema 2
        ...
        donors: list[Any] = [mkdonor(donor) for donor in self.allDonors()]
        output: dict[str, Any] = {
            "schema": 2,
            "donors": donors,
        }
        ...
        output["recipients"] = [
            mkrecipient(recipient) for recipient in self.allRecipients()
        ]
        if isinstance(self, DynamicInstance):
            output["failing_transplants"] = [
                {"donor": t.donor.id, "recipient": t.recipient.id}
