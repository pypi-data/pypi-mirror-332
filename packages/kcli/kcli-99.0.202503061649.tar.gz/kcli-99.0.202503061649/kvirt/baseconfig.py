...
{{ cluster }}-{{ num }}:
 ...
 nets: {{ nets }}"""
    ...
    def create_workflow(self, workflow, overrides={}, outputdir=None, run=True, basedir='.'):
        cwd = os.getcwd()
        ...
        for entry in files:
            if not isinstance(entry, dict):
                entry = {'origin': entry}
            origin = f"{basedir}/{os.path.expanduser(entry['origin'])}"
            if 'path' in entry:
        ...
        for entry in scripts:
            origin = f"{basedir}/{os.path.expanduser(entry)}"
