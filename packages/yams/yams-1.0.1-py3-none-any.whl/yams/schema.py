...
class EntitySchema(PermissionMixIn, ERSchema):
    ...
    def relation_definition(
        ...
        randomly. If also take_first is False, a ValueError exception will be raised.
        """
        ...
        if target_type is None:
            ...
            if len(types) != 1 and not take_first:
                raise ValueError(
                    "no target_type has been specified and there are several relation definitions "
                    "for relation_type %s: %s and take_first is set at False"
                )
    ...
    def advertise_new_add_permission(self) -> None:
        ...
        * else copy the `update` rule for `add`
        ...
        if "add" not in self.permissions:
            raise Exception("you must set the 'add' permission explicitly for now one")
