...
from logilab.common import attrdict, nullobject
...
class SubjectRelation:
    ...
    def __init__(
        ...
        if "meta" in kwargs:
            raise ValueError("meta argument is not supported anymore")
...
class metadefinition(autopackage):
    ...
    def __new__(
        ...
        for relation_name, relation_definition in list(classdict.items()):
            if isinstance(relation_definition, SubjectRelation):
...
class EntityType(Definition, metaclass=metadefinition):
    ...
    @classmethod
    def _ensure_relation_type(cls: Type["EntityType"], relation: SubjectRelation) -> bool:
    ...
    @classmethod
    def add_relation(
        cls: Type["EntityType"], relation_definition: SubjectRelation, name: Optional[str] = None
    ) -> None:
    ...
    def insert_relation_after(
        ...
        name: str,
        relation_definition: SubjectRelation,
    ) -> None:
    ...
    @classmethod
    def get_relations(cls: Type["EntityType"], name: str) -> Generator[SubjectRelation, Any, None]:
    ...
    @classmethod
    def get_relation(cls: Type["EntityType"], name: str) -> SubjectRelation:
        """Return relation definitions by name. Fails if there is multiple one."""
        relations: Tuple[SubjectRelation, ...] = tuple(cls.get_relations(name))
