...
def search(query: str, limit: int):
    ...
    try:
        ...
        if not os.path.exists("commits.json"):
            with console.status("[bold green]Generating commit history..."):
                changelog = Itera()
                commits = changelog.generate_changelog(num_commits=100)  # Get last 100 commits
                with open("commits.json", "w") as f:
                    json.dump(commits, f)

        from .graphrag import GraphRAG
        
        with console.status("[bold green]Initializing semantic search..."):
