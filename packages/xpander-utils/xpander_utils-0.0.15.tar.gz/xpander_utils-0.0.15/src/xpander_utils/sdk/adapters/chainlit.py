...
from ...generic import get_sub_agent_id_from_oas_by_name
...
class ChainlitAdapter(SDKAdapter):
    ...
    def __init__(
        ...
        """
        super().__init__(api_key, agent_id, base_url, organization_id, should_reset_cache)
    ...
    @cl.step(type="tool", name="xpander.ai")
    async def process_tool_calls(self, tool_calls: Dict[int, ToolCall], local_tools: Dict[str, Callable] = None) -> str | None:
        """
        ...
        for tool_call in tool_calls.values():
            ...
            if is_sub_agent:
                if is_agent_end_tool:
                    tool_name = f"-> {self.agent.name} (Sub agent) -> Thought & Finish"
                else:
                    tool_name = f"-> {self.agent.name} (Sub agent) -> {tool_call.name}"
