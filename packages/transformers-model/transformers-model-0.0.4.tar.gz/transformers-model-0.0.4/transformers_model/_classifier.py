...
from torch_model_hub import TextCNN, RNNAttention
from transformers import AutoModel, BertConfig, ErnieConfig, AutoTokenizer, AutoConfig


class TokenClassifier(nn.Module):

    def __init__(self, backbone, classifier, max_length: int):
        super().__init__()
...
class AutoCNNTokenClassifier(TokenClassifier):
    ...
    def __init__(self, pretrained_path, num_classes: int, max_length: int = 256, config = None):
        ...
        super().__init__(backbone, classifier, max_length)

        
class BertCNNTokenClassifier(AutoCNNTokenClassifier):

    def __init__(self, pretrained_path, num_classes, max_length: int = 256):
...
class AutoRNNAttentionTokenClassifier(TokenClassifier):
    ...
    def __init__(self, pretrained_path, num_classes, max_length: int = 256, config = None):
        config = config or AutoConfig.from_pretrained(pretrained_path)
        ...
        for param in bert.parameters():
            param.requires_grad_(False)
        super().__init__(bert, classifier, max_length)
...
class ErnieRNNAttentionTokenClassifier(AutoRNNAttentionTokenClassifier):
    ...
    def __init__(self, pretrained_path, num_classes: int, max_length: int = 256):
        config = ErnieConfig.from_pretrained(pretrained_path)
        super().__init__(pretrained_path, num_classes, max_length, config)
...
class TextClassifier(TokenClassifier):
    ...
    def __init__(self, pretrained_path, backbone, classifier, max_length: int):
        super().__init__(backbone, classifier, max_length)
...
class AutoCNNTextClassifier(TextClassifier):
    ...
    def __init__(self, pretrained_path, num_classes, max_length: int = 256, config = None):
        ...
        for param in backbone.parameters():
            param.requires_grad_(False)
        super().__init__(pretrained_path, backbone, classifier, max_length)
