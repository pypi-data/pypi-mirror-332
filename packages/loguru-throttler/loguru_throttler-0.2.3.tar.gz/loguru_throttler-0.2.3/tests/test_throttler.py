...
class TestThrottleSink(unittest.TestCase):
    ...
    def test_throttling_statistics(self):
        """Test that throttling statistics are correctly tracked and reported."""
        # Create a throttler with statistics reporting enabled but without auto-reporting
        throttler = ThrottleSink(
            throttle_time=0.1,
            report_stats=True,
            report_interval=10,  # Long interval to prevent auto-reporting
        )
        ...
        # Subsequent calls within throttle window should return False (message throttled)
        for _ in range(5):
            self.assertFalse(throttler(record))

        # Check that statistics are being tracked
        self.assertEqual(throttler.total_throttled, 5)
        self.assertEqual(len(throttler.throttled_counts), 1)

        # Get statistics manually
        stats = throttler.get_statistics()
        self.assertEqual(stats["total_throttled"], 5)
        self.assertEqual(len(stats["throttled_messages"]), 1)
        self.assertIn("cache_size", stats)
        self.assertIn("throttle_time", stats)
        self.assertIn("report_interval", stats)

        # Manually reset statistics
        throttler.throttled_counts.clear()
        throttler.total_throttled = 0

        # Verify reset worked
        stats = throttler.get_statistics()
        self.assertEqual(stats["total_throttled"], 0)
        self.assertEqual(len(stats["throttled_messages"]), 0)
