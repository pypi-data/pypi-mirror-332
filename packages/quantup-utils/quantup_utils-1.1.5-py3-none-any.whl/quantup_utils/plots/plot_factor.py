...
from ..builtin import adaptive_round
...
def plot_factor(
        ...
        # # transformations
        dropna: bool = False,   # ! <- fillna
        sort_levels: bool = False, ascending: Optional[bool] = None,  # adopts to `sort_levels`
        # # sampling
        most_common: int = 13,
        ...
        size: float = 5, width_adjust: float = 1.2,
        barwidth: float = .5,
        scale: Scale = "linear",
    ...
    dropna: bool = False,
        works exactly the same as in `pd.Series(.).value_counts(., dropna)`;
    sort_levels: bool = False,
        do sort levels alphabetically? if not then levels are sorted according to frequency;
    ascending: Optional[bool] = None,
        if None (deafult) then adopts to `sort_levels` i.e.:
        if `sort_levels = False` (default) then levels are sorted alphabetically in ascending order,
        if `sort_levels = True` then levels are sorted by frequency in descending order;
