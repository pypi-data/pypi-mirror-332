...
if __name__ == '__main__':
    ...
    if use_pp:
        ...
        # dask.config.set(scheduler='processes', num_workers = ncpu - 4) #use all but 4 cores
...
#%% Forecasting        
    if "forecast" in mode:
        ...
        # products =  ['PGA', 'SA(0.6)', 'SA(1.0)', 'SA(2.0)', 'SA(5.0)']
        products =  ['PGA', 'SA(1.0)']
        ...
        if use_pp:
            ...
            for imt in products:
                ...
                iml = []
                for i in iter:
                    iml.append(compute_IMT_exceedance(rx_lat[i], rx_lon[i], distances[i].flatten(), fr, p, lambdas, time_D, lambdas_perc, m_range, m_pdf, m_cdf, model, imt=imt, IMT_min = 0.0, IMT_max = 2.0, rx_label = i))
