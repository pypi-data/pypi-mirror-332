...
class SSLCertConfigurator(AlteonConfigurator):
    ...
    def _read(self, parameters: SSLCertParameters) -> SSLCertParameters:
        log.debug('ssl cert read start: parmeters: {0}'.format(parameters))
        self._read_device_beans(parameters)
        if self._beans:
#            generateResult = self._enum_to_int(EnumSlbSslCertsGenerate, self._beans[SlbNewSslCfgCertsTable].Generate)
#            log.debug('generateResult {0}'.format(generateResult))
#            if generateResult == 5:
            if self._beans[SlbNewSslCfgCertsTable].Generate == EnumSlbSslCertsGenerate.generated:
                log.debug('ssl cert read - generated.')
                ...
                log.debug('ssl cert read - not generated.')
