...
arg_acc   = "    input genome files as NCBI accession number (*.txt)"
arg_set   = "    external setting file as JSON format (*.json)"
arg_out   = "    target folder of output data files as JSON (*.json)"
...
def checkInputs(args: dict, command_name: str):
    ...
    try:
        ...
        if acc is not None:
            ready = download_acc(acc, command_name)
            single_genbanks += ready
    ...
    if output is not None:
        ...
        for curve in curve_list:
            ...
            contents.append({name: {
                ...
                'z': [round(value, 3) for value in z]
            }})
