...
def download_acc(acc, command_name=__name__):
    """
    ...
    for item in to_download:
        ...
        save_path = os.path.join(CACHE_PATH, item) + '.gb'

        if not os.path.exists(save_path):
            run(f"ncbi-acc-download -F genbank {item}")
            cur_name = item + '.gbk'
            if os.path.exists(cur_name):
                os.replace(cur_name, save_path)
