...
class BusinessLogic:
    ...
    def get_measure_format(self, measure_names: List[str] = None, return_id: bool = False) -> Dict[str, Dict]:
        ...
        try:
            param_types = [
                ["measure_names", measure_names, list, None],
                ["return_id", return_id, bool, False]
            ]
            check_f = error_handler.check_types()
            for param_info in param_types:
                check_f(param_info)
    ...
    @timing
    def cleanup_multisphere_data(self, cube_name: str, dimension_name: str,
        ...
        try:
            param_types = [
                ["cube_name", cube_name, str],
                ["dimension_name", dimension_name, str],
                ["sql_params", sql_params, dict],
                ["is_update", is_update, bool, True]
            ]
            check_f = error_handler.check_types()
            for param_info in param_types:
                check_f(param_info)
        ...
        if len(datasources) != 1:
            return self._raise_exception(PolymaticaException, "There must be one data source!",
                                         with_traceback=False)
        datasource = datasources[0]
        datasource_id = datasource.get("id")
        if datasource.get("server_type") not in DB_SOURCE_TYPES:
            return self._raise_exception(PolymaticaException, "Data source must be DB-type!",
                                         with_traceback=False)

        # Проверяем SQL-запрос
        query = sql_params["query"]
        datasource["sql_query"] = query
        sql_query_preview = self.execute_manager_command(command_name="user_cube",
                                                         state="data_preview_request",
                                                         cube_id=cube_id,
                                                         datasource=datasource)
        column_name = self.h.parse_result(sql_query_preview, "preview_result")[0]
        if len(column_name) != 1:
            return self._raise_exception(PolymaticaException,
        ...
        field_id = next((dim.get("field_id") for dim in dims if dim.get("db_field") == dimension_name), None)
        if not field_id:
            return self._raise_exception(PolymaticaException, "Dimension not found!",
                                         with_traceback=False)

        # Собираем sql_params и выполняем команду cleanup.
        sql_params["datasource_id"] = datasource_id
        sql_params["field_id"] = field_id
        cleanup = {"querying_datasources": [sql_params], "start_update_after": is_update}
        cleanup_result = self.execute_manager_command(command_name='user_cube',
                                                      state='cleanup',
                                                      cube_id=cube_id,
                                                      cleanup=cleanup)
