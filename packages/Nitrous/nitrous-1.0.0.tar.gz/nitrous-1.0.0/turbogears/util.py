...
_all__ = ['Bunch', 'DictObj', 'DictWrapper', 'Enum', 'setlike',
   ...
   'ensure_sequence', 'has_arg', 'get_kw_defaults', 'to_kw', 'from_kw',
   'adapt_call', 'call_on_stack', 'remove_keys', 'arg_index',
...
def get_kw_defaults(func):
    """Produce a mapping of a callable's keyword parameters to defaults.

    Given a callable, return a mapping from the name of each keyword parameter
    to the corresponding default value defined on that parameter.
    ...
    for parameter_name in sig.parameters:
        if sig.parameters[parameter_name].default != inspect._empty:
            defaults[parameter_name] = sig.parameters[parameter_name].default
...
def to_kw(func, args, kw, start=0):
    ...
    for parameter_name in spec.args:
        if not args or parameter_name in kw:
            break
        else:
            v = args.pop(0)
            kw[parameter_name] = v
...
def from_kw(func, args, kw, start=0):
    ...
    argnames, varargs, varkw, defaults = getfullargspec(func)[:4]
...
def adapt_call(func, args, kw, start=0):
    ...
    """
    argnames, varargs, kwargs = getfullargspec(func)[:3]
    del argnames[:start]
...
def has_arg(func, argname):
    ...
    return argname in getfullargspec(func)[0]
