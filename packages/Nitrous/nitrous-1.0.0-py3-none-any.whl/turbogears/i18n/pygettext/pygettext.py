...
def _visit_pyfiles(list, dirname, names):
    ...
    if '_py_ext' not in globals():
        global _py_ext
        _py_ext = importlib.machinery.all_suffixes()[0]
...
def _get_modpkg_path(dotted_name, pathlist=None):
    ...
    extension module.
    """
    # Split off the top-most name
    parts = dotted_name.split('.', 1)

    if len(parts) > 1:
        # We have a dotted path, import top-level package
        top_package = parts[0]
        remainder_path = parts[1]
        top_spec = importlib.util.find_spec(top_package, pathlist)

        # If the top-level package can't be found, return None
        if top_spec is None:
            return None

        # If it's a package, we go deeper
        if top_spec.submodule_search_locations:
            new_pathlist = top_spec.submodule_search_locations
            return _get_modpkg_path(remainder_path, new_pathlist)
        else:
            # It's not a package, so we don't go deeper
            return None
    else:
        # Plain name
        spec = importlib.util.find_spec(dotted_name, pathlist)

        # If there's no spec, or it's a built-in or extension module, return None
        if spec is None or (spec.origin is None or spec.origin == 'built-in'):
            return None
        else:
            return spec.origin if spec.submodule_search_locations is None else spec.submodule_search_locations[0]
...
def extract_genshi_strings(filename, options=None):
    ...
    try:
        ...
        print("Extracting from Genshi template", filename, file=sys.stderr)
        raise
            ...
            for k, v in rentries:
                ...
                # location comments are different b/w Solaris and GNU:
                    ...
                    for filename, lineno in v:
                        ...
                        print(_(
                            '# File: %(filename)s, line: %(lineno)s') % d, file=fp)
                ...
                if k: # do not output empty msgid
                    print('msgid', normalize(k, options.escape), file=fp)
                    print('msgstr ""\n', file=fp)
        ...
        try:
            ...
            if os.path.splitext(filename)[-1].lower() == '.kid':
                try:
                    ...
                    print("Kid eater exception:", e, file=sys.stderr)

            elif os.path.splitext(filename)[-1].lower() == '.html':
                try:
                    eater.extract_genshi_strings()
                except Exception as e:
                    print("Genshi eater exception:", e, file=sys.stderr)
