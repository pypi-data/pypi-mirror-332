...
def lin_regr(x:List,y:List, origin:bool=False):    
    x = np.array(x)
    y = np.array(y)
    n = len(x)
    sum_x = np.sum(x)
    ...
    if origin:
        ...
        sa = sy * np.sqrt(1 / (sum_x2))*t_value
        sb = 0
        ...
        t_value = stats.t.ppf((1 + 0.95) / 2, df=n-2)
        # Berechnung der Unsicherheiten von a und b
        sa = sy * np.sqrt(n / (n * sum_x2 - sum_x**2))*t_value
        sb = sy * np.sqrt(sum_x2 / (n * sum_x2 - sum_x**2))*t_value
    return [a,b,sa,sb]
