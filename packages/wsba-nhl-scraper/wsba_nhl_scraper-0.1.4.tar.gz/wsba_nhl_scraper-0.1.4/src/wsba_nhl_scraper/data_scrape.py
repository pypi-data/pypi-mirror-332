...
def parse_html(html_data):
    ...
    for row in html_data:
            if row[0] == "#" or row[4] in remove:
                ...
                try: dfp['away_on_6']
                except: dfp['away_on_6'] = ""
                try: dfp['home_on_6']
                except: dfp['home_on_6'] =""
...
def retreive_players(json):
    ...
    for id, player in zip(list(home_id['playerId']),list(home_id['playerName'])):
        home_players.update({player.upper():
                        {"id":id}})

    away_players = {}
    away_id = roster.loc[roster['teamId']==away]
    for id, player in zip(list(away_id['playerId']),list(away_id['playerName'])):
        away_players.update({player.upper():
                        {"id":id}})
...
def combine_shifts(home_shift,away_shift,json,game_id):
    away_shifts = parse_shifts(away_shift,retreive_players(json),game_id)
    home_shifts = parse_shifts(home_shift,retreive_players(json),game_id)

    return pd.concat([away_shifts,home_shifts]).sort_values(by=['period','start','player'])