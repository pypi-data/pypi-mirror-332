...
from volsite_postgres_common.db import FnT
...
class SQL:
    ...
    def input_2_iid(att: str) -> str:
        return f"({Arg.input}->>'{att}')::{FnT.TEXT}"

    @staticmethod
    def input_2_uuid(att: str) -> str:
        return f"({Arg.input}->>'{att}')::{FnT.UUID}"

    @staticmethod
    def input_2_bigint(att: str) -> str:
        return f"({Arg.input}->>'{att}')::{FnT.BIGINT}"

    @staticmethod
    def input_2_int(att: str) -> str:
        return f"({Arg.input}->>'{att}')::{FnT.INT}"
    ...
    def input_iid_2_bigint(att: str) -> str:
        return f"{CFn.id_2_bigint}( ({Arg.input}->>'{att}')::{FnT.TEXT} )"

    @staticmethod
    def input_iid_2_int(att: str) -> str:
        return f"{CFn.id_2_int}( ({Arg.input}->>'{att}')::{FnT.TEXT} )"
