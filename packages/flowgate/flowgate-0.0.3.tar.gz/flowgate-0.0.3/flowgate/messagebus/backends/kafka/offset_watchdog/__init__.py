...
from flowgate.metrics import base_metric, statsd
from flowgate.utils import import_backend

BACKENDS_PATH = "flowgate.messagebus.backends.kafka.offset_watchdog.backends"
BACKENDS = {
...
class OffsetWatchdog:
    ...
    def __init__(self, config: dict, importer: Callable = import_backend) -> None:
        ...
        logger.debug(
            "Using offset watchdog backend", backend=backend_path, config=self.config
        )
