...
from flowgate.helpers.consumer import AvroConsumer
from flowgate.helpers.message import Message
from flowgate.helpers.producer import AvroProducer

from flowgate import metrics
from flowgate.messagebus.backends import MessageBusBackend
from flowgate.messagebus.backends.kafka.config import (
    get_consumer_config,
    get_offset_watchdog_config,
    get_producer_config,
)
from flowgate.messagebus.backends.kafka.offset_watchdog import OffsetWatchdog
...
class KafkaAvroBackend(MessageBusBackend):
    ...
    @metrics.call_counter("flowgate.messagebus.kafka.handle.count")
    @metrics.timed("flowgate.messagebus.kafka.handle.time")
    def _handle(
