'''
    ############################################################################
    ######################## Print Tricks & More - version 0.3.191 #############
    ############################################################################
...
TODO_QUICK = """
    ...
    - pt.easy_imports:
        - Debate on adding a quick-exit if not running file as __main__:
            - I keep thinking that this would be useful. We could save a tiny amount of processing. 
            - However, then we would have to run the code from the actual main entry point, 
            and have all import lines relevant from that, or from the script's location
            (as it's running as __main__), which means we can't get to and run the script 
            we want from any file up from it except for the main area we wanted. 
            - So I believe that this would acutally only hurt, not help. 
            - Plus, we already have an early-exit check if the file is already in the
            system path (like another easy_import() call in another script already did
            this job, then we can skip the rest of the code). 
        ...
        - Passing blank pt.easy_imports()... should not just do cwd?
            - We do want the cwd added, because then we can add anything
            from that level (should be the 'root' level). 
            - But wouldn't we also want the folder that this file is 
            located in? So we can manage things from around this area 
            directory location as well? 
            - Add both? Mostly likely not.... 
            - I guess if we just use cwd it's pretty much doing what it'd be
            doing if we were putting in the main directory by name... 
            -And adding the location we are in could just be done via the
            argument or function that adds all directories from the current
            file location to the target import_path. 
            
    - pt.t() - revised for optimum accounting for time:
        - SIMPLE!!
            - Start the timer at the very end of the first pt.t("var") call. 
            - End the timer at the very start of the next pt.t("var") call. 
            - Now practically none of pt.t function time will be timed. only the call
            and the return statement. 
                - To get rid of these two as well, also add a function call just after
                next pt.t() start that has a function call that has the same amount of 
                args, and returns the same amount, and has the equivalent amount of
                "if" check statements that we need for our main pt.t to do the 
                proper timing for the first call vs the next etc.
                - Now we can use this time as the 'delete time'.
    ...
    - pt.every / pt.after:
        ...
        if it should run or not. 
            - It'd be faster if they only got the traceback if they needed to print. 
            - And my new pt is based on instanced classes, so if this is a different
            instance than another, we will also know without needing to traceback
            (test the differences in speed). 
        ...
        - passing a function to unlock:
            - Instead of using the if statement like this:  
                " if pt.after(seconds=1)
                    do_function() "
            - We can instead do: 
                " pt.after(seconds=1, do_function) "
            
            - It should be very low overhead to see if a function was passed or not. 
...
TODO_LONG_TERM = """
    ...
    - pt.c() 
        - 'colors' var should probably also just allow 'color'
        - it's confusing that I want to change the color but must pass
        a 'colors' var...
    ...
    - pt.r(): re-organize: new classes 2024:
        - pt.enable_after(3)            ## loops
        - pt.enable_after(seconds=1)    ## seconds
        - pt.enable_every(3)            ## loops
        - pt.enable_every(seconds=1)    ## seconds
        - pt.enable_precice(loops_on=3, loops_off=1, seconds_on=1, seconds_off=0.5)
...
TODO_Errors = """
    ...
    pt() - failure to traceback (no line numbers etc.)
        NOTE: This is a pretty big deal because we can't even figure out which
        pt() line is causing it, as we don't have any data for it. 
...
class pt:
    ...
    ## NOTE: LEARN FROM THIS: The beginnigns of trying to get this working
    ## when either a file or folder is passed (it'll add both the file
    ## and its parent to path, but it broke the intermediate_path logic somehow)
    # @staticmethod
    # def _handle_full_path(path, add_intermediate_paths=False):
    ...
    #             ## If the path is a file, add both the file and its
    #             ## parent folder to the sys.path
    #             if os.path.isfile(path):
    #                 ## Append the file path to sys.path first
    #                 pt.append_path_and_print(path)
    #                 ## Retrieve and append the parent directory of the file to sys.path if not already included
    #                 parent_dir = os.path.dirname(path)
    #                 # if parent_dir not in sys.path:
    #                 #     pt.append_path_and_print(parent_dir)
                
    #             ## Handle intermediate paths for both files and directories
    #             if add_intermediate_paths:
    #                 # pt('true')
    #                 pt.add_paths_to_sys_path(target_path=path, add_intermediate_paths=add_intermediate_paths)
    #             else:
    #                 ## If it's a directory and no intermediate paths need to be added, just append the directory
                ...
                if _bytes_size < 800 or tot_len_str < terminal_width:
                    if tot_len_str < terminal_width:
                        ...
                        try:
                            ...
                            print_str += f"\n    >{num}>  {C.t2}Unknown{C.er}: {C.t3}{var_as_str}{C.er} - {var_type_as_str}, Length: {length_var}"
