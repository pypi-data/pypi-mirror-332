...
version = version_raw = "2.0.0-beta4"
if "-" in version:
...
if not gpgme_h:
    if os.path.exists(os.path.join("/usr", "include", "gpgme.h")):
        gpgme_h = os.path.join("/usr", "include", "gpgme.h")
    else:
...
def up_to_date(source, target):
    return os.path.exists(target) and os.path.getmtime(source) <= os.path.getmtime(
        target
    )
...
class BuildExtFirstHack(build):
    def _read_header(self, header, cflags):
        ...
        return subprocess.check_output(
            os.environ.get("CPP", "cc -E").split() + cflags + [tmp_include]
        ).decode("utf-8")
    ...
    def _generate(self):
        ...
        for source, target in (
            (in_srcdir(n), self._in_build_base(n))
    ...
    def run(self):
        ...
        swige.sources.extend(
            (self._in_build_base("gpgme.i"), self._in_build_base("helpers.c"))
        )
        swige.swig_opts.extend(
            ["-I" + self.build_base, "-outdir", os.path.join(self.build_lib, "gpg")]
        )
