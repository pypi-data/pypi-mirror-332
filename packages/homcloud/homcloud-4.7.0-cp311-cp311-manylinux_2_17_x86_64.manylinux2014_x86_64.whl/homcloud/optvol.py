...
def find_lp_solver(name, options):
    ...
    if name == "coin" or name == "coin-cmd":
        return pulp.COIN_CMD(**options)
    if name == "cplex":
        return pulp.CPLEX(**options)
    if name == "cplex-cmd":
        return pulp.CPLEX_CMD(**options)
    if name == "gurobi":
        return pulp.GUROBI(**options)
    if name == "glpk":
        return pulp.GLPK(**options)
    if name == "coinmp-dll":
        return pulp.COINMP_DLL(**options)
    if name == "cplex-dll":
        return pulp.CPLEX_DLL(**options)
    if name == "cplex-py":
        return pulp.CPLEX_PY(**options)
