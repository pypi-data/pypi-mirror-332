...
def download_jira_attachment(jira, issue_key, temp_folder, extensions=None):
    """
    선택된 JIRA 이슈에서 첨부된 특정 확장자의 파일을 다운로드하여 처리
    
    :param jira: JIRA 객체
    :param issue_key: 선택된 JIRA 이슈 키 (예: 'ABC-123')
    :param temp_folder: 다운로드할 파일을 저장할 폴더 경로
    :param extensions: 다운로드할 파일 확장자의 리스트 (예: ['xlsx', 'csv'])
    :return: 다운로드한 파일의 DataFrame 또는 None
    """
    
    print(f"조회할 이슈 키: {issue_key}")  # 디버깅용 출력
    try:
        jira_issue = jira.issue(issue_key)  # 🔥 이슈 키를 다시 JIRA 이슈 객체로 변환
        print(f"이슈 객체 로드 완료: {jira_issue}")  # 디버깅용 출력
    except Exception as e:
    ...
    if len(attachments) == 1:
        selection = 0
        print(f"{attachments[0][0]} 파일을 자동으로 다운로드합니다.")
    else:
        print("다운로드 가능한 파일 목록:")
    ...
    filename, url = attachments[selection]
    ...
    if filename.endswith('.xlsx'):
        ...
        load_df = pd.read_excel(xls, sheet_name=xls.sheet_names[sheet_selection])
        return load_df
