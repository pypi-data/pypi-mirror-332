...
def write_omikuji_train_file(X: csr_matrix, Y: csr_matrix, file_path: str):
    ...
    os.remove(temp_file)
    logger.info(f"Wrote {count_valid} lines (header: {num_examples}, {num_features}, {num_labels}) to '{file_path}'")
...
def assert_both_files_same_length(text_file: str, label_file: str):
    ...
    if num_texts != num_labels:
        raise ValueError(f"Number of non-empty lines in text file ({num_texts}) "
