...
from rara_subject_indexer.supervised.omikuji_helpers import assert_both_files_same_length, split_indices, write_omikuji_train_file
...
def test_omikuji_model_workflow(temp_files):
    ...
    train_results = omikuji_model.train(
    ...
    assert set(train_results.keys()) == {"model_paths", "metrics", "n_classes"}
    assert set(train_results["metrics"].keys()) == {"precision", "recall", "f1"}
    assert set(train_results["model_paths"].keys()) == {"prefix", "model", "vectorizer", "binarizer", "config"}
...
def test_rakun_kw_extraction():
    text_content = "ühed dokumendid on siin\naga mõni teine on seal\n"\
                   "mõned hoopis aga taamal\nmõned olid kaugel\nmõned olid lähedal"
    expected_keywords = [('dokument', 0.528), ('kaugel', 0.4), ('taamal', 0.4)]
...
def test_omikuji_indexer():
    ...
    preds = indexer.find_keywords("Tantsime kõik lapaduud, lapaduud")
