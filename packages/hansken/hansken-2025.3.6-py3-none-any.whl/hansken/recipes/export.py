...
def safe_name(trace, num=None, split=1000, stream='raw', template='{trace.image_id}_{trace.id}_{stream}_{trace.name}'):
    """
    Generate a file name for a trace. The resulting file name contains only
    word characters, underscores, dots and dashes and gets truncated to a max
    length.
    ...
    :param template: format string used as the file name, 'illegal' characters
        get replace with an underscore;
    ...
    file_name = template.format(trace=trace,
                                ...
                                split=split,
                                stream=stream)
    # replace anything that's not a word-character or a small selection of filename-safe punctuation characters
    name = re.sub(r'[^\w.-]', '_', file_name.strip())
    if len(name) > MAX_FILENAME_LEN:
        short_name = name[:MAX_FILENAME_LEN]
        log.warn('truncating file name to {} characters: {} -> {}', MAX_FILENAME_LEN, name, short_name)
        return short_name
