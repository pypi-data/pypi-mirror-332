...
class Scope(Enum):
    """
    Possible values for the scope of a `.Trace`. Values that correspond to
    'in scope' (`None` or  `.in_scope`) are falsy, making them suitable to
    check whether a trace is in scope.
    """
    ...
    def __bool__(self):
        # out of scope should be falsy (see class doc)
        return self is not Scope.out_of_scope
...
class Trace(AbstractTrace):
    ...
    def __init__(self, source, context=None):
        ...
        # invocations of the update method are counted, see Trace.update method
        self._update_count = 0

        self._tags = None
        self._notes = None
        self._privileged = Trace._UNINITIALIZED  # None is a valid value, use signal value
        self._scope = Trace._UNINITIALIZED  # None is a valid value, use signal value
        self._audits = None
    ...
    def scope(self):
        """
        The scope state of this trace, either `None` or one of `.Scope`. Note that `None` is not a valid value
        when *setting* the `.scope` attribute, an operation that requires authorization.
        """
        if self._scope is Trace._UNINITIALIZED:
            self._scope = self.get('user.annotated.scope', None)

            if self._scope:
                # value has been set, turn it into a Scope
                try:
                    self._scope = Scope(self._scope)
                except ValueError as e:
                    # 'reset' the attribute as if freshly constructed
                    self._scope = Trace._UNINITIALIZED
                    raise ValueError(
                        'unknown scope state: {}'.format(self.get('user.annotated.scope'))
                    ) from e
    ...
    def scope(self, status):
        status = Scope(status)
        self.context.mark_scope(self.uid, status)
        self._scope = status
    ...
    def update(self, key_or_updates=None, value=None, data=None, overwrite=False):
        ...
        """
        # count invocations of the update method
        self._update_count += 1

        if self._update_count > 1:
            log.warn("This trace ({}) is being updated more than once. This could result in lost updates, please "
                     "supply a mapping with all batched updates instead of single keys and values.", self.id)
