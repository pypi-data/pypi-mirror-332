...
def basic_auth_str(username, password):
    """
    Encode a username and password into a Basic authentication string.

    .. note::

        Both username and password are UTF-8 encoded in the result, which
        differs from the implementation found in `requests`. Using Latin1 /
        ISO-8859-1 has been shown to *NOT* work with KeyCloak.

    :param username: the username to include in the auth string
    :param password: the password to include in the auth string
    :return: A Basic authentication header using UTF-8 encoded username and
        password
    """
    # encode the parameters if they're not already (a bytes is assumed to be pre-encoded)
    if not isinstance(username, bytes):
        username = username.encode('utf-8')
    if not isinstance(password, bytes):
        password = password.encode('utf-8')

    # construct the Basic authentication header as per RFC 2617
    return 'Basic ' + b64encode(b':'.join((username, password)))
...
def resolve(username=None, password=None, base_url=None, idp_id=None, idp_url=None, idp_realm=None, sso_url=None,
    ...
    ``HANSKEN_IDP_ENDPOINT``, and ``HANSKEN_IDP_REALM`` before determining the
    type to be used:
    ...
    if idp_url and not username:
        ...
        # after the probe there's also no username (yet)
...
class KeycloakAuth(SAMLAuthBase):
    ...
    def handle_paos_request(self, response):
        ...
        saml_response = self.session.post(self.idp_url,
                                          headers={'Content-Type': 'text/xml',
                                                   'Accept': '*/*',
                                                   'Authorization': basic_auth_str(self.username(), self.password())},
