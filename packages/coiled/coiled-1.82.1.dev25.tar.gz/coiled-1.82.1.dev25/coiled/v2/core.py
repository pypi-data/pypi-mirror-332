...
class CloudV2(OldCloud, Generic[IsAsynchronous]):
    ...
    @track_context
    async def _get_cluster_by_name(self, name: str, workspace: str | None = None) -> int:
...
@list_docstring
def list_clusters(account=None, workspace=None, max_pages: int | None = None):
    with CloudV2() as cloud:
        return cloud.list_clusters(workspace=workspace or account, max_pages=max_pages)


@delete_docstring
def delete_cluster(name: str, account: str | None = None, workspace: str | None = None):
    with CloudV2() as cloud:
