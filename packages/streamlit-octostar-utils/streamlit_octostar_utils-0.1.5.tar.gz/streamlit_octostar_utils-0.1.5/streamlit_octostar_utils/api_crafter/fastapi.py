...
class CommonParsers(object):
    async def parse_form_list(
        form_data: str, format=Literal["csv", "python"]
    ) -> Optional[List[str]]:
        try:
            if not form_data:
                return None
            ...
            raise ValidationError(
                f"Parsing failed in parse_from_list: {str(e.__class__)} {str(e)}"
            )
    ...
    def include(route):
        def decorator(func):
            ...
            if getattr(route, func_name, None):
                raise ValueError(
                    f"Cannot have two functions with the same name {func_name} within the same Route!"
                )
            ...
            async def send_task(
                ...
                if op not in self.endpoints.keys():
                    raise StarletteHTTPException(
                        401, f"Route {op} is forbidden for NiFi."
                    )
    ...
    class ZipResponseModel(object):
        ...
        def get(self):
            ...
            with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
                ...
                zip_file.writestr(
                    "response.dict", json.dumps({"status": self.status}, indent=4)
                )
            zip_buffer.seek(0)
            headers = {}
...
class DefaultErrorRoute(Route):
    error_responses = {
        ...
        501: {
            ...
            "content": {
                "application/json": {
                    "example": {
                        "message": "This method (with these parameters) is not implemented!",
                        "status": "error",
                    }
                }
            },
        ...
        400: {
            ...
            "content": {
                "application/json": {
                    "example": {
                        "message": "These two lists must have the same length!",
                        "status": "error",
                    }
                }
            },
        ...
        422: {
            ...
            "content": {
                "application/json": {
                    "example": {
                        "message": "Validation error on 0 -> filename!",
                        "status": "error",
                    }
                }
            },
    ...
    def format_error(exc, body=bytes(), debug=False):
        ...
        if isinstance(exc, StarletteHTTPException):
            ...
            status_code = 400
        elif isinstance(exc, ValidationError) or isinstance(
