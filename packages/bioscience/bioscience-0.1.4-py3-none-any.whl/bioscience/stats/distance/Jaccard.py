...
def jaccard(dataset, deviceCount = 0, mode = 1, debug = False):
    """
    ...
    if (dataset is not None):
        ...
        if mode == 2: # NUMBA: CPU Parallel mode
            ...
            oModel = __jaccardSequential(dataset, debug)
            deviceCount = 0
            sMode = "CPU Sequential"
    
    return oModel

def __jaccardSequential(dataset, debug):
    ...
    for pattern, value in enumerate(resultsCorrelation):
        ...
        if r1 < iRows and r2 < iRows:
            ...
            for i in range(iCols):
                if dataset.data[r1][i] == 1 and dataset.data[r2][i] == 1:
                    iIntersect += 1

                if dataset.data[r1][i] == 1 or dataset.data[r2][i] == 1:
                    iUnion += 1
            
            dJaccard = None
            if iUnion != 0:
                dJaccard = iIntersect / iUnion
    ...
    oCorrelationResults = CorrelationModel(name=JACCARD, results=resultsCorrelation, rows = iRows, executionTime=fExecutionTime)
