...
def weightedJaccard(dataset, deviceCount = 0, mode = 1, debug = False):
    """
    ...
    if (dataset is not None):
        ...
        if mode == 2: # NUMBA: CPU Parallel mode
            ...
            oModel = __weightedJaccardSequential(dataset, debug)
            deviceCount = 0
            sMode = "CPU Sequential"
    
    return oModel

def __weightedJaccardSequential(dataset, debug):
    ...
    for pattern, value in enumerate(resultsCorrelation):
        ...
        if r1 < iRows and r2 < iRows:
            ...
            for i in range(iCols):
                minSum += min(dataset.data[r1][i], dataset.data[r2][i])
                maxSum += max(dataset.data[r1][i], dataset.data[r2][i])
            
            dJaccard = None
            if maxSum != 0:
                dJaccard = minSum / maxSum
        
            resultsCorrelation[pattern] = dJaccard
    
    if debug == True:
        end_time = time.time()
        fExecutionTime = end_time - start_time
    
    oCorrelationResults = CorrelationModel(name=WEIGHTEDJACCARD, results=resultsCorrelation, rows = iRows, executionTime=fExecutionTime)
