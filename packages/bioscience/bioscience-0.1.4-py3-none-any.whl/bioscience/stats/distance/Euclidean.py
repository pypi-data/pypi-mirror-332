...
def euclidean(dataset, deviceCount = 0, mode = 1, debug = False):
    """
    ...
    if (dataset is not None):
        ...
        if mode == 2: # NUMBA: CPU Parallel mode
            ...
            oModel = __euclideanSequential(dataset, debug)
            deviceCount = 0
            sMode = "CPU Sequential"
    
    return oModel

def __euclideanSequential(dataset, debug):
    ...
    for pattern, value in enumerate(resultsCorrelation):
        ...
        if r1 < iRows and r2 < iRows:
            ...
            differences = dataset.data[r1] - dataset.data[r2]
            squaredDifferences = differences ** 2
            sumSquaredDifferences = sum(squaredDifferences)
            euclideanDistance = sumSquaredDifferences ** 0.5
            resultsCorrelation[pattern] = euclideanDistance
    
    if debug == True:
        end_time = time.time()
        fExecutionTime = end_time - start_time
    
    oCorrelationResults = CorrelationModel(name=EUCLIDEAN, results=resultsCorrelation, rows = iRows, executionTime=fExecutionTime)
