...
def log_odds(dataset, deviceCount = 0, mode = 1, debug = False):
    """
    ...
    if (dataset is not None):
        ...
        if mode == 2: # NUMBA: CPU Parallel mode
            ...
            oModel = __logOddsSequential(dataset, debug)
            deviceCount = 0
            sMode = "CPU Sequential"
    
    return oModel

def __logOddsSequential(dataset, debug):
    ...
    for pattern, value in enumerate(resultsCorrelation):
        ...
        if r1 < iRows and r2 < iRows:
            ...
            for x, y in zip(dataset.data[r1], dataset.data[r2]):
                if x == 1 and y == 1:
                    a += 1
                elif x == 0 and y == 1:
                    b += 1
            ...
            resultsCorrelation[pattern] = logOddsRatio
    
    if debug == True:
        end_time = time.time()
        fExecutionTime = end_time - start_time
    
    oCorrelationResults = CorrelationModel(name=LOG_ODDS, results=resultsCorrelation, rows = iRows, executionTime=fExecutionTime)
