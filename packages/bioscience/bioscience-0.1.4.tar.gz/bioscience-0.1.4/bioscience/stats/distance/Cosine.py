...
def cos(dataset, deviceCount = 0, mode = 1, debug = False):
    """
    ...
    if (dataset is not None):
        ...
        if mode == 2: # NUMBA: CPU Parallel mode
            ...
            oModel = __cosSequential(dataset, debug)
            deviceCount = 0
            sMode = "CPU Sequential"
    
    return oModel

def __cosSequential(dataset, debug):
    ...
    for pattern, value in enumerate(resultsCorrelation):
        ...
        if r1 < iRows and r2 < iRows:
            ...
            dotProduct = sum(dataset.data[r1][i] * dataset.data[r2][i] for i in range(iCols))
            magX = sum(dataset.data[r1][i]**2 for i in range(iCols)) ** 0.5
            magY = sum(dataset.data[r2][i]**2 for i in range(iCols)) ** 0.5            
            
            cosineSimilarity = None
            if (magX * magY) != 0:
                cosineSimilarity = dotProduct / (magX * magY)
            resultsCorrelation[pattern] = cosineSimilarity
    
    if debug == True:
        end_time = time.time()
        fExecutionTime = end_time - start_time
    
    oCorrelationResults = CorrelationModel(name=COSINE, results=resultsCorrelation, rows = iRows, executionTime=fExecutionTime)
