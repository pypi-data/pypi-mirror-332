...
def chat_gpt(prompt: str) -> str:
    ...
    try:
        ...
        if resp.status_code != 200:
            ...
            return LatexNodes2Text().latex_to_text(resp.text)
    except Exception as e:
        return "Ошибка соединения"

def create_neiro_answer(messages, model="openai-reasoning"):
    """Доступные модели:
    ...
    try:
        ...
        response = requests.post(f"https://text.pollinations.ai/openai", headers={"Content-Type": "application/json"}, data=json.dumps(payload), timeout=60.0)
        response.raise_for_status()
        api_response = response.json()
        response_content = api_response["choices"][0]["message"]["content"]
        return LatexNodes2Text().latex_to_text(response_content)
    except requests.exceptions.RequestException as e:
...
def generate_image(prompt, width=1024, height=1024, seed=None, model='turbo'):
    if seed is None:
