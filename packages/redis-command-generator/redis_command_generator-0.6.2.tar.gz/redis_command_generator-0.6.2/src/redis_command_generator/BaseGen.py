...
class BaseGen():
    ...
    max_cmd_cnt: int = 1000  # Maximum number of commands to execute, may be interrupted by exceeding memory cap. Set to 0 for infinite
    mem_cap: float = 70  # Memory cap percentage
    pipe_every_x: int = 500  # Execute pipeline every X commands
    def_key_size: int = 3  # Default key size
    def_key_pref: str = '' # Default key prefix
    exclude_cmds: tuple = ()  # Commands to exclude from random selection
    logfile: str = None  # Optional log file path to write debug information to
    maxmemoryMB: int = None  # Override INFO's maxmemory (useful when unavailable, for e.g. in cluster mode)
    ...
    def _get_rand_cmd(self) -> Callable[[redis.client.Pipeline, str], None]:
        method_names = [name for name in dir(self) if callable(getattr(self, name)) and not name.startswith('_') and name not in self.exclude_cmds]
        name = random.choice(method_names)
    ...
    def _check_mem_cap(self, redis_obj: redis.Redis):
        ...
        if self.maxmemoryMB:
            max_mem = self.maxmemoryMB * BYTES_IN_MB
        else:
