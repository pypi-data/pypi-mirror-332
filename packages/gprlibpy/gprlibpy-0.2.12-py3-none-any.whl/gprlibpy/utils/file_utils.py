...
class FileUtils:
    ...
    @classmethod
    @contextmanager
    def get_file_from_url(cls, url: str) -> typing.ContextManager:
        """
        ...
        try:
            ...
            if err.strerror == 404:
                ...
                raise
    @staticmethod
    def list_folders_and_files(directory: Path) -> typing.Tuple[typing.List[Path], typing.List[Path]]:
        """
        Recursively lists folders and files within a given directory.

        :param directory: The root directory to scan.
        :return: A tuple containing (list of folders, list of files).
        """
        ...
        for item in directory.rglob("*"):  # Recursively iterate
            if item.is_dir():
                folders.append(item)
            elif item.is_file():
                files.append(item)
    ...
    def solve_file_path(
        ...
        return (
            cls.get_file_from_url(uri_or_path)
